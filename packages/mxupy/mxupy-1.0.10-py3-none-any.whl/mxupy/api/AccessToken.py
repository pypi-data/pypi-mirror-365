...
class AccessToken:
    ...
    def check(self, name: str, request: Request) -> IM:
        ...
        # [TODO]应该在安全中配置
        if name == 'bigOAINet.db.member.UserControl.login' or name == 'bigOAINet.db.member.UserControl.register':
            return IM()
        ...
        if not token:
            return IM(False, 'Missing or invalid token', None, status.HTTP_401_UNAUTHORIZED)
            # raise HTTPException(status_code = status.HTTP_401_UNAUTHORIZED, detail = "Missing or invalid token")
        
        # 解析令牌
        im = self.decode(token)
        if im.error:
            return im
