...
def init_main_routes(app):
    ...
    def route_explore():
        ...
        try:
            # Extract API endpoints from the JSON context data
            logger.debug("Fetching data from instructables.com API")
            ...
            page_context = None
            script_tag = soup.select("#js-page-context")
            if script_tag:
                try:
                    page_context = json.loads(script_tag[0].string)
                    logger.debug("Successfully extracted page context JSON")
                except Exception as e:
                    logger.error(f"Error parsing page context JSON: {str(e)}")

            # Use the API endpoints to fetch category data
            categories = [
                "circuits",
                "workshop",
                "design",
            ...
            for category in categories:
                try:
                    # Use the appropriate API endpoint for each category
                    api_url = f"https://www.instructables.com/json-api/getHomepageFeatured?limit=5&category={category}"
                    logger.debug(
                        f"Fetching data for category {category} from {api_url}"
                    )

                    category_data = urlopen(api_url)
                    category_json = json.loads(category_data.read().decode())
                        ...
                        category_items.append(
                            {
                                ...
                                "link": item["url"],
                                "img": (
                                    proxy(item["downloadUrl"])
                                    if "downloadUrl" in item
                                    else ""
                                ),
                                "author": item["author"] if "author" in item else "",
                                "channel": (
                                    item["classifications"][0]["channels"][0]["title"]
                                    if "classifications" in item
                                    and item["classifications"]
                                    else ""
                                ),
                                "channel_link": (
                                    f"/{item['classifications'][0]['name']}/{item['classifications'][0]['channels'][0]['name']}/"
                                    if "classifications" in item
                                    and item["classifications"]
                                    else ""
                                ),
                    ...
                    sections.append(
                        (category.capitalize(), f"/{category}", category_items)
                    )
                    logger.debug(
                        f"Processed {len(category_items)} items for category {category}"
                    )

                except Exception as e:
                    logger.error(f"Error processing category {category}: {str(e)}")
                    raise
                    sections.append((category.capitalize(), f"/{category}", []))

            return render_template("index.html", title="Explore", sections=sections)

        except Exception as e:
            logger.error(f"Error rendering explore page: {str(e)}")
            print_exc()
            ...
            return render_template(
                ...
                message="Instructables.com recently redesigned their site. We're experiencing issues with our scraper.",
            )
