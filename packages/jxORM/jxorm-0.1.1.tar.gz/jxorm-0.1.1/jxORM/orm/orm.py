...
from jxORM.orm.jxUtils import logger, checkAssert, extract_str_from_quotes, GetClsName, CID, Now, transValue2DB, transValue2Datetime, judgeTimeIsCurrent, getRenameTableName, getDateTime_to, stringAdd, defaultValue
    ...
    class Wrapper(cls):  # 继承原类
        ...
        def _set(self, init=False, **kwargs):
            for key, value in kwargs.items():
                ...
                if c is None:
                    continue
                checkAssert(not init and not c.is_key(key), f'{clsName}::_set[{key}] 不支持设置主键')
                dt = c.get_type(key)
                if dt is not None:
