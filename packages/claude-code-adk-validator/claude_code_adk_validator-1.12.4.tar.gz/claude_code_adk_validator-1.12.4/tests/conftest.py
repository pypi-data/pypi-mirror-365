...
def run_validation_factory() -> Callable[..., Tuple[int, str, str]]:
    ...
    Supports two modes:
    1. Convenience mode: run_validation_factory(tool_data, use_ci_mode=True)
    ...
    @overload
    def _run_validation(
        tool_data: Dict[str, Any], *, use_ci_mode: bool = False
    ) -> Tuple[int, str, str]:
    ...
    def _run_validation(
        ...
        *,
        ...
        if tool_data is not None and args is not None:
            raise ValueError("Cannot specify both 'tool_data' (convenience mode) and 'args' (raw mode).")
        if tool_data is None and args is None:
            raise ValueError("Must specify either 'tool_data' (convenience mode) or 'args' (raw mode).")
        ...
        if tool_data is not None:
            # --- Convenience Mode ---
            if env is not None:
                raise ValueError("'env' argument is only for raw mode. Use 'use_ci_mode' for convenience mode.")
            
            if use_ci_mode:
                final_env.pop("GEMINI_API_KEY", None)
                final_env["CI"] = "true"
                final_env["CLAUDE_TEST_BRANCH"] = "feature-test-branch"
            ...
            # --- Raw Mode ---
            if use_ci_mode:
                raise ValueError("'use_ci_mode' is only for convenience mode.")
            final_args = args
            if env is not None:
...
@pytest.fixture
def remove_api_key_env(monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.fixture
def set_ci_flag_env(monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.fixture
def ci_mode_env(remove_api_key_env: None, set_ci_flag_env: None) -> None:
    """Sets environment variables to simulate a CI environment."""
...
@pytest.fixture
def no_api_key_env(remove_api_key_env: None) -> None:
    """Ensures no API key is set for fallback testing."""
    pass  # Composition through fixture dependencies
...
@pytest.fixture
def tdd_validator_no_api() -> TDDValidator:
    """Provides a TDD validator instance without API key for fallback testing."""
    return TDDValidator(api_key=None)
