...
class TestHybridFactoryDemo:
    ...
    def test_convenience_mode_usage(self, run_validation_factory: Any) -> None:
        ...
        assert returncode == 0, f"Expected success in CI mode, stderr: {stderr}"
    ...
    def test_raw_mode_usage(self, run_validation_factory: Any) -> None:
        ...
        returncode, stdout, stderr = run_validation_factory(
            ...
            env={"CI": "true"}  # Custom environment
        )
        assert returncode == 0, f"Expected success in raw mode, stderr: {stderr}"

    @pytest.mark.quick
    def test_composable_environment_fixtures(self, run_validation_factory: Any, set_ci_flag_env: Any, remove_api_key_env: Any) -> None:
        """Demo: Using composable environment fixtures."""
        # The environment is set up by the fixtures
        tool_data = {
            "tool": "Write", 
            "input": {"file_path": "demo.txt", "content": "Test with composable fixtures"},
            "conversation_context": "",
        }
        
        # No need to specify use_ci_mode=True, the environment is already set up
        returncode, stdout, stderr = run_validation_factory(tool_data)
        assert returncode == 0, f"Expected success with environment fixtures, stderr: {stderr}"
    ...
    def test_error_handling_mixed_modes(self, run_validation_factory: Any) -> None:
        ...
        with pytest.raises(ValueError, match="Cannot specify both 'tool_data'.*and 'args'"):
    ...
    def test_error_handling_no_arguments(self, run_validation_factory: Any) -> None:
        ...
        with pytest.raises(ValueError, match="Must specify either 'tool_data'.*or 'args'"):
