...
from claude_code_adk_validator.hybrid_validator import HybridValidator
...
'''
        ...
        print(f"File Categorization: {result.get('file_categorization', 'Not provided')}")
        ...
        assert not result["approved"], f"Dashboard update should be blocked but was approved: {result}"
        
        # Check if blocked for TDD reasons
        tdd_blocked = not result.get("tdd_approved", True)
        assert tdd_blocked, f"Should be blocked for TDD reasons. TDD approved: {result.get('tdd_approved')}"
        
        # When both security and TDD fail, both should be shown in reason
        if not result.get("security_approved", True) and not result.get("tdd_approved", True):
            assert "TDD:" in result.get("reason", ""), f"TDD reason should be visible in primary reason when both fail. Reason: {result.get('reason')}"
