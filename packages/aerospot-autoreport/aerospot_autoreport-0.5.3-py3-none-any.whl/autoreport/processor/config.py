...
def create_updated_config(updated_data, report_structure_file=None):
    ...
    if pred_data is not None and not pred_data.empty:
        ...
        def get_water_quality_grade(indicator, values):
            ...
            # 按数量排序，获取主要等级
            sorted_grades = sorted(significant_grades.items(), key=lambda x: x[1], reverse=True)
        ...
        INDICATOR_DESCRIPTIONS = {
            "nh3n": "氨氮（NH3-N）指水中以游离氨（NH3）和铵离子（NH4+）形式存在的氮。氨氮是水体中的营养素，可导致水富营养化现象产生，是水体中的主要耗氧污染物，对鱼类及某些水生生物有毒害。",
            ...
            "chla": "叶绿素a是浮游植物生物量的重要指标，其含量可反映水体的初级生产力和富营养化程度。",
