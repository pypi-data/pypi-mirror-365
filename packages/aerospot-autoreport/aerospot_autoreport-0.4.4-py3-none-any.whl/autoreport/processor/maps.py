...
GLOBAL_KRIGING_METHOD = 'universal_kriging'  # 🎯 当前使用：泛克里金高斯模型
...
def kriging_interpolation(points, values, grid_lon, grid_lat, method='auto'):
    ...
    if method == 'linear':
        ...
        # 自动模式：按优先级尝试
        methods_to_try = ['universal_kriging', 'ordinary_kriging_spherical', 'ordinary_kriging_exponential']
    ...
    for method_name in methods_to_try:
        ...
        try:
            ...
            if config.get('enforce_positive', False):
                ...
                z_transformed, transform_params = transform_data_for_kriging(z, transform_method)
            ...
            if method_name == 'universal_kriging':
                ...
                logger.info(f"搜索半径: {search_radius:.6f}, 最近点数: {config['n_closest_points']}")
            ...
            if np.any(valid_mask):
                min_val, max_val = np.min(z_pred[valid_mask]), np.max(z_pred[valid_mask])
                negative_count = np.sum(z_pred[valid_mask] < 0)
                logger.info(f"{config['description']}成功，网格大小: {z_pred.shape}, 值范围: [{min_val:.3f}, {max_val:.3f}], 负值数量: {negative_count}")
            else:
...
def enhanced_interpolation_with_neighborhood(all_data, grid_resolution=200, method='linear', 
                                           ...
                                           fixed_bounds=None, kml_boundary_path=None):
    """
    ...
    if fixed_bounds is not None:
        ...
        if boundary_method == 'alpha_shape':
            ...
            if kml_boundary_path and os.path.exists(kml_boundary_path):
                ...
                logger.warning(f"KML文件不存在或路径无效: {kml_boundary_path}，回退到alpha_shape")
                boundary_points = compute_alpha_shape(points)
                lon_min, lon_max = boundary_points[:, 0].min(), boundary_points[:, 0].max()
                lat_min, lat_max = boundary_points[:, 1].min(), boundary_points[:, 1].max()
        else:  # 默认使用凸包
    ...
    # 根据地理范围和期望空间分辨率自适应计算像素数
    # 这里不再采用固定总像素数，而是根据经纬度范围和期望的空间分辨率（单位：度/像素）动态计算网格分辨率。
    # 这样可以保证无论地理范围大小，插值图像的空间分辨率都能满足需求，避免大范围时画面变糊、小范围时像素过多浪费资源。
    # 例如，期望空间分辨率为0.00002度/像素（约2米/像素），可根据实际需求调整。
    desired_resolution = 0.00002  # 单位：度/像素，可根据需要调整
    ...
    logger.info(f"创建插值网格: {lat_pixels} x {lon_pixels} (长宽比: {aspect_ratio:.3f})")
    ...
    if boundary_method == 'kml' and kml_boundary_path and os.path.exists(kml_boundary_path):
        ...
        if kml_boundary_points is not None:
            ...
            if np.any(points_inside_mask):
                ...
                logger.info(f"从 {len(points_inside_kml)} 个KML范围内的真实数据点中选择最近点作为边界值")
            else:
            ...
            # 使用扩展的数据集进行克里金插值
            grid_values = kriging_interpolation(extended_points, extended_values, grid_lon, grid_lat, method=GLOBAL_KRIGING_METHOD)
        else:
            # 如果无法获取KML边界点，使用克里金插值
            grid_values = kriging_interpolation(points, values, grid_lon, grid_lat, method=GLOBAL_KRIGING_METHOD)
    else:
        # 非KML方法使用克里金插值
        grid_values = kriging_interpolation(points, values, grid_lon, grid_lat, method=GLOBAL_KRIGING_METHOD)
    ...
    if boundary_method == 'kml':
        ...
        if kml_boundary_path and os.path.exists(kml_boundary_path):
            ...
            boundary_mask = create_kml_boundary_mask(grid_lon, grid_lat, kml_boundary_path)
        else:
...
class SatelliteMapGenerator:
    ...
    def generate_indicator_map(self):
        ...
        for indicator in self.indicator_columns:
            ...
            if supports_grading:
                map_types = ['distribution', 'interpolation', 'clean_interpolation_svg', 'level']
            ...
            for type in map_types:
                ...
                if type == 'distribution':
                    ...
                    if interpolation_data:
                        self.interpolation_cache = interpolation_data  # 缓存插值结果供clean版本使用
                    ...
                    result, _ = generate_clean_interpolation_map(
                        ...
                        grid_resolution=300, transparent_bg=True, output_format='svg', 
                    ...
                    result = generate_level_indicator_map(
                        ...
                        self.satellite_geo_bounds, self.data_geo_bounds, self.all_points_outside, 
                        self.Z, getattr(self, 'grid_lon', None), getattr(self, 'grid_lat', None)
                    )
                    # 清理插值数据
                    self.Z = None
                    self.grid_lon = None
...
# 创建单个以点带面图 - 使用增强插值算法
def generate_interpolation_indicator_map(data, indicator, satellite_info, save_path, satellite_geo_bounds, data_geo_bounds, all_points_outside, water_mask, kml_boundary_path=None):
    ...
    try:
        # 决定使用的边界方法
        boundary_method = 'kml' if kml_boundary_path and os.path.exists(kml_boundary_path) else 'alpha_shape'
...
def generate_clean_interpolation_map(data, indicator, save_path, grid_resolution=200, 
                                   transparent_bg=True, output_format='png', satellite_info=None, kml_boundary_path=None,
    ...
    try:
        ...
        if precomputed_interpolation is not None:
            ...
            grid_values, grid_lon, grid_lat, boundary_mask, boundary_points = enhanced_interpolation_with_neighborhood(
                prepared_data,
                grid_resolution=300,  # 与generate_interpolation_indicator_map保持一致
                method='linear',
                neighborhood_radius=3,  # 与generate_interpolation_indicator_map保持一致
                boundary_method=boundary_method,
                indicator_col=indicator,
                fixed_bounds=None,  # 与generate_interpolation_indicator_map保持一致
                kml_boundary_path=kml_boundary_path
            )
...
def generate_level_indicator_map(indicator, satellite_info, save_path, satellite_geo_bounds, data_geo_bounds, all_points_outside, Z, grid_lon=None, grid_lat=None):
    """
    ...
    im = main_ax.imshow(grade_map, extent=extent,
