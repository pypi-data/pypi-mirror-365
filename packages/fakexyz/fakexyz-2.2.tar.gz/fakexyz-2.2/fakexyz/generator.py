...
class FakeXYZ:
    ...
    def _get_suggestions(self, user_input, limit=3):
        ...
        for match, score in name_matches:
            if score >= 60: # Lowered threshold for better matching
        ...
        # Prioritize prefix matching for country codes
        for country_data in self.supported_countries_list:
            if country_data["country_code"].lower().startswith(user_input_lower) and \
               f"{country_data['country_flag']} {country_data['country']} (<code>{country_data['country_code']}</code>)" not in suggestions:
                suggestions.append(f"{country_data['country_flag']} {country_data['country']} (<code>{country_data['country_code']}</code>)")
        
        # If not enough suggestions from prefix, use fuzzy matching for country codes with a lower threshold
        if len(suggestions) < limit:
            ...
            for match, score in code_matches:
                if score >= 50 and f"{country_codes[match]['country_flag']} {country_codes[match]['country']} (<code>{country_codes[match]['country_code']}</code>)" not in suggestions: # Lowered threshold and avoid duplicates
                    country_data = country_codes[match]
                    suggestions.append(f"{country_data['country_flag']} {country_data['country']} (<code>{country_data['country_code']}</code>)")
        
        # Ensure unique suggestions and limit to the requested number
