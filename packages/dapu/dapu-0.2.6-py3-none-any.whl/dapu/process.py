...
class DapuProcess:
    ...
    def __init__(self, args: list | DapuContext | None):
        ...
        if isinstance(args, DapuContext): # reusing context (worker -> job -> manager chain)
            ...
            logger.debug(f"from list, work_dir is {work_dir}")
            ...
            logger.debug(f"hub is inited")
            self.context = DapuContext(work_dir, hub)
            logger.debug(f"context is inited")
            self.context.flags = args[1:] if len(args) > 1 and args[1] is not None else []
            logger.debug(f"start prepare log (discord)")
            self.prepare_logger("DEBUG") # discord level
            logger.debug(f"logs prepared")
    ...
    def prepare_logger(self, discord_level: str = "ERROR"):
        if self.context and self.context.flags:
            flags: list[str] = self.context.flags
        else:
            flags = []
