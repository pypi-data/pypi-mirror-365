...
from dapu.perks import halt, real_path_from_list, replace_dict, is_interactive, prepare_schema_name
from dapu.perks import version_string_from_timeint
...
class DapuProcess:
    ...
    def __init__(self, args: list | DapuContext | None):
        ...
        if isinstance(args, DapuContext):
            ...
            logger.debug(f"from object, work_dir is {DapuProcess.context.work_dir}")
            return
        ...
        if isinstance(args, list):
            ...
            DapuProcess.context = self.context
            logger.debug(f"from list, work_dir is {DapuProcess.context.work_dir}")

    def prepare_logger(self):
        if self.context and self.context.flags:
            flags = self.context.flags
        else:
            flags = {}
        logger.remove() # if not remove then apprise got messages twice
        if not 'debug' in flags:
        ...
        if 'discord' in flags:
            ...
            if discord_hook_id and discord_hook_token:
                ...
                level = "ERROR"
                logger.add(notifier.notify, level=f"{level}", filter= FilterDict({"apprise": False})) # type: ignore - notifier.notify tagastab bool, pylance arvab, et vaja None
                logger.info(f"Discord logging for {level} is turned ON")
    ...
    def task_id_eventlog(cls, flag: str, content: str|None = None) -> Callable: # decorator! very special!
        ...
        def inner(func: Callable[..., list[tuple]]) -> Callable:
            def wrapper(*args: Any, **kwargs: Any) -> int |None:
                result_set = func(*args, **kwargs) # moving toward DataCapsule here instead of list
