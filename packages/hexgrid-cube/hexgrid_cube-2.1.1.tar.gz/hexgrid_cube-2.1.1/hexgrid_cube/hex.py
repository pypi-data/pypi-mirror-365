...
class Hex:
    ...
    def successive_neighbours(self, direction: int, depth: int) -> list["Hex"]:
        """
        Return a list of neighbouring Hexes, excluding `self`.

        Parameters
        ----------
        direction: int
            the direction to take neighbours from
        depth: int
            the number of successive neighbours to include

        Returns
        -------
        list[Hex]
            a list of neighbouring Hexes
        """
        if depth < 1:
            raise ValueError("Depth must be at least 1.")
        ret = [self.neighbour(direction)]
        for n in range(depth-1):
            ret.append(ret[-1].direction(direction))
        return ret

    def to_vector(self) -> Vector:
        """Return a Vector with coordinates identical to the Hex's."""
