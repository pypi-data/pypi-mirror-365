...
def check_for_updates():
    current_version = str(open("VERSION", "r").read().strip())
...
def start():
    ...
    parser.add_argument(
        "--version", action="version", version=f"vsai {open('VERSION', 'r').read().strip()}"
    )
    parser.add_argument("-V", action="version", version=f"vsai {open('VERSION', 'r').read().strip()}")
