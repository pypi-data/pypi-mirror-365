    ...
    if latest_version > current_version:
        ...
        if user_input in ["yes", "y"]:
            ...
            if platform.lower().startswith("win"):
                upgrade_cmd = f'start /B cmd /c "pip install vsai -U && {exe} {script} {' '.join(args)}"'
                subprocess.Popen(upgrade_cmd, shell=True)
            else:
                upgrade_cmd = (
                    f"{exe} -m pip install vsai -U && {exe} {script} {' '.join(args)}"
                )
                subprocess.Popen(upgrade_cmd, shell=True)
            print(
                "Upgrading vsai... The application will now exit and restart after upgrade."
            )
            time.sleep(1)
            os._exit(0)
        else:
...
def start():
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"vsai {VERSION}",
    )
    parser.add_argument("-V", action="version", version=f"vsai {VERSION}")
