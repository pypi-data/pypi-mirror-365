    ...
    if latest_version > current_version:
        ...
        if user_input in ["yes", "y"]:
            if platform.lower().startswith("win"):
                upgrade_cmd = f'start /B cmd /c "pip install vsai -U && cls && vsai"'
                Popen(upgrade_cmd, shell=True)
            else:
                upgrade_cmd = (
                    f"pip3 install vsai -U && clear && vsai"
                )
                Popen(upgrade_cmd, shell=True)
            os._exit(0)
        else:
...
def start():
    check_for_updates()
    parser = argparse.ArgumentParser(description="vsai")
    parser.add_argument(
