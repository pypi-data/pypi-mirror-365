...
class CurationAPIClient:
    ...
    # Property endpoints  
    def create_property(self, property_data: Dict, source_id: Optional[int] = None) -> Dict:
        """
        Create a new property.
        
        Args:
            property_data: Property data dictionary
            source_id: Optional source ID to automatically prefix technical_name
        """
        if source_id and 'technical_name' in property_data:
            source = self.get_source(source_id)
            source_tech_name = source.get('technical_name')
            if source_tech_name:
                tech_name = property_data['technical_name']
                if not tech_name.startswith(f"{source_tech_name}_"):
                    property_data = property_data.copy()
                    property_data['technical_name'] = f"{source_tech_name}_{tech_name}"
        elif not source_id and 'technical_name' in property_data:
            print(f"Warning: Creating property '{property_data['technical_name']}' without source prefix. Consider passing source_id parameter.")
