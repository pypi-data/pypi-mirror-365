...
class CompileCacheSubmit:
    ...
    def __init__(self, repo_info: List[Dict], 
        ...
        # 编译相关配置
        self._build_command = build_command   # 编译命令
        ...
        for repo_dict in self._repo_info:
            for repo_name, repo_config in repo_dict.items():
                ...
                try:
                    ...
                    repo_manager.CloneRepository(branch=target_commit, 
                                                 clean=False, 
                                                 shallow=False, 
                                                 custom_init_func=self._code_init_func)
    ...
    def execute_build(self) -> bool:
        ...
        self._logger.info(f"编译命令: {self._build_command}")
        ...
        try:
            ...
            with file_util.ChangeDirectory(self._build_workdir):
                ...
                # 调用外部注入的编译执行器函数
                build_success = self._build_executor(self._build_command)
    ...
    def run_submit_phase(self, diff_ninja_log_path: str, p4_ignore_url: str) -> bool:
        """
