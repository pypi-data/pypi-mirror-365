    ...
    **kwargs
    ...
    if scheme == 1:
        while convergence > tol:
            if silent is False:
                print(
                    r"iDEA.Godby_inversion: convergence = {1:.5}, tolerance = {1:.5}".format(
                        convergence, tol
                    ),
                    end="\r",
                )
    ...
    if scheme == 2: 
        if occs_calc == False:
            while convergence > tol:
                if silent is False:
                    print(
                        r"iDEA.Wang-Parr_inversion: convergence = {1:.5}, tolerance = {1:.5}".format(
                            convergence, tol
                        ),
                        end="\r",
                    )
                ...
                update_term = ((target_n - n)/(denom))*0.001
        ...
        if occs_calc == True:
            while convergence > tol:
                if silent is False:
                    print(
                        r"iDEA.Wang-Parr_inversion.thermal: convergence = {0:.5}, tolerance = {1:.5}".format(
                            convergence, tol
                        ),
                        end="\r",
                    )
                ...
                Eis = 2*state.up.energies
                F_occs = 1/(1+np.exp((Eis-chem_u)/tau))
                weighted_density = F_occs * n # need to recalculate weights using the occupation function since ONLY EIGENVALUES will change
                denom = np.zeros(len(n))
                for i in range(int(s_fictitious.count/2)):
                    denom += F_occs * (2*state.up.orbitals[:,i]**2)*(1/(2*state.up.energies[i]))
