...
class LLMClientRedis:
    ...
    def _create_llm_resources_json(path: str):
        ...
        # 加载
        with importlib.resources.files('src').joinpath('config/llm_resources.json').open(encoding="utf-8") as f:
    ...
    def _create_config_ini(path: str):
        ...
        with importlib.resources.files('src').joinpath('config/config.ini').open(encoding="utf-8") as f:
            template: str = f.read()
