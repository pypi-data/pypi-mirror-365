...
def run_test_login_command(shell_exec_path: str | Path, key: str, env=None) -> str:
	full_cmd = f'{TEST_COMMAND_PREFIX} {key}"'
	output = run_login_command(str(shell_exec_path), full_cmd, env=env, log_message_prefix='Running test command\n\t$ ')
	LOGGER.info(f'output={output}')
	return full_cmd


def run_login_command(shell_exec_path: str, cmd: str, timeout_seconds=3, env=None, log_message_prefix=''):
	"""
	...
	child = pexpect.spawn(
		shell_command,
		encoding='utf-8', timeout=5+2+timeout_seconds+1, env=env,
		# maxread=1,
		echo=False,
		dimensions=(40, 120) # WARNING: this is important, if not large enough, text will be truncated, causing matches to fail
	)
