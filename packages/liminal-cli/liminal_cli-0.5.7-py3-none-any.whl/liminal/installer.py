...
def test_environment():
	...
	if not atuin.is_the_users_existing_atuin_env_supported():
...
def test_correctly_setup():
	...
	full_command = run_test_login_command(
		Shell().exec_path,
		f'installed {datetime_utcnow()} {uuid4()}',
		env=os.environ.copy() | atuin.neccessary_atuin_env_vars(),
	)
	...
	for _i in range(6):
		expected_details = atuin.local_history_row(full_command)
		if expected_details:
			break
		time.sleep(0.5)
	assert expected_details, f'Test command not found in local history. {count_before_test=} {count_after=}.'
...
class ShellConfig:
	...
	def generate_extension_file(self):
		...
		content = template.format(shell_name=shell.name, bash_preexec_path=path_replace_home_with_var(bash_preexec_path))
...
def _main():
	...
	atuin.configure_atuin(Server.SYNC_ADDRESS, freq='1hr') # preven sync on following register command?
	...
	atuin.configure_atuin(Server.SYNC_ADDRESS, freq='0')
	did_import_history = atuin.import_existing_history()
...
def main():
	...
	with sync_log.open(mode='w') as output_file:
		try:
			subprocess.run(['nohup', sys.executable, '-m', 'liminal.cli', 'shell', 'sync', '--install-id', INSTALL_SESSION.install_id], 
				capture_output=False, check=True, stdout=output_file, stderr=output_file,
			)
...
def _report_install_issue():
	...
	payload = {
		...
		'liminal_install_token': INSTALL_SESSION.install_id if INSTALL_SESSION else None,
