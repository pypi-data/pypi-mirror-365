...
def set_track_name(ctx: Context, track_index: int, name: str) -> str:
    ...
    try:
        ...
        return f"Renamed track to: {name}"
    except Exception as e:
        logger.error(f"Error setting track name: {str(e)}")
        return f"Error setting track name: {str(e)}"

@mcp.tool()
def set_track_volume(ctx: Context, track_index: int, volume_db: float) -> str:
    """
    Set the volume of a track.
    
    Parameters:
    - track_index: The index of the track to adjust
    - volume_db: The new volume value in decibels (usually between -70 and 6)
    """
    try:
        # Validate volume_db value
        if volume_db < -70.0:
            volume_db = -70.0  # Min value (silence)
        elif volume_db > 6.0:
            volume_db = 6.0  # Max value
        ...
        # No need to use result["volume_db"] as it may not be accurate in the response
        # Just use the validated value we sent
        return f"Set volume of track {track_index} to {volume_db} dB"
    except Exception as e:
...
def get_clip_notes(ctx: Context, track_index: int, clip_index: int) -> str:
    ...
    try:
        ...
        # Format the notes in a readable way
        notes = result.get("notes", [])
        clip_length = result.get("clip_length", 0)
        ...
        for note in notes:
            ...
            formatted_notes.append(f"{note_name} at beat {start:.2f} for {duration:.2f} beats (velocity: {velocity})")
            
        return f"Clip length: {clip_length} beats\nNotes:\n" + "\n".join(formatted_notes)
    except Exception as e:
...
def set_device_parameter(ctx: Context, track_index: int, device_index: int, parameter_name: str, value: float) -> str:
    ...
    - parameter_name: The name of the parameter to set (e.g., 'Attack')
    ...
    try:
        ...
        if result.get("status") == "success":
            ...
            return f"Error setting parameter: {result.get('message', 'Unknown error')}"
    except Exception as e:
