...
class TreeSitterParser(BaseParser):
    ...
    def _initialize_parsers(self):
        ...
        if not TREE_SITTER_AVAILABLE:
            logger.warning("tree-sitter-languages not available, parsers will be limited")
            return
        ...
        class FallbackAST:
            ...
            def _create_text_nodes(self, code: str):
                ...
                if self.language == 'python':
                    ...
                    func_pattern = r'^\s*(?:static\s+|inline\s+|extern\s+)?(?:void|int|char|float|double|\w+\s*\*?)\s+(\w+)\s*\([^)]*\)\s*\{'
                elif self.language == 'javascript':
                    func_pattern = r'\b(?:function\s+(\w+)|(\w+)\s*=\s*function|\b(\w+)\s*=\s*\([^)]*\)\s*=>)'
                else:
                    func_pattern = r'\b(?:def|function|func)\s+(\w+)[^:]*:'
