...
class MCPRegistry:
    ...
    def register_mcp_tool(self, tool_name: str, script:str, requirements:str) -> None:
        try:
            if self.registry_url:
                code = self._merge_mcp_code(script, requirements)
                logging.info(f"Registering MCP : name={tool_name}, code=\n{code}'")
                mcpbox_url = asyncio.run(self._call_remove_mcp_tool(tool_name))
                mcpbox_url = asyncio.run(self._call_add_mcp_tool(tool_name, code))
                logging.info("register_mcp: register tool success in McpBox with url: '%s'", mcpbox_url)
        except Exception as e:
