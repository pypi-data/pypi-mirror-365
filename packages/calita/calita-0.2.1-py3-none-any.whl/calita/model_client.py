...
from calita.utils import AlitaError
...
class LiteAIClient(ModelClient): #zhangx
    ...
    def __init__(self, config: Dict[str, Any]):
        model = config.get("agent", {}).get("primary_llm", "openai/qwen-turbo")
        self.model = model
        api_key = config.get("api", {}).get("openai_api_key")
        api_base = config.get("api", {}).get("openai_api_url")
        temperature = config.get("api", {}).get("temperature", 0.7)
        max_tokens = config.get("api", {}).get("max_tokens", 16384)
