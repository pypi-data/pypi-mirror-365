...
class CodeRunnerSandbox:
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.execution_timeout: int = int(config.get("execution_timeout", 600))
        logging.info("E2bCodeRunner initialized with execution timeout of %d seconds", self.execution_timeout)

    def run_script(self, function_name:str, script: str, verify_code: str, dependencies: List[str]) -> Tuple[str, bool]:
        sandbox = None
        try:
            sandbox = Sandbox(timeout=self.execution_timeout)
