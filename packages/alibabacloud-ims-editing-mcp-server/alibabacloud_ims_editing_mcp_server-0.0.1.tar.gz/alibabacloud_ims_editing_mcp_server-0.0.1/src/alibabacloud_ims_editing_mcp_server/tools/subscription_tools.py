...
def get_user_subscription_level():
    """
        <tags>'获取会员订阅版本'</tags>
        <toolDescription>该工具主要功能是获取客户当前的会员订阅版本。Basic: 入门版，Standard: 标准版，Premium：旗舰版，Trial：试用版。权限范围：入门版 < 标准版 < 试用版 < 旗舰版</toolDescription>
    ...
    response = invoke_api("GetPrepaidServiceInfo", payload)
    subscription_list = response["PrepaidServiceInfoList"]

    max_level = None
    max_level_int = -1
    level_map = {"None": -1, "Basic": 0, "Standard": 1, "Trial": 2, "Premium": 3}
