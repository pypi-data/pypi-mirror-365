...
class LeeBounds:
    ...
    def fit(self, data: pd.DataFrame, outcome: str, treatment: str, 
        ...
        # Calculate naive difference in means (among selected observations)
        treated_selected_mean = np.mean(Y[(D == 1) & (S == 1)])
        control_selected_mean = np.mean(Y[(D == 0) & (S == 1)])
        naive_diff = treated_selected_mean - control_selected_mean
        
        # Store results directly in self
        self.lower_bound = lower_bound
        self.upper_bound = upper_bound
        self.naive_diff = naive_diff
        self.p1 = p1
        self.p0 = p0
    ...
    def summary(self) -> str:
        ...
        - Naive difference in means: {self.naive_diff:.4f}
    ...
    def plot(self, figsize: Tuple[int, int] = (12, 8)) -> plt.Figure:
        """
        Create a horizontal line visualization of the Lee bounds with markers for zero and naive difference.
        ...
        """
        if not hasattr(self, 'lower_bound'):
            raise ValueError("No results available. Please fit the model first.")
        
        # Use the stored naive difference in means
        naive_diff = self.naive_diff
        
        fig, ax = plt.subplots(figsize=figsize)
        
        # Set y position for the horizontal line
        y_pos = 0.5
        
        # Draw horizontal line between bounds
        ax.hlines(y=y_pos, xmin=self.lower_bound, xmax=self.upper_bound, 
                 color='red', linewidth=3, alpha=0.8, label='Treatment effect bounds')
        
        # Add markers for key points
        # Zero marker (cross)
        ax.scatter(0, y_pos, color='black', marker='x', s=100, linewidth=3,
                  label='Zero effect', zorder=5)
        
        # Naive difference marker (dot)
        ax.scatter(naive_diff, y_pos, color='blue', marker='o', s=100, 
                  label=f'Naive difference ({naive_diff:.3f})', zorder=5)
        
        # Lower and upper bound markers (squares)
        ax.scatter(self.lower_bound, y_pos, color='red', marker='s', s=100,
                  label=f'Lower bound ({self.lower_bound:.3f})', zorder=5)
        ax.scatter(self.upper_bound, y_pos, color='red', marker='s', s=100,
                  label=f'Upper bound ({self.upper_bound:.3f})', zorder=5)
        
        # Add confidence intervals as dashed horizontal lines on the main line
        if not np.isnan(self.lower_bound_ci[0]):
            # Lower bound confidence interval (dashed line extending from lower bound)
            ax.hlines(y=y_pos, xmin=self.lower_bound_ci[0], xmax=self.lower_bound,
                     color='red', linestyle='--', linewidth=2, alpha=0.7,
                     label=f'Lower bound CI ({self.lower_bound_ci[0]:.3f}, {self.lower_bound_ci[1]:.3f})')
            
            # Upper bound confidence interval (dashed line extending from upper bound)
            ax.hlines(y=y_pos, xmin=self.upper_bound, xmax=self.upper_bound_ci[1],
                     color='red', linestyle='--', linewidth=2, alpha=0.7,
                     label=f'Upper bound CI ({self.upper_bound_ci[0]:.3f}, {self.upper_bound_ci[1]:.3f})')
        
        # Customize the plot
        ax.set_xlabel('Treatment Effect')
        ax.set_ylabel('')
        ax.set_title('Lee (2009) Treatment Effect Bounds', fontsize=14, fontweight='bold')
        ax.set_ylim(0, 1)
        ax.set_yticks([])
        ...
        # Reduce white space by tightening y-axis limits
        ax.set_ylim(0.2, 0.8)
