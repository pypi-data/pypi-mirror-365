...
def _add_arguments(parser: ArgumentParser):
    ...
    group.add_argument('--sort', choices=['size', 'files', 'dirs', 'path'], default=None, help="Sort by the given column.")
    ...
    mutex.add_argument('-l', '--link', action='store_const', dest='list_path_types', const='link')
    mutex.add_argument('--other', action='store_const', dest='list_path_types', const='other')
...
def _main():
    ...
    except DependencyMissingException as err:
        sys.stderr.write(f"{Color.RED}ERROR: {err.message}{Color.RESET}\n")
        sys.exit(1)
...
class DirStatOperation:
    ...
    def get_headers(self, *, path_length_indication: int|None):
        headers = [
            ...
            Header('links', length=7, format='{0:,.0f}', right_aligned=True),
            Header('others', length=7, format='{0:,.0f}', right_aligned=True),
            Header('size', display='(size)', length=10, format=self.size_format, right_aligned=True),
        ]
        if sys.platform == 'win32' and get_win32api():
            headers += [
                HeaderSeparator,
    ...
    def _get_common_data(self, item: PathInfo|PathCounter) -> dict[str,Any]:
        ...
        data = { 
            ...
            'links': item.total_link_count,
            'others': item.total_other_count,
    ...
    def build_exporter(self, paths: list[Path]):
        ...
        return exporter_class(headers, out=self.out, sort_column=self.sort_column, sort_reverse=self.sort_column in {'size', 'files'}, normalize_paths=self.root_dir)
    ...
    def analyze_directory(self, dir: Path, *, depth: int, dir_paths: Iterable[Path]|None = None) -> PathCounter:
        if self._logger.isEnabledFor(logging.DEBUG):
            ...
            if self.terminal:
                write_erasable(f"{Color.GRAY}Analyze {dir} …{Color.RESET}\n", stderr=True, freq=1.0)
        ...
        for path in dir_paths:
            ...
            if self.recurse and path.is_dir():
    ...
    def analyze_path(self, path: Path) -> PathInfo:
        ...
        if sys.platform == 'win32':
            win32api = get_win32api(no_warn=True)
            if win32api:
                # Recall
                info.is_recall = False
                if info.nature != PathNature.DIRECTORY:
                    attrs = win32api.GetFileAttributes(str(info.path)) # type: ignore
...
class ListPathsOperation(DirStatOperation):
    def __init__(self, root_dir: str|os.PathLike|None = None, *, paths_type: Literal['all','file','dir','link','other','recall','no-recall'], recurse: bool|None = None, one_file_system: bool|None = None, max_depth: int|None = None, sort_column: str|None = None, **options):
        if max_depth is None:
            max_depth = -1
        if 'recall' in paths_type:
            get_win32api(required=True)
        super().__init__(root_dir, recurse=recurse, one_file_system=one_file_system, max_depth=max_depth, sort_column=sort_column, **options)
    ...
    def is_match(self, item: PathInfo) -> bool:
        ...
        elif self.paths_type == 'link':
            return item.nature == PathNature.SYMLINK
        elif self.paths_type == 'other':
            return item.nature not in {PathNature.REGULAR_FILE, PathNature.DIRECTORY, PathNature.SYMLINK}
...
class PathInfo:
    ...
    def total_link_count(self):
        return (1 if self.nature == PathNature.SYMLINK else 0) + (self.dir_counter.total_link_count if self.dir_counter is not None else 0)
    
    @property
    def total_other_count(self):
        return (1 if self.nature not in {PathNature.DIRECTORY, PathNature.SYMLINK, PathNature.REGULAR_FILE} else 0) + (self.dir_counter.total_other_count if self.dir_counter is not None else 0)
...
class PathCounter:
    ...
    def append(self, item: PathInfo|PathCounter):
        if isinstance(item, PathInfo):
            ...
            elif item.nature == PathNature.SYMLINK:
                self.total_link_count += 1
            elif item.nature != PathNature.REGULAR_FILE:
                self.total_other_count += 1
            ...
            self.total_link_count += item.total_link_count
            self.total_other_count += item.total_other_count
    ...
    def total_file_count(self):
        return self.total_count - (self.total_dir_count + self.total_link_count + self.total_other_count)
...
class DependencyMissingException(Exception):
    def __init__(self, pypi_name: str):
        message = self.build_message(pypi_name)
        super().__init__(message)
        self.message = message
        self.pypi_name = pypi_name

    @classmethod
    def build_message(cls, pypi_name: str, *, required = False):
        return f'Missing {"required" if required else "recommended"} dependency "{pypi_name}".{Color.RESET}\nPlease run the following command to install the missing dependency: {Color.CYAN}pip install {pypi_name}{Color.RESET}'
    

def get_win32api(*, required = False, no_warn = False):
    global _win32api, _win32api_warned
    pypi_name = 'pywin32'
    ...
    if _win32api is None:
        try:
            ...
            if required:
                raise DependencyMissingException(pypi_name) from None
            _win32api = False

    if not _win32api and not _win32api_warned and not no_warn:
        message = DependencyMissingException.build_message(pypi_name, required=required)
        logging.getLogger(__name__).warning(f"{Color.YELLOW}{message}{Color.RESET}")
        _win32api_warned = True
    
    return _win32api

_win32api = None
_win32api_warned = False
...
_last_erasable: float|None = None
_erasable: list[int] = [] # ROADMAP[zut]: rename

def write_erasable(text: str, *, stderr = False, newline=False, freq: float|int|None = None):  # ROADMAP[zut]: arguments stderr & freq, rename
    """
    ...
    """
    global _last_erasable
    
    t = time_ns()
    if freq is not None and _last_erasable is not None and (t - _last_erasable) / 1E9 < freq:
        return
    ...
    if newline:
        _erasable.insert(0, 0)
    
    file.flush()
    _last_erasable = t


def erase_erasable():  # ROADMAP[zut]: rename
    """
    ...
    for i, nb_chars in enumerate(_erasable):
        if i == 0:
...
class Color:
    ...
    if os.environ.get('NO_COLORS', '').lower() in {'1', 'yes', 'true', 'on'} or not sys.stderr.isatty():
        NO_COLORS = True
        for _ in dir():
            if isinstance(_, str) and _[0] != '_' and _ not in ['NO_COLORS']: #FIXME[zut]: DISABLED => NO_COLORS
                locals()[_] = ''
