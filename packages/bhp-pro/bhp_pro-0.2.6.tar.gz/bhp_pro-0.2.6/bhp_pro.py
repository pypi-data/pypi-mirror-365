...
def payloadhunter():
    ...
    def test_payload(proxy, target, payload, payload_num):
        try:
            ...
            result = subprocess.run(cmd, input=curl_payload, capture_output=True, text=True)

            status = "SUCCESS" if result.returncode == 0 else "FAILED"
            if status == "SUCCESS":
                print(f"{Fore.GREEN}[+] Payload {payload_num} OK on {target}")
            else:
                print(f"{Fore.RED}[-] Payload {payload_num} FAILED on {target}")
    ...
    def save_results(results):
        if not results:
            print(f"{Fore.YELLOW}No successful results to save.")
            return None
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        try:
            with open(filename, 'w') as f:
                ...
                for result in results:
                    ...
                    f.write(f"\nResponse:\n{result['response']}\n")
                    f.write("\n" + "="*40 + "\n")
            print(f"\n{Fore.GREEN}Saved successful results to: {filename}")
            return filename
        except Exception as e:
    ...
    def main111():
        ...
        all_results = []
        max_threads = 20           # Number of threads per batch
        batch_size = 50            # Hosts per batch
        ...
        for batch_num, batch in enumerate(chunked(bug_hosts, batch_size), 1):
            ...
            with ThreadPoolExecutor(max_workers=max_threads) as executor:
                futures = []
                for host in batch:
                    print(f"{Fore.MAGENTA}[=] Testing host: {host}{Style.RESET_ALL}")
                    for i, payload in enumerate(build_payloads(ssh, host), 1):
                        futures.append(executor.submit(test_payload, proxy, host, payload, i))
