...
def payloadhunter():
    ...
    def test_payload(proxy, target, payload, payload_num):
        try:
            ...
            if "200" in status_line:
                print(f"{Fore.GREEN}[+] Payload {payload_num} OK on {target} (Status: 200)")
                return {
                    "target": target,
                ...
                print(f"{Fore.YELLOW}[!] Payload {payload_num} got non-200 response on {target} (Status: {status_line})")
                return {
                    "target": target,
                    "payload_num": payload_num,
                    "payload": payload,
    ...
    def main111():
        ...
        all_results = []
        max_threads = 25           # Number of threads per batch
