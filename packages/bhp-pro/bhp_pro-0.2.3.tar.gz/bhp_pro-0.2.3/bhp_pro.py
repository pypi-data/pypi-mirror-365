...
REQUIRED_MODULES = [
    ...
    'inquirer', 'shutil', 'cryptography', 'queue', 'colorama'
]
...
def install_dependencies():
    ...
    for i, module in enumerate(REQUIRED_MODULES, 1):
        simple_progress(i, len(REQUIRED_MODULES), f"Python {module}")
        try:
            __import__(module)
...
def open_port_checker():
    ...
    from colorama import Style  # <-- Add this import
...
def payloadhunter():
    ...
    def expand_cidr(cidr):
        try:
            ...
            print(f"{Fore.RED}Error expanding CIDR: {str(e)}{Style.RESET_ALL}")
            return []
    ...
    def get_targets(prompt):
        while True:
            ...
            if is_file(target):
                ...
                if targets:
                    print(f"{Fore.GREEN}Loaded {len(targets)} targets from file{Style.RESET_ALL}")
                    return targets
                continue
            ...
            if is_cidr(target):
                ...
                if targets:
                    print(f"{Fore.GREEN}Expanded to {len(targets)} IPs from CIDR{Style.RESET_ALL}")
                    return targets
                continue
            ...
            print(f"{Fore.RED}Invalid input - must be IP, domain, CIDR, or file path{Style.RESET_ALL}")
        ...
        while True:
            ...
            try:
                ...
                if all([parsed.scheme, parsed.netloc]):
                    return proxy
                print(f"{Fore.RED}Invalid format. Use proxy:port or http://proxy:port{Style.RESET_ALL}")
            except:
                print(f"{Fore.RED}Invalid proxy format{Style.RESET_ALL}")

    def build_payloads(ssh, host):
        # Return payloads in exact format used for testing
        return [
    ...
    def save_results(results):
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"proxy_test_results_{timestamp}.txt"
        ...
        try:
            with open(filename, 'w') as f:
                ...
                for result in results:
                    ...
                    f.write(f"=== Payload {result['payload_num']} ===\n")
                    f.write(f"Status: {result['status']}\n")
                    f.write(f"\nPayload Used:\n{result['payload']}\n")
                    f.write(f"\nResponse:\n{result['response']}\n")
                    f.write("\n" + "="*40 + "\n")
            
            print(f"\n{Fore.GREEN}All results saved to {filename}{Style.RESET_ALL}")
            return filename
        except Exception as e:
            print(f"{Fore.RED}Failed to save results: {str(e)}{Style.RESET_ALL}")
            return None

    def test_payload(proxy, target, payload, payload_num):
        try:
            # Convert [crlf] to actual line endings for curl
            ...
            cmd = [
                "curl", "-s", "-x", proxy,
                "--max-time", "10",
                "--data-binary", "@temp_payload.txt",
                f"http://{target}"
            ]
            
            print(f"\n{Fore.CYAN}=== Testing Payload {payload_num} on {target} ==={Style.RESET_ALL}")
            ...
            # Show condensed response info
            if result.stdout:
                print(f"{Fore.BLUE}Response snippet:{Style.RESET_ALL}")
                print(result.stdout[:200].strip() + ("..." if len(result.stdout) > 200 else ""))
            
            return {
                "target": target,
                "payload_num": payload_num,
                "payload": payload,  # Original payload with [crlf]
    ...
    def main111():
        ...
        # Get SSH server (single target)
        ssh = get_targets("Enter SSH server (e.g us1.vip.xyz): ")[0]
        
        # Get bug hosts (can be multiple)
        bug_hosts = get_targets("Enter bug host(s) (domain/IP/CIDR/file.txt): ")
        
        all_results = []
        
        for host in bug_hosts:
            print(f"\n{Fore.MAGENTA}=== Testing host: {host} ==={Style.RESET_ALL}")
            
            for i, payload in enumerate(build_payloads(ssh, host), 1):
                result = test_payload(proxy, host, payload, i)
        ...
        if results_file:
            print(f"\nComplete results saved to: {Fore.CYAN}{results_file}{Style.RESET_ALL}")
