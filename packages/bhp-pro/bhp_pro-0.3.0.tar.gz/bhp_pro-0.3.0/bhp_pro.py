...
def payloadhunter():
    ...
    def signal_handler(signum, frame):
        print(f"\n{Fore.RED}Interrupted (signal {signum}). Cleaning up temp files and exiting...{Style.RESET_ALL}")
        cleanup_temp_files()
        sys.exit(1)

    signal.signal(signal.SIGINT, signal_handler)   # Handle Ctrl+C

    # Register Ctrl+Z (SIGTSTP) only if available (Unix/Linux/macOS)
    ...
    def test_payload(proxy, target, payload, payload_num, results_file=None):
        try:
            ...
            if any(status_line.startswith(f"HTTP/1.1 {code}") for code in ("200", "301")):
                result_data = {
                    "target": target,
