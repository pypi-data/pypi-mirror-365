...
def _set_config(name: str, value, *, dry_run = False, system = False):
    ...
    if current == value:
        ...
        _logger.info(f"npm: {Color.YELLOW}would{Color.RESET} set {Color.CYAN}%s{Color.RESET} from {Color.GRAY}%s{Color.RESET} to {Color.CYAN}%s", name, current, value)
        return
    else:
        _logger.info(f"npm: set {Color.CYAN}%s{Color.RESET} from {Color.GRAY}%s{Color.RESET} to {Color.CYAN}%s", name, current, value)
