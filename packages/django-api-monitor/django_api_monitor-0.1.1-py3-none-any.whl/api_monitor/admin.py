...
class APIAccessLogAdmin(admin.ModelAdmin):
    ...
    search_fields = (
        'client_fingerprint__user__username',
        'view_name',
        'url_path',
        'request_method',
        'memory_usage',
    ...
    def get_username(self, obj):
        return obj.client_fingerprint.user.username if obj.client_fingerprint and obj.client_fingerprint.user else None

    get_username.short_description = 'Username'
    get_username.admin_order_field = 'client_fingerprint__user__username'
    ...
    def export_as_csv(self, request, queryset):
        ...
        for data in queryset:
            client_fingerprint = getattr(data, 'api_access_log_client_fingerprint', None)
            writer.writerow([
                data.timestamp,
                data.request_method,
                data.url_path,
                data.status_code,
                data.view_name,
                getattr(client_fingerprint.user, 'username', '') if client_fingerprint else '',
                getattr(client_fingerprint, 'token', '') if client_fingerprint else '',
                getattr(client_fingerprint, 'ip_address', '') if client_fingerprint else '',
                getattr(client_fingerprint, 'user_agent', '') if client_fingerprint else '',
                getattr(client_fingerprint, 'country', '') if client_fingerprint else '',
                getattr(client_fingerprint, 'session_id', '') if client_fingerprint else '',
                getattr(client_fingerprint, 'cookie_data', '') if client_fingerprint else '',
                data.execution_time,
