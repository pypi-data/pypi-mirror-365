    ...
    def __init__(
        ...
        *,
        ...
        if not github_repo_name and not source_dir:
            typer.echo("Please provide either a GitHub repo name or a source directory.")
            raise typer.Exit(1)
        ...
        try:
            if zip:
                ...
                if self.github_service:
                    github_repo_id = await self.github_service.get_repo_id(self.github_repo_name)
                    task = await self.api.schedule_drift_task_for_github(
                        github_repo_id, self.github_ref, plan_json
                    )
                else:
                    raise ValueError("GitHub service is required when using GitHub repo name.")
            else:
        ...
        if self.output_dir and zip_results:
            ...
            if self.github_service and self.github_repo_name:
                ...
                await self.github_service.add_pr_comment(
                    self.github_repo_name, pr_number, "Drift fix created successfully"
                )
