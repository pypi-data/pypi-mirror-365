...
class ChatDeepAI(BaseChatModel):
    ...
    def validate_environment(self) -> Self:
        ...
        # Validate and normalize chat style - now more flexible
        if not validate_chat_style(self.chat_style, strict=False):
            # Only warn if the style is completely invalid (empty/None)
            logger.warning(
                f"Invalid chat style '{self.chat_style}'. Using 'chatgpt-alternative' as fallback."
            )
            self.chat_style = "chatgpt-alternative"
        else:
            # Normalize known aliases, but keep unknown styles as-is
            normalized_style = normalize_chat_style(self.chat_style)
            if normalized_style != self.chat_style:
                logger.info(f"Chat style '{self.chat_style}' normalized to '{normalized_style}'")
                self.chat_style = normalized_style
            else:
                # Unknown style - log info but allow it
                if self.chat_style not in get_chat_styles().keys():
                    logger.info(f"Using custom chat style '{self.chat_style}' - this may work with extended DeepAI styles")
    ...
    def get_available_chat_styles(self) -> Dict[str, Dict[str, Any]]:
        """
        Get information about available chat styles from local definitions.
    ...
    def get_all_extended_styles(self) -> List[Dict[str, Any]]:
        """
        Get all available chat styles including extended styles from DeepAI API.
        
        This method fetches styles from the DeepAI API using the styles module,
        providing access to a much larger collection of chat styles beyond the
        built-in ones.
        
        Returns:
            List[Dict[str, Any]]: List of all available chat styles with metadata
            
        Example:
            >>> chat = ChatDeepAI()
            >>> all_styles = chat.get_all_extended_styles()
            >>> for style in all_styles[:3]:
        ...
        try:
            ...
            logger.warning("Styles module not available, returning basic chat styles")
            # Convert local styles to the extended format
            local_styles = get_chat_styles()
            extended_format = []
            for i, (handle, info) in enumerate(local_styles.items()):
                extended_format.append({
                    'id': i + 1,
                    'url_handle': handle,
                    'name': info.get('description', handle.replace('-', ' ').title()),
                    'description': info.get('description', ''),
                    'genre': 'General',
                    'usage': 1000,
                    'image': f'chat-style-image/default/{handle}.jpg'
                })
