...
class RedisMessageQueue:
    ...
    async def _graceful_shutdown(self) -> None:
        ...
        try:
            ...
            if hasattr(self, '_monitor_service') and self._monitor_service:
                try:
                    await asyncio.wait_for(
                        self._monitor_service.stop_delay_processing(), 
                        timeout=10.0
                    )
                except asyncio.TimeoutError:
                    self._logger.error("【stop】停止调度器服务超时")
                except Exception as e:
                    self.log_error("【stop】停止调度器服务失败", e)
            ...
            await asyncio.wait_for(
                self._wait_for_local_queue_empty(),
                timeout=30.0
            )
            ...
            await asyncio.wait_for(
                self._wait_for_consumers_finish(),
                timeout=30.0
            )
            ...
            self._logger.error("【stop】优雅停机超时")
        except Exception as e:
