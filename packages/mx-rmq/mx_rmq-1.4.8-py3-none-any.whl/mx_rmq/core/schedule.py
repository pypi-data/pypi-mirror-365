...
class ScheduleService:
    ...
    async def stop_delay_processing(self) -> None:
        ...
        if not self.is_running:
            self.context.log_debug("延时任务调度器未运行，无需停止")
            return
            
        self.is_running = False
        self.context.log_info("开始停止延时任务调度器...")
        ...
        try:
            ...
            if self.scheduler_task:
                try:
                    ...
                    self.context.log_error("停止延时任务调度器超时，强制结束")
                    # 如果超时，确保任务被取消
                    ...
                    self.context.log_error(f"停止延时任务调度器时发生异常: {e}")

            self.context.log_debug("延时任务调度器已停止")
        except Exception as e:
            self.context.log_error(f"停止延时任务调度器过程中发生错误: {e}")
        finally:
    ...
    async def pubsub_listener(self) -> None:
        ...
        self.context.log_info("pubsub_listener close! 💀")
    async def periodic_fallback(self) -> None:
        ...
        while self.is_running:
            ...
            try:
                ...
                self.context.log_error("兜底检查执行失败", e)
        self.context.log_info("periodic_fallback close! 💀")
    async def try_process_expired_tasks(self) -> None:
        """尝试处理过期任务"""
        try:
            lua_script: AsyncScript = self.context.lua_scripts["process_delay_message"]
            delay_tasks_key = self.context.get_global_key(GlobalKeys.DELAY_TASKS)
            pending_key = self.context.get_topic_key("", TopicKeys.PENDING).rstrip(":")
            result = await lua_script(
                keys=[delay_tasks_key, pending_key], args=[]
            )
            if result:
                self.context._logger.info(f"处理延时任务成功, result={result}")
        except Exception as e:
            self.context.log_error("处理延时任务失败", e)
