...
# Set up logging and save original file descriptors
...
for scenario in "${SCENARIOS_TO_RUN[@]}"; do
    ...
    exec 5>&1 6>&2  # Save current stdout and stderr for this scenario
    ...
    # Restore stdout/stderr to the main log (if file descriptors exist)
    if [ -e /proc/self/fd/5 ] && [ -e /proc/self/fd/6 ]; then
        exec 1>&5 5>&-
        exec 2>&6 6>&-
    fi
done
...
# Clean up file descriptors before exit
