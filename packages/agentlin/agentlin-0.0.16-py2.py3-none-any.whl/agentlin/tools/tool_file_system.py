...
def glob(pattern: str, path: Optional[str] = None, case_sensitive: bool = False,
    ...
    try:
        ...
        # 使用glob模块查找文件
        found_files = glob_module.glob(search_pattern, recursive=True)
        ...
        for file_path in found_files:
            ...
            if not should_ignore and respect_git_ignore:
                for ignore_pattern in gitignore_patterns:
                    if fnmatch.fnmatch(rel_path, ignore_pattern) or fnmatch.fnmatch(file_name, ignore_pattern):
                        should_ignore = True
                        break
            ...
            if not should_ignore:
                ...
                if not case_sensitive:
                    ...
                    if fnmatch.fnmatch(rel_path.lower(), pattern_lower):
...
def search_file_content(pattern: str, path: Optional[str] = None, include: Optional[str] = None) -> str:
    ...
    try:
        ...
        if include:
            # 使用指定的glob模式
            search_pattern = os.path.join(search_path, include)
            search_files = glob_module.glob(search_pattern, recursive=True)
        else:
            # 搜索大多数文本文件
            search_files = []
            for root, dirs, files in os.walk(search_path):
                # 跳过常见的忽略目录
                dirs[:] = [d for d in dirs if d not in {'.git', 'node_modules', '__pycache__', '.vscode', '.idea', 'venv', 'env'}]

                for file in files:
                    file_path = os.path.join(root, file)
                    # 只搜索可能的文本文件
                    if _is_likely_text_file(file_path):
        ...
        for file_path in search_files:
            ...
            try:
                ...
                if file_matches:
                    rel_path = os.path.relpath(file_path, search_path)
                    matches.append((rel_path, file_matches))
                    file_count += 1
