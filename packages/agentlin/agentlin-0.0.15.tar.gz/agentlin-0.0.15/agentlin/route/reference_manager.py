...
class ReferencableItem(BaseModel):
...
def hash_block_list(block_list: list[Block]) -> str:
    ...
    """
    return str(hash(tuple(block_list)))  # Simple hash based on tuple representation
    ...
    def process_tool_result(self, tool_result: ToolResult) -> ToolResult:
        ...
        for i, content in enumerate(message_content):
            if "id" in content:
                ...
                elif i + 1 < len(message_content) and "id" not in message_content[i + 1]:
                    id_to_referencable_result[reference_id]["message_content_end_idx"] = i + 1
        ...
        for id, result in id_to_referencable_result.items():
            ...
            id_to_number_start_end[reference_number] = (reference_number, message_content_start_idx, message_content_end_idx)
        ...
        for i, content in enumerate(message_content):
            if "id" in content:
                ...
                if i == start_idx:
                    new_message_content.append({"type": "text", "text": f"<referencable-result>\nID: {reference_number}\n"})
