"""
Multi File Read Tool for AgentLin

参考 TypeScript 版本的 read_many_files 工具实现，支持通过路径或 glob 模式读取多个文件的内容。
...
除非 'useDefaultExcludes' 为 false，否则默认排除适用于常见非文本文件和大型依赖目录。
    ...
    def _is_within_root(self, path_to_check: str, root_directory: str) -> bool:
        """检查路径是否在给定的根目录内"""
    ...
    def _process_single_file_content(self, file_path: str, offset: Optional[int] = None,
        ...
        try:
            ...
            relative_path = os.path.relpath(file_path, self.target_directory).replace('\\', '/')
            ...
            error_message = str(e)
            display_path = os.path.relpath(file_path, self.target_directory).replace('\\', '/')
            return {
    ...
    def _find_files_by_patterns(self, patterns: List[str], exclude_patterns: List[str],
                               respect_git_ignore: bool = True) -> Set[str]:
        ...
        if respect_git_ignore:
            gitignore_path = os.path.join(self.target_directory, '.gitignore')
        ...
        for pattern in patterns:
            ...
            if os.path.isabs(pattern):
                pattern = os.path.relpath(pattern, self.target_directory)

            full_pattern = os.path.join(self.target_directory, pattern)

            # 使用 glob 查找文件
            try:
                matches = glob.glob(full_pattern, recursive=True)
                for match in matches:
                    if os.path.isfile(match) and self._is_within_root(match, self.target_directory):
    ...
    async def execute(self, params: ToolParams) -> ToolResult:
        ...
        try:
            ...
            # 查找文件
            files_to_consider = self._find_files_by_patterns(
                search_patterns, effective_excludes, respect_git_ignore
            )
            ...
            if not files_to_consider:
                return ToolResult(
                    ...
                    block_list=[],
                    data={'processed_files': [], 'skipped_files': [], 'target_directory': self.target_directory}
                )
            ...
            for file_path in sorted_files:
                result = self._process_single_file_content(file_path)
            ...
            # 构建显示消息
            display_message = f"### ReadManyFiles Result (Target Dir: `{self.target_directory}`)\n\n"
...
# 便利函数，与项目中其他工具保持一致
def create_multi_files_read_tool(target_directory: str = None) -> MultiFilesReadTool:
    """创建多文件读取工具实例"""
    return MultiFilesReadTool(target_directory)
...
def read_many_files(
    ...
    Returns:
        包含处理结果的字典
    """
    tool = MultiFilesReadTool(target_directory)
