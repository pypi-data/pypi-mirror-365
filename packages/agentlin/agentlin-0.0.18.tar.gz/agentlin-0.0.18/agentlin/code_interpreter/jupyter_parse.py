...
from agentlin.core.agent_schema import content_to_text, generate_hash_id, generate_short_uuid
    ...
    if iopub_msg["msg_type"] == "stream":
        ...
        if "data" in iopub_msg["content"]:
            ...
            if MIME_TOOL_RESPONSE in data:
                ...
                return text_response(iopub_msg, text=output)
...
def parse_msg_list_to_tool_response(msg_list: list[Optional[dict]], mode: Literal["simple", "full", "debug"]) -> ToolResponse:
    ...
    for i, msg in enumerate(msg_list):
        ...
        if response:
            ...
            logger.debug(content_to_text(response.get("message_content", [])))
