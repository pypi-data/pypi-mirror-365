"""
基于RabbitMQ的Agent消息队列实现，提供多Agent协调、时间同步和可扩展的消息通信能力。

实现原理：
1. 基于RabbitMQ的robust连接，支持自动重连和错误恢复
2. 支持Agent间消息通信和时间同步机制
3. 类型安全的消息处理和结构化日志记录
4. 可扩展的消息处理架构，支持不同类型的Agent实现
"""
...
class AgentConnectionError(AgentMessageQueueError):
    """Agent连接异常"""

    pass


class AgentMessageError(AgentMessageQueueError):
    """Agent消息处理异常"""
...
def _create_logger_for_agent(log_dir: str, agent_id: str) -> Logger:
    ...
    new_logger.add(
        sink=sys.stdout,
        format="{time:YYYY-MM-DD HH:mm:ss} | {extra[agent_id]: <8} | {level: <8} | {message}",
        level="INFO",
...
class AgentMessageQueue(ABC):
    """
    基于RabbitMQ的Agent消息队列，支持多Agent协调、时间同步和RPC调用。

    提供以下功能：
    1. Agent间消息通信，支持点对点和广播通信
    2. 时间同步机制，确保多Agent系统的一致性
    ...
    def __init__(
        ...
        Args:
            agent_id: Agent唯一标识符，如果为None则自动生成UUID
        ...
        # 初始化日志记录
        self.logger = _create_logger_for_agent(self.LOG_DIR, self.agent_id)
        self.logger.info(f"初始化Agent {self.agent_id}，RabbitMQ主机: {self.rabbitmq_host}:{self.rabbitmq_port}")
    ...
    async def connect(self):
        ...
        支持自动重连和错误恢复，对多Agent协调研究至关重要。
        """
        delay = self.reconnect_initial_delay

        while not self._shutdown:
            try:
                self.logger.info(f"尝试连接到RabbitMQ: {self.rabbitmq_host}:{self.rabbitmq_port}")
                ...
                self.logger.info("成功连接到RabbitMQ并初始化基础设施")
                return

            except (AMQPConnectionError, AMQPChannelError) as e:
                self.logger.error(f"连接失败: {e}. {delay}秒后重试...")
                await asyncio.sleep(delay)
                delay = min(delay * 2, self.reconnect_max_delay)
            except Exception as e:
                self.logger.error(f"连接设置时发生意外错误: {e}. {delay}秒后重试...")
                await asyncio.sleep(delay)
                delay = min(delay * 2, self.reconnect_max_delay)
    ...
    async def _setup_agent_infrastructure(self):
        """设置Agent通信基础设施"""
        if not self.channel:
            raise AgentConnectionError("Channel未初始化，无法设置Agent基础设施")
        ...
        self.logger.debug(f"声明交换机 '{AGENT_EXCHANGE_NAME}' (类型: {AGENT_EXCHANGE_TYPE})")
        ...
        self.logger.debug(f"声明独占队列 '{self.routing_key}'")

        # 绑定队列到交换机
        await self.agent_queue.bind(self.agent_exchange, routing_key=self.routing_key)
        self.logger.debug(f"绑定队列 '{self.routing_key}' 到交换机 '{AGENT_EXCHANGE_NAME}'")

    async def _setup_time_infrastructure(self):
        """设置时间同步基础设施"""
        if not self.channel:
            raise AgentConnectionError("Channel未初始化，无法设置时间同步基础设施")
        ...
        self.logger.debug(f"声明时间交换机 '{TIME_EXCHANGE_NAME}' (类型: {TIME_EXCHANGE_TYPE})")
        ...
        self.logger.info(f"时间路由键: {time_routing_key}")
        ...
        self.logger.info(f"Agent {self.agent_id} 绑定到时间交换机 '{TIME_EXCHANGE_NAME}'")

    async def _setup_rpc_infrastructure(self):
        """设置RPC基础设施"""
        if not self.channel:
            raise AgentConnectionError("Channel未初始化，无法设置RPC基础设施")
        ...
        self.logger.debug(f"声明RPC交换机 '{RPC_EXCHANGE_NAME}' (类型: {RPC_EXCHANGE_TYPE})")

        # 创建接收RPC请求的队列（持久化，非独占）
        self.rpc_queue = await self.channel.declare_queue(
            name=f"rpc_requests.{self.agent_id}",
            durable=True,
            exclusive=False,
        )

        # 绑定RPC请求队列到交换机
        await self.rpc_queue.bind(self.rpc_exchange, routing_key=f"rpc.{self.agent_id}")
        self.logger.debug(f"创建并绑定RPC请求队列 'rpc_requests.{self.agent_id}'")

        # 创建接收RPC响应的队列（临时的，独占的）
        self.reply_queue = await self.channel.declare_queue(
            name=f"rpc_replies.{self.agent_id}.{uuid.uuid4().hex[:8]}",
            durable=False,
            exclusive=True,
            auto_delete=True,
        )
        self.logger.debug(f"创建RPC回复队列 '{self.reply_queue.name}'")
    ...
    async def publish_time(self, msg_type: str, payload: dict, routing_key: str) -> None:
        ...
        if not self.time_exchange:
            self.logger.error("时间交换机未初始化，无法发布时间消息")
            return
        ...
        try:
            ...
            self.logger.debug(f"发布时间消息: {msg_type} -> {routing_key}")
        except Exception as e:
            self.logger.error(f"发布时间消息失败: {e}")
    ...
    async def send_message(
        ...
        if not self.agent_exchange:
            self.logger.error("交换机未初始化，无法发送消息")
            return False
        ...
        try:
            ...
            self.logger.debug(f"发送消息到 {recipient_id}: {message_type}")
            return True

        except (AMQPConnectionError, AMQPChannelError) as e:
            self.logger.error(f"发送消息到 '{recipient_id}' 失败: {e}. 尝试重新连接...")
            await self.reconnect()
            return False
        except Exception as e:
            self.logger.error(f"发送消息到 '{recipient_id}' 时发生意外错误: {e}")
            return False
    ...
    async def broadcast_message(
        ...
        """
        if not self.agent_exchange:
            self.logger.error("交换机未初始化，无法广播消息")
            return False
        ...
        try:
            ...
            self.logger.debug(f"广播消息: {message_type} -> {target_pattern}")
            return True
    ...
    def register_rpc_method(self, method_name: str, method: Callable):
        ...
        self.logger.info(f"注册RPC方法: {method_name}")
    ...
    def register_method(self, method_name: str, method):
        return self.register_rpc_method(method_name, method)

    def register(self, method_name: str = None):
        return self.register_rpc(method_name)

    async def call(self, target_agent_id: str, method_name: str, *args, timeout=None, **kwargs):
        return await self.call_rpc(target_agent_id, method_name, *args, timeout=timeout, **kwargs)

    def call_sync(self, target_agent_id: str, method_name: str, *args, timeout=None, **kwargs):
        return self.call_rpc_sync(target_agent_id, method_name, *args, timeout=timeout, **kwargs)

    async def consume(self):
        """
        开始异步消费消息

        启动常规Agent通信和时间同步消息的消息消费。
        对系统环境中的多Agent协调至关重要。
        """
        if not self.agent_queue:
            self.logger.error("队列未初始化，无法开始消费")
            return

        # 开始消费常规Agent消息
        self.agent_consumer_tag = await self.agent_queue.consume(self.on_message)
        self.logger.info("开始消费Agent消息")

        if not self.time_queue:
            self.logger.error("时间队列未初始化，无法开始消费时间同步消息")
            return

        # 开始消费时间同步消息
        self.time_consumer_tag = await self.time_queue.consume(self.on_message)
        self.logger.info("开始消费时间同步消息")

        # 开始消费RPC消息
        await self._start_rpc_consumption()

    async def _start_rpc_consumption(self):
        """开始消费RPC消息"""
        if not self.rpc_queue or not self.reply_queue:
            self.logger.error("RPC队列未初始化，无法开始RPC消费")
            return

        # 开始消费RPC请求
        self.rpc_consumer_tag = await self.rpc_queue.consume(self.on_message)
        self.logger.info("开始消费RPC请求")

        # 开始消费RPC响应
        self.reply_consumer_tag = await self.reply_queue.consume(self.on_message)
        self.logger.info("开始消费RPC响应")
            ...
            try:
                ...
                try:
                    ...
                    if msg_type:
                        ...
                        self.logger.warning(f"无法解析消息格式: {msg_data}")
                        return
                ...
                self.logger.error(f"JSON解码错误: {e}. 消息: {message.body}")
            except Exception as e:
                self.logger.error(f"处理消息时出错: {e}. 消息: {message.body}")
    ...
    async def _dispatch_message(self, message: AgentMessage):
        ...
        if handler:
            try:
                ...
                self.logger.debug(f"处理消息: {message_type} (来自: {message.sender})")
            except Exception as e:
                self.logger.error(f"处理消息 {message_type} 时出错: {e}")
        else:
            # 交给抽象方法处理
    ...
    async def _handle_stop_agent(self, message: AgentMessage):
        """处理停止Agent消息"""
        self.logger.info(f"Agent {self.agent_id} 收到停止信号，正在关闭")
        self.stop()
    ...
    async def _handle_time_tick(self, message: AgentMessage):
        ...
        try:
            ...
            self.logger.debug(f"处理时间同步: {time_tick.current_time}")
        except Exception as e:
            self.logger.error(f"处理时间同步消息失败: {e}")
    ...
    async def _handle_agent_status(self, message: AgentMessage):
        ...
        try:
            ...
            self.logger.debug(f"处理Agent状态: {status_message.agent_id} -> {status_message.status}")
        except Exception as e:
            self.logger.error(f"处理Agent状态消息失败: {e}")
    ...
    async def _handle_rpc_request(self, message: AgentMessage):
        ...
        try:
            ...
            self.logger.debug(f"收到RPC请求: {rpc_request.method_name}")
            ...
            self.logger.error(f"处理RPC请求失败: {e}")
            # 发送错误响应
            try:
                payload = message.payload or {}
                ...
                self.logger.error(f"发送RPC错误响应失败: {send_error}")
    ...
    async def _handle_rpc_response(self, message: AgentMessage):
        ...
        try:
            ...
            if correlation_id in self.pending_requests:
                ...
                self.logger.debug(f"解析RPC响应，correlation_id: {correlation_id}")
            else:
                self.logger.warning(f"收到未知correlation_id的RPC响应: {correlation_id}")

        except Exception as e:
            self.logger.error(f"处理RPC响应失败: {e}")
    ...
    async def _send_rpc_request(self, target_agent_id: str, rpc_request: RPCRequest):
        ...
        self.logger.debug(f"发送RPC请求到 {target_agent_id}.{rpc_request.method_name}")
    ...
    async def _execute_rpc_method(self, rpc_request: RPCRequest) -> RPCResponse:
        ...
        try:
            ...
            self.logger.error(f"执行RPC方法 '{method_name}' 失败: {e}")
            return RPCResponse(
                request_id=rpc_request.request_id,
                correlation_id=rpc_request.correlation_id,
                success=False,
    ...
    async def _send_rpc_response(self, response: RPCResponse, reply_to: str, correlation_id: str):
        """发送RPC响应"""
        if not self.channel:
            self.logger.error("Channel未初始化，无法发送RPC响应")
            return
        ...
        self.logger.debug(f"发送RPC响应，request_id: {response.request_id}")

    @abstractmethod
    async def _handle_regular_message(self, message: AgentMessage):
        """
        处理来自其他Agent的常规（非系统）消息

        必须由具体的Agent类实现，以定义Agent特定的消息处理行为。
    ...
    async def handle_time_tick(self, time_tick: TimeTickMessage):
        ...
        更新Agent的当前时间和tick ID。对于在确定性系统模式下
        保持所有Agent间一致的时间进展至关重要。

        Args:
            time_tick: 包含时间信息的消息
        """
        current_time_str = time_tick.current_time
        self.logger.debug(f"收到时间同步消息: {current_time_str}")
        ...
        try:
            ...
            self.logger.error(f"Agent {self.agent_id} 收到无效的时间同步格式: {current_time_str}, 错误: {e}")
    ...
    async def handle_agent_status(self, status_message: AgentStatusMessage):
        ...
        Args:
            status_message: Agent状态消息
        """
        self.logger.info(f"收到Agent状态更新: {status_message.agent_id} -> {status_message.status}")
    ...
    async def reconnect(self):
        ...
        try:
            ...
            raise AgentConnectionError(f"Reconnection failed: {e}")
    ...
    async def close(self):
        ...
        保证研究的可重现性。
        """
        self.logger.info(f"开始关闭Agent {self.agent_id}")

        # 取消消息消费者
        if self.agent_queue and self.agent_consumer_tag:
            try:
                await self.agent_queue.cancel(self.agent_consumer_tag)
                self.logger.info("已取消Agent消息消费")
            except Exception as e:
                self.logger.error(f"取消Agent消息消费失败: {e}")

        if self.time_queue and self.time_consumer_tag:
            try:
                await self.time_queue.cancel(self.time_consumer_tag)
                self.logger.info("已取消时间同步消息消费")
            except Exception as e:
                self.logger.error(f"取消时间同步消息消费失败: {e}")

        # 取消RPC消费者
        if self.rpc_queue and self.rpc_consumer_tag:
            try:
                await self.rpc_queue.cancel(self.rpc_consumer_tag)
                self.logger.info("已取消RPC请求消费")
            except Exception as e:
                self.logger.error(f"取消RPC请求消费失败: {e}")

        if self.reply_queue and self.reply_consumer_tag:
            try:
                await self.reply_queue.cancel(self.reply_consumer_tag)
                self.logger.info("已取消RPC响应消费")
            except Exception as e:
                self.logger.error(f"取消RPC响应消费失败: {e}")
        ...
        self.logger.debug("等待挂起的发布者确认完成")

        # 关闭连接
        if self.channel:
            try:
                await self.channel.close()
                self.logger.info("通道已关闭")
            except Exception as e:
        ...
        if self.connection and not self.connection.is_closed:
            try:
                ...
                self.logger.error(f"关闭RabbitMQ连接失败: {e}")
    ...
    async def run(self):
        ...
        try:
            await self.consume()
            self.logger.info(f"Agent {self.agent_id} 开始运行")
            await self._stop_event.wait()
        except Exception as e:
            self.logger.error(f"Agent运行时出错: {e}")
            raise
        finally:
            await self.close()
    ...
    def stop(self):
        ...
        设置关闭标志并触发停止事件以进行优雅的Agent终止。
        """
        self.logger.info(f"收到停止信号: Agent {self.agent_id}")
        self._shutdown = True
    ...
    async def process_time_tick(self, msg: Dict[str, Any]):
        ...
        委托给handle_time_tick方法进行Agent特定的时间处理逻辑。
...
# 便捷函数，用于创建Agent实例
async def create_agent_message_queue(
    agent_id: str,
    ...
    **kwargs,
    ...
    Args:
        agent_id: Agent ID
        message_handlers: 自定义消息处理器字典
        rabbitmq_host: RabbitMQ主机
        rabbitmq_port: RabbitMQ端口
        **kwargs: 其他初始化参数

    Returns:
        已初始化的AgentMessageQueue实例
    """
    # 这是一个工厂函数示例，实际使用时需要具体的实现类
    # 因为AgentMessageQueue是抽象类
    raise NotImplementedError(
        "AgentMessageQueue is abstract. Use a concrete implementation or create one.",
    )
...
async def create_rpc_client(
    ...
    **kwargs,
    ...
    Returns:
        已连接的AgentMessageQueue实例
    """
    # 由于AgentMessageQueue是抽象类，这里需要具体的实现
    # 在实际使用中，应该创建具体的子类实例
    raise NotImplementedError(
        "AgentMessageQueue is abstract. Create a concrete implementation that inherits from AgentMessageQueue.",
    )
...
async def create_rpc_server(
    ...
    **kwargs,
    ...
    Returns:
        已连接并注册方法的AgentMessageQueue实例
    """
    # 由于AgentMessageQueue是抽象类，这里需要具体的实现
    # 在实际使用中，应该创建具体的子类实例，然后注册方法
    raise NotImplementedError(
        "AgentMessageQueue is abstract. Create a concrete implementation that inherits from AgentMessageQueue.",
    )


class SimpleAgentMessageQueue(AgentMessageQueue):
    """
    简单的AgentMessageQueue实现，用于测试和示例
    """

    async def _handle_regular_message(self, message: AgentMessage):
        """处理常规消息的简单实现"""
        self.logger.info(f"收到常规消息: {message.message_type} (来自: {message.sender})")
        if message.payload:
            self.logger.debug(f"消息载荷: {message.payload}")
...
async def create_simple_agent(
    ...
    **kwargs,
    ...
    Returns:
        已初始化的SimpleAgentMessageQueue实例
    """
    agent = SimpleAgentMessageQueue(
        agent_id=agent_id,
...
async def create_simple_rpc_server(
    agent_id: str,
    methods: Dict[str, Callable],
    rabbitmq_host: str = "localhost",
    rabbitmq_port: int = 5672,
    **kwargs,
    ...
    Returns:
        已连接并注册方法的SimpleAgentMessageQueue实例
    """
    agent = SimpleAgentMessageQueue(
        agent_id=agent_id,
        rabbitmq_host=rabbitmq_host,
    ...
    for method_name, method in methods.items():
        agent.register_rpc_method(method_name, method)
