...
class OutputGenerator:
    ...
    def __init__(self, config: BrassConfig, storage: BrassStorage):
        ...
        # Initialize Best Practices Recommendation Engine
        self.best_practices_engine = BestPracticesRecommendationEngine(project_path=config.project_root)
    ...
    def _calculate_enterprise_readiness(self, security_issues, code_issues, todos, entities) -> Dict:
        ...
        critical_security = len([s for s in security_issues if s.get('data', {}).get('severity') == 'critical'])
    ...
    def generate_all_outputs(self) -> Dict[str, Path]:
        ...
        try:
            ...
            try:
                ...
                try:
                    observations_count = len(self.dcp.get_observations_by_type('code_issue'))
                    logger.error(f"ENTERPRISE_DEBUG_STATE: code_issues={observations_count}")
                    security_count = len(self.dcp.get_observations_by_type('security_issue'))
                    logger.error(f"ENTERPRISE_DEBUG_STATE: security_issues={security_count}")
                    todo_count = len(self.dcp.get_observations_by_type('todo'))
    ...
    def generate_privacy_report(self) -> Path:
        """
        Generate privacy analysis report using PrivacyReportGenerator.
        
        Returns:
            Path to generated .brass/PRIVACY_ANALYSIS.md file
        """
        try:
            from ..privacy import PrivacyReportGenerator
            
            # PrivacyReportGenerator expects project root, not .brass directory
            project_root = str(self.output_dir.parent)
            generator = PrivacyReportGenerator(project_root)
            
            # PrivacyReportGenerator.generate_report() returns string path
            report_path_str = generator.generate_report()
            report_path = Path(report_path_str)
            
            logger.info(f"Generated privacy analysis report: {report_path}")
            return report_path
            
        except Exception as e:
            import traceback
            logger.error(f"PRIVACY_REPORT_FAILURE: {type(e).__name__}: {e}")
            logger.error(f"PRIVACY_REPORT_STACK: {traceback.format_exc()}")
            # Re-raise to maintain current error handling behavior
            raise e
    ...
    def _deduplicate_by_location(self, observations):
        ...
        for obs in observations:
            ...
            if location_key not in location_map:
                location_map[location_key] = {
                    'primary_observation': copy.deepcopy(obs),
                    'consolidated_issues': [copy.deepcopy(data)],
                    'location': location_key
                }
            else:
                # Add to consolidated issues for this location
                location_map[location_key]['consolidated_issues'].append(copy.deepcopy(data))
        ...
        for location_data in location_map.values():
            ...
            if len(consolidated_issues) > 1:
                primary_data = primary_obs.get('data', {})
                primary_data['consolidated_issues'] = consolidated_issues
                primary_data['issue_count'] = len(consolidated_issues)
                primary_data['description'] = f"Multiple issues detected: {', '.join([issue.get('pattern_name', issue.get('description', 'Unknown')) for issue in consolidated_issues])}"
    ...
    def _build_all_data_types_analysis(self, todos, code_issues, security_issues, 
        ...
        # which creates separate .brass/PRIVACY_ANALYSIS.md files via PrivacyReportGenerator
        
        # 8. Best Practices Recommendations (Replacing old Strategic Recommendations)
        sections.append("## 🎯 Best Practices Recommendations\n")
        
        # Log integration with intelligence generation pipeline
        logger.info("OUTPUT_GENERATOR: Integrating evidence-based best practices system into intelligence pipeline")
        
        # Analyze project to generate recommendations
        ...
        recommendations = self.best_practices_engine.generate_recommendations(analysis, limit=6)
        
        # Format recommendations for output
        formatted_recs = self.best_practices_engine.format_recommendations_for_output(recommendations)
