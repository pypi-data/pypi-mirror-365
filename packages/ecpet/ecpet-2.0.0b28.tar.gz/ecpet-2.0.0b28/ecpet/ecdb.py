...
def ingest_toa5(params):
    ...
    :param params: Tuple of (filename, station_name, table_name, free_id)
    :type params: tuple
    ...
    if ecfile.toa5_check(infile):
            ...
            for retry in range(MAX_RETRIES):
                try:
                    ...
                    if "UNIQUE constraint failed" in str(
                        ...
                        logger.error(
                            f'Failed to insert file after {MAX_RETRIES} '
                            f'attempts: {e}')
                        conn.close()
                        return
        ...
        response = table_prepare(cur, dbtable, dbcolumns, dbtypes)
        if len(response) > 1:
            logger.critical(
                "ERROR: multiple matching tables. Fix the database.")
            conn.close()
            return
...
def ingest_df(df, station_name=None, table_name=None, time_column=None):
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            # Process DataFrame (better error handling)
            dw = df.copy()
            ...
            for col in tab_skeleton_cols:
                if col not in dw.columns:
                    if col == 'TIMESTAMP':
                        if time_column and time_column in dw.columns:
                            dw['TIMESTAMP'] = [
                                timestamp_fill(
                            ...
                            logger.error(
                                ...
                                'TIMESTAMP or time_column mut be a '
                                'valid column')
                            raise ValueError('Missing time column')
                    elif col == 'RECORD':
                        dw['RECORD'] = [x + 1 for x in range(len(df.index))]
                    elif col in ['head_id', 'file_id']:
                        dw[col] = ""
                else:
                    logger.warning(f"{col} not in df but in "
            ...
            try:
                dw.to_sql(dbtable.replace(':', '_'), conn,
                ...
                conn.rollback()
                logger.debug(
                    f'to_sql failed, using manual insertion: {sql_error}')
                # Manual insertion
                nl = 0
                to_db = []
                ...
                conn.commit()
                logger.info(
                    f'Successfully ingested {len(dw)} rows (manual method)')
                return True
            ...
            if any(keyword in error_msg for keyword in
                   ['locked', 'busy', 'constraint']):
                if attempt < MAX_RETRIES - 1:
                    import time
                    wait_time = 0.1 * (2 ** attempt)  # Exponential backoff
                    logger.warning(
                        f'Database conflict on attempt {attempt + 1}, waiting {wait_time}s')
...
def ingest(infiles, force=False, nproc=0, station_name=None,
    ...
    if nproc == 1:
        ...
        # Windows-specific: Use spawn method for multiprocessing
        ...
        if nproc == 0:
            pool = Pool(initializer=init_worker_process, initargs=(dbfile,))
        else:
            pool = Pool(nproc, initializer=init_worker_process)
        # try:
        for result in pool.imap(ingest_toa5, pro_para):
...
def conf_to_db(conf):
    ...
    :param conf: Configuration object to store
    :type conf: object

    Persists processing configuration for reproducibility and checkpointing.
    """
    #
    try:
        conn = sqlite3.connect(dbfile)
    except:
        logger.critical('unable to open project file %s' % dbfile)
        sys.exit(1)
    cur = conn.cursor()
    #
    # test if table already exists, create it if not
    ...
    for k, v in conf.items():
        ...
        vv = ''.join(x if x != '"' else '""' for x in v)
        values = '"' + '","'.join((k, vv)) + '"'
        sql = 'INSERT OR REPLACE INTO "%s" (%s) VALUES (%s);' % (
