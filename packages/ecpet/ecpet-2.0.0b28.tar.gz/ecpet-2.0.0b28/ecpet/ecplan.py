...
def planarfit_to_file(conf, intervals):
    ...
    #
    plfitpath = conf.get('Parmdir')
    plfitbase = conf.get('PlfName')
...
def planarfit(conf, intervals):
    ...
    plcalc = pd.Series()
    pfvalid = conf.get('PFValid', 'Par', kind='float', unlist=True, na=0.)
    try:
