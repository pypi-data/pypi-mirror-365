    ...
    class Page_Columns(wx.adv.WizardPageSimple):
        ...
        def get_column_info(self):
            ...
            header = ecfile.toa5_get_header(os.path.join(
                self.wizard.guilist.get('RawDir'), file))
...
#            try:
        ...
        def freq_guess(self, event):
            ...
            fs = ecfile.toa5_get_times(files[0], count=True)
...
#            time_last = dateutil.parser.parse(fs[1][0:16]+':00 UTC')
        ...
        def Leave(self, event):
            if self.radio_auto.GetSelection() == 1 and not self.time_check():
                ErrorBox('beginning date/time is not earlier than end date/time')
                ok = False
            else:
                ok = True
            if ok and not self.freq_check():
                ok = WarningBox('Warning: frequency out of range 0.1..50 Hz')
            else:
                ok = True
            if ok:
                self.store_settings()
