...
logger = logging.getLogger('ragflow_mcp_server_continue')
...
async def serve() -> Server:
    server = Server("ragflow-mcp-server-v0-2-0")
    ...
    async def handle_call_tool(
        ...
        if name == "list_datasets":
            ...
            return [types.TextContent(type="text", text=f"与检索相关的内容")]
            ...
            # try:
            #     result = ragflow.retrieve(dataset_id, question)
            #     return [types.TextContent(type="text", text=json.dumps(result, ensure_ascii=False, indent=2))]
            # except Exception as e:
            #     return [types.TextContent(type="text", text=f"Error: {str(e)}")]
    ...
    async def _run():
        ...
        async with mcp.server.stdio.stdio_server() as (read_stream, write_stream):
            ...
            await server.run(
                ...
                write_stream,
                InitializationOptions(
                    server_name="ragflow-mcp-server-v0-2-0",
