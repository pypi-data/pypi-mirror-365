...
def _is_time_valid(hour: int, minute: int, second: int, nanosecond: int) -> bool:
    ...
    def get_quarter(self) -> int:
        """
        Returns the quarter of the year based on Persian month.
        Quarter 1 = Farvardin–Khordad (0–2)
        Quarter 2 = Tir–Shahrivar (3–5)
        Quarter 3 = Mehr–Azar (6–8)
        Quarter 4 = Dey–Esfand (9–11)
        """
        return (self.tm_mon // 3) + 1

    def get_week_number(self) -> int:
        """
        Returns the simple week number in the Persian year.
        Week starts on Shanbeh (Saturday), and Farvardin 1 is always in week 1.
        """
        return (self.tm_yday // 7) + 1

    def get_week_number_iso(self) -> int:
        """
        Returns an ISO-like week number for Persian calendar.
        Week starts on Shanbeh (Saturday), and the first week must contain at least 4 days.
        """
        first_day_of_year = from_persian_components(self.tm_year, 0, 1)
        if first_day_of_year:
            first_wday = first_day_of_year.tm_wday  # 0 = Shanbeh
            offset = (7 - first_wday) % 7
            if offset < 4:
                start_day = 0
            else:
                start_day = 7 - first_wday
            week_num = ((self.tm_yday - start_day) // 7) + 1
            return max(1, week_num)
        return 1

    def is_weekend(self) -> bool:
        """
        Returns True if the date falls on the Iranian weekend (Thursday or Friday).
        """
        return self.tm_wday in (5, 6)

    def week_start_date(self) -> 'Tm':
        """
        Returns a new Tm object representing the start of the week (Shanbeh) for the current date.
        """
        start_timestamp = self.to_timestamp() - (self.tm_wday * 86400)
        return at_utc(start_timestamp)

    def is_workday(self) -> bool:
        """
        Returns True if the date is a workday (Saturday to Wednesday).
        """
        return self.tm_wday in (0, 1, 2, 3, 4)

    def days_in_month(self) -> int:
        """
        Returns the number of days in the current Persian month.
        """
        days_in_month = [
            31, 31, 31, 31, 31, 31,
            30, 30, 30, 30, 30, 29
        ]
        if self.tm_mon == 11 and self.is_leap():
            return 30
        return days_in_month[self.tm_mon]

    def calendar_grid(self) -> list:
        """
        Returns a 6-week calendar grid (42 days) for the current Persian month.
        Each cell is a Tm object.
        """
        first_day = from_persian_components(self.tm_year, self.tm_mon, 1)
        if not first_day:
            return []

        delta_days = first_day.tm_wday
        start_timestamp = first_day.to_timestamp() - delta_days * 86400
        days = []
        for i in range(42):  # 6 weeks * 7 days
            day_tm = at_utc(start_timestamp + i * 86400)
            days.append(day_tm)
        return days
