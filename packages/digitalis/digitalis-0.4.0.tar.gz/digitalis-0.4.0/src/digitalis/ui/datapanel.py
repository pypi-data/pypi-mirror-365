...
class DataView(Widget, can_focus=True):
    ...
    topic: reactive[ChannelData | None] = reactive(None)
    ...
    async def watch_topic(self, topic: ChannelData | None) -> None:
    ...
    async def watch_data(self, data: ChannelMessage | str | None) -> None:
        ...
        schema = data.schema
        if schema is None:
            static.update(f"Unsupported schema: {schema}")
            switcher.current = static.id
            return
    ...
    async def action_switch_panel(
        ...
        if new_panel_type:
            ...
            if notify and isinstance(self.data, ChannelMessage):
