...
"""
    ...
    def update_diag(self, data: ChannelMessage) -> None:
        if data.schema != "diagnostic_msgs/msg/DiagnosticArray":
            return
        if data.channel_id != self._last_channel_id:
            # TODO: cache this?
            self.data.clear()
            self._newest_timestamp = 0
            self._last_channel_id = data.channel_id
        ...
        for status in data.message.status:
            ...
            self.data[key] = StaleStatusWrapper(
                status=status,
                is_stale=False,
