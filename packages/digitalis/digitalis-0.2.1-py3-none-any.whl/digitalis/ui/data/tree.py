...
from digitalis.reader.base import ChannelData, ChannelMessage
from digitalis.ui.data.base import BasePanel, Settings
...
class TreeView(Tree):
    ...
    def watch_data(self, channel_message: ChannelMessage | None) -> None:
        """Adds data from ChannelMessage to a node.

        Args:
            channel_message: ChannelMessage containing schema, timestamp, and message data.
        """
        ...
        def add_node(name: str, node: TreeNode, obj: Any, expand_depth: int) -> None:
            ...
            if not hasattr(obj, "__slots__"):
                ...
                if name:
                    label = Text.assemble(
                        Text.from_markup(f"[b]{name}[/b]="), highlighter(repr(obj))
                    )
                else:
                    label = Text(repr(obj))
        ...
        schema_name = "n/a"
        if channel_message.schema:
            schema_name = channel_message.schema
...
class LazyTree(Widget, BasePanel):
    data: reactive[ChannelMessage | None] = reactive(None)
    ...
    def watch_data(self, _channel_message: ChannelMessage | None) -> None:
        self._update_date()

    def on_message(self, message: ChannelMessage) -> None:
        pass

    def action_set_topic(self, topic: ChannelData) -> None:
        """Set the topic for this panel."""
        logging.info(f"Setting topic to {topic}")
