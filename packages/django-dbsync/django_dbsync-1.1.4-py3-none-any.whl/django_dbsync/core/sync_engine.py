        ...
        try:
            ...
            for col_name, col_info in model_columns.items():
                if col_name in existing_columns:
                    ...
                    try:
                        if engine == 'mysql':
                            self.inspector.cursor.execute("""
                                SELECT COLUMN_DEFAULT, COLUMN_KEY, EXTRA
                            ...
                            if row:
                                ...
                                db_unique = row[1] == 'UNI'
                                db_extra = row[2]
                                # If column is auto_increment, it's a PK
                    ...
                    # Unique sync (skip if primary key or redundant unique on PK)
                    if not is_primary_key and db_unique != model_unique:
                        # Check for redundant unique key on PK
                        if engine == 'mysql':
                            # Query for unique keys on this column
                            self.inspector.cursor.execute("""
                                SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
                                WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = %s AND COLUMN_NAME = %s AND NON_UNIQUE = 0
                            """, (table_name, col_name))
                            unique_count = self.inspector.cursor.fetchone()[0]
                            # If PK, skip unique
                            if unique_count > 0 and is_primary_key:
                                continue
                        if engine == 'mysql':
                            if model_unique:
