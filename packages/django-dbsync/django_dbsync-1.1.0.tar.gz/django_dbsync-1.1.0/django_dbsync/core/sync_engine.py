...
class SyncEngine:
    ...
    def find_existing_table_for_model(self, model):
        ...
        if expected_lower in table_lower_map:
            ...
            if actual_name != expected_table:
                ...
                # For MySQL, also check case-insensitive conflicts
                engine = self.inspector.get_database_engine()
                if engine == 'mysql':
                    # Check if there's a case-insensitive conflict
                    if expected_lower in table_lower_map and table_lower_map[expected_lower] != expected_table:
                        logger.warning(f"MySQL case-insensitive conflict: table '{actual_name}' exists, cannot rename to '{expected_table}'")
                        
                        # Generate manual SQL command for this conflict
                        if self.dry_run:
                            if engine == 'mysql':
                                manual_cmd = f"RENAME TABLE `{actual_name}` TO `{expected_table}`;"
                            elif engine == 'postgresql':
                                manual_cmd = f'ALTER TABLE "{actual_name}" RENAME TO "{expected_table}";'
                            elif engine == 'sqlite':
                                manual_cmd = f'ALTER TABLE "{actual_name}" RENAME TO "{expected_table}";'
                            else:
                                manual_cmd = f"-- Manual rename command for {engine}: {actual_name} -> {expected_table}"
                            
                            self.manual_commands.append({
                                'old_name': actual_name,
                                'new_name': expected_table,
                                'command': manual_cmd,
                                'reason': 'MySQL case-insensitive conflict resolution'
                            })
                    ...
                    if expected_lower in table_lower_map and table_lower_map[expected_lower] != expected_table:
                        ...
                        if self.dry_run:
                            ...
                            self.manual_commands.append({
                                ...
                                'new_name': expected_table,
                                'command': manual_cmd,
                                'reason': 'MySQL case-insensitive conflict resolution (default table)'
                            })
        ...
        for table in existing_tables:
            ...
            if table_lower in possible_old_names and table_lower != expected_lower:
                ...
                if engine == 'mysql':
                    # Check if there's a case-insensitive conflict
                    if expected_lower in table_lower_map and table_lower_map[expected_lower] != expected_table:
                        logger.warning(f"MySQL case-insensitive conflict: table '{table}' exists, cannot rename to '{expected_table}'")
                        
                        # Generate manual SQL command for this conflict
                        if self.dry_run:
                            if engine == 'mysql':
                                manual_cmd = f"RENAME TABLE `{table}` TO `{expected_table}`;"
                            elif engine == 'postgresql':
                                manual_cmd = f'ALTER TABLE "{table}" RENAME TO "{expected_table}";'
                            elif engine == 'sqlite':
                                manual_cmd = f'ALTER TABLE "{table}" RENAME TO "{expected_table}";'
                            else:
                                manual_cmd = f"-- Manual rename command for {engine}: {table} -> {expected_table}"
                            
                            self.manual_commands.append({
                                'old_name': table,
                                'new_name': expected_table,
                                'command': manual_cmd,
                                'reason': 'MySQL case-insensitive conflict resolution (possible old name)'
                            })
    ...
    def _rename_table(self, old_name, new_name):
        ...
        try:
            ...
            engine = self.inspector.get_database_engine()
            if engine == 'mysql':
                # MySQL is case-insensitive, so check if any table with same name (case-insensitive) exists
                table_lower_map = {table.lower(): table for table in existing_tables}
