...
class SyncEngine:
    ...
    def __init__(self, database_alias='default', dry_run=False, auto_approve=False):
        ...
        # Debug logging
        logger.info(f"SyncEngine initialized - dry_run: {dry_run}, auto_approve: {auto_approve}")
    ...
    def reset_dry_run(self):
        ...
        self.dry_run = False
        logger.info("Dry run mode reset to False")
                        ...
                        if needs_alter:
                            ...
                            if self.dry_run:
                                ...
                                try:
                                    ...
                                    logger.info(f"Verifying column change for '{col_name}' in '{table_name}'")
                                    ...
                                    if verify_result:
                                        ...
                                        if verify_match:
                                            ...
                                            if self._verify_column_change_effective(table_name, col_name, base_def, actual_def):
                                                logger.info(f"Column change verified as effective for '{col_name}'")
                                                if debug_mode:
                                                    print(f"✅ Column change verified as effective")
                                            else:
                                                logger.warning(f"Column change may not have been fully applied for '{col_name}'")
                                                if debug_mode:
                                        ...
                                        logger.warning(f"Could not get CREATE TABLE for verification of '{col_name}'")
                                        if debug_mode:
...
"""
        ...
        try:
            ...
            try:
                ...
                if "Cannot add or update a child row" in error_msg:
                    ...
                    logger.warning(user_friendly_msg)
                    if debug_mode:
    ...
    def _fix_existing_foreign_key_column(self, table_name, col_name, field):
        ...
        try:
            ...
            logger.info(f"Fixing FK: {table_name}.{col_name} -> {referenced_table}.{referenced_col}")
            if debug_mode:
            ...
            if engine == 'mysql':
                ...
                logger.info(f"Found {len(existing_constraints)} existing FK constraints to drop")
                if debug_mode:
                ...
                for constraint_name, ref_table in existing_constraints:
                    ...
                    try:
                        ...
                        logger.info(f"Successfully dropped FK {constraint_name}")
                        if debug_mode:
                            print(f"✅ Successfully dropped FK {constraint_name}")
                    except Exception as drop_error:
                        logger.error(f"Error dropping FK {constraint_name}: {drop_error}")
                        if debug_mode:
            ...
            logger.info(f"Adding new FK constraint for {table_name}.{col_name}")
            if debug_mode:
            ...
            if result:
                logger.info(f"Successfully fixed FK column '{col_name}' in '{table_name}'")
                if debug_mode:
                    print(f"✅ Successfully fixed FK column '{col_name}' in '{table_name}'")
            else:
                logger.error(f"Failed to fix FK column '{col_name}' in '{table_name}'")
                if debug_mode:
    ...
    def _check_foreign_key_constraint_exists(self, table_name, col_name, field):
        ...
        try:
            ...
            logger.info(f"Checking FK constraint: {table_name}.{col_name} -> {referenced_table}.{referenced_col}")
            ...
            if engine == 'mysql':
                ...
                constraints = self.inspector.cursor.fetchall()
                logger.info(f"Found {len(constraints)} FK constraints for {table_name}.{col_name}")
                ...
                for constraint_name, ref_table, ref_col in constraints:
                    logger.info(f"Constraint: {constraint_name} -> {ref_table}.{ref_col}")
                    ...
                    if ref_table == referenced_table and ref_col == referenced_col:
                        logger.info(f"Found matching FK constraint: {constraint_name}")
                        if debug_mode:
                            print(f"   ✅ Found matching FK constraint: {constraint_name}")
                        return True
                
                logger.info(f"No matching FK constraint found for {table_name}.{col_name} -> {referenced_table}.{referenced_col}")
                if debug_mode:
    ...
    def _verify_column_change_effective(self, table_name, col_name, expected_def, actual_def):
        ...
        try:
            ...
            logger.warning(f"Error verifying column change effectiveness: {e}")
            return False
