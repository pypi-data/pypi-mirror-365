        ...
        try:
            ...
            for col_name, col_info in model_columns.items():
                if col_name in existing_columns:
                    ...
                    # Extract the base type from the model definition (e.g., VARCHAR(100) from 'VARCHAR(100) NULL')
                    model_type = col_info['definition'].split()[0].upper()
                    # For MySQL, db_type may be e.g. 'int(11)', 'varchar(100)', so compare base type
            ...
            for col_name, col_info in model_columns.items():
                if col_name in existing_columns:
                    ...
                    # For DB default, need to query information_schema or pg_catalog
                    db_default = None
                    db_unique = False
                    try:
                        if engine == 'mysql':
                    ...
                    if model_default is not None and not callable(model_default):
                        ...
                        db_default_norm = str(db_default) if db_default is not None else None
                        model_default_norm = str(model_default)
