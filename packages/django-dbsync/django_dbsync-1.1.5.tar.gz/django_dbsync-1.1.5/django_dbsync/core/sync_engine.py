        ...
        try:
            ...
            for col_name, col_info in model_columns.items():
                if col_name in existing_columns:
                    ...
                    try:
                        if engine == 'mysql':
                            ...
                            if row:
                                ...
                                if db_extra and 'auto_increment' in db_extra.lower():
                                    is_primary_key = True
                        elif engine == 'postgresql':
                            self.inspector.cursor.execute("""
                    ...
                    # Build full column definition for MySQL
                    if engine == 'mysql':
                        # Always use MODIFY COLUMN with full definition for any change
                        full_def = col_info['definition']
                        # Add NULL/NOT NULL
                        if model_null:
                            full_def += ' NULL'
                        else:
                        ...
                        if model_default is not None and not callable(model_default):
                            if isinstance(model_default, bool):
                                default_val = 1 if model_default else 0
                            else:
                                default_val = model_default
                            full_def += f' DEFAULT {repr(default_val)}'
                        # Only apply if any difference
                        needs_alter = False
                        if db_null is not None and model_null is not None and db_null != model_null:
                            needs_alter = True
                        if db_default_norm != model_default_norm:
                            needs_alter = True
                        # Type difference already handled above, but ensure we catch all
                        ...
                        if db_type_base != model_type_base or db_type.upper() != model_type:
                            needs_alter = True
                        if needs_alter:
                            alter_query = f'ALTER TABLE `{table_name}` MODIFY COLUMN `{col_name}` {full_def}'
                            if self.dry_run:
                                logger.info(f"[DRY RUN] Would alter column '{col_name}' in '{table_name}' to: {full_def}")
                                result['actions'].append(f"Would alter column '{col_name}' in '{table_name}' to: {full_def}")
                            else:
                                try:
                                    with transaction.atomic(using=self.database_alias):
                                        self.inspector.cursor.execute(alter_query)
                                    logger.info(f"Altered column '{col_name}' in '{table_name}' to: {full_def}")
                                    result['actions'].append(f"Altered column '{col_name}' in '{table_name}' to: {full_def}")
                                except Exception as e:
                        ...
                        # For each difference, use the appropriate ALTER COLUMN
                        if db_null is not None and model_null is not None and db_null != model_null:
            ...
            for col_name, col_info in model_columns.items():
                if col_name in existing_columns:
                    ...
                    field_type = type(field).__name__
                    if field_type in ['ForeignKey', 'OneToOneField']:
                        # Check for actual FK constraint in DB
                        try:
                            engine = self.inspector.get_database_engine()
                            related_model = field.related_model
                            related_table = related_model._meta.db_table
                            related_column = related_model._meta.pk.column
                            fk_exists = False
                            if engine == 'mysql':
                                self.inspector.cursor.execute("""
                                ...
                                """, (table_name, col_name, related_table, related_column))
                                fk_exists = self.inspector.cursor.fetchone()[0] > 0
                            elif engine == 'postgresql':
                                self.inspector.cursor.execute("""
                                    SELECT COUNT(*) FROM information_schema.key_column_usage
                                    WHERE table_name = %s AND column_name = %s
