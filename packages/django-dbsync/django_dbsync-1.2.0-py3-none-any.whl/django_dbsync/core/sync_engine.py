...
"""
    ...
    def _defaults_equivalent(self, db_default, model_default, model_type):
        ...
        if model_type.upper() in ['BOOLEAN', 'TINYINT(1)']:
            if model_default is False and (db_default is None or str(db_default).lower() in ['0', 'false', 'none']):
                return True
            if model_default is True and (db_default is None or str(db_default).lower() in ['1', 'true', 'none']):
                return True
