...
class Std_Json_Sampler:
    ...
    """
    def __init__(self, data: List[Dict] | Std_Json, cate_field: str, is_multi_label: bool = False, splitter: str = '|') -> None:
        """
        初始化采样器，按指定字段对数据进行分类
    ...
    def sample(self, 
        ...
        Args:
            ...
            seed: 随机种子，用于复现采样结果
            insufficient_strategy: 当类别样本不足时的处理策略，'error'（默认）或 'warn'
        ...
        if n is not None:
            ...
            for category, count in tqdm(n.items(),desc="Sampling..."):
                ...
                # 计算实际采样数量
                actual_count = self._calculate_sample_count(count, total_in_category)
                
                # 检查样本是否充足并处理
                if actual_count > total_in_category:
                ...
                # 从该类别中随机采样
                samples = random.sample(self.data_by_category[category], actual_count)
                ...
                print(f"类别 '{category}' 采样数量: {actual_count}")
                sampled_cate_counts[category] = actual_count
