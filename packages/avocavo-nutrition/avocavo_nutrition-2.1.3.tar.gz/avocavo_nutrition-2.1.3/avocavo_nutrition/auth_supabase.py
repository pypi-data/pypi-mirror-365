...
class SupabaseAuthManager:
    ...
    def _get_supabase_config(self) -> bool:
        ...
        try:
            ...
            try:
                response = requests.get(f"{self.base_url}/api/auth/supabase-config", timeout=10)
            except requests.exceptions.SSLError:
                # Fallback for SSL certificate issues
                print("⚠️  SSL certificate issue detected, using fallback...")
                response = requests.get(f"{self.base_url}/api/auth/supabase-config", timeout=10, verify=False)
                ...
                if parsed_url.path == '/callback':
                    ...
                    if error:
                        ...
                        try:
                            ...
                            # Check if this is the common NULL field scanning error
                            error_str = str(e)
                            if 'converting NULL to string is unsupported' in error_str or 'recovery_token' in error_str:
                                self.server.callback_result = {'success': False, 'error': 'Supabase NULL field error - use legacy auth instead'}
                            else:
