...
class ChurnModelBuilder:
    ...
    def build_lightgbm(self):
        """
        Builds 5 LightGBM models with different hyperparameter variants and class imbalance handling.

        Returns:
            List of (params_dict, trained_model)
        """
        # Step 1: Compute scale_pos_weight from training data
        pos_count = np.sum(self.y_train == 1)
        neg_count = np.sum(self.y_train == 0)
        scale_pos_weight = round(neg_count / pos_count, 3)
        ...
        # Step 3: Add fixed parameters to each combination
        models = []
        for params in combinations:
            full_params = {
                **params,
            ...
            model.fit(self.X_train, self.y_train)
            models.append((full_params, model))
