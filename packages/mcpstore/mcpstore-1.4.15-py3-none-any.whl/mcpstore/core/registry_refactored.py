...
class ServiceRegistry:
    ...
    这是MCPStore相对于FastMCP的独特价值：
    - 每个Agent都有独立的服务空间
    - Store级别使用main_client_id作为特殊的Agent
    - 完全隔离的多租户架构
    
    重构后简化设计：
    - 直接使用FastMCP Client，移除复杂的会话抽象
    - 保留Agent级别隔离的核心价值
    - 简化数据结构，提高性能
    - 移除重复的连接管理，依赖FastMCP的连接生命周期
    """
    ...
    def __init__(self):
        ...
        # agent_id -> {tool_name: service_name} - 工具到服务的映射
