...
class ConfigProcessor:
    """
    增强版配置处理器：处理用户配置和FastMCP配置之间的转换
    
    设计理念：
    1. 对用户极度友好：支持多种输入格式
    2. 对FastMCP严格：确保格式完全符合要求
    3. 智能推断：自动处理transport、环境变量等
    4. 错误友好：提供清晰的错误信息和建议
    """
    ...
    def _get_service_config(cls, service_name: str) -> Dict[str, Any]:
        ...
        if service_name in cls.KNOWN_SERVICES:
            logger.debug(f"Using known configuration for service: {service_name}")
            return deepcopy(cls.KNOWN_SERVICES[service_name])
