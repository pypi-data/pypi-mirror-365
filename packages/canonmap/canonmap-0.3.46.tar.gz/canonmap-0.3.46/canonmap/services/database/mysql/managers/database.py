...
class DatabaseManager:
    ...
    def create_database(
        ...
        with get_cursor(conn) as cursor:
            ...
            if already_exists:
                logger.info(f"Database '{database_name.database_name}' already exists")
            else:
                logger.info(f"Database '{database_name.database_name}' does not exist, creating it...")
                cursor.execute(f"CREATE DATABASE `{database_name.database_name}`")
                if self.autocommit:
                    conn.commit()
                logger.info(f"Database '{database_name.database_name}' created successfully")
        
        self.connection_manager.database = database_name.database_name
