...
async def connect_remote(
    ...
    async def _try_get_service():
        try:
            server = await asyncio.wait_for(_get_server(), server_timeout)
        except asyncio.TimeoutError:
            logger.debug(f"Failed to connect to servers: {server_urls}")
            return
        try:
            service = await asyncio.wait_for(_get_service(server), service_timeout)
            logger.debug(f"Service {service_name_or_id} is available on servers: {server_urls}")
            return service
        except asyncio.TimeoutError:
            logger.debug(f"Failed to get service {service_name_or_id} on servers: {server_urls}")
            return

    return await _try_get_service()
