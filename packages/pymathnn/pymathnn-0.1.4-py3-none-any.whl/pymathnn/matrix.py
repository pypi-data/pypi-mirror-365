...
class Matrix:
    def __init__(self, shape, init='zeros'):
    ...
    def sum(self, axis=None):
        return self.matrix.sum() if axis is None else self.matrix.sum(axis=axis)

    def multiply(self, scalar_or_matrix) -> 'Matrix':
        if isinstance(scalar_or_matrix, (int, float)):
            try:
                risultato = Matrix(self.shape)
                risultato.matrix = self.matrix * float(scalar_or_matrix)
            except ValueError:
                raise TypeError(f"The scalae '{scalar_or_matrix}' can not converted to float.")
            return risultato

        elif isinstance(scalar_or_matrix, Matrix):
            if self.shape[-1] != scalar_or_matrix.shape[0]:
                raise ValueError(f"Shape incompatibile per prodotto: {self.shape} @ {scalar_or_matrix.shape}")

            result_shape = (self.shape[0], scalar_or_matrix.shape[1]) if len(self.shape) == 2 else (scalar_or_matrix.shape[1],)
            risultato = Matrix(result_shape)
            risultato.matrix = self.matrix @ scalar_or_matrix.matrix
            return risultato

        else:
            raise TypeError(f"Tipo non supportato per multiply(): {type(scalar_or_matrix)}")
    ...
    def transpose(self):
        ...
        result = Matrix((cols, rows))  # la nuova matrice trasposta
    ...
    def reshape(self, shape):
        ...
        total_required = shape[0] * shape[1]

        # 🔒 Protezione contro tipi non iterabili
        if isinstance(self.matrix, (float, int, np.float64, np.int64)):
            flat = [float(self.matrix)]
        elif isinstance(self.matrix, np.ndarray):
            flat = self.matrix.flatten().tolist()
        elif isinstance(self.matrix, list):
            flat = [item for row in self.matrix for item in row]
        else:
            raise TypeError(f"❌ Tipo non supportato per reshape: {type(self.matrix)}")

        # 🩹 Padding o taglio
        if len(flat) < total_required:
            flat += [0.0] * (total_required - len(flat))
        else:
            flat = flat[:total_required]

        # 🔁 Ricostruzione
        reshaped = [flat[i * shape[1]:(i + 1) * shape[1]] for i in range(shape[0])]
        ...
        if nome == "softmax":
            ...
            for i in range(self.shape[0]):
                for j in range(self.shape[1]):
                    self.matrix[i][j] = activations[nome](self.matrix[i][j])
        return self
    ...
    def deactivation(self, nome: str):
        ...
        if nome == "softmax":
            self.matrix = deactivations[nome](self.matrix)  # Assume compatibilità con vettori/matrici
        else:
            for i in range(self.shape[0]):
                for j in range(self.shape[1]):
                    self.matrix[i][j] = deactivations[nome](self.matrix[i][j])
        return self
