...
class AgentServer:
    ...
    def _add_routes(self) -> None:
        ...
        @self.router.get(f"/{agent_name}/v1/health")
        async def health_check() -> Dict[str, Any]:
            return {"status": "healthy", "timestamp": time.time()}
        
        @self.router.get(f"/{agent_name}/v1/info", response_model=AgentInfo)
        ...
        if not callable(controller_fn):
            raise ValueError(f"Agent {self.agent_instance.name} does not have an 'invoke' method")
        
        self.router.add_api_route(
            path=f"/{agent_name}/v1/invoke",
