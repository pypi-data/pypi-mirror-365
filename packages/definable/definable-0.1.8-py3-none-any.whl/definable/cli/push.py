...
@click.option(
    ...
    help="Deployment server URL (default: http://localhost:8080)",
)
@click.option("--name", "-n", help="Agent name (defaults to name from agent.yaml)")
@click.option(
    "--config",
    "-c",
    default="agent.yaml",
...
def push_command(
    deployment_server: str, name: Optional[str], config: str, check: bool
) -> None:
    ...
    try:
        ...
        if not agent_name:
            try:
                ...
                click.echo(
                    "Warning: Could not load agent name from config. Using 'unnamed-agent'"
                )
