...
class Auth:
    ...
    async def change_password(self, current_password: str, new_password: str) -> bool:
        ...
        try:
            ...
            raise AuthenticationError(f"Password change request failed: {str(e)}")
    ...
    async def request_password_reset(self, email: str) -> Dict[str, str]:
        ...
        try:
            response = await self._client.post(
                f"{self.base_url}/api/v1/auth/password-reset",
                json={"email": email},
            )
            response.raise_for_status()
            return response.json()
            ...
            raise AuthenticationError(f"Password reset request failed: {str(e)}")
