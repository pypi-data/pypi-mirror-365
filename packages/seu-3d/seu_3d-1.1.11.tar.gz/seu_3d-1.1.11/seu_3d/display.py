    ...
    def selectXY_tab(self):
        ...
        def select_xyz():
            ...
            self.show_x_range = widgets.RangeSlider(
                value=(x_min, x_max), min=x_min - (abs(x_min) * 0.9), max=x_max * 1.1, step=0.1, label="X Range"
            )
            self.show_y_range = widgets.RangeSlider(
                value=(y_min, y_max), min=y_min - (abs(y_min) * 0.9), max=y_max * 1.1, step=0.1, label="Y Range"
            )
            self.show_z_range = widgets.RangeSlider(
                value=(z_min, z_max), min=z_min - (abs(z_min) * 0.9), max=z_max * 1.1, step=0.1, label="Z Range"
            )
