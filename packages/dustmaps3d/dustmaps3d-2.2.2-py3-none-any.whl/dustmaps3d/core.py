...
def load_data():
    ...
    def msg(english, *args):
        if is_china_user():
            translations = {
                "Downloading": f"[dustmaps3d] 正在下载 {GZ_FILENAME}（约400MB）...",
                "Trying primary": "[dustmaps3d] 正在尝试主源（GitHub）...",
                "Primary failed": "[dustmaps3d] 主源失败，尝试备用源（NADC）...",
                "Saved": "[dustmaps3d] ✅ 下载完成，保存于: {}",
                "Extracting": "[dustmaps3d] 解压文件中: {}",
                "Uncompressed": "[dustmaps3d] ✅ 解压完成，保存为: {}",
                "Invalid FITS file": "[dustmaps3d] 无效的 FITS 文件: {}",
                "Not a valid gzip": "[dustmaps3d] ❌ 非法 gzip 文件，可能是错误网页",
                "Decompress failed": "[dustmaps3d] 解压失败: {}",
                "Invalid after extract": "[dustmaps3d] 解压后仍无法读取，文件无效。",
                "Download failed": "[dustmaps3d] 下载失败 (尝试 {}/{}): {}",
                "Both sources failed": "[dustmaps3d] 无法从 GitHub 和 NADC 下载数据\n主源错误: {}\n备用源错误: {}",
                "Server resume failed": "[dustmaps3d] 服务器不支持断点续传，重试下载。"
            }
            return translations.get(english, english).format(*args)
        else:
            defaults = {
                "Downloading": f"[dustmaps3d] Downloading {GZ_FILENAME} (~400MB)...",
                "Trying primary": "[dustmaps3d] Trying primary source (GitHub)...",
                "Primary failed": "[dustmaps3d] Primary source failed. Trying backup (NADC)...",
                "Saved": "[dustmaps3d] ✅ Data has been saved to: {}",
                "Extracting": "[dustmaps3d] Extracting {} ...",
                "Uncompressed": "[dustmaps3d] ✅ Uncompressed to: {}",
                "Invalid FITS file": "[dustmaps3d] Invalid FITS file: {}",
                "Not a valid gzip": "[dustmaps3d] ❌ Downloaded file is not a valid gzip file. Possibly an error page.",
                "Decompress failed": "[dustmaps3d] Failed to decompress file: {}",
                "Invalid after extract": "[dustmaps3d] Downloaded file is still invalid after extraction.",
                "Download failed": "[dustmaps3d] Download failed (attempt {}/{}): {}",
                "Both sources failed": "[dustmaps3d] Failed to download from both sources.\nPrimary: {}\nBackup: {}",
                "Server resume failed": "[dustmaps3d] Server didn't support resume. Restarting."
            }
            return defaults.get(english, english).format(*args)
            ...
            try:
                ...
                with requests.get(url, stream=True, headers=headers, timeout=30) as r:
                    if existing and r.status_code != 206:
                        print(msg("Server resume failed"))
                ...
                print(msg("Download failed", attempt, max_retries, e))
        else:
            raise RuntimeError(msg("Download failed", max_retries, max_retries, "max retries exceeded"))

        temp_file.rename(path)
        print(msg("Saved", path))
    ...
    if not LOCAL_DATA_PATH.exists() or not is_fits_valid(LOCAL_DATA_PATH):
        ...
        try:
            print(msg("Trying primary"))
            download_with_resume(primary_url, LOCAL_GZ_PATH)
        except Exception as e1:
            print(msg("Primary failed"))
            cleanup()
            try:
                download_with_resume(backup_url, LOCAL_GZ_PATH)
            except Exception as e2:
                cleanup()
                raise RuntimeError(msg("Both sources failed", e1, e2))

        if not is_valid_gzip(LOCAL_GZ_PATH):
            cleanup()
            raise RuntimeError(msg("Not a valid gzip"))

        try:
            print(msg("Extracting", LOCAL_GZ_PATH))
            with gzip.open(LOCAL_GZ_PATH, 'rb') as f_in, open(LOCAL_DATA_PATH, 'wb') as f_out:
                shutil.copyfileobj(f_in, f_out)
            LOCAL_GZ_PATH.unlink()
            print(msg("Uncompressed", LOCAL_DATA_PATH))
        except Exception as e:
            raise RuntimeError(msg("Decompress failed", e))

        if not is_fits_valid(LOCAL_DATA_PATH):
            cleanup()
            raise RuntimeError(msg("Invalid after extract"))

    return Table.read(LOCAL_DATA_PATH, format='fits').to_pandas()
