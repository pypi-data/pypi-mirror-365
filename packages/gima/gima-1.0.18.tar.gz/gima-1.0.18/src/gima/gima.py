...
def prompt(gitRepo, readCommand = False):
    ...
    prompt = Settings.PREFIX_LINE\
            + Logger.BLUE + 'gim > ' + Logger.BLUE_END \
            + ('[' + (status).rjust(18, " ") + '] ' if status is not None else '') \
            + Logger.BLUE + str(path) + Logger.BLUE_END \
            + ' > '
    if readCommand:
...
def scan():
    ...
    for subdir, dirs, files in os.walk(currentDir):
        ...
        if gitRepo.isGitRepo():
            ...
            # else:
                Logger.logInfo("{} > [A]dd/[I]gnore/ignore [R]ecursivelly/[S]kip for now".format(Logger.BLUE + subdir + Logger.BLUE_END))
...
def groupOneRepo(repoPath):
    ...
    prompt = Settings.PREFIX_LINE\
            + Logger.BLUE + 'gim > ' + Logger.BLUE_END \
            + Logger.BLUE + str(repoPath) + Logger.BLUE_END \
