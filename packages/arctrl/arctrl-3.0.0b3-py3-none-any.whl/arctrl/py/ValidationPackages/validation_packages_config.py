...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        ...
        def _arrow3352(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow3351(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3350(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3349(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow3348(__unit: None=None) -> IEnumerable_1[str]:
                            def _arrow3347(__unit: None=None) -> IEnumerable_1[str]:
                                return singleton("}")

                            return append(singleton("]"), delay(_arrow3347))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow3348))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow3349))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow3350))

            return append(singleton("{"), delay(_arrow3351))

        return join("\n", to_list(delay(_arrow3352)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow3355(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow3354(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow3354, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow3355))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr3356
