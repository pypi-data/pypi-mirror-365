...
class FsWorkbook(IDisposable):
    ...
    def RemoveWorksheet(self, name: str) -> None:
        self_1: FsWorkbook = self
        def _arrow204(__unit: None=None) -> FsWorksheet:
            try: 
                def _arrow203(ws: FsWorksheet) -> bool:
                    return ws.Name == name

                return default_arg(try_find_1(_arrow203, self_1._worksheets), None)
        ...
        ignore(remove_in_place(_arrow204(), self_1._worksheets, ObjectExpr205()))
...
FsWorkbook_reflection = _expr206
