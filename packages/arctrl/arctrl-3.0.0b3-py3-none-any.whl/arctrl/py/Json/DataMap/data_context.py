...
def encoder(dc: DataContext) -> IEncodable:
    ...
    def _arrow2833(oa: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa)

    def _arrow2835(oa_1: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_1)

    def _arrow2840(oa_2: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_2)

    def _arrow2842(value: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2844(value_2: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2846(value_4: str, dc: Any=dc) -> IEncodable:
    ...
    values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([("data", encoder_1(dc)), try_include("explication", _arrow2833, DataContext__get_Explication(dc)), try_include("unit", _arrow2835, DataContext__get_Unit(dc)), try_include("objectType", _arrow2840, DataContext__get_ObjectType(dc)), try_include("description", _arrow2842, DataContext__get_Description(dc)), try_include("generatedBy", _arrow2844, DataContext__get_GeneratedBy(dc)), try_include("label", _arrow2846, DataContext__get_Label(dc))]))
...
def _arrow2857(get: IGetters) -> DataContext:
    data: Data
    object_arg: IRequiredGetter = get.Required
    data = object_arg.Field("data", decoder_1)
    def _arrow2850(__unit: None=None) -> OntologyAnnotation | None:
        object_arg_1: IOptionalGetter = get.Optional
        return object_arg_1.Field("explication", OntologyAnnotation_decoder)

    def _arrow2851(__unit: None=None) -> OntologyAnnotation | None:
        object_arg_2: IOptionalGetter = get.Optional
        return object_arg_2.Field("unit", OntologyAnnotation_decoder)

    def _arrow2853(__unit: None=None) -> OntologyAnnotation | None:
    ...
    def _arrow2856(__unit: None=None) -> str | None:
        object_arg_6: IOptionalGetter = get.Optional
        return object_arg_6.Field("generatedBy", string)

    return DataContext__ctor_Z780A8A2A(data.ID, data.Name, data.DataType, data.Format, data.SelectorFormat, _arrow2850(), _arrow2851(), _arrow2853(), _arrow2854(), _arrow2855(), _arrow2856(), data.Comments)


decoder: Decoder_1[DataContext] = object(_arrow2857)
