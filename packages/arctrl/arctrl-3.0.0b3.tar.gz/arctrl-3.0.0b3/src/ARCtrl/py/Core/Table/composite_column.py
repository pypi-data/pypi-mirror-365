...
class CompositeColumn(Record):
    ...
    def TryGetColumnUnits(self, __unit: None=None) -> Array[OntologyAnnotation] | None:
        this: CompositeColumn = self
        def _arrow635(__unit: None=None) -> IEnumerable_1[OntologyAnnotation]:
            def _arrow634(cell: CompositeCell) -> IEnumerable_1[OntologyAnnotation]:
                return singleton(cell.AsUnitized[1]) if cell.is_unitized else empty()

            return collect(_arrow634, this.Cells)

        arr: Array[OntologyAnnotation] = list(to_array(delay(_arrow635)))
...
CompositeColumn_reflection = _expr637
