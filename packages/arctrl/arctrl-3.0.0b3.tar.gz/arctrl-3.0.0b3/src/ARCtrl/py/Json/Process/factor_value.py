...
def ISAJson_encoder(id_map: Any | None, fv: FactorValue) -> IEncodable:
    def f(fv_1: FactorValue, id_map: Any=id_map, fv: Any=fv) -> IEncodable:
        ...
        def _arrow2402(value: str, fv_1: Any=fv_1) -> IEncodable:
        ...
        def _arrow2405(value_2: Factor, fv_1: Any=fv_1) -> IEncodable:
            return encoder_1(id_map, value_2)

        def _arrow2406(value_3: Value_1, fv_1: Any=fv_1) -> IEncodable:
            return encoder_2(id_map, value_3)

        def _arrow2407(oa: OntologyAnnotation, fv_1: Any=fv_1) -> IEncodable:
            return OntologyAnnotation_ISAJson_encoder(id_map, oa)

        values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([try_include("@id", _arrow2402, ISAJson_genID(fv_1)), try_include("category", _arrow2405, fv_1.Category), try_include("value", _arrow2406, fv_1.Value), try_include("unit", _arrow2407, fv_1.Unit)]))
    ...
    if id_map is not None:
        def _arrow2413(fv_3: FactorValue, id_map: Any=id_map, fv: Any=fv) -> str:
            return ISAJson_genID(fv_3)

        return encode(_arrow2413, f, fv, id_map)
...
def _arrow2424(get: IGetters) -> FactorValue:
    def _arrow2417(__unit: None=None) -> str | None:
        object_arg: IOptionalGetter = get.Optional
        return object_arg.Field("@id", Decode_uri)

    def _arrow2419(__unit: None=None) -> Factor | None:
        object_arg_1: IOptionalGetter = get.Optional
        return object_arg_1.Field("category", decoder_1)

    def _arrow2420(__unit: None=None) -> Value_1 | None:
    ...
    return FactorValue(_arrow2417(), _arrow2419(), _arrow2420(), _arrow2423())


ISAJson_decoder: Decoder_1[FactorValue] = object(_arrow2424)
