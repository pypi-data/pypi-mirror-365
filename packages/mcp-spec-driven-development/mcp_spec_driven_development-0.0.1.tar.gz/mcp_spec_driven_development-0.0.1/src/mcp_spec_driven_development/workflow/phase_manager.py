...
from .models import PhaseType, PhaseStatus, WorkflowState
...
class PhaseManager:
    ...
    def start_requirements_phase(self, feature_name: str) -> WorkflowState:
        ...
        if workflow.current_phase != PhaseType.REQUIREMENTS:
            raise ValueError(f"Cannot start requirements phase from {workflow.current_phase.value}")
    ...
    def complete_requirements_phase(self, feature_name: str) -> WorkflowState:
        ...
        if workflow.current_phase != PhaseType.REQUIREMENTS:
            raise ValueError(f"Cannot complete requirements phase from {workflow.current_phase.value}")
    ...
    def approve_requirements_phase(self, feature_name: str) -> WorkflowState:
        ...
        if workflow.current_phase != PhaseType.REQUIREMENTS:
            raise ValueError(f"Cannot approve requirements phase from {workflow.current_phase.value}")
    ...
    def start_design_phase(self, feature_name: str) -> WorkflowState:
        ...
        if workflow.current_phase != PhaseType.DESIGN:
            raise ValueError(f"Cannot start design phase from {workflow.current_phase.value}")
        
        if workflow.phase_status[PhaseType.REQUIREMENTS] != PhaseStatus.APPROVED:
            raise ValueError("Requirements phase must be approved before starting design")
    ...
    def complete_design_phase(self, feature_name: str) -> WorkflowState:
        ...
        if workflow.current_phase != PhaseType.DESIGN:
            raise ValueError(f"Cannot complete design phase from {workflow.current_phase.value}")
    ...
    def approve_design_phase(self, feature_name: str) -> WorkflowState:
        ...
        if workflow.current_phase != PhaseType.DESIGN:
            raise ValueError(f"Cannot approve design phase from {workflow.current_phase.value}")
    ...
    def start_tasks_phase(self, feature_name: str) -> WorkflowState:
        ...
        if workflow.current_phase != PhaseType.TASKS:
            raise ValueError(f"Cannot start tasks phase from {workflow.current_phase.value}")
    ...
    def complete_tasks_phase(self, feature_name: str) -> WorkflowState:
        ...
        if workflow.current_phase != PhaseType.TASKS:
            raise ValueError(f"Cannot complete tasks phase from {workflow.current_phase.value}")
    ...
    def approve_tasks_phase(self, feature_name: str) -> WorkflowState:
        ...
        if workflow.current_phase != PhaseType.TASKS:
            raise ValueError(f"Cannot approve tasks phase from {workflow.current_phase.value}")
    ...
    def can_transition_to_phase(self, feature_name: str, target_phase: PhaseType) -> bool:
        ...
        if target_phase == PhaseType.REQUIREMENTS:
            ...
            return (workflow.phase_status[PhaseType.REQUIREMENTS] == PhaseStatus.APPROVED and
                    workflow.phase_status[PhaseType.DESIGN] == PhaseStatus.APPROVED)
