...
class ValidationTools:
    ...
    async def handle_validate_document(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        if not document_type or not content:
            return [TextContent(type="text", text="Error: document_type and content are required")]
        ...
        try:
            ...
            if document_type == "requirements":
                ...
                return [TextContent(type="text", text=f"Invalid document type: {document_type}")]
            
            # Format validation results
            response = self._format_validation_results(document_type, results, feature_name)
    ...
    def _format_validation_results(self, document_type: str, results: List[ValidationResult], feature_name: str = "") -> str:
...
"""
        ...
        if errors:
            ...
            response += "3. **Re-validate** - Run validation again after making changes\n"
            response += "4. **Seek approval** - Present corrected document to user\n"
        elif warnings:
            response += "1. **Consider addressing warnings** - These improve document quality\n"
            response += "2. **Document is acceptable** - Can proceed with approval if warnings are acceptable\n"
            response += "3. **Present to user** - Get explicit approval before proceeding\n"
        else:
    ...
    async def handle_get_validation_checklist(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            return [TextContent(type="text", text=f"Error getting validation checklist: {str(e)}")]
...
"""
    ...
    async def handle_explain_validation_error(self, arguments: Dict[str, Any]) -> List[TextContent]:
        """Handle explain_validation_error tool call."""
        error_type = arguments.get("error_type", "")
        document_type = arguments.get("document_type", "")
        
        if not error_type or not document_type:
            return [TextContent(type="text", text="Error: error_type and document_type are required")]
        ...
        try:
            ...
            return [TextContent(type="text", text=f"Error explaining validation error: {str(e)}")]
...
"""
    ...
    async def handle_validate_requirement_traceability(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        if not requirements_content:
            return [TextContent(type="text", text="Error: requirements_content is required")]
...
"""
            ...
            if tasks_content:
                ...
                orphaned_refs = self._find_orphaned_task_references(req_ids, tasks_content)
                if orphaned_refs:
            ...
            if design_content and tasks_content:
                design_issues = sum(1 for covered in design_coverage.values() if not covered)
                task_issues = sum(1 for covered in task_coverage.values() if not covered)
                ...
                if design_issues == 0 and task_issues == 0:
                    ...
                    if design_issues > 0:
                        response += f"- {design_issues} requirements not addressed in design\n"
                    if task_issues > 0:
                        response += f"- {task_issues} requirements not referenced in tasks\n"
            ...
            return [TextContent(type="text", text=f"Error validating traceability: {str(e)}")]
    ...
    def _check_design_coverage(self, req_ids: List[str], design_content: str) -> Dict[str, bool]:
        ...
        for req_id in req_ids:
            ...
            covered = f"requirement {req_id}" in design_content.lower() or f"req {req_id}" in design_content.lower()
            coverage[req_id] = covered
        
        return coverage
    
    def _check_task_coverage(self, req_ids: List[str], tasks_content: str) -> Dict[str, bool]:
    ...
    def _find_orphaned_task_references(self, req_ids: List[str], tasks_content: str) -> List[str]:
