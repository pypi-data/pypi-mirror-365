...
class TaskValidator:
    ...
    NON_CODING_INDICATORS = [
        'user testing', 'user acceptance', 'deployment', 'production',
        'performance metrics', 'user training', 'documentation creation',
        'business process', 'marketing', 'communication', 'user feedback',
        'analyze usage', 'gather metrics', 'deploy to', 'user interview'
    ]
    
    # Vague language indicators
    VAGUE_INDICATORS = [
        'set up the system', 'make it work', 'implement properly',
        'add security', 'create responsive design', 'user management',
        'handle errors', 'optimize performance', 'improve user experience'
    ]
    
    # Actionable coding verbs
    CODING_VERBS = [
        'implement', 'create', 'write', 'build', 'add', 'develop',
        'code', 'program', 'construct', 'design', 'integrate',
        'test', 'validate', 'configure', 'setup', 'initialize'
    ]
    ...
    def validate(self, content: str, requirements_content: Optional[str] = None) -> List[ValidationResult]:
        """
        ...
        if requirements_content:
            results.extend(self._validate_requirements_traceability(tasks, requirements_content))
    ...
    def _parse_tasks(self, lines: List[str]) -> List[TaskItem]:
        ...
        for i, line in enumerate(lines):
            ...
            if match:
                ...
                if stripped_line.startswith('-') and not self.task_regex.match(stripped_line):
    ...
    def _validate_task(self, task: TaskItem, lines: List[str]) -> List[ValidationResult]:
    ...
    def _validate_task_actionability(self, task: TaskItem, section_name: str) -> List[ValidationResult]:
        ...
        for indicator in self.NON_CODING_INDICATORS:
            if indicator in full_text:
                results.append(ValidationResult(
                    ...
                    location=ValidationLocation(section=section_name, line=task.line_start),
    ...
    def _validate_task_requirements(self, task: TaskItem, section_name: str) -> List[ValidationResult]:
    ...
    def _validate_task_specificity(self, task: TaskItem, section_name: str) -> List[ValidationResult]:
        ...
        implementation_details = [detail for detail in task.details if not self.requirements_ref_regex.search(detail)]
        ...
        full_text = f"{task.title} {' '.join(task.details)}".lower()
        has_specifics = any(keyword in full_text for keyword in [
            'class', 'method', 'function', 'file', 'component', 'module',
            'interface', 'api', 'endpoint', 'database', 'table', 'model'
        ])
    ...
    def _validate_task_numbering(self, tasks: List[TaskItem]) -> List[ValidationResult]:
        ...
        for task in main_tasks:
            if task.number != str(expected_main):
                results.append(ValidationResult(
                    ...
                    location=ValidationLocation(section=f'Task {task.number}', line=task.line_start),
        ...
        for parent_num, subtasks in subtasks_by_parent.items():
            ...
            sorted_subtasks = sorted(subtasks, key=lambda t: float(t.number.split('.')[1]) if '.' in t.number else 0)
            expected_sub = 1
            for subtask in sorted_subtasks:
                ...
                if subtask.number != expected_number:
                    results.append(ValidationResult(
                        ...
                        location=ValidationLocation(section=f'Task {subtask.number}', line=subtask.line_start),
    ...
    def _validate_task_sequencing(self, tasks: List[TaskItem]) -> List[ValidationResult]:
        ...
        setup_keywords = ['setup', 'initialize', 'create structure', 'foundation', 'configure']
    ...
    def _validate_requirements_traceability(self, tasks: List[TaskItem], requirements_content: str) -> List[ValidationResult]:
