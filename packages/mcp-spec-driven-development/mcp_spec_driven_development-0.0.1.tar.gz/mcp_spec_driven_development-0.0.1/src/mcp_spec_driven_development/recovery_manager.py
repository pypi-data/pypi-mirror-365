...
from .workflow.models import PhaseType, PhaseStatus, WorkflowState
...
class StateSnapshot:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "phase_statuses": {phase.value: status.value for phase, status in self.phase_statuses.items()},
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'StateSnapshot':
        ...
        return cls(
            ...
            phase_statuses={PhaseType(k): PhaseStatus(v) for k, v in data["phase_statuses"].items()},
...
class RecoveryManager:
    ...
    def __init__(self, backup_dir: Optional[Path] = None, logger: Optional[logging.Logger] = None):
    ...
    def create_state_snapshot(self, workflow_state: WorkflowState, metadata: Optional[Dict[str, Any]] = None) -> StateSnapshot:
        ...
        if len(self._state_snapshots[workflow_state.feature_name]) > 10:
            self._state_snapshots[workflow_state.feature_name] = self._state_snapshots[workflow_state.feature_name][-10:]
        
        # Persist snapshot to disk
        self._persist_snapshot(snapshot)
        
        self.logger.info(f"Created state snapshot for {workflow_state.feature_name} at phase {workflow_state.current_phase.value}")
        return snapshot
    
    def rollback_to_snapshot(self, feature_name: str, snapshot_index: int = -1) -> StateSnapshot:
        """
        ...
        try:
            ...
            self.logger.info(f"Executing rollback plan for {feature_name}: {recovery_plan.description}")
    ...
    def execute_graceful_degradation(self, component: str, error: Exception) -> Dict[str, Any]:
        """
    ...
    def get_alternative_paths(self, failed_operation: str, context: Dict[str, Any]) -> List[str]:
        """
        ...
        return alternatives.get(failed_operation, [
    ...
    def _persist_snapshot(self, snapshot: StateSnapshot) -> None:
        ...
        try:
            ...
            snapshot_file = feature_dir / f"snapshot_{snapshot.timestamp.strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _handle_validation_recovery(self, error: SpecDrivenDevelopmentError) -> RecoveryPlan:
    ...
    def _handle_workflow_recovery(self, error: SpecDrivenDevelopmentError) -> RecoveryPlan:
    ...
    def _handle_content_recovery(self, error: SpecDrivenDevelopmentError) -> RecoveryPlan:
    ...
    def _create_generic_recovery_plan(self, error: SpecDrivenDevelopmentError) -> RecoveryPlan:
