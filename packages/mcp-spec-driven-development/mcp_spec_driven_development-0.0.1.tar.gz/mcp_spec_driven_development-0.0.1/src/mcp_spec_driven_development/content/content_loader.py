...
class ContentLoader:
    ...
    def __init__(self, logger: Optional[logging.Logger] = None):
        ...
        self.error_handler.register_fallback_handler(
            "content_methodology", 
            self.fallback_provider.get_fallback_methodology
        )
        self.error_handler.register_fallback_handler(
            "content_template", 
            self.fallback_provider.get_fallback_template
        )
        self.error_handler.register_fallback_handler(
            "content_example", 
            self.fallback_provider.get_fallback_example
        )
    ...
    def get_methodology_content(self, topic: str) -> str:
        ...
        try:
            ...
            if fallback_content:
                self.logger.warning(f"Using fallback methodology content for topic: {topic}")
    ...
    def get_template_content(self, template_type: str) -> str:
        ...
        try:
            ...
            # File doesn't exist, try fallback
            fallback_content = self.fallback_provider.get_fallback_template(template_type)
            if fallback_content:
                self.logger.warning(f"Using fallback template content for type: {template_type}")
                return fallback_content
    ...
    def get_example_content(self, example_type: str) -> str:
        ...
        try:
            ...
            if fallback_content:
                self.logger.warning(f"Using fallback example content for type: {example_type}")
                return fallback_content
    ...
    def validate_content_structure(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            if not self.data_dir.exists():
                issues["general"].append(f"Data directory does not exist: {self.data_dir}")
                return issues
            ...
            if not methodology_dir.exists():
                ...
                expected_files = ["workflow.md", "ears-format.md", "phase-transitions.md"]
                for expected_file in expected_files:
                    file_path = methodology_dir / expected_file
                    if not file_path.exists():
                        issues["methodology"].append(f"Missing methodology file: {expected_file}")
            ...
            if not templates_dir.exists():
                ...
                expected_templates = ["requirements-template.md", "design-template.md", "tasks-template.md"]
                for expected_template in expected_templates:
                    file_path = templates_dir / expected_template
                    if not file_path.exists():
                        issues["templates"].append(f"Missing template file: {expected_template}")
            ...
            if examples_dir.exists():
                ...
                if not example_files:
                    issues["examples"].append("Examples directory exists but contains no example files")
