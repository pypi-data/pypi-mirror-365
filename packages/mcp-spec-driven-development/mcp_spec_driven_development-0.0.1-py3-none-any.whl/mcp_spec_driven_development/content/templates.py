...
class TemplateRepository:
    ...
    def _load_builtin_templates(self) -> Dict[str, str]:
        ...
        templates[TemplateType.REQUIREMENTS.value] = self._load_template_file("requirements-template.md")
        templates[TemplateType.DESIGN.value] = self._load_template_file("design-template.md")
        templates[TemplateType.TASKS.value] = self._load_template_file("tasks-template.md")
        
        # Load Chinese templates
        templates[f"{TemplateType.REQUIREMENTS.value}-zh"] = self._load_template_file("requirements-template-zh.md")
        templates[f"{TemplateType.DESIGN.value}-zh"] = self._load_template_file("design-template-zh.md")
        templates[f"{TemplateType.TASKS.value}-zh"] = self._load_template_file("tasks-template-zh.md")
...
{% endfor %}"""
    ...
    def validate_template_format(self, template_type: TemplateType, content: str) -> bool:
        """Validate that content matches expected template format."""
        try:
            # Basic validation - check for required sections
            if template_type == TemplateType.REQUIREMENTS:
                return "# Requirements Document" in content and "## Requirements" in content
