...
class DesignValidator:
    ...
    def validate(self, content: str, requirements_content: Optional[str] = None) -> List[ValidationResult]:
        ...
        if requirements_content:
            results.extend(self._validate_requirements_traceability(content, requirements_content))
        ...
        for i in range(1, len(heading_levels)):
            ...
            if current_level > prev_level + 1:
                results.append(ValidationResult(
                    ...
                    location=ValidationLocation(section='document', line=current_line),
    ...
    def _validate_required_sections(self, sections: Dict[str, DesignSection]) -> List[ValidationResult]:
        ...
        for required_section in self.REQUIRED_SECTIONS:
            if required_section not in sections:
                ...
                results.append(ValidationResult(
                    ...
                    location=ValidationLocation(section=required_section, line=sections[required_section].line_start),
                    suggestion=f'Add content to the {required_section} section'
                ))
        
        return results
    
    def _validate_architecture_section(self, content: str, sections: Dict[str, DesignSection]) -> List[ValidationResult]:
        ...
        if not mermaid_diagrams:
            results.append(ValidationResult(
                ...
                location=ValidationLocation(section='Architecture', line=arch_section.line_start),
            ...
            for diagram in mermaid_diagrams:
                if 'graph' not in diagram.lower() and 'flowchart' not in diagram.lower():
                    results.append(ValidationResult(
                        type='warning',
    ...
    def _validate_components_section(self, sections: Dict[str, DesignSection]) -> List[ValidationResult]:
        ...
        if not component_matches:
            results.append(ValidationResult(
                ...
                location=ValidationLocation(section='Components and Interfaces', line=comp_section.line_start),
            ...
            for component_name in component_matches:
                ...
                component_section_text = self._extract_component_section(comp_content, component_name)
                
                if 'Purpose' not in component_section_text:
                    results.append(ValidationResult(
                        type='warning',
                        message=f'Component "{component_name}" missing purpose description',
                        location=ValidationLocation(section='Components and Interfaces'),
    ...
    def _validate_requirements_traceability(self, design_content: str, requirements_content: str) -> List[ValidationResult]:
