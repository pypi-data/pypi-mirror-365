...
from ..validation.task_validator import TaskValidator, TaskItem
...
class TaskExecutionTools:
            ...
            Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "issue_type": {
                            "type": "string",
                            "enum": ["unclear_requirements", "technical_difficulty", "dependency_issues", "testing_problems", "general"],
    ...
    async def handle_get_task_details(self, arguments: Dict[str, Any]) -> List[TextContent]:
        """Handle get_task_details tool call."""
        feature_name = arguments.get("feature_name", "")
        task_number = arguments.get("task_number", "")
        
        if not feature_name or not task_number:
            return [TextContent(type="text", text="Error: feature_name and task_number are required")]
        
        try:
            # Load task document
            tasks_content = self._load_tasks_document(feature_name)
            if not tasks_content:
                return [TextContent(type="text", text=f"No tasks document found for feature: {feature_name}")]
            ...
            if not task:
                return [TextContent(type="text", text=f"Task {task_number} not found in {feature_name}")]
...
"""
            ...
            if task.requirements_refs:
                ...
                if requirements_content:
                    for req_ref in task.requirements_refs:
                        req_context = self._extract_requirement_context(requirements_content, req_ref)
                        if req_context:
                            response += f"#### Requirement {req_ref}\n{req_context}\n\n"
                else:
                    response += "Requirements document not available for detailed context.\n"
            else:
            ...
            return [TextContent(type="text", text=f"Error getting task details: {str(e)}")]
    
    async def handle_get_task_context(self, arguments: Dict[str, Any]) -> List[TextContent]:
...
"""
            ...
            if design_content:
                response += "\n## Design Context\n"
                design_context = self._extract_relevant_design_context(design_content, task)
            ...
            if related_tasks:
                ...
                for related_task in related_tasks:
                    status = self._get_task_status_from_content(tasks_content, related_task.number)
            ...
            return [TextContent(type="text", text=f"Error getting task context: {str(e)}")]
    
    async def handle_get_task_dependencies(self, arguments: Dict[str, Any]) -> List[TextContent]:
...
"""
            ...
            if dependencies['prerequisites']:
                ...
                for prereq in dependencies['prerequisites']:
                    status = self._get_task_status_from_content(tasks_content, prereq.number)
                    status_icon = "✅" if status == "completed" else "⏸️" if status == "not_started" else "🔄"
            ...
            if dependencies['dependents']:
                ...
                for dependent in dependencies['dependents']:
                    status = self._get_task_status_from_content(tasks_content, dependent.number)
                    response += f"- **Task {dependent.number}**: {dependent.title} ({status})\n"
            else:
            ...
            if dependencies['parallel']:
                ...
                for parallel in dependencies['parallel']:
                    status = self._get_task_status_from_content(tasks_content, parallel.number)
                    response += f"- **Task {parallel.number}**: {parallel.title} ({status})\n"
            
            # Execution order guidance
            response += "\n### Execution Order Guidance\n"
            
            if dependencies['prerequisites']:
                incomplete_prereqs = [p for p in dependencies['prerequisites'] 
                                    if self._get_task_status_from_content(tasks_content, p.number) != "completed"]
            ...
            return [TextContent(type="text", text=f"Error analyzing task dependencies: {str(e)}")]
    
    async def handle_get_task_troubleshooting(self, arguments: Dict[str, Any]) -> List[TextContent]:
...
"""
            ...
            if issue_type == "unclear_requirements":
                ...
                response += self._get_dependency_troubleshooting(task, tasks, tasks_content)
...
"""
            ...
            return [TextContent(type="text", text=f"Error getting troubleshooting guidance: {str(e)}")]
    ...
    async def handle_list_tasks(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            if filter_status != "all":
                filtered_tasks = []
                for task in tasks:
                    task_status = self._get_task_status_from_content(tasks_content, task.number)
...
"""
            ...
            for main_task in main_tasks:
                status = self._get_task_status_from_content(tasks_content, main_task.number)
                ...
                if main_task.number in subtasks_by_parent:
                    ...
                    for subtask in subtasks_by_parent[main_task.number]:
                        sub_status = self._get_task_status_from_content(tasks_content, subtask.number)
                        sub_icon = self._get_status_icon(sub_status)
                        response += f"  - {sub_icon} {subtask.number}: {subtask.title} ({sub_status})\n"
            
            # Add summary statistics
            total_tasks = len(tasks)
            completed_count = sum(1 for task in tasks if self._get_task_status_from_content(tasks_content, task.number) == "completed")
            in_progress_count = sum(1 for task in tasks if self._get_task_status_from_content(tasks_content, task.number) == "in_progress")
...
"""
    ...
    async def handle_get_next_task(self, arguments: Dict[str, Any]) -> List[TextContent]:
...
"""
            ...
            task_status = self._get_task_status_from_content(tasks_content, next_task.number)
...
"""
            ...
            if task_status == "in_progress":
                response += "- This task is already in progress and should be completed\n"
            elif not dependencies['prerequisites']:
                response += "- No prerequisite tasks - can start immediately\n"
            else:
                completed_prereqs = [p for p in dependencies['prerequisites'] 
                                   if self._get_task_status_from_content(tasks_content, p.number) == "completed"]
...
"""
    ...
    def _load_requirements_document(self, feature_name: str) -> Optional[str]:
        ...
        try:
            return self.content_loader.load_spec_document(feature_name, PhaseType.REQUIREMENTS)
        except:
            return None
    ...
    def _load_design_document(self, feature_name: str) -> Optional[str]:
        ...
        try:
            return self.content_loader.load_spec_document(feature_name, PhaseType.DESIGN)
        except:
            return None
    
    def _find_task_by_number(self, tasks: List[TaskItem], task_number: str) -> Optional[TaskItem]:
    ...
    def _get_task_status_from_content(self, tasks_content: str, task_number: str) -> str:
    ...
    def _extract_requirement_context(self, requirements_content: str, req_ref: str) -> Optional[str]:
    ...
    def _extract_relevant_design_context(self, design_content: str, task: TaskItem) -> str:
        ...
        if relevant_sections:
            for section_title, section_content in relevant_sections[:2]:  # Limit to 2 most relevant
                context += f"\n{section_title}\n"
    ...
    def _find_related_tasks(self, tasks: List[TaskItem], current_task: TaskItem) -> List[TaskItem]:
        ...
        if current_task.is_subtask:
            for task in tasks:
                if task.parent_task == current_task.parent_task and task.number != current_task.number:
                    related.append(task)
...
"""
    ...
    def _analyze_task_dependencies(self, tasks: List[TaskItem], current_task: TaskItem) -> Dict[str, List[TaskItem]]:
        ...
        for task in tasks:
            ...
            if task_num < current_num:
                ...
                if current_task.is_subtask and task.parent_task == current_task.parent_task:
                    if task_num < current_num:
    ...
    def _get_recommended_sequence(self, tasks: List[TaskItem], current_task: TaskItem) -> List[str]:
        ...
        if current_task.is_subtask:
            sequence.insert(1, f"Ensure parent task {current_task.parent_task} context is understood")
            sequence.append("Verify integration with other subtasks")
        
        return sequence
    
    def _find_next_recommended_task(self, tasks: List[TaskItem], tasks_content: str) -> Optional[TaskItem]:
        ...
        for task in tasks:
            ...
            if status == "not_started":
                ...
                for prereq in dependencies['prerequisites']:
                    prereq_status = self._get_task_status_from_content(tasks_content, prereq.number)
    ...
    def _get_requirements_troubleshooting(self, task: TaskItem, feature_name: str) -> str:
...
"""
    ...
    def _get_dependency_troubleshooting(self, task: TaskItem, tasks: List[TaskItem], tasks_content: str) -> str:
...
"""
    ...
    async def handle_update_task_status(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        if not feature_name or not task_number or not new_status:
            return [TextContent(type="text", text="Error: feature_name, task_number, and status are required")]
        ...
        try:
            ...
            validation_result = self._validate_status_change(tasks, task, new_status, tasks_content)
            if not validation_result["valid"]:
                return [TextContent(type="text", text=f"Status change not allowed: {validation_result['reason']}")]
            
            # Update the task status in the document
            updated_content = self._update_task_status_in_content(tasks_content, task_number, new_status)
            
            if updated_content == tasks_content:
                return [TextContent(type="text", text=f"Task {task_number} status was already {new_status}")]
...
"""
            ...
            if new_status == "completed":
                ...
                dependent_tasks = self._analyze_task_dependencies(tasks, task)["dependents"]
                if dependent_tasks:
                    response += "\n#### Tasks Now Available\n"
                    for dep_task in dependent_tasks:
                        dep_deps = self._analyze_task_dependencies(tasks, dep_task)
                        all_prereqs_done = all(
                            self._get_task_status_from_content(updated_content, prereq.number) == "completed"
            ...
            if new_status == "completed":
                ...
                if next_task:
                    ...
                    response += "- All tasks completed! Feature implementation is done.\n"
            elif new_status == "in_progress":
                response += "- Focus on completing this task\n"
                response += "- Use task execution tools for guidance and troubleshooting\n"
            else:  # not_started
    ...
    async def handle_validate_task_execution_order(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            # Analyze dependencies
            dependencies = self._analyze_task_dependencies(tasks, task)
            current_status = self._get_task_status_from_content(tasks_content, task_number)
...
"""
            ...
            if current_status == "not_started":
                if not dependencies['prerequisites'] or all(
                    self._get_task_status_from_content(tasks_content, prereq.number) == "completed"
                    for prereq in dependencies['prerequisites']
                ):
            ...
            if task.is_subtask:
                ...
                response += f"- Complete all subtasks before considering parent task complete\n"
                
                # Find sibling subtasks
                siblings = [t for t in tasks if t.parent_task == task.parent_task and t.number != task.number]
                if siblings:
                    response += f"- Sibling subtasks: {', '.join(s.number for s in siblings)}\n"
            
            # Suggest next tasks if this one is completed
            if current_status == "completed":
                next_candidates = []
                for candidate in tasks:
                    if self._get_task_status_from_content(tasks_content, candidate.number) == "not_started":
                        candidate_deps = self._analyze_task_dependencies(tasks, candidate)
                        if all(
                            self._get_task_status_from_content(tasks_content, prereq.number) == "completed"
                ...
                if next_candidates:
                    ...
                    for candidate in next_candidates[:3]:  # Show top 3
                        response += f"- **Task {candidate.number}**: {candidate.title}\n"
            
            return [TextContent(type="text", text=response)]
            
        except Exception as e:
            return [TextContent(type="text", text=f"Error validating task execution order: {str(e)}")]
    
    async def handle_get_task_progress(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            if not tasks:
                return [TextContent(type="text", text=f"No tasks found in {feature_name}")]
            
            # Calculate progress statistics
            total_tasks = len(tasks)
            completed_tasks = sum(1 for task in tasks if self._get_task_status_from_content(tasks_content, task.number) == "completed")
            in_progress_tasks = sum(1 for task in tasks if self._get_task_status_from_content(tasks_content, task.number) == "in_progress")
...
"""
            ...
            for main_task in main_tasks:
                ...
                task_subtasks = [t for t in subtasks if t.parent_task == main_task.number]
                if task_subtasks:
                    completed_subtasks = sum(1 for st in task_subtasks if self._get_task_status_from_content(tasks_content, st.number) == "completed")
            ...
            if completed_tasks > 0:
                response += f"- **Completion Rate**: {completed_tasks} tasks completed\n"
            ...
            for task in tasks:
                if self._get_task_status_from_content(tasks_content, task.number) == "not_started":
                    dependencies = self._analyze_task_dependencies(tasks, task)
                    if dependencies['prerequisites']:
            ...
            if in_progress_tasks > 0:
                response += "- Focus on completing in-progress tasks before starting new ones\n"
            ...
            if completion_percentage >= 80:
                ...
                response += "- Good progress made - continue with systematic task completion\n"
            else:
                response += "- Project is in early stages - focus on foundational tasks first\n"
            
            return [TextContent(type="text", text=response)]
            
        except Exception as e:
            return [TextContent(type="text", text=f"Error getting task progress: {str(e)}")]
    
    # Helper methods for task status management
    
    def _validate_status_change(self, tasks: List[TaskItem], task: TaskItem, new_status: str, tasks_content: str) -> Dict[str, Any]:
    ...
    def _update_task_status_in_content(self, content: str, task_number: str, new_status: str) -> str:
