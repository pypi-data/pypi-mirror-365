...
from ..workflow.models import PhaseType, PhaseStatus
...
class WorkflowManagementTools:
    ...
    async def handle_create_workflow(self, arguments: Dict[str, Any]) -> List[TextContent]:
...
"""
    ...
    async def handle_get_workflow_status(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            if not summary:
                return [TextContent(type="text", text=f"No workflow found for feature: {feature_name}")]
...
"""
            ...
            if current_status == 'not_started':
                ...
                if summary['can_proceed']:
                    response += f"- Ready to move to next phase: {summary['next_phase']}\n"
                    response += f"- Use `transition_phase` with action 'start' for next phase\n"
            ...
            try:
                ...
                return [TextContent(type="text", text=f"Error getting workflow status: {str(e)}")]
    
    async def handle_transition_phase(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        if not feature_name or not action:
            return [TextContent(type="text", text="Error: feature_name and action are required")]
        ...
        try:
            ...
            if phase_str:
                try:
                    ...
                    return [TextContent(type="text", text=f"Invalid phase: {phase_str}")]
            else:
            ...
            if action == "start":
                ...
                return [TextContent(type="text", text=f"Invalid action: {action}. Use 'start', 'complete', or 'approve'")]
...
"""
            ...
            if action == "start":
                ...
                response += f"- Complete the phase and use 'complete' action when ready\n"
            elif action == "complete":
                response += f"- {target_phase.value.title()} phase is now ready for review\n"
                response += f"- Present the completed document to user for approval\n"
                response += f"- Use 'approve' action after receiving explicit user approval\n"
            elif action == "approve":
            ...
            try:
                error = error_handler.handle_workflow_error(
                    ...
                    context={"feature_name": feature_name, "action": action, "phase": phase_str}
                )
                return [TextContent(type="text", text=error.get_formatted_message())]
            except Exception:
            ...
            try:
                error = error_handler.handle_workflow_error(
                    ...
                    current_phase=phase_str,
                    attempted_action="phase_transition",
                    context={"feature_name": feature_name, "action": action, "phase": phase_str, "original_error": str(e)}
                )
                return [TextContent(type="text", text=error.get_formatted_message())]
            except Exception:
                return [TextContent(type="text", text=f"Error during phase transition: {str(e)}")]
    ...
    async def handle_navigate_backward(self, arguments: Dict[str, Any]) -> List[TextContent]:
        """Handle navigate_backward tool call."""
        feature_name = arguments.get("feature_name", "")
        target_phase_str = arguments.get("target_phase", "")
        
        if not feature_name or not target_phase_str:
            return [TextContent(type="text", text="Error: feature_name and target_phase are required")]
        
        try:
            target_phase = PhaseType(target_phase_str)
        except ValueError:
            return [TextContent(type="text", text=f"Invalid target phase: {target_phase_str}")]
        
        try:
            # Check if backward navigation is allowed
            can_navigate = self.state_tracker.can_navigate_backward(feature_name, target_phase)
            
            if not can_navigate:
                return [TextContent(type="text", text=f"Cannot navigate backward to {target_phase.value} phase from current state")]
            
            # Perform backward navigation
            success = self.state_tracker.navigate_backward(feature_name, target_phase)
            
            if not success:
                return [TextContent(type="text", text=f"Failed to navigate backward to {target_phase.value} phase")]
...
"""
            ...
            try:
                error = error_handler.handle_workflow_error(
                    ...
                    context={"feature_name": feature_name, "target_phase": target_phase_str, "original_error": str(e)}
                )
                return [TextContent(type="text", text=error.get_formatted_message())]
            except Exception:
                return [TextContent(type="text", text=f"Error during backward navigation: {str(e)}")]
    
    async def handle_check_transition_requirements(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            can_transition = self.phase_manager.can_transition_to_phase(feature_name, target_phase)
...
"""
            ...
            try:
                ...
                return [TextContent(type="text", text=f"Error checking transition requirements: {str(e)}")]
    
    async def handle_get_approval_guidance(self, arguments: Dict[str, Any]) -> List[TextContent]:
...
"""
            ...
            try:
                ...
                return [TextContent(type="text", text=f"Error getting approval guidance: {str(e)}")]
