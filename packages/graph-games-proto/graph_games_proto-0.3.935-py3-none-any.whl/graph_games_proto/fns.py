...
class LegalActionParams(PClass):
    t = field(type=str)
    deck_idx = field(type=(int, type(None)), initial=None)
    max = field(type=(int, type(None)), initial=None)
    def __todict__(self):
        return {
            "t": self.t,
    ...
    def __fromdict__(d):
        return LegalActionParams(
            t=d["t"],
            deck_idx=d.get("deck_idx"),  # Handle None case
...
class State(PClass):
    legal_actions = field(type=list)  # List[LegalAction]
    ...
    def __fromdict__(d):
        return State(
            legal_actions=[LegalAction.__fromdict__(x) for x in d["legal_actions"]],
...
def getinitialstate(game_config):
    ...
    sample_hooks = [
        Hook(
            ...
            uuid=str(uuid4()),
            when="start",
            code="print('Sample Hook 1 executed')"
        ),
    ]

    return run_initialization_hooks(state, sample_hooks, True)


def run_initialization_hooks(state, hooks, log=False):
    return reduce(
        lambda s, h: run_hook(s, h, log), 
        hooks, 
        state
    )
...
def append_to_legal_actions_for_each_player(game, name, legal_action_params):
    ...
    return game.set(legal_actions=game.legal_actions)


def run_hook(state, hook, log=False):
    code_string = """def handler(game):
    game = shuffle_all_decks(game)  
    game = deal_cards_to_each_player(game, 0, 4)
    game = deal_cards_to_each_player_discard_tray(game, 1, 2)
    game = distribute_all_piles(game)
    game = flip_cards(game, 0, 5)
    game = append_to_legal_actions_for_each_player(
        game, 
...
def getprivatestate(s, player_idx):
    ...
    return PrivateState(
        ...
        legals_actions_2 = get_legal_actions(s, player_idx),
