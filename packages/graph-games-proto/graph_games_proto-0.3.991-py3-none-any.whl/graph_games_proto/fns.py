...
class LegalActionConfig(PClass):
    ...
    claim_path = field(type=(LegalActionClaimPath, type(None)), initial=None)  # LegalActionClaimPath
    def __todict__(self):
        return {
            "name": self.name,
            ...
            "claim_path": self.claim_path.__todict__() if self.claim_path else None,
        }
    @staticmethod
    def __fromdict__(d):
        return LegalActionConfig(
            ...
            claim_path=LegalActionClaimPath.__fromdict__(d["claim_path"]) if d.get("claim_path") else None,
        )
...
def append_default_legal_actions_for_initial_player(game, log=False):
...
def get_claimable_paths(game, player_idx):
    paths = game.game_config.fig.board_config.board_paths
    return [
        path for path in paths 
        if is_path_claimable(game, player_idx, path)
    ]
...
HOOK_NAMESPACE = {
    ...
    'get_claimable_paths': get_claimable_paths,
}


def is_path_claimable(game, player_idx, path):
    print("****************************************** Inside is_path_claimable")
    cards = game.players[player_idx].cards
    segments = path.path.segments
    print(f"Player cards: {[card.__todict__() for card in cards]}")
    print(f"Path segments: {segments}")
    print(f"Deck.units: {[unit.__todict__() for unit in game.decks[0].units]}")

    non_null_segments = [seg for seg in segments if seg is not None]
    null_segments = [seg for seg in segments if seg is None]

    print(f"Non-null segments: {non_null_segments}")
    print(f"Null segments: {null_segments}")

    # sample = []
    # for segment in non_null_segments:
    #     if segment.unit is not None:
    #         sample.append(segment.unit)
...
def handler(game, action):
    ...
    if action.legal_action.config.name == "INITIAL-GOAL-DISCARD":
        return append_default_legal_actions_for_initial_player(game)
    return game
"""
