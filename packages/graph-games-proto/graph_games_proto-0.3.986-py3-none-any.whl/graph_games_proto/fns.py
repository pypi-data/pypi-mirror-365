...
DEFAULT_LEGAL_ACTIONS_HOOK = """def handler(game, player_idx):
    ...
    claimable_paths = game.game_config.fig.board_config.board_paths
    # get_claimable_paths(game, player_idx)

    print(f"len(claimable_paths): {len(claimable_paths)}")

    for claimable_path in claimable_paths:
        game = append_to_legal_actions(
            game,
            LegalAction(
                player_idx=player_idx,
