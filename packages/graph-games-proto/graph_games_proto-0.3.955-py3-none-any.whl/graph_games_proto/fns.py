...
class LegalActionParams(PClass):
...
class ActionDiscard(PClass):
    card_idxs = field(type=list)  # List[int]
    def __todict__(self):
        return {
            "card_idxs": self.card_idxs,
        }
    @staticmethod
    def __fromdict__(d):
        return ActionDiscard(
            card_idxs=d["card_idxs"]
        )
...
def getinitialstate(game_config):
    ...
    state = State(
        ...
        piles=[
            Pile(
                ...
                num_pieces=num_segment_pieces_per_player,
                pieces=[
                    Piece(idx=piece_idx, pile_idx=player_idx) for piece_idx in range(num_segment_pieces_per_player)
                ],
            ) for player_idx in range(game_config.num_players)
        ],
        ...
        decks=[
            Deck(
                ...
                units=[],
                facedown_stack=[
                    Card(idx=idx, is_wild=False, deck_idx=0, unit_idx=idx) for idx in range(gettotaldeckcards(fig))
                ],
                faceup_spots=[],
                discard=[],
            ),
            Deck(
                ...
                units=[],
                facedown_stack=[
                    Card(idx=idx, is_wild=False, deck_idx=1, unit_idx=idx) for idx in range(getnumroutecards(fig))
                ],
                faceup_spots=[],
                discard=[],
            )
    ...
    return run_initialization_hooks(state, INITIALIZATION_HOOKS, True)


def run_initialization_hooks(state, hooks, log=False):
    return reduce(
        lambda s, h: run_initialization_hook(s, h, log), 
        hooks, 
        state
    )

    
def run_handle_action_hooks(state, action, hooks, log=False):
    return reduce(
        lambda s, h: run_handle_action_hook(s, action, h, log), 
        hooks, 
        state
    )


def run_after_handle_action_hooks(state, hooks, log=False):
    return reduce(
        lambda s, h: run_after_handle_action_hook(s, h, log), 
        hooks, 
        state
    )
...
def handle_discard_action(game, action):
    if not game or not action or not action.discard:
        return game

    deck_idx = action.legal_action.discard.deck_idx
    max_discard = action.legal_action.discard.max
    ...
    for discard_tray_idx in action.discard.card_idxs:
...
def append_to_legal_actions_for_each_player(game, name, legal_action_params):
    ...
    for player in game.players:
        game.legal_actions_2.append(
            LegalAction(
                ...
                discard=legal_action_params.discard,
            )
        )
...
def run_handle_action_hook(state, action, hook, log=False):
    ...
    namespace = {
        ...
        'LegalActionParams': LegalActionParams,
...
def run_after_handle_action_hook(state, hook, log=False):
...
def run_initialization_hook(state, hook, log=False):
...
def getnextstate2(s, a, log):
    return run_handle_action_hooks(s, a, HANDLE_ACTION_HOOKS, True)
...
INIT_HOOK_3 = """def handler(game):
    return deal_cards_to_each_player_discard_tray(game, 1, 2)
"""
...
INIT_HOOK_6 = """def handler(game):
    return append_to_legal_actions_for_each_player(
        game, 
        "INITIAL-2ND-DECK-DISCARD",
        LegalActionParams(
            discard=LegalActionDiscard(
                deck_idx=1,
                max=1,
            ),
        )
...
AFTER_HANDLE_ACTION_HOOK_1 = """
def handler(game, action):
    if action['name'] == "INITIAL-DECK2-DISCARD":
        if len(game['legal_actions']) > 0:
            return appends_default_legal_actions_for_initial_player!(game)
"""
...
AFTER_HANDLE_ACTION_HOOKS = [
    # Hook(
    #     name="SampleHookAfterHandleAction1",
    #     uuid=str(uuid4()),
    #     when="after_handle_action",
    #     code=AFTER_HANDLE_ACTION_HOOK_1,
    # ),
]
