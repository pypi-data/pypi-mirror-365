...
class LegalAction(PClass):
    name = field(type=str)
    player_idx = field(type=int)
    params = field(type=LegalActionParams)
    def __todict__(self):
        return {
            "name": self.name,
            "player_idx": self.player_idx,
            "params": self.params.__todict__(),
        }
    @staticmethod
    def __fromdict__(d):
        return LegalAction(
            name=d["name"],
            player_idx=d["player_idx"],
            params=LegalActionParams.__fromdict__(d["params"])
        )
...
def getnextstate(s, action, log):
    ...
    return run_after_handle_action_hooks(next, AFTER_HANDLE_ACTION_HOOKS, True)
...
def handler(game, user, submitted_action):
    if submitted_action['name'] == "INITIAL-DECK2-DISCARD":
        if len(game['legal_actions']) > 0:
            return
        appends_default_legal_actions_for_initial_player!(
            game
        )
"""
