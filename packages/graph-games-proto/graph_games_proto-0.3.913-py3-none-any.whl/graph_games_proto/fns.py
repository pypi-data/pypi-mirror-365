    ...
    state = State(
        ...
        decks=[
            Deck(
                ...
                units=[],
                cards=[
                    Card(uuid=str(uuid4()), is_wild=False, deck_idx=0, unit_idx=i) for i in range(gettotaldeckcards(fig))
                ],
            ),
            Deck(
                uuid=str(uuid4()),
                units=[],
                cards=[
                    Card(uuid=str(uuid4()), is_wild=False, deck_idx=1, unit_idx=i) for i in range(getnumroutecards(fig))
                ],
            )
        ],
...
def shuffle_deck(game, deck_idx):
    ...
    rng = game.rng
    shuffled_cards = list(deck.cards)
    rng.shuffle(shuffled_cards)
    
    shuffled_deck = deck.set(cards=shuffled_cards)
...
def run_hook(state, hook, log=False):
    ...
    game = deal_cards_to_each_player(game, 1, 3)
...
def getpublicstate(s):
    return PublicState(
        ...
        players=[None for p in s.players],
