...
class State(PClass):
    default_legal_actions_code = field(type=(str, type(None)), initial=None)  # Optional[str]
    legal_actions_2 = field(type=list)  # List[LegalAction]
    ...
    def __todict__(self):
        return {
            "default_legal_actions_code": self.default_legal_actions_code,
            "legal_actions_2": [x.__todict__() for x in self.legal_actions_2],
    ...
    def __fromdict__(d):
        return State(
            default_legal_actions_code=d.get("default_legal_actions_code"),
            legal_actions_2=[LegalAction.__fromdict__(x) for x in d["legal_actions_2"]],
...
def getinitialstate(game_config):
    ...
    state = run_set_default_legal_actions_hook(state, DEFAULT_LEGAL_ACTIONS_HOOK, True)
    return state


def run_set_default_legal_actions_hook(state, hook, log=False):
    return state.set(default_legal_actions_code=hook)
...
def append_to_legal_actions_for_initial_player(game, legal_action_config):
    ...
    game.legal_actions_2.append(
        LegalAction(
            ...
            name=legal_action_config.name,
            discard=legal_action_config.discard,
            draw=legal_action_config.draw,
        )
    )
...
def append_default_legal_actions_for_initial_player(game):
    print("****************************************** Inside append_default_legal_actions_for_initial_player")
    return temp_namespace_fn(game, game.player_idxs[0])
...
HOOK_NAMESPACE = {
    ...
    'temp_namespace_fn': temp_namespace_fn,
}
...
INIT_HOOK_6 = """def handler(game):
    return append_to_legal_actions_for_each_player(
        game, 
        LegalActionConfig(
            name="INITIAL-GOAL-DECK-DISCARD",
...
def handler(game, action):
    ...
    if action.legal_action.name == "INITIAL-GOAL-DECK-DISCARD":
