...
class Deck(PClass):
    ...
    faceup_stack = field(type=list, initial=[])  # List[Card]
    faceup_spots = field(type=list, initial=[])  # List[Card]
    def __todict__(self):
        return {
            "uuid": self.uuid,
            "units": [unit.__todict__() for unit in self.units],
            "facedown_stack": [card.__todict__() for card in self.facedown_stack],
            "faceup_stack": [card.__todict__() for card in self.faceup_stack],
    ...
    def __fromdict__(d):
        return Deck(
            ...
            faceup_stack=[Card.__fromdict__(card) for card in d["faceup_stack"]],
...
def getinitialstate(game_config):
    ...
    state = State(
        ...
        players=[Player(pieces=[], cards=[], discard_tray=[]) for _ in range(game_config.num_players)],
...
def shuffle_deck(game, deck_idx):
    ...
    shuffled_deck = deck.set(facedown_stack=shuffled_cards, faceup_stack=[], faceup_spots=[])
...
def run_hook(state, hook, log=False):
    ...
    game = deal_cards_to_each_player_discard_tray(game, 1, 3)
