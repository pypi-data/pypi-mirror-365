...
def print_usage():
    print("Usage: pulseaudio_device_control OPERATION DEVICE_TYPE [AMOUNT]")
    print("    OPERATION: \"raise\", \"lower\", \"mute\", or \"next\"")
    print("    DEVICE_TYPE: \"sink\" or \"source\"")
    print("    AMOUNT: Optional integer % change for raise and lower\n")
...
def parse_arguments():
    ...
    if len(sys.argv) == 4:
        amount = float(sys.argv[3]) * 0.01

    return operation, device_type, amount


def raise_volume(pulse, device, amount):
    if device.mute:
    ...
    pulse.volume_change_all_chans(device, amount)


def lower_volume(pulse, device, amount):
    pulse.volume_change_all_chans(device, -amount)
...
def main():
    operation, device_type, amount = parse_arguments()
                ...
                if device.name == default_device_name:
                    match operation:
                        case Operation.RAISE:
                            raise_volume(pulse, device, amount)
                        case Operation.LOWER:
                            lower_volume(pulse, device, amount)
                        case Operation.MUTE:
