...
class Utils:
    ...
    @staticmethod
    def datetime_now_format(format: str = "%d/%m/%Y %H:%M:%S"):
        Utils.datetime_now().strftime(format)
...
class FileUtils:
    ...
    @staticmethod
    def create_upload_file(path: str, filename: str = None, content_type: str = None) -> UploadFile:

        with open(path, "rb") as f:
            content = f.read()
        stream = io.BytesIO(content)
        return UploadFile(filename=filename or path.split("/")[-1], file=stream, headers={"content_type": content_type})
