...
from panCG.bin.callCNS import run_callCNS
...
from panCG.bin.cnsIndexAssign import run_cnsIndexAssign
from panCG.bin.cnsIndexMerge import run_cnsIndexMerge
from panCG.bin.cnsRecall import run_cnsRecall
from panCG.bin.cnsIndexSort import run_cnsIndexSort

from panCG.bin.geneMapMerge import run_geneMapMerge
from panCG.bin.geneClustering import run_geneClustering
from panCG.bin.geneIndexAssign import run_geneIndexAssign
...
class MyStart:
    ...
    def setup_GeneIndex(self):
        GeneIndex_parser = self.subparsers.add_parser('GeneIndex', help='build gene index',
                                                      formatter_class=argparse.RawDescriptionHelpFormatter)
        # input option
        input_option = GeneIndex_parser.add_argument_group(title="required input")
        input_option.add_argument("-c", "--config", dest="config", required=True,
                                  help="The input configuration file")
        input_option.add_argument("-w", "--workDir", dest="workDir", required=True,
                                  help="Output result directory")
        ...
        # diamond option
        diamond_option = GeneIndex_parser.add_argument_group(title="diamond parameter")
        ...
        # jcvi option
        jcvi_option = GeneIndex_parser.add_argument_group(title="jcvi parameter")
        ...
        # OrthoFinder option
        orthofinder_option = GeneIndex_parser.add_argument_group(title="OrthoFinder parameter")
        orthofinder_option.add_argument("--orthofinder_threads", dest="orthofinder_threads",
                                        type=int, default=10,
                                        help="number of threads [default: %(default)s]")

        # assign option
        assign_option = GeneIndex_parser.add_argument_group(title="assign parameter")
        ...
        # paralogs option
        paralogs_option = GeneIndex_parser.add_argument_group(title="paralogs parameter")
    ...
    def setup_CnsIndex(self):
        CnsIndex_parser = self.subparsers.add_parser('CnsIndex', help='build CNS index',
                                                     formatter_class=argparse.RawDescriptionHelpFormatter)
        # input option
        input_option = CnsIndex_parser.add_argument_group(title="required input")
        input_option.add_argument("-c", "--config", dest="config", required=True,
                                  help="The input configuration file")
        input_option.add_argument("-w", "--workDir", dest="workDir", required=True,
                                  help="Output result directory")
        ...
        # halLiftover option
        halLiftover_option = CnsIndex_parser.add_argument_group(title="halLiftover parameter")
        ...
        # blastn option
        blastn_option = CnsIndex_parser.add_argument_group(title="halLiftover parameter")
        ...
        # jcvi option
        jcvi_option = CnsIndex_parser.add_argument_group(title="jcvi parameter")
        jcvi_option.add_argument("--jcvi_threads", dest="jcvi_threads", type=int, default=6,
                                 help="number of jcvi threads [default: %(default)s]")
        jcvi_option.add_argument("--jcvi_parallel", dest="jcvi_parallel", type=int, default=10,
                                 help="number of running jcvi simultaneously [default: %(default)s]")
        ...
        # map option
        map_option = CnsIndex_parser.add_argument_group(title="map parameter")
        ...
        # cluster option
        cluster_option = CnsIndex_parser.add_argument_group(title="cluster parameter")
        ...
        # assign option
        assign_option = CnsIndex_parser.add_argument_group(title="assign parameter")
        ...
        # merge option
        merge_option = CnsIndex_parser.add_argument_group(title="merge parameter")
        ...
        # recall option
        recall_option = CnsIndex_parser.add_argument_group(title="recall parameter")
    ...
    def setup_genePavAsso(self):
        ...
        input_option.add_argument("--pan_gene_file", dest="pan_gene_file", required=True,
                                  help="The pan-Gene file obtained from the `GeneIndex`")
    ...
    def set_GLSS(self):
        ...
        input_option.add_argument("-c", "--config", dest="config", required=True,
                                  help="The configuration file in `GeneIndex`")
        input_option.add_argument("-w", "--workDir", dest="workDir", required=True,
                                  help="The workDir result directory in `GeneIndex`")
        input_option.add_argument("-r", "--reference", dest="reference", required=True,
    ...
    def set_CLSS(self):
        ...
        # input_option.add_argument("-c", "--config", dest="config", required=True,
        #                           help="The configuration file in `CnsIndex`")
    ...
    def set_CnsGeneLink(self):
        ...
        input_option.add_argument("-c", "--cns_index", dest="cns_index_file", required=True,
                                  help="The output pan-CNS file in `CnsIndex`")
        input_option.add_argument("-g", "--gene_index", dest="gene_index_file", required=True,
                                  help="The output pan-Gene file in `GeneIndex`")
        input_option.add_argument("-l", "--link_yaml", dest="link_yaml", required=True,
                                  help="The contig yaml file. Annotation file recording each gene of each species in the `GeneIndex`")
    ...
    def run_callCns(self):
        ...
        self.decorated_func(run_callCNS, self.logger, self.logger, config, workDir, reference, args_dict)
        self.decorated_func(run_CE_RmCDS_to_CNS, self.logger, self.logger, config, workDir, reference, args_dict)

    def run_GeneIndex(self):
        args = self.parser.parse_args()
        args_dict = vars(args)
        config = args.config
        workDir = args.workDir
        reference = args.reference
        self.decorated_func(run_geneMapMerge, self.logger, self.logger, config, workDir, reference, args_dict)
        self.decorated_func(run_geneClustering, self.logger, self.logger, config, workDir, reference, args_dict)
        self.decorated_func(run_geneIndexAssign, self.logger, self.logger, config, workDir, reference, args_dict)
        self.decorated_func(parse_paralogs, self.logger, self.logger, config, workDir, reference, args_dict)

    def run_CnsIndex(self):
        args = self.parser.parse_args()
        args_dict = vars(args)
        config = args.config
        workDir = args.workDir
        ...
        self.decorated_func(run_cnsMapMerge, self.logger, self.logger,
                            config, workDir, geneWorkDir, geneConfig, reference, args_dict)
        self.decorated_func(run_cnsClustering, self.logger, self.logger, config, workDir, reference)
        self.decorated_func(run_cnsIndexAssign, self.logger, self.logger, config, workDir, reference, args_dict)
        self.decorated_func(run_cnsIndexMerge, self.logger, self.logger, config, workDir, reference, args_dict)
        self.decorated_func(run_cnsRecall, self.logger, self.logger, config, workDir, reference, args_dict)
        self.decorated_func(run_cnsIndexSort, self.logger, self.logger, config, workDir, reference)
    ...
    def run_CnsSyntenyNet(self):
        ...
        geneWorkDir = args.geneWorkDir
        geneConfig = args.geneConfig
        run_CNS_synteny_net(self.logger, panCns, cnsConfig, workDir, reference, geneWorkDir, geneConfig, args_dict)
    ...
    def run(self):
        ...
        if args.command == 'callCns':
            self.run_callCns()
        elif args.command == 'CnsIndex':
            self.run_CnsIndex()
        elif args.command == 'GeneIndex':
            self.run_GeneIndex()
