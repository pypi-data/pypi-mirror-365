...
# from panCG.lib.base import MyBedTool
...
class CNSsAnchor(BasePan):
    ...
    """
    halLiftover_cmd = os.path.join(CACTUS_PATH, "halLiftover")
    makeblastdb_cmd = os.path.join(BLAST_PATH, "makeblastdb")
    blastn_cmd = os.path.join(BLAST_PATH, "blastn")
    blastn_threads = 6
    jcvi_threads = 6
...
def run_rescue_cns_anchor(cnsAnchor, geneWorkDir, geneConfigData):
    ...
    gene_block_file = os.path.join(geneWorkDir, "JCVIDir", f"{cnsAnchor.que}.{cnsAnchor.ref}",
                                   f"{cnsAnchor.que}.{cnsAnchor.ref}.simple")
    que_gene_bed_file = geneConfigData["species"][cnsAnchor.que]["longest_pep_bed"]
    ref_gene_bed_file = geneConfigData["species"][cnsAnchor.ref]["longest_pep_bed"]
