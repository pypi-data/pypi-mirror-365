...
def push_ut_test_results_to_jama(version: str, input_folder: str) -> bool:
    """
    ...
    try:
        ...
        if warnings:
            print("\nWARNINGS:")
            print("| Test Case                | Document Key           | Message")
            print("|--------------------------|------------------------|------------------------")
            for warning in warnings:
                print(f"| {warning['test_case']:<24} | {warning['doc_key']:<20} | {warning['message']}")
            print()

        # Report errors
        if errors:
            print("\nERRORS:")
            print("| Test Case                | Document Key           | Error")
            print("|--------------------------|------------------------|------------------------")
            for error in errors:
                print(f"| {error['test_case']:<24} | {error['doc_key']:<20} | {error['error']}")
            print()

        if success_count > 0:
            if error_count > 0:
                logging.warning(f"Processed {success_count} test runs but {error_count} had failures")
                # Don't raise error for partial success, just log warnings
            else:
                logging.info(f"Successfully processed {success_count} test runs for version {version}")
            return True
        else:
            logging.warning("No test runs were successfully updated")
            return False
