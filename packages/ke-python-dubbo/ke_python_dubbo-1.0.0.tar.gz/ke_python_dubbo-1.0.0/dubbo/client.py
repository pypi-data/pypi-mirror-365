...
class ZkRegister(object):
    ...
    def __init__(self, hosts, path, user, password, application_name='search_platform'):
        """
    ...
    def __resubscribe(self):
        ...
        for interface in self.hosts.keys():
            self.zk.get_children(self.path + DUBBO_ZK_PROVIDERS.format(interface), watch=self._watch_children)
            self.zk.get_children(self.path + DUBBO_ZK_CONFIGURATORS.format(interface), watch=self._watch_configurators)
    ...
    def get_provider_host(self, interface):
        ...
        if interface not in self.hosts:
            ...
            try:
                if interface not in self.hosts:
                    path = self.path + DUBBO_ZK_PROVIDERS.format(interface)
                    # print(path)
    ...
    def _get_configurators_from_zk(self, interface):
        ...
        :return:
        """
        configurators = self.zk.get_children(self.path + DUBBO_ZK_CONFIGURATORS.format(interface), watch=self._watch_configurators)
        if configurators:
    ...
    def _watch_configurators(self, event):
        ...
        # 试图从配置中取出权重相关的信息
        configurators = self.zk.get_children(self.path + DUBBO_ZK_CONFIGURATORS.format(interface),
    ...
    def _register_consumer(self, providers):
        ...
        consumer_path = self.path + DUBBO_ZK_CONSUMERS.format(fields['interface'])
    ...
    def _routing_with_wight(self, interface):
        ...
        if interface not in self.weights or not self.weights[interface]:
            return random.choice(list(hosts))
