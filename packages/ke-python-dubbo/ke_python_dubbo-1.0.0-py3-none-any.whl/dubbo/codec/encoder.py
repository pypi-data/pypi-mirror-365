...
"""
20250727协议修复参考：
http://hessian.caucho.com/download/hessianlib.py
    ...
    * object
    ...
    def _get_parameter_types(self, arguments):
        ...
        for argument in arguments:
            # print("argument: " + str(argument) + " type: " + str(type(argument)) + "\n")
    ...
    def _get_class_name(self, _class):
        ...
        if isinstance(_class, bool):  # bool类型的判断必须放在int类型判断的前面
            ...
            if len(_class) == 0:
                # 空列表使用 java.util.List 接口
                return 'Ljava/util/List;'
            # 对于非空列表，也使用 java.util.List 接口
            # 这样更符合 Java 的类型系统，因为 Python 的 list 对应 Java 的 List 接口
            return 'Ljava/util/List;'
        else:
    ...
    def _encode_list(self, value):
        ...
        length = len(value)
        if length == 0:
            # 空列表使用 java.util.ArrayList 类型
            _type = 'Ljava/util/ArrayList;'
            result.append(0x70)  # 空列表标记
        ...
        # 根据第一个元素确定列表类型，使用正确的 Java 类型描述符
        if isinstance(value[0], bool):
            _type = 'Ljava/util/List;'  # 使用 java.util.List 接口
        elif isinstance(value[0], int):
            # 对于整数，根据值范围判断是 Integer 还是 Long
            if MIN_INT_32 <= value[0] <= MAX_INT_32:
                _type = 'Ljava/util/List;'  # List<Integer>
            else:
                _type = 'Ljava/util/List;'  # List<Long>
        elif isinstance(value[0], float):
            _type = 'Ljava/util/List;'  # List<Double>
        elif isinstance(value[0], str):
            _type = 'Ljava/util/List;'  # List<String>
        elif isinstance(value[0], Object):
            _type = 'Ljava/util/List;'  # List<Object>
        else:
