...
class List(list):  # TODO weg (or should be unneeded virtually always)
...
def project_directory() -> annize.fs.Path:
    annize_config_rootpath = annize.flow.run_context.object_by_name(annize.flow.run_context.RunContext._ANNIZE_CONFIG_ROOTPATH__NAME)
    projectfiles_subdir = os.path.dirname(annize_config_rootpath)
    projectdir = _get_data("project_directory", None)
    if not projectdir:
        projectdir = str(annize.project.loader.project_root_directory(annize_config_rootpath))
    if not os.path.isabs(projectdir):
        projectdir = f"{projectfiles_subdir}/{projectdir}"
    projectdir = os.path.abspath(projectdir)
    return annize.fs.Path(projectdir)
