        ...
        arg_parser = argparse.ArgumentParser(
            description=None if only_documentation else annize.i18n.tr("an_Cli_welcome").format(
                ...
                readme_file=repr(f"file://{annize.asset.data.readme_pdf(
                    annize.i18n.current_culture().iso_639_1_lang_code)}"),
...
class Commands:#TODO
    ...
    class ConsoleRunner(annize.flow.runner.Runner):

        def show_task_chooser(self):
            tasks = self.get_tasks()
            itask = annize.user_feedback.choice_dialog("Choose task  TODO", tasks)
            self.set_selected_task(None if (itask is None) else tasks[itask])

        def show_task_execution(self):
            print("TODO task execution starts")

        def show_task_execution_success(self):
            print("TODO task execution succ", self.get_success_state())
    ...
    def __init__(self, project: str, with_answers_from_json_file: t.Iterable[str],
                 with_answers_from_json_string: t.Iterable[str], with_answer: t.Iterable[t.Tuple[str, str]], **_):#TODO
    ...
    def do(self, task_name: str, **_):
        project = annize.project.Project.load(self.__project or self.project_default)
#            annize.user_feedback._add_controller_to_context(  # TODO xx  also for studio()
#               controller=annize.user_feedback.static.StaticUserFeedbackController(self.__answers),
#              context=project.context, priority_index=100_000)
        with annize.i18n.annize_user_interaction_culture():  # TODO ?!?! (we already have that somewhere else?! (two times!))
            with annize.ui.app("runner", project=project, run_task=task_name) as (app, app_ctrl):
                app_ctrl.run()

    def studio(self, **_):
        project = annize.project.Project.load(self.__project or self.project_default)
        with annize.i18n.annize_user_interaction_culture():  # TODO ?!?! (we already have that somewhere else?! (two times!))
            with annize.ui.app("studio", project=project) as (app, app_ctrl):
                app_ctrl.run()
