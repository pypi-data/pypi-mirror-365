...
class Package(annize.fs.FilesystemContent):
    ...
    def _path(self):
        with annize.i18n.get_culture("en"):  # TODO zz
    ...
    def _mkpackage_prepareinfos(cls, build: _BuildInfo, tmpdir: annize.fs.TInputPath) -> None:
        ...
        build.authorstring = f"{build.author.fullname} <{build.author.email or 'unknown@unknown'}>"
...
Files: *
Copyright: {datetime.datetime.now().strftime("%Y")} {build.author.fullname}
