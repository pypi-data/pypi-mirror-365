        ...
        if isinstance(nodemat.node, annize.project.ObjectNode):
            ...
            try:
                ...
                raise annize.project.MaterializerError(
                    f"No item '{nodemat.node.type_name}' in feature module '{nodemat.node.feature}'") from ex
            clssspec = inspect.getfullargspec(clss)
            if len(clssspec.args) > 1 or clssspec.varargs:
                raise annize.project.MaterializerError(f"Callable '{nodemat.node.type_name}' in '{nodemat.node.feature}'"
