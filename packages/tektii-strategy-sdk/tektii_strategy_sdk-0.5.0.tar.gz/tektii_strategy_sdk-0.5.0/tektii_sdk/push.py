...
from tektii_sdk import __version__
...
def create_strategy_version(config: PushConfig) -> CreateStrategyVersionResponseDTO:
    ...
    headers = {
        ...
        "User-Agent": f"tektii-sdk/{__version__}",
    }
...
def push_strategy(
    ...
    if config_path:
        from tektii_sdk.cli import load_config

        strategy_config = load_config(config_path)
    validation_result = validate_module(module_path, class_name, config=strategy_config)
