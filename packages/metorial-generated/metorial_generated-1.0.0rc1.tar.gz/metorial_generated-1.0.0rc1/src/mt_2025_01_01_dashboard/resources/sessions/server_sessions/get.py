...
class mapSessionsServerSessionsGetOutput:
    ...
    def from_dict(data: Dict[str, Any]) -> SessionsServerSessionsGetOutput:
        ...
        mcp=data.get('mcp') and {
            ...
            "server": data.get('mcp', {}).get('server') and {
        ...
        session=data.get('session') and {
            ...
            "usage": data.get('session', {}).get('usage') and {
        ...
        server_deployment=data.get('server_deployment') and {
            ...
            "server": data.get('server_deployment', {}).get('server') and {
        ...
        connection=data.get('connection') and {
            "object": data.get('connection', {}).get('object'),
            "id": data.get('connection', {}).get('id'),
            "client": data.get('connection', {}).get('client') and {
                "user_agent": data.get('connection', {}).get('client', {}).get('user_agent'),
                "anonymized_ip_address": data.get('connection', {}).get('client', {}).get('anonymized_ip_address')
            },
            "created_at": data.get('connection', {}).get('created_at') and datetime.fromisoformat(data.get('connection', {}).get('created_at')),
            "started_at": data.get('connection', {}).get('started_at') and datetime.fromisoformat(data.get('connection', {}).get('started_at')),
            "ended_at": data.get('connection', {}).get('ended_at') and datetime.fromisoformat(data.get('connection', {}).get('ended_at'))
        },
        created_at=data.get('created_at') and datetime.fromisoformat(data.get('created_at'))
        )

    @staticmethod
    def to_dict(value: Union[SessionsServerSessionsGetOutput, Dict[str, Any], None]) -> Optional[Dict[str, Any]]:
        if value is None:
            return None
