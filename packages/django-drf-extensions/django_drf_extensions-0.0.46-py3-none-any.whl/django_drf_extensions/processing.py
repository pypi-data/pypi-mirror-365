...
def is_multi_table_inherited_model(model_class):
    ...
    # Check if the model has any parent models
    if not model_class._meta.parents:
        return False
    
    # Check if any of the parent models are concrete (have their own table)
    for parent in model_class._meta.parents.values():
...
def safe_bulk_create(model_class, instances, **kwargs):
    ...
    """
    try:
        # Try bulk_create first - Django will raise ValueError for multi-table inherited models
        return model_class.objects.bulk_create(instances, **kwargs)
    except ValueError as e:
        if "Can't bulk create a multi-table inherited model" in str(e):
            # Fall back to individual saves for multi-table inherited models
            logger.info("Detected multi-table inherited model, using individual saves")
            created_instances = []
...
def safe_bulk_update(model_class, instances, fields, **kwargs):
    ...
    """
    try:
        # Try bulk_update first - Django will raise ValueError for multi-table inherited models
        return model_class.objects.bulk_update(instances, fields, **kwargs)
    except ValueError as e:
        if "Can't bulk update a multi-table inherited model" in str(e):
            # Fall back to individual saves for multi-table inherited models
            logger.info("Detected multi-table inherited model, using individual saves for update")
            updated_count = 0
            for instance in instances:
...
def get_model_manager(model_class, manager_info=None):
    """
    Get the appropriate manager for a model, respecting custom managers if provided.
    
    Args:
        model_class: The Django model class
        manager_info: Optional dict with manager information from ViewSet
    
    Returns:
        The appropriate manager to use for database operations
    """
    if manager_info and manager_info.get('manager_class') != 'Manager':
        # Use custom manager if specified
        try:
            manager_class = import_string(f"{manager_info['manager_module']}.{manager_info['manager_class']}")
            return manager_class()
        except (ImportError, AttributeError):
            logger.warning(f"Could not import custom manager {manager_info['manager_class']}, falling back to default")
...
def async_create_task(self, serializer_class_path: str, data_list: list[dict], user_id: int | None = None, manager_info: dict | None = None):
    """
    ...
    try:
        ...
        manager = get_model_manager(model_class, manager_info)
        ...
        if instances_to_create:
            ...
            with transaction.atomic():
                # Use the custom manager for bulk operations
                try:
                    created_instances = manager.bulk_create(new_instances)
                except ValueError as e:
                    if "Can't bulk create a multi-table inherited model" in str(e):
                        # Fall back to individual saves for multi-table inherited models
                        logger.info("Detected multi-table inherited model, using individual saves")
...
@shared_task(bind=True)
def async_update_task(self, serializer_class_path: str, updates_list: list[dict], user_id: int | None = None, manager_info: dict | None = None):
    """
    ...
    try:
        ...
        instances_dict = {
            ...
            for instance in manager.filter(id__in=ids_to_update)
        }
        ...
        if valid_updates:
            ...
            with transaction.atomic():
                ...
                try:
                    updated_count = manager.bulk_update(instances_to_update, fields_list, batch_size=1000)
                except ValueError as e:
                    if "Can't bulk update a multi-table inherited model" in str(e):
                        # Fall back to individual saves for multi-table inherited models
                        logger.info("Detected multi-table inherited model, using individual saves for update")
                        ...
                        for instance in instances_to_update:
                            instance.save(update_fields=fields_list)
                            updated_count += 1
                    else:
                        raise
...
@shared_task(bind=True)
def async_replace_task(self, serializer_class_path: str, replacements_list: list[dict], user_id: int | None = None, manager_info: dict | None = None):
    """
    ...
    try:
        ...
        instances_dict = {
            instance.id: instance 
            for instance in manager.filter(id__in=ids_to_replace)
        }
        ...
        if valid_replacements:
            ...
            with transaction.atomic():
                ...
                try:
                    replaced_count = manager.bulk_update(instances_to_replace, fields_list, batch_size=1000)
                except ValueError as e:
                    if "Can't bulk update a multi-table inherited model" in str(e):
                        # Fall back to individual saves for multi-table inherited models
                        logger.info("Detected multi-table inherited model, using individual saves for replace")
                        replaced_count = 0
                        for instance in instances_to_replace:
                            instance.save(update_fields=fields_list)
                            replaced_count += 1
                    else:
                        raise
...
@shared_task(bind=True)
def async_delete_task(self, model_class_path: str, ids_list: list[int], user_id: int | None = None, manager_info: dict | None = None):
    """
    ...
    try:
        ...
        with transaction.atomic():
            ...
            deleted_count, _ = manager.filter(id__in=ids_list).delete()
...
def async_get_task(
    ...
    try:
        ...
        if "ids" in query_data:
            ...
            queryset = manager.filter(id__in=ids_list)
    ...
    try:
            ...
            try:
                ...
                if serializer.is_valid():
                    ...
                    if unique_filter:
                        ...
                        existing_instance = manager.filter(**lookup_filter).first()
        ...
        if instances_to_update:
            ...
            with transaction.atomic():
                ...
                if fields_to_update:
                    try:
                        updated_count = manager.bulk_update(update_instances, fields_to_update, batch_size=1000)
                    except ValueError as e:
                        if "Can't bulk update a multi-table inherited model" in str(e):
                            # Fall back to individual saves for multi-table inherited models
                            logger.info("Detected multi-table inherited model, using individual saves for update")
