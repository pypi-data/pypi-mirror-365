...
def safe_bulk_create(model_class, instances, **kwargs):
    ...
    try:
        ...
        if "Can't bulk create a multi-table inherited model" in str(e):
            ...
            print(f"DEBUG: Detected multi-table inherited model, using individual saves", file=sys.stderr)
            created_instances = []
...
def safe_bulk_update(model_class, instances, fields, **kwargs):
    ...
    try:
        ...
        if "Can't bulk update a multi-table inherited model" in str(e):
            # Fall back to individual saves for multi-table inherited models
            print(f"DEBUG: Detected multi-table inherited model, using individual saves for update", file=sys.stderr)
            updated_count = 0
            for instance in instances:
...
class OperationsMixin:
    ...
    def bulk_get(self, request):
        ...
        # Get the queryset to pass manager information
        queryset = self.get_queryset()
        manager_info = {
            'model_path': f"{queryset.model.__module__}.{queryset.model.__name__}",
            'manager_class': queryset.model._default_manager.__class__.__name__,
            'manager_module': queryset.model._default_manager.__class__.__module__,
        }
        
        task = async_get_task.delay(
            model_class_path, serializer_class_path, query_data, user_id, manager_info
        )
    ...
    def bulk_create(self, request):
        ...
        task = async_create_task.delay(serializer_class_path, data_list, user_id, manager_info)
    ...
    def bulk_update(self, request):
        ...
        task = async_update_task.delay(serializer_class_path, data_list, user_id, manager_info)
    ...
    def bulk_replace(self, request):
        ...
        task = async_replace_task.delay(serializer_class_path, data_list, user_id, manager_info)
    ...
    def bulk_delete(self, request):
        ...
        task = async_delete_task.delay(model_class_path, ids_list, user_id, manager_info)
    ...
    def _bulk_upsert(self, request, data_list, unique_fields_param):
        ...
        task = async_upsert_task.delay(
            serializer_class_path, data_list, unique_fields, update_fields, user_id, manager_info
        )
