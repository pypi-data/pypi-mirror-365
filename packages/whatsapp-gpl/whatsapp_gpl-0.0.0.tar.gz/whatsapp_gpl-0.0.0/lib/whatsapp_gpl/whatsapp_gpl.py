...
class WhatsappGplZerounIntezarAgler:
    def __init__(self):
        self._lib = None
    ...
    def initializedwhatsapp_gplZerounIntezarAglerNativeCallbackFunction(self) -> None:
        lib = self
    ...
    def ensureInitialized(self, libraryPath: str) -> None:
        ...
        if libraryPath == "":
            # libraryPath = os.path.abspath(os.path.join(os.path.dirname(__file__), 'lib/libwhatsapp_gpl.so'))
            libraryPath = 'libwhatsapp_gpl.so'
        # Load library
        self._lib = ctypes.CDLL(libraryPath)
 
        self._lib.Invokewhatsapp_gplZerounIntezarAglerNativeFunction.restype = ctypes.c_char_p
        self._lib.Invokewhatsapp_gplZerounIntezarAglerNativeFunction.argtypes = [ctypes.c_char_p]
        self._callback = self.initializedwhatsapp_gplZerounIntezarAglerNativeCallbackFunction()

        self._lib.Initializedwhatsapp_gplZerounIntezarAglerNativeCallbackFunction(self._callback)
    ...
    def invokeRaw(self, parameters: dict) -> dict:
        ...
        # Kirim ke C
        result_ptr = self._lib.Invokewhatsapp_gplZerounIntezarAglerNativeFunction(input_bytes)
