...
#     """
#     Capture a screenshot using Chrome DevTools Protocol and return it as a NumPy array.
...
#         "fromSurface": True
#     })
#     return np.frombuffer(base64.b64decode(result["data"]), dtype=np.uint8)


def get_cdp_screenshot(driver, save_path="local_debugging/screenshot.png") -> np.ndarray:
    """
    Capture a screenshot using Chrome DevTools Protocol, save it to disk,
    and return it as a NumPy array (BGR format).

    :param driver: Selenium WebDriver instance.
    :param save_path: Path to save the screenshot (PNG).
    :return: Screenshot as a NumPy array in BGR format.
    """
    result = driver.execute_cdp_cmd("Page.captureScreenshot", {
        "format": "png",
...
class TurnstileMatcher:
    ...
    def __init__(
        ...
        """
        Initialize the TurnstileMatcher.
