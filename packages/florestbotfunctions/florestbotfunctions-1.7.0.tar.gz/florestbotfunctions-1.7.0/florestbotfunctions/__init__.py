...
class FunctionsObject:
    ...
    def cut_link(self, url: str, proxies: dict[str, str] = None) -> str:
        ...
        if request.text != 'limited':
            ...
            request = requests.get(f'https://clck.ru/--', params={"url":url}, headers=self.headers, proxies=proxies if proxies else self.proxies)
            return request.text
    def detect_new_kworks(self, category: int = 11, pages: int = 1, delay: int = 300, message: str = 'Новый кворк!\nИмя кворка: $name$\nЦена: $amout$\nСсылка: $url$\nАвтор: $username$\nВремя активности: $time$.', telegram_bot_token: str = None, telegram_chat_id: int = None):
        """Привет! Эта функция - враппер для отслеживания новых предложений на бирже Kwork.\ncategory: категория для поиска. 11 - это программирование.\npages: какое количество страниц проверять?\ndelay: задержка между проверками биржи.\nmessage: сообщение, которое пишется в консоль и в бота. $name$ - имя кворка, $amout$ - цена, $url$ - ссылка, $username$ - ник покупателя, $time$ - время активности кворка.\n### ДЛЯ ОТПРАВКИ В TELEGRAM\ntelegram_bot_token: токен бота для отправки.\ntelegram_chat_id: ID пользователя, которому бот должен написать, или группы. Не забудьте его запустить с помощью команды /start."""
        ...
        while True:
            ...
            for kwork in new_kworks:
                if kwork.url in new:
                    ...
                    formatted_message = message.replace('$name$', kwork.name).replace('$amout$', str(kwork.possible_price_limit)).replace('$url$', kwork.url).replace('$username$', kwork.username).replace('$time$', kwork.date_active)
                    print(formatted_message)
                    if all([telegram_bot_token, telegram_chat_id]):
                        requests.post(f'https://api.telegram.org/bot{telegram_bot_token}/sendMessage', data={"text":formatted_message, "chat_id":telegram_chat_id}, proxies=self.proxies).raise_for_status()
...
class AsyncFunctionsObject:
    ...
    async def detect_new_kworks(self, category: int = 11, pages: int = 1, delay: int = 300, message: str = 'Новый кворк!\nИмя кворка: $name$\nЦена: $amout$\nСсылка: $url$\nАвтор: $username$\nВремя активности: $time$.', *, telegram_bot_token: str = None, telegram_chat_id: int = None):
        """Привет! Эта функция - враппер для отслеживания новых предложений на бирже Kwork.\ncategory: категория для поиска. 11 - это программирование.\npages: какое количество страниц проверять?\ndelay: задержка между проверками биржи.\nmessage: сообщение, которое пишется в консоль и в бота. $name$ - имя кворка, $amout$ - цена, $url$ - ссылка, $username$ - ник покупателя, $time$ - время активности кворка.\n### ДЛЯ ОТПРАВКИ В TELEGRAM\ntelegram_bot_token: токен бота для отправки.\ntelegram_chat_id: ID пользователя, которому бот должен написать, или группы. Не забудьте его запустить с помощью команды /start."""
        return await asyncio.to_thread(self.sync_functions_object.detect_new_kworks, category, pages, delay, message, telegram_bot_token, telegram_chat_id)
