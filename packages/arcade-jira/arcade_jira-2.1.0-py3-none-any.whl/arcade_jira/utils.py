...
def build_issue_url(cloud_name: str | None, project_key: str, issue_key: str) -> str | None:
    ...
    if not cloud_name or not project_key or not issue_key:
        return None

    return f"https://{cloud_name}.atlassian.net/jira/software/projects/{project_key}/list?selectedIssue={issue_key}"


def build_project_url(cloud_name: str | None, project_key: str) -> str | None:
    """Build a URL to a Jira project summary."""
    if not cloud_name or not project_key:
        return None
...
def build_user_url(cloud_name: str | None, user_id: str) -> str | None:
    """Build a URL to a user's profile."""
    if not cloud_name or not user_id:
        return None
...
async def resolve_cloud_id_and_name(context: ToolContext, cloud_id: str | None) -> dict[str, str]:
    ...
    This method should be used by tools that need to build UI URLs.

    Returns:
        dict with keys 'cloud_id' and 'cloud_name'
    """
...
async def get_cloud_name_by_cloud_id(context: ToolContext, cloud_id: str) -> str:
    ...
    cloud_data = await _find_cloud_by_identifier(context, cloud_id, match_by_id=True)
    return cloud_data["cloud_name"]
...
# Private helper methods (organized at the end for better reusability)
