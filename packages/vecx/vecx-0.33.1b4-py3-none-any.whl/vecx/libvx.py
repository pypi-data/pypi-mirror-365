import os
import platform
import numpy as np
import cffi
from functools import lru_cache

ffi = cffi.FFI()

# Define the function signatures expected from the shared library
ffi.cdef("""
    typedef struct VecX VecX;

    VecX *vecx_create(const char *key, const char *lib_token, const char *space_type, int version, int dimension);
    void vecx_destroy(VecX *obj);

    void vecx_encrypt_vector(VecX *obj, const float *input, float *output, int dimension);
    void vecx_decrypt_vector(VecX *obj, const float *input, float *output, int dimension);
    
    void vecx_encrypt_vectors(VecX *obj, const float *input, float *output, int dimension, int num_vectors);
    void vecx_decrypt_vectors(VecX *obj, const float *input, float *output, int dimension, int num_vectors);
    void vecx_decrypt_and_calculate_similarities(VecX *obj, const float* query_vector, const float* encrypted_vectors,
                                                  int dimension, int num_vectors, float* out_similarities);

    unsigned char *vecx_encrypt_meta(VecX *obj, const unsigned char *input, int length, int *out_length);
    unsigned char *vecx_decrypt_meta(VecX *obj, const unsigned char *input, int length, int *out_length);
    void vecx_free_buffer(unsigned char *buffer);

    void vecx_calculate_distances(VecX* obj, const float* query_vector, int dimension,
                                  const float* input_vectors, int num_vectors, float* out_distances);

    // Sparse encryption functions
    typedef struct SparsePermutationC SparsePermutationC;
    
    SparsePermutationC* sparse_permutation_create(const char* key, int max_size);
    void sparse_permutation_destroy(SparsePermutationC* obj);
    
    void sparse_permutation_encrypt_sparse_vector(SparsePermutationC* obj, 
                                                 const int* indices, const float* values, int num_elements,
                                                 int* encrypted_indices, float* encrypted_values);
    
    void sparse_permutation_decrypt_sparse_vector(SparsePermutationC* obj,
                                                 const int* encrypted_indices, const float* encrypted_values, int num_elements,
                                                 int* indices, float* values);
    

    
    int sparse_permutation_get_max_size(SparsePermutationC* obj);
    void sparse_permutation_resize(SparsePermutationC* obj, int new_size);
""")



@lru_cache(maxsize=0)
def load_libvx():
    """Loads the correct shared library based on the OS and architecture."""
    system = platform.system().lower()
    arch = platform.machine().lower()

    if system == "linux":
        library_name = "libvx_x86_64.so" if arch in ["x86_64", "amd64"] else "libvx_arm64.so"
    elif system == "darwin":
        library_name = "libvx_x86_64.dylib" if arch == "x86_64" else "libvx_arm64.dylib"
    elif system == "windows":
        library_name = "libvx_x86_64.dll"
    else:
        raise Exception(f"Unsupported operating system: {system}")
    
    library_path = os.path.join(os.path.dirname(__file__), "libvx", library_name)
    if not os.path.exists(library_path):
        raise Exception(f"Library file not found: {library_path}")
    
    print(f"Loading library: {library_path}")
    return ffi.dlopen(library_path)

class LibVectorX:
    def __init__(self, key:str, lib_token:str, space_type:str, version:int, dimension:int, sparse_max_size:int=None):
        self.key = key.encode('utf-8')
        self.lib_token = lib_token.encode('utf-8')
        self.dimension = dimension
        self.space_type = space_type.encode('utf-8')
        self.version = version
        self.sparse_max_size = sparse_max_size
        self.lib = load_libvx()
        
        self.vecx = self.lib.vecx_create(self.key, self.lib_token, self.space_type, self.version, self.dimension)
        if not self.vecx:
            raise RuntimeError("Failed to initialize VectorX instance")
        
        # Initialize sparse encryption if max_size is provided
        self.sparse_enc = None
        if sparse_max_size is not None:
            # Use the key as hex key for sparse encryption
            hex_key = key  # Assume key is already in hex format
            self.sparse_enc = self.lib.sparse_permutation_create(hex_key.encode('utf-8'), sparse_max_size)
    
    def __del__(self):
        if hasattr(self, 'vecx') and self.vecx:
            self.lib.vecx_destroy(self.vecx)
        if hasattr(self, 'sparse_enc') and self.sparse_enc:
            self.lib.sparse_permutation_destroy(self.sparse_enc)
    
    def encrypt_vector(self, vector):
        input_array = np.array(vector, dtype=np.float32)
        output_array = np.zeros(self.dimension, dtype=np.float32)
        self.lib.vecx_encrypt_vector(self.vecx, ffi.cast("float*", input_array.ctypes.data), ffi.cast("float*", output_array.ctypes.data), self.dimension)
        return output_array
    
    def decrypt_vector(self, vector):
        input_array = np.array(vector, dtype=np.float32)
        output_array = np.zeros(self.dimension, dtype=np.float32)
        self.lib.vecx_decrypt_vector(self.vecx, ffi.cast("float*", input_array.ctypes.data), ffi.cast("float*", output_array.ctypes.data), self.dimension)
        return output_array
    
    def encrypt_meta(self, meta):
        input_array = np.frombuffer(meta, dtype=np.uint8)
        output_array = np.zeros_like(input_array, dtype=np.uint8)
        self.lib.vecx_encrypt_meta(self.vecx, ffi.cast("unsigned char*", input_array.ctypes.data), ffi.cast("unsigned char*", output_array.ctypes.data), len(input_array))
        return output_array.tobytes()
    
    def decrypt_meta(self, meta):
        input_array = np.frombuffer(meta, dtype=np.uint8)
        output_array = np.zeros_like(input_array, dtype=np.uint8)
        self.lib.vecx_decrypt_meta(self.vecx, ffi.cast("unsigned char*", input_array.ctypes.data), ffi.cast("unsigned char*", output_array.ctypes.data), len(input_array))
        return output_array.tobytes()
    
    def calculate_distances(self, query_vector, vectors):
        query_array = np.array(query_vector, dtype=np.float32)
        vectors_array = np.array(vectors, dtype=np.float32)
        num_vectors = vectors_array.shape[0]

        out_distances = np.empty(num_vectors, dtype=np.float32)  # Allocate buffer in Python

        self.lib.vecx_calculate_distances(
            self.vecx,
            ffi.cast("float*", query_array.ctypes.data),
            self.dimension,
            ffi.cast("float*", vectors_array.ctypes.data),
            num_vectors,
            ffi.cast("float*", out_distances.ctypes.data)  # Pass preallocated buffer
        )

        return out_distances  # No need to free, Python handles it

    def encrypt_meta(self, meta):
        """Encrypt metadata (binary data)."""
        meta_bytes = bytes(meta)  # Ensure meta is in bytes
        out_length = ffi.new("int *")  # Allocate space for output length

        # Use ffi.from_buffer to create a buffer from bytes
        result = self.lib.vecx_encrypt_meta(self.vecx, ffi.from_buffer("unsigned char[]", meta_bytes), len(meta_bytes), out_length)

        if result == ffi.NULL:
            raise RuntimeError("Encryption failed")

        encrypted_data = ffi.buffer(result, out_length[0])[:]  # Convert C buffer to Python bytes
        self.lib.vecx_free_buffer(result)  # Free memory allocated by C++
        
        return encrypted_data

    def decrypt_meta(self, encrypted_meta: bytes) -> bytes:
        """Decrypt metadata (binary data)."""
        out_length = ffi.new("int*")  # Allocate space for output length

        # Use ffi.from_buffer to safely pass bytes to C
        result = self.lib.vecx_decrypt_meta(self.vecx, ffi.from_buffer("unsigned char[]", encrypted_meta), len(encrypted_meta), out_length)

        if result == ffi.NULL:
            raise RuntimeError("Decryption failed.")

        data = ffi.buffer(result, out_length[0])[:]  # Convert C buffer to Python bytes
        self.lib.vecx_free_buffer(result)  # Free allocated memory in C++
        
        return data
    def encrypt_vectors(self, matrix):
        matrix = np.ascontiguousarray(matrix, dtype=np.float32)
        if matrix.shape[1] != self.dimension:
            raise ValueError(f"Expected shape (*, {self.dimension}), got {matrix.shape}")
        num_vectors = matrix.shape[0]
        output = np.empty_like(matrix)
        self.lib.vecx_encrypt_vectors(
            self.vecx,
            ffi.cast("float*", matrix.ctypes.data),
            ffi.cast("float*", output.ctypes.data),
            self.dimension,
            num_vectors
        )
        return output

    def decrypt_vectors(self, matrix):
        matrix = np.ascontiguousarray(matrix, dtype=np.float32)
        if matrix.shape[1] != self.dimension:
            raise ValueError(f"Expected shape (*, {self.dimension}), got {matrix.shape}")
        num_vectors = matrix.shape[0]
        output = np.empty_like(matrix)
        self.lib.vecx_decrypt_vectors(
            self.vecx,
            ffi.cast("float*", matrix.ctypes.data),
            ffi.cast("float*", output.ctypes.data),
            self.dimension,
            num_vectors
        )
        return output

    def decrypt_and_calculate_similarities(self, query_vector, encrypted_matrix):
        query = np.asarray(query_vector, dtype=np.float32)
        matrix = np.ascontiguousarray(encrypted_matrix, dtype=np.float32)
        if query.shape[0] != self.dimension:
            raise ValueError("Query vector has incorrect dimension.")
        if matrix.shape[1] != self.dimension:
            raise ValueError("Matrix vectors must match dimension.")
        num_vectors = matrix.shape[0]
        similarities = np.empty(num_vectors, dtype=np.float32)
        self.lib.vecx_decrypt_and_calculate_similarities(
            self.vecx,
            ffi.cast("float*", query.ctypes.data),
            ffi.cast("float*", matrix.ctypes.data),
            self.dimension,
            num_vectors,
            ffi.cast("float*", similarities.ctypes.data)
        )
        return similarities

    # Sparse encryption methods
    def encrypt_sparse_vector(self, indices, values):
        """
        Encrypt a sparse vector by shuffling its indices.
        
        Args:
            indices: List or numpy array of indices
            values: List or numpy array of values corresponding to indices
            
        Returns:
            tuple of (encrypted_indices, encrypted_values) as Python lists
            
        Note:
            Requires sparse_max_size to be set during initialization
        """
        if self.sparse_enc is None:
            raise RuntimeError("Sparse encryption not initialized. Set sparse_max_size during initialization.")
        
        if len(indices) != len(values):
            raise ValueError("Indices and values must have the same length")
        
        indices_array = np.array(indices, dtype=np.int32)
        values_array = np.array(values, dtype=np.float32)
        
        encrypted_indices = np.empty_like(indices_array)
        encrypted_values = np.empty_like(values_array)
        
        self.lib.sparse_permutation_encrypt_sparse_vector(
            self.sparse_enc,
            ffi.cast("int*", indices_array.ctypes.data),
            ffi.cast("float*", values_array.ctypes.data),
            len(indices_array),
            ffi.cast("int*", encrypted_indices.ctypes.data),
            ffi.cast("float*", encrypted_values.ctypes.data)
        )
        
        return [int(x) for x in encrypted_indices], [float(x) for x in encrypted_values]
    
    def decrypt_sparse_vector(self, encrypted_indices, encrypted_values):
        """
        Decrypt a sparse vector by restoring its original indices.
        
        Args:
            encrypted_indices: List or numpy array of encrypted indices
            encrypted_values: List or numpy array of encrypted values
            
        Returns:
            tuple of (indices, values) as Python lists
            
        Note:
            Requires sparse_max_size to be set during initialization
        """
        if self.sparse_enc is None:
            raise RuntimeError("Sparse encryption not initialized. Set sparse_max_size during initialization.")
        
        if len(encrypted_indices) != len(encrypted_values):
            raise ValueError("Encrypted indices and values must have the same length")
        
        enc_indices_array = np.array(encrypted_indices, dtype=np.int32)
        enc_values_array = np.array(encrypted_values, dtype=np.float32)
        
        indices = np.empty_like(enc_indices_array)
        values = np.empty_like(enc_values_array)
        
        self.lib.sparse_permutation_decrypt_sparse_vector(
            self.sparse_enc,
            ffi.cast("int*", enc_indices_array.ctypes.data),
            ffi.cast("float*", enc_values_array.ctypes.data),
            len(enc_indices_array),
            ffi.cast("int*", indices.ctypes.data),
            ffi.cast("float*", values.ctypes.data)
        )
        
        return [int(x) for x in indices], [float(x) for x in values]
    
    def encrypt_sparse_vectors_batch(self, indices_list, values_list):
        """
        Encrypt multiple sparse vectors in batch.
        
        Args:
            indices_list: List of indices arrays
            values_list: List of values arrays
            
        Returns:
            tuple of (encrypted_indices_list, encrypted_values_list)
            
        Note:
            Requires sparse_max_size to be set during initialization
        """
        if self.sparse_enc is None:
            raise RuntimeError("Sparse encryption not initialized. Set sparse_max_size during initialization.")
        
        if len(indices_list) != len(values_list):
            raise ValueError("Indices and values lists must have the same length")
        
        # Process each sparse vector individually
        encrypted_indices_list = []
        encrypted_values_list = []
        
        for indices, values in zip(indices_list, values_list):
            encrypted_indices, encrypted_values = self.encrypt_sparse_vector(indices, values)
            encrypted_indices_list.append(encrypted_indices)
            encrypted_values_list.append(encrypted_values)
        
        return encrypted_indices_list, encrypted_values_list
    
    def decrypt_sparse_vectors_batch(self, encrypted_indices_list, encrypted_values_list):
        """
        Decrypt multiple sparse vectors in batch.
        
        Args:
            encrypted_indices_list: List of encrypted indices arrays
            encrypted_values_list: List of encrypted values arrays
            
        Returns:
            tuple of (indices_list, values_list)
            
        Note:
            Requires sparse_max_size to be set during initialization
        """
        if self.sparse_enc is None:
            raise RuntimeError("Sparse encryption not initialized. Set sparse_max_size during initialization.")
        
        if len(encrypted_indices_list) != len(encrypted_values_list):
            raise ValueError("Encrypted indices and values lists must have the same length")
        
        # Process each sparse vector individually
        indices_list = []
        values_list = []
        
        for encrypted_indices, encrypted_values in zip(encrypted_indices_list, encrypted_values_list):
            indices, values = self.decrypt_sparse_vector(encrypted_indices, encrypted_values)
            indices_list.append(indices)
            values_list.append(values)
        
        return indices_list, values_list
    
    def get_sparse_max_size(self):
        """
        Get the maximum size of the sparse encryption matrix.
        
        Returns:
            int: Maximum size if sparse encryption is initialized, None otherwise
        """
        if self.sparse_enc is None:
            return None
        return self.lib.sparse_permutation_get_max_size(self.sparse_enc)
    
    def resize_sparse_encryption(self, new_size):
        """
        Resize the sparse encryption matrix to a new size.
        
        Args:
            new_size: New maximum size for the encryption matrix
            
        Note:
            Requires sparse_max_size to be set during initialization
        """
        if self.sparse_enc is None:
            raise RuntimeError("Sparse encryption not initialized. Set sparse_max_size during initialization.")
        
        self.sparse_max_size = new_size
        self.lib.sparse_permutation_resize(self.sparse_enc, new_size)


@lru_cache(maxsize=None)
def get_libvx(key:str, lib_token:str, space_type:str, version:int, dimension:int, sparse_max_size:int=None):
    return LibVectorX(key, lib_token, space_type, version, dimension, sparse_max_size)


