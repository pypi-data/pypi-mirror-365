...
class CheckDefaultWriteAccess(Check):
    ...
    def check_metadata(
        ...
        sections = [s for s in meta.sections() if s.name in self.DEFAULT_STANZAS]
        ...
        for section in sections:
            ...
            if section.has_option("access"):
                latest_result = self._check_section(section)
                found_access = True
        ...
        if latest_result:
            ...
            yield FailMessage(
                ...
                file_name=meta.get_relative_path(),
            )

    def _check_section(self, section: "ConfigurationSection" | "MergedConfigurationSection"):
        access_option = section.get_option("access")
