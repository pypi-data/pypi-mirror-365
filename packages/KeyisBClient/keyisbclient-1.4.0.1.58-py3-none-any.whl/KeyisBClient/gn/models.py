...
class GNRequest:
    ...
    def serialize(self, frame: bool = False, mode: int = 0) -> bytes:
        ...
        return _pack(mode, self.stream, len(blob)) + blob if frame else blob

    @staticmethod
    def deserialize(data: bytes, frame: bool = False, mode: int = 0) -> 'GNRequest':
        ...
        unpacked: dict = msgpack.loads(data, raw=False)
        _url = Url(unpacked[2])
        if not _url.method:
            _url.method = unpacked[1]

        pr = unpacked.get(3, {}).get(1)

        if pr is True:
            pr = 'gn:proxy:request-to-real-server'
...
class GNResponse:
    ...
    @staticmethod
    def deserialize(data: bytes, frame: bool = False, mode: int = 0) -> 'GNResponse':
        if frame:
            if len(data) < 8:
                raise Exception('len')
        ...
        if frame:
            cm = unpacked.get(3, {}).get(1)
            if cm is True:
                cm = 'ok'

        return GNResponse(
            command=unpacked.get(1, 'gn:not_command'),
            payload=unpacked.get(2),
            stream=stream if stream is not None else unpacked.get(3)
        )
