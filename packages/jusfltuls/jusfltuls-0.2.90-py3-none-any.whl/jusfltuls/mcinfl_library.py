...
# def export_measurement_to_file(measurement, database, filepath):
#     client = InfluxDBClient(database=database)
#     query = f'SELECT * FROM "{measurement}"'
#     result = client.query(query)
#     points = list(result.get_points(measurement=measurement))
#     if not points:
#         return f"No data in measurement {measurement} of {database}"
#     keys = points[0].keys()
#     with open(filepath, 'w', newline='') as f:
#         writer = csv.DictWriter(f, fieldnames=keys)
#         writer.writeheader()
#         writer.writerows(points)
#     return f"Exported {measurement} from {database} to {filepath}"


# def import_measurement_from_file(filepath, measurement, database):
...
#         points = []
#         for row in reader:
#             # Convert row to InfluxDB line protocol point dict
#             # Assuming all fields are fields, no tags, and time is optional
#             point = {
...
#             points.append(point)
#         if not points:
#             return f"No data in file {filepath}"
#         client.write_points(points)
#     return f"Imported {measurement} into {database} from {filepath}"
...
def restore_portable_get_oldname(PATH, database=None):
    ...
    with open(manifest_file[0], "r") as f:
        ...
        with open(LOGFILENAME, "a") as f:
            f.write(f"\nmanifest:    {manidbs}  {type(manidbs)}  ==set\n\n")
            f.write(f"\nmanifest:    {manidbs}  {len(manidbs)}    ==1\n\n")
            ##f.write(f"\nmanifest:    {manidbs}  {manidbs[0]}    ==1\n\n")

        if len(manidbs) > 1:
            with open(LOGFILENAME, "a") as f:
                f.write(f"\nmanifest file databases != 1 {manidbs} \n\n")
            return None
