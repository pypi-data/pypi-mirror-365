...
class Graph():
    def __init__(self,
        ...
        self._EdgeConceptStr = f"_graph{self._graph_id}_Edge"
    ...
    @cached_property
    def Edge(self):
        """Lazily define and cache the self.Edge concept, and associated logic."""
        ...
        where(_Edge, not_(_Edge.src)).define(
            Error.new(message="All `Edge`s must have a `src`.", edge=_Edge)
        )

        # All `Edge`s must have a `dst`:
        where(_Edge, not_(_Edge.dst)).define(
            Error.new(message="All `Edge`s must have a `dst`.", edge=_Edge)
        )

        # If weighted, all `Edge`s must have a `weight`:
        if self.weighted:
            where(_Edge, not_(_Edge.weight)).define(
        ...
        # If not weighted, no `Edge`s may have a `weight`:
            ...
            where(_Edge, _Edge.weight).define(
        ...
        # the other node's `dst` and vice versa.
            ...
            where(
                edge_a, edge_b,
                edge_a < edge_b, # implies edge_a != edge_b
                where(edge_a.src == edge_b.src, edge_a.dst == edge_b.dst) |
                where(edge_a.src == edge_b.dst, edge_a.dst == edge_b.src)
            ).define(
                Error.new(
                    message=(
                        "Multi-edges are not allowed when `aggregator=None`. "
                    ...
                    edge_a=edge_a,
                    edge_b=edge_b,
                    edge_a_src=edge_a.src,
                    edge_b_src=edge_b.src,
                    edge_a_dst=edge_a.dst,
                    edge_b_dst=edge_b.dst,
                )
            )
    ...
    def _edge(self):
        ...
        consume the `Edge` concept's `src` and `dst` relationships.
        """
        ...
        if self.directed:
            define(
                _edge_rel(Edge.src, Edge.dst)
            )
        elif not self.directed:
            define(
                _edge_rel(Edge.src, Edge.dst),
                _edge_rel(Edge.dst, Edge.src)
            )
    ...
    def _weight(self):
        ...
        consume the `Edge` concept's `src`, `dst`, and `weight` relationships.
        """
        ...
        if self.directed and self.weighted:
            define(
                _weight_rel(Edge.src, Edge.dst, Edge.weight),
            )
        elif self.directed and not self.weighted:
            define(
                _weight_rel(Edge.src, Edge.dst, 1.0),
            )
        elif not self.directed and self.weighted:
            define(
                _weight_rel(Edge.src, Edge.dst, Edge.weight),
                _weight_rel(Edge.dst, Edge.src, Edge.weight),
            )
        elif not self.directed and not self.weighted:
            define(
                _weight_rel(Edge.src, Edge.dst, 1.0),
                _weight_rel(Edge.dst, Edge.src, 1.0),
            )
    ...
    # At this time, _isolated_node is primarily a helper for preferential_attachment.
    @cached_property
    ...
    def is_connected(self):
        ...
        **Disconnected Graph Example**
        ...
        ...     Edge(src=n1, dst=n2),
        ...     Edge(src=n3, dst=n2),
        ...     Edge(src=n4, dst=n5)  # This edge creates a separate component
        ... )
        >>>
