...
class TestVibeTests:
    ...
    def model(self, request):
        """Get model from environment variable, command line, or use default."""
        import os
        
        # Try environment variable first (for PyPI installs)
        env_model = os.environ.get('VIBETEST_MODEL')
        if env_model:
            return env_model
            
        # Try command line argument (for development)
        return getattr(request.config.option, 'model', 'gemma3:4b')
    
    @pytest.fixture
    def iterations(self, request):
        """Get number of iterations from environment variable, command line, or use default."""
        import os
        
        # Try environment variable first (for PyPI installs)
        env_iterations = os.environ.get('VIBETEST_ITERATIONS')
        if env_iterations:
            try:
