...
class SlackService(SlackLibClient):
    ...
    def get_members(self, exclude_bots: bool = True) -> dict[str, SlackProfile]:
        ...
        return {
            ...
            and not (exclude_bots and member.is_bot)
        }
    ...
    def get_users_id_name(self, exclude_bots: bool = True) -> dict[str, str]:
        ...
        return {
            ...
            if not user.deleted
            and not (exclude_bots and user.is_bot)
        }
    ...
    def get_user_id_email(self, email: str) -> Optional[str]:
        ...
        """
        return next(
            (user.id for user in self._get_users() if user.email == email),
            None,
        )
    ...
    def _get_users(self) -> Iterator[SlackProfile]:
        """
    ...
    def fetch_members(self, exclude_bots: bool = True) -> list[SlackProfile]:
        """
        Gets a list of all users in the workspace.
