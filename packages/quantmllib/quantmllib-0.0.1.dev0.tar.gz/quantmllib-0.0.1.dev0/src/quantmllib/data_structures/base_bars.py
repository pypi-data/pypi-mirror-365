...
class BaseBars(ABC):
    ...
    def batch_run(self, file_path_or_df: str | Iterable[str] | pd.DataFrame, verbose: bool = True, to_csv: bool = False,
                  output_path: str | None = None) -> pd.DataFrame | None:
        """
    ...
    def _batch_iterator(self, file_path_or_df: str | Iterable[str] | pd.DataFrame) -> Generator[pd.DataFrame, None, None]:
        """
        ...
        if isinstance(file_path_or_df, (list, tuple)):
            ...
            for file_path in file_path_or_df:
                yield from pd.read_csv(file_path, chunksize=self.batch_size, parse_dates=[0])

        elif isinstance(file_path_or_df, str):
            self._read_first_row(file_path_or_df)
            yield from pd.read_csv(file_path_or_df, chunksize=self.batch_size, parse_dates=[0])

        elif isinstance(file_path_or_df, pd.DataFrame):
            yield from _crop_data_frame_in_batches(file_path_or_df, self.batch_size)
    ...
    def run(self, data: list | tuple | pd.DataFrame) -> list:
        """
    ...
    def _update_high_low(self, price: float) -> float | float:
        """
