...
def _improve_clusters(corr_mat: pd.DataFrame, clusters: dict, top_clusters: dict) -> (
...
def cluster_kmeans_top(corr_mat: pd.DataFrame, repeat: int = 10) -> pd.DataFrame | dict | pd.Series | bool:
    """
...
def get_onc_clusters(corr_mat: pd.DataFrame, repeat: int = 10) -> pd.DataFrame | dict | pd.Series:
    """
