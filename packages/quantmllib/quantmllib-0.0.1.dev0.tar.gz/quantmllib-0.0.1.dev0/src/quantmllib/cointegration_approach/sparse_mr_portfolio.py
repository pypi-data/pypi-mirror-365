...
from sklearn.linear_model import Lasso, MultiTaskLasso, lasso_path
from sklearn.preprocessing import StandardScaler, normalize

from quantmllib.optimal_mean_reversion import OrnsteinUhlenbeck
...
class SparseMeanReversionPortfolio:
    ...
    def sparse_eigen_deflate(self, sdp_result: np.array, cardinality: int, tol: float = 1e-6, max_iter: int = 100,
        ...
        while iteration <= max_iter:
            if verbose:
                print(f"Iteration: {iteration}, Objective function value: {f_old}")
            # Power step
            s = np.squeeze(sdp_result @ x)
            g = 2 * s
