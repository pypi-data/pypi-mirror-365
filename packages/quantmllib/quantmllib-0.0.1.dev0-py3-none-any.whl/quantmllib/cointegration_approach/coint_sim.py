...
class CointegrationSimulation:
    ...
    def load_params(self, params: dict, target: str = "price"):
        ...
        if target not in target_types:
            raise ValueError(f"Invalid parameter dictionary type. Expect one of: {target_types}")
    ...
    def verify_ar(self, price_matrix: np.array) -> tuple[float, float | None]:
        """
    ...
    def verify_coint(self, price_series_x: np.array, price_series_y: np.array,
                     x_name: str = "Share S1", y_name: str = "Share S2") -> tuple[float, float | None]:
        """
