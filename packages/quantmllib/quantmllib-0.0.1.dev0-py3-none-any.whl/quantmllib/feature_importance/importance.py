...
from quantmllib.cross_validation.cross_validation import ml_cross_val_score
...
def mean_decrease_accuracy(model, X, y, cv_gen, clustered_subsets=None, sample_weight_train=None,
    ...
    * The CV must be purged and embargoed.
    ...
    # TODO: Verify the accuracy of bellow implementation, as it was created by ChatGPT
    results = []
    for i in range(n_repeat):
        res = _mean_decrease_accuracy_round(
            model, X, y, cv_gen,
    ...
    if len(results) == 1:
        return results[0]
    # Average mean and std across repeats
    mean_df = pd.concat([r["mean"] for r in results], axis=1).mean(axis=1)
    std_df = pd.concat([r["mean"] for r in results], axis=1).std(axis=1) * (len(results))**-0.5
    out = pd.DataFrame({"mean": mean_df, "std": std_df})
    return out
...
def plot_feature_importance(importance_df, oob_score, oos_score, save_fig=False, output_path=None):
    ...
    plt.title(f"Feature importance. OOB Score:{round(oob_score, 4)}; OOS score:{round(oos_score, 4)}")
