...
class AbstractModelFingerprint(ABC):
    ...
    def plot_effects(self) -> plt.figure:
        ...
        if self.pair_wise_effect is None:
            ...
            ax3.bar(*zip(*self.pair_wise_effect["norm"].items(), strict=False))

        ax1.set_title("Linear effect")
        ax1.bar(*zip(*self.linear_effect["norm"].items(), strict=False))

        ax2.set_title("Non-Linear effect")
        ax2.bar(*zip(*self.non_linear_effect["norm"].items(), strict=False))
    ...
    def _get_feature_values(self, X: pd.DataFrame, num_values: int) -> None:
        ...
        self.feature_column_position_mapping = dict(zip(X.columns, range(0, X.shape[1]), strict=False))
    ...
    def _get_pairwise_effect(self, pairwise_combinations: list, model: object, X: pd.DataFrame, num_values) -> dict:
        ...
        for pair in pairwise_combinations:
            ...
            for x_k, y_cdf_k in zip(self.feature_values[col_k], y_cdf_k_centered, strict=False):
                for x_l, y_cdf_l in zip(self.feature_values[col_l], y_cdf_l_centered, strict=False):
