...
class DashGraph:
    ...
    def _generate_layout(self):
        ...
        layout_input = [
            html.H1(f"{graph_type} from {self.graph.get_matrix_type()} matrix"),
            html.Hr(),
            dbc.Row(
                [
    ...
    def _style_colours(self):
        ...
        if self.colour_groups:
            ...
            for item in keys:
                new_colour = {
                    "selector": f'node[colour_group="{item}"]',
                    "style": {
                        "background-color": f"{self.colour_groups[item]}",
                    }
                }
    ...
    def _assign_sizes(self):
        ...
        new_sizes = {
            "selector": "node",
            "style": {
                "width": f"mapData(size, {min_size}, {max_size}, 25, 250)",
                "height": f"mapData(size, {min_size}, {max_size}, 25, 250)",
            }
        }
        ...
        stylesheet = \
            [
                ...
                },
                {
                    "selector": "[weight => 0]",
                    "style": {
                        "width": f"mapData(weight, 0, {max(self.weights)}, 1, 8)",
                    }
                },
                {
                    "selector": "[weight < 0]",
                    "style": {
                        "width": f"mapData(weight, 0, {min(self.weights)}, 1, 8)",
                    }
                }
            ]
        return stylesheet
