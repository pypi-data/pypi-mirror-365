...
class ToolUsageDisplay:
    ...
    def display_agent_usage(self, agent_name: str) -> None:
        ...
        table = Table(title=f"🤖 {agent_name.title()} Agent - Tool Usage", show_header=True)
    ...
    def display_overall_usage(self) -> None:
        ...
        header_table = Table(title="🔧 Overall Tool Usage Statistics", show_header=True)
        ...
        if usage['most_popular_tools']:
            tools_table = Table(title="🏆 Most Popular Tools", show_header=True)
    ...
    def display_tool_details(self, tool_name: str) -> None:
        ...
        # Tool details table
        table = Table(title=f"🔧 {tool_name} - Detailed Usage", show_header=True)
        table.add_column("Metric", style="cyan")
        table.add_column("Value", style="white")
        ...
        # Recent usage
        if usage['recent_usage']:
            recent_table = Table(title="🕐 Recent Usage", show_header=True)
            ...
            while True:
                ...
                if usage['agent_activity']:
                    for agent, calls in usage['agent_activity'].items():
                        ...
                        if recent:
                            self.console.print(f"🤖 {agent}: {calls} calls (Last: {recent['tool']} at {recent['time']})")
                        else:
                            self.console.print(f"🤖 {agent}: {calls} calls")
    ...
    def clear_usage_history(self) -> None:
        ...
        self.console.print("[green]✅ Tool usage history cleared[/green]")