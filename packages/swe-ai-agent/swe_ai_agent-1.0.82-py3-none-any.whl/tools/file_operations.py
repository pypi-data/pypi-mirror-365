...
# Import our debug and diff utilities
import sys
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from utils.dump import dump
from utils.diff_display import show_file_diff, show_progressive_diff
...
class FileOperations:
    ...
    def __init__(self, base_path: str = None, show_diffs: bool = False, debug_mode: bool = False):
        """
        ...
        Args:
            ...
            show_diffs: Whether to show diff visualizations during file operations
            debug_mode: Whether to enable debug dump output
        """
    ...
    def _show_create_diff(self, file_path: str, content: str):
        """Show diff for file creation."""
        from utils.progress_tracker import RealTimeInterface
        import inspect
        
        # Try to get real-time interface from call stack
        try:
            for frame_info in inspect.stack():
                frame_locals = frame_info.frame.f_locals
                if 'real_time_interface' in frame_locals:
                    real_time_interface = frame_locals['real_time_interface']
                    if hasattr(real_time_interface, 'display_diff'):
                        real_time_interface.display_diff(file_path, "", content)
                        return
        except:
            pass
        ...
        diff_output = show_file_diff("", content, file_path)
        console.print(f"\n[bold yellow]📋 New File: {file_path}[/bold yellow]")
        console.print(Panel(
            diff_output,
            title=f"[bold]Created: {file_path}[/bold]",
            border_style="green",
            expand=False
        ))
        console.print()
    ...
    def create_file(self, file_path: str, content: str = "", is_directory: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if is_directory:
                ...
                if self.show_diffs:
                    self._show_create_diff(file_path, content)
    ...
    def edit_file(self, file_path: Optional[str] = None, text: str = "", start_line: int = 1, 
                  end_line: Optional[int] = None) -> Dict[str, Any]:
        """
        Edit a file by replacing content at specific line numbers with debug and diff support.
        ...
        Returns:
            Dict with 'success', 'old_text', 'new_text', 'message', 'diff', and optional 'error'
        """
        try:
            if file_path:
            ...
            # Debug: Show operation parameters
            if self.debug_mode:
                dump(file_path, start_line, end_line, len(text))
            ...
            if self.debug_mode:
                dump(len(lines), original_content[:100] + "..." if len(original_content) > 100 else original_content)
            ...
            # Generate diff visualization if enabled
            diff_output = ""
            if self.show_diffs:
                diff_output = show_file_diff(original_content, new_content, target_path.name)
                print("📝 File Edit Diff:")
                print(diff_output)
            ...
            if self.debug_mode:
                dump(f"Edit completed successfully", len(new_lines))
            ...
            if self.show_diffs:
                result["diff_display"] = diff_output
            
            return result
            
        except Exception as e:
            if self.debug_mode:
                dump(f"Edit failed with error: {str(e)}")
            return {
                "success": False,
    ...
    def write_file(self, file_path: str, content: str, append: bool = False) -> Dict[str, Any]:
        """
        Write content to a file, completely replacing or appending with debug and diff support.
        ...
        Returns:
            Dict with 'success', 'message', 'diff', and optional 'error'
        """
        try:
            full_path = self.base_path / file_path
            full_path.parent.mkdir(parents=True, exist_ok=True)
            ...
            # Debug: Show operation parameters
            if self.debug_mode:
                dump(file_path, len(content), append)
            
            # Read original content if file exists for diff comparison
            original_content = ""
            if full_path.exists():
                with open(full_path, 'r', encoding='utf-8') as f:
                    original_content = f.read()
            ...
            # Prepare new content
            if append and original_content:
            ...
            # Generate diff visualization if enabled
            diff_output = ""
            if self.show_diffs and (original_content or not append):
                diff_output = show_file_diff(original_content, new_content, full_path.name)
                print(f"📝 File {'Append' if append else 'Write'} Diff:")
                print(diff_output)
            ...
            if self.debug_mode:
                dump(f"Write completed successfully", len(new_content))
            ...
            if self.debug_mode:
                dump(f"Write failed with error: {str(e)}")
            return {
                "success": False,
    ...
    def replace_in_file(self, file_path: Optional[str] = None, old_text: str = "", new_text: str = "", 
                       max_replacements: int = -1) -> Dict[str, Any]:
        """
        Replace text in a file with debug and diff support.
        ...
        Returns:
            Dict with 'success', 'replacements_made', 'message', 'diff', and optional 'error'
        """
        try:
            if file_path:
                target_path = self.base_path / file_path
            ...
            # Debug: Show operation parameters
            if self.debug_mode:
                dump(file_path, old_text[:50] + "..." if len(old_text) > 50 else old_text, 
                     new_text[:50] + "..." if len(new_text) > 50 else new_text, max_replacements)
            ...
            if max_replacements == -1:
                ...
                new_content = original_content.replace(old_text, new_text, max_replacements)
                replacements_made = min(original_content.count(old_text), max_replacements)
            
            if replacements_made == 0:
                return {
                    "success": False,
                    "message": f"Text not found in file: '{old_text[:100]}{'...' if len(old_text) > 100 else ''}'",
            ...
            if self.debug_mode:
                dump(f"Replace completed successfully", replacements_made)
            
            result = {
                "success": True,
                "replacements_made": replacements_made,
                "message": f"Replaced {replacements_made} occurrence(s) in {target_path.name}",
                "diff": diff_output
            }
            ...
            if self.debug_mode:
                dump(f"Replace failed with error: {str(e)}")
            return {
                "success": False,
                "message": f"Error replacing text in file: {str(e)}",
                "error": str(e)
            }
