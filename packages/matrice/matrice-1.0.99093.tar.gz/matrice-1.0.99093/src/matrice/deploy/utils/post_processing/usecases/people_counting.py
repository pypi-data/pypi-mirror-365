...
class PeopleCountingUseCase(BaseProcessor):
    ...
    def _get_current_timestamp_str(self, stream_info: Optional[Dict[str, Any]], precision=False, frame_id: str=None) -> str:
        ...
        if precision:
            if stream_info.get("input_stream", {}).get("feed_type", "live") == "disk":
                if frame_id:
                    start_time = int(frame_id)/stream_info.get("input_stream", {}).get("original_fps", 30)
                else:
                    start_time = stream_info.get("input_stream", {}).get("start_frame", 30)/stream_info.get("input_stream", {}).get("original_fps", 30)
        ...
        if stream_info.get("input_stream", {}).get("feed_type", "live") == "disk":
            ...
            stream_time_str = stream_info.get("input_stream", {}).get("stream_info", {}).get("stream_time", "")
            if stream_time_str:
