...
class VehicleMonitoringUseCase(BaseProcessor):
    ...
    def _get_current_timestamp_str(self, stream_info: Optional[Dict[str, Any]], precision=False) -> str:
        if not stream_info or isinstance(stream_info, str):
            return "00:00:00.00" if not precision else "00:00:00.000000"
        if precision:
        ...
        if stream_info.get("feed_type", "live") == "disk":
            ...
            stream_time_str = stream_info.get("stream_info", {}).get("stream_time", "") if isinstance(stream_info.get("stream_info"), dict) else ""
            if stream_time_str:
                try:
    ...
    def _get_start_timestamp_str(self, stream_info: Optional[Dict[str, Any]], precision=False) -> str:
        if not stream_info or isinstance(stream_info, str):
            return "00:00:00" if not precision else "00:00:00.000000"
        if precision:
            if stream_info.get("feed_type", "live") == "disk":
                return "00:00:00"
            else:
