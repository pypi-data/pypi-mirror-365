...
class AdvancedCustomerServiceUseCase(BaseProcessor):
    ...
    def _generate_per_frame_agg_summary(self, processed_data, analytics_results, config, context, stream_info=None):
        ...
        for idx, frame_id in enumerate(frame_ids):
            ...
            # --- Per-frame timestamp logic (high-precision, never default to 00:00:00.00) ---
            current_timestamp = None
            if stream_info:
                input_settings = stream_info.get("input_settings", {})
                stream_type = input_settings.get("stream_type", "video_file")
                # Prefer high-precision video/stream timestamps if available
                if stream_type == "video_file":
                    video_timestamps = stream_info.get("video_timestamp")
                    if isinstance(video_timestamps, list) and len(video_timestamps) > idx:
                        ts_val = video_timestamps[idx]
                        if len(ts_val) > 8:
                            current_timestamp = ts_val[:12]  # e.g. HH:MM:SS.ms
                        else:
                            current_timestamp = ts_val[:8]
                    elif isinstance(video_timestamps, str):
                        ts_val = video_timestamps
                        if len(ts_val) > 8:
                            current_timestamp = ts_val[:12]
                        else:
                            current_timestamp = ts_val[:8]
                elif stream_type == "stream":
                    stream_times = stream_info.get("stream_time")
                    if isinstance(stream_times, list) and len(stream_times) > idx:
                        ts_val = stream_times[idx]
                        if len(ts_val) > 8:
                            current_timestamp = ts_val[:12]
                        else:
                            current_timestamp = ts_val[:8]
                    elif isinstance(stream_times, str):
                        ts_val = stream_times
                        if len(ts_val) > 8:
                            current_timestamp = ts_val[:12]
                        else:
                            current_timestamp = ts_val[:8]
            # If still not found or looks like a zero timestamp, estimate from frame index and FPS
            if not current_timestamp or current_timestamp.startswith("00:00:00"):
                if fps:
                    total_seconds = idx / fps
                elif wallclock_start_time is not None:
                    total_seconds = time.time() - wallclock_start_time
                else:
                    total_seconds = 0
                ...
                current_timestamp = f"{hours:02d}:{minutes:02d}:{seconds:02d}.{centiseconds:02d}"
