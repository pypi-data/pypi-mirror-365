...
class FlareAnalysisUseCase(BaseProcessor):
        ...
        try:
            ...
            while True:
                ...
                for detection in frame_detections:
                    ...
                    bbox_height = (bbox["ymax"] - bbox["ymin"]) if config.bbox_format == "xmin_ymin_xmax_ymax" else bbox["height"]
                    relative_height = (bbox_height / frame_height) * 100 if frame_height else 0.0
                    flare_record = {
                        "frame_id": frame_key,
    ...
    def _count_categories(self, detections: List[Dict], config: FlareAnalysisConfig) -> Dict:
        ...
        for det in detections:
            ...
            detections_list.append({
                ...
                "main_color": main_color,
                "relative_height": det.get("relative_height")
            })
        return {
    ...
    def _check_alerts(self, summary: Dict, frame_number: Any, config: FlareAnalysisConfig) -> List[Dict]:
        ...
        for detection in summary["detections"]:
            relative_height = detection.get("relative_height", 0.0)
            if relative_height > 40.0:  # Add alert only if relative height > 40%
                category = detection.get("category", "unknown")
                alerts.append({
                    "alert_type": getattr(config.alert_config, 'alert_type', ['Default']) if hasattr(config.alert_config, 'alert_type') else ['Default'],
                    "alert_id": f"alert_{category}_{frame_key}",
                    "incident_category": self.CASE_TYPE,
                    "threshold_level": 40.0,
    ...
    def _generate_tracking_stats(
        ...
        high_precision_reset_timestamp = self._get_current_timestamp_str(stream_info, precision=True)
        camera_info = self.get_camera_info_from_stream(stream_info)
        ...
        for cat, count in per_category_count.items():
            color_info = ", ".join([f"{color}: {count}" for color, count in counting_summary.get("categories", {}).get(cat, {}).items()])
            human_text_lines.append(f"\t{cat}: {count} ({color_info})")
        human_text_lines.append(f"TOTAL SINCE {start_timestamp}")
        for cat, count in total_counts_dict.items():
            if count > 0:
                color_info = ", ".join([f"{color}: {count}" for color, count in counting_summary.get("categories", {}).get(cat, {}).items()])
                human_text_lines.append(f"\t{cat}: {count} ({color_info})")
        if total_detections > 0 and stream_info:
            frame_height = stream_info.get("input_settings", {}).get("height")
            if frame_height:
                total_relative_height = sum(det.get("relative_height", 0) for det in counting_summary.get("detections", []))
                avg_relative_height = total_relative_height / total_detections if total_detections > 0 else 0
    ...
    def _generate_business_analytics(self, counting_summary: Dict, alerts: List, config: FlareAnalysisConfig, stream_info: Optional[Dict[str, Any]] = None, is_empty=False) -> List[Dict]:
        if is_empty:
            return []
        return []
    ...
    def _extract_predictions(self, detections: List[Dict]) -> List[Dict[str, Any]]:
        return [{
            ...
            "bounding_box": det.get("bbox", {}),
            "main_color": det.get("main_color", "unknown"),
            "relative_height": det.get("relative_height", 0.0)
        } for det in detections]
