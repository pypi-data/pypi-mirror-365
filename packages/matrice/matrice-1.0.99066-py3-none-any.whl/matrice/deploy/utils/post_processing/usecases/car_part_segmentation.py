...
class CarPartSegmentationUseCase(BaseProcessor):
    ...
    def process(self, data: Any, config: ConfigProtocol, context: Optional[ProcessingContext] = None,
        ...
        # Store original masks to preserve through all steps
        original_detections = {id(det): det.get("masks", det.get("mask", [])) for det in data}
        self.logger.debug(f"Input masks: {[det.get('masks', det.get('mask', [])) for det in data]}")
        ...
        if config.confidence_threshold is not None:
            ...
            # Restore masks
            processed_data = [
                {**det, "masks": original_detections.get(id(det), det.get("masks", det.get("mask", [])))}
                for det in processed_data
            ]
            self.logger.debug(
                f"Masks after confidence filtering: {[det.get('masks', det.get('mask', [])) for det in processed_data]}")
        else:
            processed_data = data
        ...
        if config.index_to_category:
            ...
            self.logger.debug(
                f"Masks after category mapping: {[det.get('masks', det.get('mask', [])) for det in processed_data]}")
        ...
        if config.target_categories:
            ...
            self.logger.debug(
                f"Masks after category filtering: {[det.get('masks', det.get('mask', [])) for det in processed_data]}")
        ...
        if config.enable_smoothing:
            ...
            # Store masks before smoothing
            pre_smoothing_detections = {id(det): det.get("masks", det.get("mask", [])) for det in processed_data}
            processed_data = bbox_smoothing(processed_data, self.smoothing_tracker.config, self.smoothing_tracker)
            # Restore masks after smoothing
            processed_data = [
                {**det, "masks": pre_smoothing_detections.get(id(det), det.get("masks", det.get("mask", [])))}
                for det in processed_data
            ]
            self.logger.debug(
                f"Masks after smoothing: {[det.get('masks', det.get('mask', [])) for det in processed_data]}")
        ...
        try:
            ...
            # Store masks before tracking
            pre_tracking_detections = {id(det): det.get("masks", det.get("mask", [])) for det in processed_data}
            processed_data = self.tracker.update(processed_data)
            # Restore masks after tracking
            processed_data = [
                {**det, "masks": pre_tracking_detections.get(id(det), det.get("masks", det.get("mask", [])))}
                for det in processed_data
            ]
            self.logger.debug(
                f"Masks after tracking: {[det.get('masks', det.get('mask', [])) for det in processed_data]}")
    ...
    def _generate_events(self, counting_summary: Dict, alerts: List, config: CarPartSegmentationConfig,
        ...
        if total_detections > 0:
            ...
            event = {
                ...
                "application_name": "Car Part Segmentation System",
    ...
    def _generate_insights(self, summary: dict, config: CarPartSegmentationConfig) -> List[str]:
        """
    ...
    def _check_alerts(self, summary: dict, config: CarPartSegmentationConfig) -> List[Dict]:
        """
