...
class MaskDetectionUseCase(BaseProcessor):
    ...
    def __init__(self):
        ...
        self.target_categories = ["without_mask", "with_mask"]
    ...
    def process(self, data: Any, config: ConfigProtocol, context: Optional[ProcessingContext] = None,
        ...
        tracking_stats_list = self._generate_tracking_stats(counting_summary, alerts, summary, config, frame_number, stream_info)
    ...
    def _generate_events(self, counting_summary: Dict, alerts: List, config: MaskDetectionConfig,
        ...
        if total_detections > 0:
            ...
            if config.alert_config and hasattr(config.alert_config, 'alert_type'):
                for i in range(len(config.alert_config.alert_type)):
                    alert_settings.append({
                        "alert_type": config.alert_config.alert_type[i],
                        "incident_category": config.alert_config.alert_incident_category[i] if hasattr(config.alert_config, 'alert_incident_category') else self.CASE_TYPE,
                        "threshold_level": config.alert_config.count_thresholds if hasattr(config.alert_config, 'count_thresholds') else {},
                        "settings": {config.alert_config.alert_type[i]: config.alert_config.alert_value[i]}
                    })
            ...
            event = {
                ...
                "location_info": stream_info.get("camera_info", {}).get("location", "TODO") if stream_info else "TODO",
            ...
            level = "low"
            intensity = 0.0
            human_text_lines = ["NO INCIDENTS DETECTED:"]
            human_text = "\n".join(human_text_lines)
            alert_settings = []
            if config.alert_config and hasattr(config.alert_config, 'alert_type'):
                for i in range(len(config.alert_config.alert_type)):
    ...
    def _generate_tracking_stats(
        ...
        high_precision_timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%d-%H:%M:%S.%f UTC")

        # Build camera_info from stream_info if available
        camera_info = {
            "camera_name": "TODO",
            "camera_group": "TODO", 
            "location": "TODO"
        ...
        if stream_info and stream_info.get("camera_info"):
            camera_info.update(stream_info["camera_info"])
        ...
        print("----------TRACKING-DETECTIONSS----------------------")
        print(counting_summary.get("detections", []))
        print("-------------DETECTIONSS@2-------------------")
        print(detections)
        ...
        # Create tracking_stat in expected format matching eg.json
        tracking_stat = {
            "input_timestamp": high_precision_timestamp,
            "reset_timestamp": high_precision_timestamp,
    ...
    def _check_alerts(self, summary: dict, config: MaskDetectionConfig) -> List[Dict]:
        ...
        if hasattr(config.alert_config, 'count_thresholds') and config.alert_config.count_thresholds:
            for category, threshold in config.alert_config.count_thresholds.items():
                if category == "all" and total > threshold:  # Fixed logic: alert when EXCEEDING threshold
                    alerts.append({
                        "alert_type": getattr(config.alert_config, 'alert_type', ['Default'])[0] if hasattr(config.alert_config, 'alert_type') else 'Default',
                        "incident_category": self.CASE_TYPE,
                        "threshold_level": threshold,
                        "settings": {
                            getattr(config.alert_config, 'alert_type', ['Default'])[0] if hasattr(config.alert_config, 'alert_type') else 'Default': 
                            getattr(config.alert_config, 'alert_value', ['JSON'])[0] if hasattr(config.alert_config, 'alert_value') else 'JSON'
                        }                    
                    })
                    ...
                    if count > threshold:  # Fixed logic: alert when EXCEEDING threshold
                        alerts.append({
                            "alert_type": getattr(config.alert_config, 'alert_type', ['Default'])[0] if hasattr(config.alert_config, 'alert_type') else 'Default',
                            "incident_category": f"{category} exceeds threshold ({threshold})",
                            "threshold_level": threshold,
                            "settings": {
                                getattr(config.alert_config, 'alert_type', ['Default'])[0] if hasattr(config.alert_config, 'alert_type') else 'Default': 
                                getattr(config.alert_config, 'alert_value', ['JSON'])[0] if hasattr(config.alert_config, 'alert_value') else 'JSON'
