...
class PeopleCountingUseCase(BaseProcessor):
    ...
    def _process_frame_detections(self, frame_data: Any, config: PeopleCountingConfig, frame_id: str, stream_info: Optional[Dict[str, Any]] = None) -> tuple:
        ...
        incidents = self._generate_incidents(counting_summary, zone_analysis, alerts, config, stream_info)
    ...
    def _generate_incidents(self, counting_summary: Dict, zone_analysis: Dict, alerts: List, config: PeopleCountingConfig, stream_info: Optional[Dict[str, Any]] = None) -> List[Dict]:
        ...
        if total_people > 0:
            ...
            severity_level = self.determine_severity_level(total_people, threshold_low=3, threshold_medium=7, threshold_critical=15)
            
            # Create main people counting incident
            incident_id = f"people_counting_{self.get_total_frames_processed()}"
            incident_human_text = f"people counting detected @ {self.get_high_precision_timestamp()} [Severity Level: {severity_level}]"
            ...
            if config.alert_config and config.alert_config.count_thresholds:
                ...
                if total_people >= threshold:
                    alert_obj = self.create_alert_object(
                        "count_threshold", "count_alert_1", "people_counting", 
                        float(threshold), True, {"threshold_exceeded": total_people}
                    )
                    incident_alerts.append(alert_obj)
                    
                    incident_alert_settings.append({
                        "alert_type": "count_threshold",
                        "incident_category": "people_counting",
                        "threshold_level": threshold,
                        "ascending": True,
                        "settings": {}
                    })
            
            # Create incident using standardized method
            incident = self.create_incident(
                incident_id, "people_counting", severity_level, 
                incident_human_text, camera_info, incident_alerts, incident_alert_settings
            )
            incidents.append(incident)
        
        # Add zone-specific incidents if applicable
        if zone_analysis:
            for zone_name, zone_count in zone_analysis.items():
                zone_total = self._robust_zone_total(zone_count)
                if zone_total > 0:
                    zone_severity = self.determine_severity_level(zone_total, threshold_low=2, threshold_medium=5, threshold_critical=10)
                    
                    zone_incident_id = f"zone_occupancy_{zone_name}_{self.get_total_frames_processed()}"
                    zone_human_text = f"zone occupancy detected in {zone_name} @ {self.get_high_precision_timestamp()} [Severity Level: {zone_severity}]"
                    
                    # Set camera info location to the zone name
                    zone_camera_info = camera_info.copy()
                    zone_camera_info["location"] = zone_name
                    
                    zone_incident = self.create_incident(
                        zone_incident_id, "zone_occupancy", zone_severity,
                        zone_human_text, zone_camera_info
                    )
                    incidents.append(zone_incident)
    ...
    def _generate_business_analytics(self, counting_summary: Dict, zone_analysis: Dict, config: PeopleCountingConfig, stream_info: Optional[Dict[str, Any]] = None) -> List[Dict]:
    ...
    def _generate_events(self, counting_summary: Dict, zone_analysis: Dict, alerts: List, config: PeopleCountingConfig, stream_info: Optional[Dict[str, Any]] = None) -> List[Dict]:
        """Generate structured events for backwards compatibility (legacy format)."""
        from datetime import datetime, timezone
    ...
    def _generate_tracking_stats(self, counting_summary: Dict, zone_analysis: Dict, insights: List[str], summary: str, config: PeopleCountingConfig, frame_id: Optional[str] = None, stream_info: Optional[Dict[str, Any]] = None) -> List[Dict]:
        ...
        if config.alert_config:
            # Convert alert config to alert_settings format
            if hasattr(config.alert_config, 'count_thresholds') and config.alert_config.count_thresholds:
                for category, threshold in config.alert_config.count_thresholds.items():
                    alert_settings.append({
                        "alert_type": "count_threshold",
                        "incident_category": "people_counting" if category == "all" else category,
                        "threshold_level": threshold,
                        "ascending": True,
                        "settings": {}
                    })
        
        # Generate human_text using standardized method
        current_timestamp = self._get_current_timestamp_str(stream_info)
        start_timestamp = self._get_start_timestamp_str(stream_info)
        alerts_summary = "Alert triggered" if alerts else "None"
        
        human_text = self.generate_tracking_human_text(
            per_category_current, per_category_total, current_timestamp, start_timestamp, alerts_summary
        )
