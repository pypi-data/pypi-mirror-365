...
class VehicleMonitoringUseCase(BaseProcessor):
    ...
    def _generate_tracking_stats(self, counting_summary: Dict, alerts: List, config: VehicleMonitoringConfig,
        ...
        if total_detections > 0:
            ...
            for category, count in per_category_count.items():
                if count > 0:
                    ...
                    category_lines.append(f"  {count} {display_name}")
            human_text_lines.append(f"    - Vehicle[s] Detected: {', '.join(category_lines)}")
        else:
        ...
        for cat, count in per_category_count.items():
            human_text_lines.append(f"\t{self.CATEGORY_DISPLAY.get(cat, cat)}: {count}")
