...
def rebase_sequence_editor(
    todo_file_path: Annotated[
        Path,
        typer.Argument(help="Path to the git rebase todo file"),
    ],
):
    """This function will get called as the GIT_SEQUENCE_EDITOR, which is a hacky
    way to programatically set all commits to edit in an interactive rebase"""
    todo_file_path.write_text(todo_file_path.read_text().replace("pick", "edit"))
...
def parse_time_str(time_str: str) -> datetime.time | None:
    if not time_str:
        return None
...
def fmt(dt: datetime.datetime):
    return f"{dt.astimezone():%d %b %y %H:%M}"
