...
class c_Outlook():
    ...
    def getAccount(self, str_outAcctName = ''):
        ...
        try:
            ...
            str_ErrorMsg += '  - Email addr wanted: {} \n'.format(str_outAcctName)
            str_ErrorMsg += '  - Email addr Available on this machine : \n'
            ...
            logger.error(f'{str_ErrorMsg}|  \r  - |{err}|')
            raise err
        self.o_Acct = o_Acct
        return o_Acct
    ...
    def getInbox(self, str_inbox = ''):
        ...
        try:
            ...
            str_ErrorMsg = ' ERROR in getInbox, Cannot find the Box: \n'
            str_ErrorMsg += '  - Inbox-level folder wanted: {} \n'.format(str_inbox)
            str_ErrorMsg += '  - Inbox-level folder Available in the email Account : \n'
            # Loop on Inbox level folder
            try:
                for __fol in self.o_Acct.Folders:
                    __fol = str(__fol)
                    str_ErrorMsg += '    - {}  \n'.format(__fol)
            except: pass
            logger.error(f'{str_ErrorMsg}|  \r  - |{err}|')
    ...
    def getFolders(self, l_folders = []):
        ...
        try:
            ...
            str_ErrorMsg = ' ERROR in getFolder, Cannot find folder: \n'
            str_ErrorMsg += '  - Folder wanted: {} \n'.format(_fol)
            str_ErrorMsg += '  -   *list : {} \n'.format('|'.join(l_folders))
            str_ErrorMsg += '  - Folder Available in Inbox : \n'
            # Loop on Inbox level folder
            try:
                for __fol in o_folder.Folders:
                    __fol = str(__fol)
                    str_ErrorMsg += '    - {}  \n'.format(__fol)
            except:
                pass
    ...
    def MoveEmail(self):
        ...
        o_folder_dest = self.get_folders_destination(l_folder_dest)
        # Move
        try:
            ...
            raise err
    def get_folders_destination(self, l_folder_dest = []):
        ...
        try:
            ...
            logger.error(' ERROR in getFolderdestination: Cannot find the Folder : |{}|  \r  - |{}|'.format('|'.join(l_folder_dest), err))
            raise err
        ...
        try:
            ...
            if not str_to == '':
                ...
                if len(o_emails_sub) == 0:
                    ...
                    if self.o_emailParam.str_subject == '':
                        raise ValueError(f'Empty on filterEmails_ToCc with To: {str_to} || Subject: {self.o_emailParam.str_subject}')
            ...
            if not str_cc == '':
                ...
                if len(o_emails_sub) == 0:
                    ...
                    if self.o_emailParam.str_subject == '':
                        raise ValueError(
                            f'Empty on filterEmails_ToCc with CC: {str_to} || Subject: {self.o_emailParam.str_subject}')
                else:   o_emails = o_emails_sub
        except Exception as err:
