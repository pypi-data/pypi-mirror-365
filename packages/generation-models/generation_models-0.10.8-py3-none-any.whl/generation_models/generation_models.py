...
class SolverConfig(BaseModel):
    ...
    - Example: 8 for 8s
    """
    mip_gap_tolerance: Annotated[float, Field(ge=0.0)] = 0.0
...
class DCLosses(BaseModel):
    ...
    - Max value: 1.0
    """
    mppt_error: BoundedLossFactor = 0.0  # TODO: remove once mppt_error deprecated, equivalent to tracking_error
...
class StorageSolverOptions(BaseModel):
    ...
    *Whether or not to include DA/RT co-optimization in the Energy Markets*
    ...
    no_virtual_trades: Annotated[
        t.Optional[bool],
        get_deprecator(
            "The no_virtual_fields is deprecated and will be removed in the future. Use minimum_dam_coverage_fraction instead. "
    ...
    @model_validator(mode="after")
    @classmethod
    def coerce_energy_strategy_to_ops_strategy(cls, values):
        ...
        if values.energy_strategy is None:
            values.energy_strategy = OpsStrategy.dart_qo if values.dart else OpsStrategy.da_qo_only
            values.dart = None
        if isinstance(values.energy_strategy, EnergyStrategy):
            values.energy_strategy = values.energy_strategy.to_ops_strategy()
        elif isinstance(values.energy_strategy, MarketConfig):
            values.energy_strategy = translate_market_config_to_ops_strategy(values.energy_strategy)

        if values.energy_strategy != OpsStrategy.dart_qo:
            ...
            if values.minimum_dam_coverage_fraction > 0.0:
                raise ValueError(
...
class PVStorageModel(ImportExportLimitMixin, MarketBase):
    ...
    def check_dam_award_against_config(cls, values):
        _check_redispatch_arguments(values)
        return values
