...
class Warehouse(BaseAPIClient):
    ...
    def retrieve_with_graph_search(
        self, query: str, indexes: list[str] = [], n_objects: int = 10
    ) -> list:
        """Retrieve objects from this warehouse with graph search"""
        if not indexes:
            indexes = ["chunks"]
        response = requests.post(
            f"{self.base_url}/api/v1/projects/{self.id}/retrieve-with-graph/",
            headers=self._get_headers_without_content_type(),
        ...
        if response.status_code == 200:
            return response.json()["retrieval_results"]
        logger.error(f"Failed to retrieve with graph search: {response.text}")
        return []
    ...
    def retrieve_with_semantic_search_and_filter_with_tags(
        ...
        if excluded_tags is None:
            excluded_tags = []
        response = requests.post(
            f"{self.base_url}/api/v1/projects/{self.id}/retrieve-with-naive-and-filter-with-tags/",
            headers=self._get_headers(),
            json={
                "query": query,
                "n_objects": n_objects,
