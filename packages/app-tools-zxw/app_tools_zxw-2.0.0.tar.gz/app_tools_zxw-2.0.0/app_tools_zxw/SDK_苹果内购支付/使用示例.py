...
"""
import asyncio
from app_tools_zxw.SDK_苹果内购支付 import 苹果内购支付服务
from app_tools_zxw.SDK_苹果内购支付.测试数据 import 共享密钥, recipt, transaction_id, originalTransactionIdentifierIOS


async def 示例_验证一次性购买():
    """验证一次性购买示例"""
    # 初始化服务（使用沙盒环境）
    ...
    # 使用测试数据（实际使用时从iOS客户端获取）
    收据数据 = recipt
    交易ID = transaction_id
    
    try:
        # 验证一次性购买
        result = await apple_service.验证一次性购买(收据数据, 交易ID)
...
async def 示例_验证订阅购买():
    ...
    # 使用测试数据
    收据数据 = recipt
    原始交易ID = originalTransactionIdentifierIOS  # 可选参数
    
    try:
        # 验证订阅购买
        result = await apple_service.验证订阅购买(收据数据, 原始交易ID)
...
async def 示例_检查订阅状态():
    ...
    try:
        ...
        for i, 交易 in enumerate(订阅信息['最新交易信息']):
            print(f"  交易 {i+1}:")
            print(f"    产品ID: {交易.get('product_id')}")
            print(f"    交易ID: {交易.get('transaction_id')}")
            print(f"    过期时间: {交易.get('expires_date')}")
            print(f"    是否试用期: {交易.get('is_trial_period')}")
