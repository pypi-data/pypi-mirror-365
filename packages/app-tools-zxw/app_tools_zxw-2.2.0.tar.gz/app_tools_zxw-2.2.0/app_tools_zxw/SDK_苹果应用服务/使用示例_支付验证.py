...
from pems.config_苹果支付 import (
    ...
    共享密钥, recipt, transaction_id, originalTransactionIdentifierIOS
)
...
async def 示例_基本功能():
    ...
    async with 获取支付服务() as 支付服务:
        # 1. 验证收据
        print("1. 验证收据...")
        验证结果 = await 支付服务.验证收据_从应用收据(recipt)
        print(f"验证结果: {验证结果}")
        print(验证结果.model_dump())

        # 2. 获取最新交易
        print("\n2. 获取最新交易...")
        最新交易 = await 支付服务.获取最新交易(originalTransactionIdentifierIOS)
        print(f"最新交易: {最新交易}")
        print(最新交易.model_dump())

        # 3. 验证特定交易
        print("\n3. 验证特定交易...")
        交易验证结果 = await 支付服务.验证特定交易(transaction_id)
...
async def 示例_订阅功能():
    ...
    async with 获取支付服务() as 支付服务:
        # 获取订阅状态
        订阅状态 = await 支付服务.获取订阅状态(originalTransactionIdentifierIOS)
        print(f"订阅状态: ")
        print(f"环境: {订阅状态.环境}")
