...
from range_tools import extend_range, reverse_range
...
def get_items(sequence, indices):
    ...
    """Yield items from a sequence at specified indices.

    Args:
        sequence: The sequence to retrieve items from.
        indices: Iterable of indices to access.

    Yields:
        Items from the sequence at specified indices.
    """
    for index in indices:
        yield sequence[index]


def handle_replace_offset_range(items, replaced_offset_range, replacements):
    # type: (Iterable[T], XRange, Sequence[T]) -> Iterator[T]
    """Handle replacement of items in a specified offset range.

    Args:
        items: Original sequence of items.
        replaced_offset_range: Range of offsets to be replaced.
        replacements: Sequence of replacement items.

    Yields:
        Items from the original sequence with replacements applied where specified.
    """
    num_replacements = len(replacements)

    for offset, item in enumerate(items):
        # Replace?
        if offset in replaced_offset_range:
            replacement_offset = replaced_offset_range.index(offset)
...
class COWList(Sequence[T]):
    ...
    """
    __slots__ = ('physical_implementation', 'physical_offsets', 'cached_hash_value')
    ...
    # Helpers
    def _new_instance_with_new_physical_offsets(self, new_physical_offsets):
        # type: (SelfCOWList, XRange) -> SelfCOWList
        ...
        new_instance.physical_offsets = new_physical_offsets
    ...
    def delete(self, index_or_slice):
        ...
        physical_offsets = self.physical_offsets  # type: XRange
        length = len(physical_offsets)  # type: int
        ...
        if isinstance(index_or_slice, int):
            ...
            if -length <= index_or_slice < length:
                ...
                physical_offsets_left = physical_offsets[:offset]
                physical_offsets_right = physical_offsets[offset + 1:]

                new_items = chain(
                    get_items(physical_implementation, physical_offsets_left),
                    get_items(physical_implementation, physical_offsets_right)
                )  # type: Iterable[T]
            ...
            sliced_out_physical_offsets = physical_offsets[index_or_slice]

            # Are we actually deleting?
            if not sliced_out_physical_offsets:
                return self
            else:
                remaining_physical_offsets = (
                    physical_offset
                    for physical_offset in physical_offsets
                    if physical_offset not in sliced_out_physical_offsets
                )

                new_items = get_items(physical_implementation, remaining_physical_offsets)
    ...
    def insert_sequence(self, index, sequence):
        ...
        if -length <= index <= length:
            ...
            if not sequence_length:
                ...
                physical_offsets_left = physical_offsets[:index]
                physical_offsets_right = physical_offsets[index:]

                # Are we inserting a sequence at the end of the physical implementation?
                if not physical_offsets_right:
                    extended_physical_offsets, extended_by = extend_range(physical_offsets, sequence_length)
                    physical_length = len(physical_implementation)
                    if extended_by == XRange(physical_length, physical_length + sequence_length):
                        ...
                        return self._new_instance_with_new_physical_offsets(extended_physical_offsets)
    ...
    def pop(self, index=-1):
        ...
        physical_offsets = self.physical_offsets
        length = len(physical_offsets)
        ...
        if -length <= index < length:
            ...
            popped = physical_implementation[physical_offsets[offset]]
    ...
    def remove(self, value):
        ...
        element_iterator = get_items(physical_implementation, physical_offsets)
    ...
    def reverse(self):
        ...
        """
        return self._new_instance_with_new_physical_offsets(
            reverse_range(self.physical_offsets)
        )
        ...
        # Slice assignment
            ...
            if isinstance(value_or_iterable, Iterable):
                ...
                if isinstance(slice_assignment_action, ReplaceOffsetRange):
                    replaced_offset_range = slice_assignment_action.replaced_offset_range
                    ...
                    new_items = handle_replace_offset_range(
                        get_items(physical_implementation, physical_offsets),
    ...
    def __bool__(self):
        ...
        return len(self.physical_offsets) > 0

    def __contains__(self, value):
        # type: (SelfCOWList, object) -> bool
        return value in get_items(self.physical_implementation, self.physical_offsets)
    ...
    def __getitem__(self, index_or_slice):
        ...
        if isinstance(index_or_slice, int):
            # Is it valid?
            if -length <= index_or_slice < length:
                return physical_implementation[physical_offsets[index_or_slice]]
            else:
                raise IndexError('index out of range')
        elif isinstance(index_or_slice, slice):
            return self._new_instance_with_new_physical_offsets(physical_offsets[index_or_slice])
        else:
    ...
    def __iter__(self):
        ...
        return get_items(self.physical_implementation, self.physical_offsets)

    def __len__(self):
        # type: (SelfCOWList) -> int
        return len(self.physical_offsets)
    ...
    def __repr__(self):
        ...
        return '%s([%s])' % (
            ...
            ', '.join(map(repr, get_items(self.physical_implementation, self.physical_offsets)))
        )

    def __reversed__(self):
        # type: (SelfCOWList) -> Iterator[T]
        return get_items(self.physical_implementation, reversed(self.physical_offsets))
