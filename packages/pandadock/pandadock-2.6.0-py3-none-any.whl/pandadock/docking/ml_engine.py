...
class MLEngine(DockingEngine):
    ...
    def project_to_grid_box(self, coordinates: np.ndarray) -> np.ndarray:
        """Project coordinates to be within grid box while preserving molecular geometry"""
        min_bounds, max_bounds = self.grid_box.get_bounds()
        
        # Find the most extreme atom positions
        atom_min = np.min(coordinates, axis=0)
        atom_max = np.max(coordinates, axis=0)
        
        # Calculate needed shift to keep all atoms within bounds
        shift = np.zeros(3)
        for i in range(3):
        ...
        # Apply uniform shift to preserve molecular geometry
        projected = coordinates + shift
