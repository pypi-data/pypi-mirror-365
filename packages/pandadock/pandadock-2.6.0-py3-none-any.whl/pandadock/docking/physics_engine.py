...
class PhysicsEngine(DockingEngine):
    ...
    def sample_translations(self, num_samples: int) -> List[np.ndarray]:
        ...
        # Generate translations relative to center, not absolute positions
        # This ensures that grid_center + translation stays within bounds
        max_displacement = grid_size / 4  # Conservative displacement to leave room for molecule size
        
        for _ in range(num_samples):
            # Generate displacement from center (not absolute coordinates)
    ...
    def apply_transformation(self, coords: np.ndarray, rotation: np.ndarray, translation: np.ndarray) -> np.ndarray:
        ...
        # Ensure ALL coordinates stay within grid box bounds
        min_bounds, max_bounds = self.grid_box.get_bounds()
        
        # Check if any individual atoms are outside bounds and adjust the whole molecule
        # Find the most extreme atom positions
        atom_min = np.min(final_coords, axis=0)
        atom_max = np.max(final_coords, axis=0)
        
        # Calculate needed shifts to keep all atoms within bounds
        shift = np.zeros(3)
        for i in range(3):
            if atom_min[i] < min_bounds[i]:
                shift[i] = min_bounds[i] - atom_min[i] + 1.0  # 1Å margin
            elif atom_max[i] > max_bounds[i]:
                shift[i] = max_bounds[i] - atom_max[i] - 1.0  # 1Å margin
