...
class PDBbindBenchmark:
    ...
    def __init__(self, pdbbind_dir: str, output_dir: str, max_complexes: int = 50, 
        ...
        if grid_center_file:
            ...
            # Even without a grid center file, add hardcoded centers for known test cases
            self.add_hardcoded_grid_centers()
    ...
    def load_grid_centers(self, grid_center_file: str):
        ...
        # Add hardcoded grid centers for known test cases if not already loaded
        self.add_hardcoded_grid_centers()
        
        self.logger.info(f"Total grid centers available: {len(self.grid_centers)}")
        if self.grid_centers:
            self.logger.info(f"Available PDB codes: {list(self.grid_centers.keys())}")
    
    def add_hardcoded_grid_centers(self):
        """Add hardcoded grid centers for known test cases"""
        known_centers = {
            '10gs': (13.491, 5.790, 27.274),  # Known 10gs binding site center
            '1a30': (25.0, 30.0, 15.0),       # Example test case
        }
        
        for pdb_code, center in known_centers.items():
            if pdb_code not in self.grid_centers:
                self.grid_centers[pdb_code] = center
                self.logger.info(f"Added hardcoded grid center for {pdb_code}: {center}")
    
    def load_pdbbind_index(self) -> Dict[str, Dict]:
        """
        **FIXED**: Load PDBbind index file to get experimental affinities with robust parsing.
        """
        index_data = {}
        ...
        if not index_file:
            self.logger.warning("No PDBbind index file found. Experimental affinity comparison will be skipped.")
            return {}
        ...
        with open(index_file, 'r') as f:
            for line in f:
                ...
                # More robustly find the affinity string in the line
                affinity_str = None
                for part in parts[3:]:
                    part_lower = part.lower()
                    if 'kd=' in part_lower or 'ki=' in part_lower or 'ic50=' in part_lower:
                        affinity_str = part
                        break
        ...
        self.logger.info(f"Successfully loaded and parsed {len(index_data)} entries from PDBbind index")
        return index_data

    def parse_affinity(self, affinity_str: str) -> float:
        """
        **FIXED**: Parse affinity string (e.g., 'Kd=49uM') to pKd/pKi value with robust regex.
        """
        try:
            # Normalize string for easier parsing
            normalized_str = affinity_str.lower().replace(" ", "")

            # This robust regex handles different prefixes (kd, ki, ic50) and units (pM, nM, uM, mM, M)
            match = re.search(r'([kdi|ic50]+)[=<>]?(\d+\.?\d*e?-?\d*)((?:p|n|u|m)?m)', normalized_str)
            
            if not match:
                self.logger.warning(f"Could not parse affinity value from '{affinity_str}'")
                return np.nan

            value = float(match.group(2))
            unit = match.group(3)

            # Convert value to molar concentration
            if unit == 'pm':    # picomolar
                molar = value * 1e-12
            elif unit == 'nm':  # nanomolar
                molar = value * 1e-9
            elif unit == 'um':  # micromolar
                molar = value * 1e-6
            elif unit == 'mm':  # millimolar
                molar = value * 1e-3
            elif unit == 'm':   # molar
                molar = value
            else:
                self.logger.warning(f"Unknown affinity unit '{unit}' in '{affinity_str}'")
                return np.nan

            # Convert to pKd/pKi, handling edge case of zero molar concentration
            if molar > 0:
            ...
            self.logger.error(f"Error parsing affinity string '{affinity_str}': {e}")
            return np.nan
    ...
    def load_complexes(self) -> List[Dict]:
        ...
        for pdb_dir in pdb_dirs[:self.max_complexes]:
            ...
            # Get experimental data from our parsed index
            exp_data = index_data.get(pdb_code)
            if not exp_data:
                self.logger.warning(f"No valid experimental affinity for {pdb_code}, skipping.")
                continue
        ...
        self.logger.info(f"Loaded {len(complexes)} complexes with valid affinity data for benchmarking")
    ...
    def run_pandadock(self, complex_info: Dict, algorithm: str) -> Optional[Dict]:
        ...
        try:
            # Build PandaDock command for version 2.4.0
            cmd = [
                'pandadock',  # Updated for 2.4.0 - no longer python -m
                ...
                '--out', str(output_dir),
                '--num-poses', '10',
                '--report-format', 'json'  # Explicitly request JSON output
            ]
            
            # Add algorithm-specific parameters for PandaDock 2.4.0
            if algorithm == 'pandaml':
                cmd.extend(['--ml-rescoring'])  # Enable ML rescoring for PandaML
            elif algorithm == 'pandaphysics':
                cmd.extend(['--exhaustiveness', '8'])  # Higher exhaustiveness for physics-based
            elif algorithm == 'pandacore':
                cmd.extend(['--exhaustiveness', '8', '--n-jobs', '4'])  # Optimized for speed
            
            # Add grid center if available (try multiple variations of the PDB code)
            grid_center = None
            for pdb_variant in [pdb_code.lower(), pdb_code.upper(), pdb_code]:
                if pdb_variant in self.grid_centers:
                    grid_center = self.grid_centers[pdb_variant]
                    break
            
            if grid_center:
                x, y, z = grid_center
                cmd.extend(['--center', str(x), str(y), str(z)])
                # Add grid size to ensure consistent binding site coverage
                cmd.extend(['--size', '60', '60', '60'])
                self.logger.info(f"✅ Using grid center for {pdb_code}: {x}, {y}, {z} with size 60x60x60")
            else:
                self.logger.warning(f"❌ No grid center found for {pdb_code} (checked: {[pdb_code.lower(), pdb_code.upper(), pdb_code]})")
                self.logger.warning(f"Available grid centers: {list(self.grid_centers.keys())}")
                self.logger.warning(f"Using automatic center detection - poses may not respect intended grid boundaries!")
                # Still set size even without center for consistency
                cmd.extend(['--size', '60', '60', '60'])
            
            # Add metal-specific parameters if metal complex
            if complex_info['metal_complex']:
                cmd.extend(['--flexible-residues', 'auto'])  # Updated parameter for 2.4.0
            ...
            # Parse JSON output (PandaDock 2.4.0 format)
            ...
            # Extract best pose results from PandaDock 2.4.0 format
            ...
            if binding_affinity is None:
                # Fallback to IC50 conversion if binding_affinity is missing
                ic50_um = best_pose.get('ic50_um')
                if ic50_um is not None:
                    # Convert IC50 (μM) to approximate pKd: pKd = -log10(IC50_μM * 1e-6)
                    binding_affinity = -np.log10(ic50_um * 1e-6)
                else:
                    # Final fallback: use energy-based estimate
                    energy = best_pose.get('energy', 0)
                    binding_affinity = abs(energy) / 1.364 if energy < 0 else 5.0
            
            return {
                'score': best_pose.get('score', 0.5),
                'energy': best_pose.get('energy', 0),
                'binding_affinity': binding_affinity,
                'confidence': best_pose.get('confidence', 0.5),
                ...
                'ic50': best_pose.get('ic50', None),
                'ic50_um': best_pose.get('ic50_um', None),
                'ligand_efficiency': best_pose.get('ligand_efficiency', None),
    ...
    def generate_comprehensive_analysis(self):
        ...
        if df.empty:
            self.logger.error("No results to analyze. Benchmark may have failed. Exiting analysis.")
            return
    ...
    def convert_prediction_scale(self, predicted_values, experimental_values):
        """
        Convert predicted values to match experimental scale using linear transformation
        This handles cases where predictions might be in different units (e.g., binding energy vs pKd)
        """
        from scipy import stats
        
        # Remove invalid values
        valid_mask = (predicted_values != -0.5) & ~np.isnan(predicted_values) & ~np.isnan(experimental_values)
        ...
        # Try different transformations and pick the best one
        transformations = [
            # Identity (no transformation)
            lambda x: x,
            # Absolute value (in case values are negative binding energies)
            lambda x: np.abs(x),
            # Negative (convert binding energy to affinity-like)
            lambda x: -x,
            # Linear scaling to experimental range
            lambda x: (x - x.min()) / (x.max() - x.min()) * (exp_valid.max() - exp_valid.min()) + exp_valid.min() if x.max() != x.min() else x,
            # Convert from kcal/mol binding energy to pKd (assuming RT = 1.364 kcal/mol at 298K)
            lambda x: np.abs(x) / 1.364  # Convert kcal/mol to pKd units
        ]
        ...
        for transform in transformations:
            try:
                transformed = transform(pred_valid)
                if np.any(np.isnan(transformed)) or np.any(np.isinf(transformed)):
                    continue
                    
                # Calculate correlation
                if len(np.unique(transformed)) > 1:  # Ensure not all values are the same
                    r2 = r2_score(exp_valid, transformed)
                    if r2 > best_r2:
                        best_r2 = r2
        ...
        if best_transform is not None:
            self.logger.info(f"Applied scale correction with R² improvement: {best_r2:.3f}")
            return best_transform(predicted_values)
        else:
            self.logger.warning("No suitable scale correction found, using original values")
            return predicted_values

    def generate_affinity_correlation(self, df: pd.DataFrame):
        """Generate binding affinity correlation analysis with scale correction"""
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('Binding Affinity Prediction Performance (Scale-Corrected)', fontsize=20, fontweight='bold')
        
        # Filter data with experimental affinity (but keep failed predictions for conversion)
        valid_df = df.dropna(subset=['experimental_affinity']).copy()
        
        if len(valid_df) == 0:
            self.logger.warning("No experimental affinity data found. Plots will be empty.")
            plt.close()
            return
        
        # Convert failed predictions (-0.5) to use energy or score values where available
        self.logger.info("Converting failed predictions to use energy/score values...")
        conversion_count = 0
        
        for i, row in valid_df.iterrows():
            if row['predicted_affinity'] == -0.5:
                converted_affinity = None
                
                # Try different conversion strategies based on algorithm
                if row['algorithm'] == 'pandacore' and 'score' in row and not np.isnan(row['score']):
                    # For pandacore, use inverse score relationship (lower score = better affinity)
                    score_val = row['score']
                    # Convert score to approximate pKd (assuming typical score range 0-20)
                    converted_affinity = max(2.0, 12.0 - (score_val / 2.0))  # Map score to 2-12 pKd range
                    conversion_source = f"score {score_val:.2f}"
                
                elif row['algorithm'] == 'pandaphysics' and 'energy' in row and not np.isnan(row['energy']):
                    # For pandaphysics, use energy values (already in kcal/mol-like units)
                    energy_val = row['energy']
                    if energy_val < 0:  # Favorable binding energy
                        converted_affinity = abs(energy_val) / 1.364  # Convert to pKd
                        conversion_source = f"energy {energy_val:.2f}"
                
                # Apply conversion if successful
                if converted_affinity is not None:
                    valid_df.at[i, 'predicted_affinity'] = converted_affinity
                    conversion_count += 1
                    self.logger.info(f"Converted {row['algorithm']} {row['pdb_code']}: {conversion_source} -> affinity {converted_affinity:.2f}")
        
        self.logger.info(f"Converted {conversion_count} failed predictions using energy/score values")
        
        # Re-filter after conversion
        valid_df = valid_df[valid_df['predicted_affinity'] != -0.5]
        
        if len(valid_df) == 0:
            self.logger.warning("No valid affinity data after energy conversion. Plots will be empty.")
            plt.close()
            return
        
        # Apply scale correction for each algorithm
        self.logger.info("Applying scale correction to predicted affinities...")
        
        for algorithm in valid_df['algorithm'].unique():
            alg_mask = valid_df['algorithm'] == algorithm
            alg_data = valid_df[alg_mask]
            
            if len(alg_data) > 0:
                # Store original predictions for comparison
                valid_df.loc[alg_mask, 'predicted_affinity_raw'] = alg_data['predicted_affinity']
                
                if len(alg_data) > 1:
                    # Apply scale correction
                    corrected_predictions = self.convert_prediction_scale(
                        alg_data['predicted_affinity'].values,
                        alg_data['experimental_affinity'].values
                    )
                    
                    valid_df.loc[alg_mask, 'predicted_affinity'] = corrected_predictions
                    
                    self.logger.info(f"{algorithm}: Raw range [{alg_data['predicted_affinity'].min():.2f}, {alg_data['predicted_affinity'].max():.2f}] "
                                   f"-> Corrected range [{corrected_predictions.min():.2f}, {corrected_predictions.max():.2f}]")
                else:
                    self.logger.info(f"{algorithm}: Only 1 data point, no scale correction needed")
        ...
        for i, algorithm in enumerate(self.algorithms):
            ...
            if len(alg_data) > 0:  # Changed from > 3 to > 0 to show all data
                ...
                if len(alg_data) > 1:
                    ...
                    # Calculate Pearson correlation for additional insight
                    from scipy import stats
                    pearson_r, p_value = stats.pearsonr(alg_data['experimental_affinity'], alg_data['predicted_affinity'])
                    
                    label = f'{algorithm.upper()} (R²={r2:.2f}, r={pearson_r:.2f}, n={len(alg_data)})'
                else:
                    label = f'{algorithm.upper()} (n={len(alg_data)})'
                ...
                ax.scatter(alg_data['experimental_affinity'], alg_data['predicted_affinity'],
                          alpha=0.8, color=colors[i % len(colors)], s=80, marker=markers[i % len(markers)],
        ...
        # 2. Before/After scale correction comparison (if we have raw data)
        ax = axes[0, 1]
        if 'predicted_affinity_raw' in valid_df.columns:
            ax.set_title('Scale Correction Applied')
            for i, algorithm in enumerate(self.algorithms):
                alg_data = valid_df[valid_df['algorithm'] == algorithm]
                if len(alg_data) > 0:
                    ax.scatter(alg_data['predicted_affinity_raw'], alg_data['predicted_affinity'],
                              alpha=0.7, color=colors[i % len(colors)], s=60,
                              marker=markers[i % len(markers)], label=algorithm.upper())
            ax.set_xlabel('Raw Predicted Values')
            ax.set_ylabel('Scale-Corrected Predicted Values')
            ax.legend()
            ax.grid(True, alpha=0.3)
        else:
            # Fall back to residuals analysis
            ...
            for i, algorithm in enumerate(self.algorithms):
                ...
                if len(alg_data) > 0:
                    ...
                    ax.scatter(alg_data['experimental_affinity'], residuals, 
                              alpha=0.7, color=colors[i % len(colors)], label=algorithm.upper(), s=40)
        ...
        # 3. Algorithm comparison statistics (updated for scale-corrected data)
        ax = axes[1, 0]
        stats_data = []
        algorithm_names = []
        for algorithm in self.algorithms:
            alg_data = valid_df[valid_df['algorithm'] == algorithm]
            if len(alg_data) > 1:  # Changed from > 3 to > 1
                r2 = r2_score(alg_data['experimental_affinity'], alg_data['predicted_affinity'])
                mae = mean_absolute_error(alg_data['experimental_affinity'], alg_data['predicted_affinity'])
                ...
                from scipy import stats
                pearson_r, _ = stats.pearsonr(alg_data['experimental_affinity'], alg_data['predicted_affinity'])
                
                stats_data.append([max(0, r2), mae, rmse, abs(pearson_r)])
                algorithm_names.append(algorithm)
        
        if stats_data:
            metrics = ['R²', 'MAE', 'RMSE', '|r|']
            x = np.arange(len(metrics))
            width = 0.8 / len(algorithm_names) if algorithm_names else 0.25
            
            for i, algorithm in enumerate(algorithm_names):
                ax.bar(x + i*width, stats_data[i], width, 
                       label=algorithm.upper(), color=colors[i % len(colors)], alpha=0.8)
            
            ax.set_ylabel('Metric Value')
            ax.set_title('Affinity Prediction Metrics (Scale-Corrected)')
            ax.set_xticks(x + width * (len(algorithm_names) - 1) / 2)
            ax.set_xticklabels(metrics)
            ax.legend()
        
        # 4. Accuracy within error thresholds
        ax = axes[1, 1]
        ...
        for algorithm in self.algorithms:
            alg_data = valid_df[valid_df['algorithm'] == algorithm]
            if len(alg_data) > 0:
                errors = np.abs(alg_data['predicted_affinity'] - alg_data['experimental_affinity'])
                accuracies = [np.mean(errors <= t) * 100 for t in thresholds]
                threshold_data.append(accuracies)
                threshold_algorithms.append(algorithm)
        
        if threshold_data:
            x = np.arange(len(thresholds))
            width = 0.8 / len(threshold_algorithms) if threshold_algorithms else 0.25
            
            for i, algorithm in enumerate(threshold_algorithms):
                ax.bar(x + i*width, threshold_data[i], width, 
                       label=algorithm.upper(), color=colors[i % len(colors)], alpha=0.8)
            
            ax.set_ylabel('Accuracy (%)')
            ax.set_title('Prediction Accuracy within Error Thresholds')
            ax.set_xticks(x + width * (len(threshold_algorithms) - 1) / 2)
            ax.set_xticklabels([f'±{t} pKd' for t in thresholds])
            ax.legend()
            ax.set_ylim(0, 100)
        ...
        self.logger.info("Affinity correlation analysis completed with scale correction")
        for algorithm in algorithm_names:
            ...
            if len(alg_data) > 1:
                ...
                errors = np.abs(alg_data['predicted_affinity'] - alg_data['experimental_affinity'])
                within_1pkd = np.mean(errors <= 1.0) * 100
                
                self.logger.info(f"{algorithm.upper()}: R²={r2:.3f}, r={pearson_r:.3f}, MAE={mae:.2f}, "
                               f"Within ±1 pKd: {within_1pkd:.1f}% (n={len(alg_data)})")
