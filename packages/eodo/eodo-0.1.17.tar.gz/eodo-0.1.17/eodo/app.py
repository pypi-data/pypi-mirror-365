...
class IPv6Tool:
    def __init__(self, select_iface="", task_id=""):
        ...
        self.ipv6, self.public_ipv6 = self.get_ipv6_list(select_iface)
    ...
    def get_ipv6_list(self, select_iface=""):
        ...
        if ipv6 and not public_ipv6:
            return set(sorted(ipv6)), None
        else:
            return set(sorted(ipv6)), set(sorted(public_ipv6))
...
def update_task(task_id=""):
    ...
    if not iptool.public_ipv6 and not iptool.ipv6:
        logger.info(f"[{task_id}] 无法获取 IPV6 地址，跳过后续所有步骤，请检查网卡是否选择正确。")
        dingtalk.notice_no_public_ipv6()
        return
    if not iptool.public_ipv6 and iptool.ipv6:
        logger.info(f"[{task_id}] 无法获取公网 IPV6 地址，跳过后续所有步骤。存在地址 {iptool.ipv6}")
        dingtalk.notice_no_public_ipv6()
        return
    else:
