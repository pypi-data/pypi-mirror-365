...
def whatsapp_eval_multi_image_overlay(client, update: WhatsAppUpdate, context, overlay_top_left=None, overlay_top_right=None, overlay_bottom_left=None, overlay_bottom_right=None):
    ...
    if overlay_top_left:
        overlay_bytes = requests.get(overlay_top_left).content
        im2 = Image.open(overlay_bytes)
        im1 = overlay_images(im1, im2, "left","top", True)
    if overlay_top_right:
        overlay_bytes = requests.get(overlay_top_right).content
        im2 = Image.open(overlay_bytes)
        im1 = overlay_images(im1, im2, "right","top", True)
    if overlay_bottom_left:
        overlay_bytes = requests.get(overlay_bottom_left).content
        im2 = Image.open(overlay_bytes)
        im1 = overlay_images(im1, im2, "left","bottom", True)
    if overlay_bottom_right:
        overlay_bytes = requests.get(overlay_bottom_right).content
        im2 = Image.open(overlay_bytes)
