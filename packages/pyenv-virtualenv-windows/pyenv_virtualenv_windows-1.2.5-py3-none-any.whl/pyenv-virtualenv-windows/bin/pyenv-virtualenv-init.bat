@echo off
setlocal
REM Windows Command Line Script (Batch) to launch "pyenv-virtualenv-init.py".
REM
REM Dependencies:
REM   * pyenv
...
call python "%~dp0pyenv-virtualenv-init.py" %*
set /a RC=%ERRORLEVEL%
if %RC% equ 130 goto cancel
if %RC% equ 5 goto finish
...
:error1
echo.
echo [91mERROR:    Failed to initialize terminal shell (RC = %RC%).[0m
