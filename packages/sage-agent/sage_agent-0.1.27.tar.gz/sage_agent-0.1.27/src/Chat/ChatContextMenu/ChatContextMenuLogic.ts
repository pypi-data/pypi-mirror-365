...
export class ChatContextMenuLogic {
  ...
  public async handleItemSelect(item: MentionContext): Promise<void> {
    ...
    if (this.onContextSelected) {
      ...
      if (item.type === 'rules' && !item.content) {
        contextWithContent.content =
          await this.contextLoaders.loadTemplateContent(item.id);
      }
  ...
  public async showDropdown(): Promise<void> {
    ...
    const templateContexts = await this.contextLoaders.loadTemplateFiles();
    ...
    contextItems.set('rules', templateContexts);
