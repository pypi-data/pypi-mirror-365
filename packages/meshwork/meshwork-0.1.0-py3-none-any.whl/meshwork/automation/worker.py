...
class Worker:
    ...
    def _get_web_executor(self):
        ...
        async def automation_request(request: Request):
            ...
            if not automation:
                return JSONResponse(
                    content={"correlation": auto_request.correlation,
                             "result": {"error": f"No automation found for path '{auto_request.path}'"}},
                    status_code=404,
                    headers=headers,
                )
            ...
            try:
                ...
                return JSONResponse(
                    content={"correlation": auto_request.correlation,
                             "result": {"error": f"Automation failed: {format_exception(e)}"}},
                    status_code=500,
                    headers=headers,
                )
