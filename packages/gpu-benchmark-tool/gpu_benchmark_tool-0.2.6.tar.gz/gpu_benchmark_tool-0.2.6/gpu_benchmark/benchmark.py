...
def run_full_benchmark(handle, duration=60, enhanced=True, device_id=0):
    ...
    if enhanced:
        ...
        if not requirements["enhanced_available"]:
            print("Enhanced monitoring not available - requirements not met")
            print("Run 'gpu-benchmark enhanced-status' for detailed diagnostics")
            metrics = stress_gpu_with_monitoring(handle, duration)
        else:
            # Requirements met, bypass backend availability check and create directly
            from .backends.nvidia import NVIDIABackend
            backend = NVIDIABackend()
            
            print(f"DEBUG: Backend created: {backend is not None}")
            print(f"DEBUG: Backend type: {type(backend)}")
            
            if not backend:
                print("Enhanced monitoring not available - NVIDIA backend not found")
                print("Run 'gpu-benchmark enhanced-status' for detailed diagnostics")
                metrics = stress_gpu_with_monitoring(handle, duration)
            elif not hasattr(backend, 'create_monitor'):
                print("Enhanced monitoring not available - backend missing create_monitor method")
                print("Run 'gpu-benchmark enhanced-status' for detailed diagnostics")
                metrics = stress_gpu_with_monitoring(handle, duration)
            else:
                try:
                    # Pass device_id instead of handle for better compatibility
