...
class Store:
    ...
    def _create_table_queries(self, T: type[Q]) -> list[str]:
        queries = []
        uniques = []
        ...
        for field in dataclasses.fields(T):
            ...
            if field.metadata.get("store.type", None) == "unique":
                uniques.append(field_name)
            fields.append(field_name + ' ' + field_type)
        field_statement = ", ".join(fields)
        queries.append(f'{create} {tablename}({field_statement});')

        unique_columns = ", ".join(uniques)
        if len(uniques) > 0:
            queries.append(f"CREATE UNIQUE INDEX IF NOT EXISTS {tablename}unique ON {tablename}({unique_columns});")
        return queries
    ...
    def create_db(self) -> None:
        ...
        with self:
            for dc in self._types:
                for q in self._create_table_queries(dc):
                    cur.execute(q)
            for dc in self._types:
    ...
    def create_table(self, dc: type[Q]) -> None:
        qs = self._create_table_queries(dc)
        cur = self._connection.cursor()
        with self:
            for q in qs:
                cur.execute(q)
