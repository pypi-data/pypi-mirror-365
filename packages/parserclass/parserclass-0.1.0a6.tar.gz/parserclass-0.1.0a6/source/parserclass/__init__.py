...
def parameters_from_function(func: Callable) -> list[FunctionParameter]:
...
class FunctionArgParser[T]:
    ...
    def _parse(self, std_args: list[str] = []) -> RunTriplet[T]:
        args: Args = []
        kwargs: Kwargs = {}

        parameters = RetrievableSet(reversed(self.parameters))
        std_args.reverse()
        while len(std_args) > 0:
            if std_args[-1].startswith("--"):
                break
        ...
        while len(std_args) > 0:
            ...
            if param.type is bool:
                if len(std_args) > 0 and not std_args[-1].startswith("--"):
                    value = std_args.pop()
                else:
    ...
    def parse(self, std_args: list[str] = []) -> tuple[Args, Kwargs]:
        try:
    ...
    def parse_and_run(self, std_args: list[str] = []) -> T | None:
        try:
...
class BaseArgParser:
    ...
    subparsers: RetrievableSet[BaseArgParser | FunctionArgParser] = field(
        default_factory=RetrievableSet
    )
    ...
    def _parse(self, std_args: list[str], /, execute: bool = False) -> RunTriplet[Any]:
        try:
            if len(std_args) == 0:
            ...
            if arg in self.subparsers:
                ...
                match subparser:
                    case BaseArgParser():
                        return subparser._parse(std_args)
    ...
    def parse_and_run(self, std_args: list[str]) -> Any:
        try:
            result, args, kwargs = self._parse(std_args)
            return result(*args, **kwargs)
        except ArgumentError as err:
