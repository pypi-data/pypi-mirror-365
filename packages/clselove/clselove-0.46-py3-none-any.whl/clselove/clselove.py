...
import re, io,cv2, time, random, requests,string,names,subprocess,os,pycountry,phonenumbers
import numpy as np
...
def totel_temp(email):
    ...
    if matched_error:
        ...
        return text_code
def opencv_geetest(slice_url,bg_url,big,smail,totel):
...
def geetest(driver):
    try:
        ...
        slice_url = re.search(r'url\("(.*?)"\)', slice_el.get_attribute("style")).group(1)
        bg_url = re.search(r'url\("(.*?)"\)', bg_el.get_attribute("style")).group(1)
        slice_url = requests.get(slice_url, timeout=10).content
        bg_url = requests.get(bg_url, timeout=10).content
        dist = opencv_geetest(slice_url,bg_url,220,30,37)
        print("distance =", dist)
