...
def request(server = None, token = None, course = None,
    ...
    if (lms.api.assignment.resolve.requires_resolution(assignments)):
        resolved_assignments = lms.api.assignment.resolve.fetch_and_resolve_assignments(server, token, course, assignments)
    ...
    if (lms.api.user.resolve.requires_resolution(users)):
        resolved_users = lms.api.user.resolve.fetch_and_resolve_users(server, token, course, users)
    ...
    for assignment_index in range(len(assignments)):
        ...
        score_count += lms.api.assignment.uploadscores.direct_request(server, token, course,
