...
DEFAULT_CONFIG_FILENAME = 'edq-lms.json'
...
def _get_config(modify_parser = None, **parser_options):
    ...
    lms.log.init(level = config.get('log_level', lms.log.DEFAULT_LOGGING_LEVEL))
    ...
    try:
        ...
        raise ValueError("Failed to parse LMS server: '%s'." % config['server']) from ex
...
def get_argument_parser(description = '', course = True, **kwargs):
    ...
    parser.add_argument('--log-level', dest = 'log_level',
        action = 'store', type = str, default = lms.log.DEFAULT_LOGGING_LEVEL,
        choices = lms.log.LEVELS,
        help = 'The logging level (default: %(default)s).')

    parser.add_argument('--server', dest = 'server',
        action = 'store', type = str, default = None,
        help = 'The LMS instance to work with.')

    parser.add_argument('--token', dest = 'token',
        action = 'store', type = str, default = None,
        help = 'A LMS authentication token.')

    if (course):
        parser.add_argument('--course', dest = 'course',
            action = 'store', type = int, default = None,
            help = 'The LMS course ID.')
