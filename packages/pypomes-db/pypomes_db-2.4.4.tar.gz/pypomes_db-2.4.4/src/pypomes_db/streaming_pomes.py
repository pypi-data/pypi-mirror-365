...
def db_stream_lobs(errors: list[str] | None,
    ...
    if curr_conn:
            ...
            while next_rs:
                ...
                while row:
                    ...
                    if is_migrated:
                        ...
                        byte_count += len(lob_data)
                        byte_step += len(lob_data)
                        lob_count += 1
                        lob_step += 1
                    ...
                    if logger and lob_step >= log_trigger:
                        mins: float = (time.time() - start_step) / 60
                        logger.debug(msg=f"Streamed {lob_step} LOBs ({lob_step/mins:.2f} LOBs/min), "
                                         f"{byte_step} bytes ({(byte_step/(1024 * 1024 * mins)):.2f} GBytes/min),  "
                                         f"from {engine}.{table}.{lob_column}, "
                                         f"offset {offset_count + lob_count - lob_step}")
                        byte_step = 0
                        lob_step = 0
        ...
        if err_msg:
            ...
            mins = (time.time() - start_count) / 60
            logger.debug(msg=f"Finished streaming {lob_count} LOBs ({lob_count/mins:.2f} LOBs/min), "
                             f"{byte_count} bytes ({(byte_count/(1024 * 1024 * mins)):.2f} GBytes/min), "
