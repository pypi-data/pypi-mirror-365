    ...
    if curr_conn:
                ...
                while row:
                    ...
                    if logger and lob_step >= log_trigger:
                        ...
                        duration: str = timestamp_duration(start=start_step,
                                                           finish=finish_step)
                        logger.debug(msg=f"Streamed {lob_step} LOBs in {duration} "
                                         f"({lob_step/mins:.2f} LOBs/min), "
                                         f"{byte_step} bytes ({byte_step/(mins * 1024 ** 2):.2f} MBytes/min),  "
                                         f"from {engine}.{table}.{lob_column}, "
        ...
        if err_msg:
            ...
            duration: str = timestamp_duration(start=start_count,
                                               finish=finish_count)
            logger.debug(msg=f"Finished streaming {lob_count} LOBs in  {duration} "
                             f"({lob_count/mins:.2f} LOBs/min), "
                             f"{byte_count} bytes ({byte_count/(mins * 1024 ** 2):.2f} MBytes/min), "
