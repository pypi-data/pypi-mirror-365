...
absl::StatusOr<std::vector<float>> GenerateQueryFixedDimensionalEncoding(
  ...
  Eigen::Map<const MatrixRowMajor> embedding_matrix(
  ...
  int32_t projection_dim =
      use_identity_encoding ? config.dimension() : config.projection_dimension();
  ...
  for (int repetition_num = 0; repetition_num < config.num_repetitions();
    ...
    if (config.num_simhash_projections() > 0) {
      sketch_matrix =
          embedding_matrix * internal::SimHashMatrixFromSeed(
    ...
    if (use_identity_encoding) {
      ...
      projected_matrix =
          embedding_matrix *
          internal::AMSProjectionMatrixFromSeed(config.seed() + repetition_num,
                                                config.dimension(),
                                                projection_dim);
    }
