...
class MyTestCase(unittest.TestCase):
    ...
    def test_process(self):
        ...
        class Clock(Process):
            ...
            def on_yield(self):
                ...
                self.env.timeout(1, self.on_yield, ())  # `tuple()` means no argument for `on_yield()`
    ...
    def test_event(self):
        ...
        class Machine(Process):
            ...
            def on_yield(self):
                out.add(f"{self.name} time: {int(self.env.get_now())} state: {self.state}")
                self.env.timeout(1, self.on_yield, tuple())  # `tuple()` means no argument for `on_yield()`
    ...
    def test_resource(self):
        ...
        class MyProcess(Process):
            ...
            def run(self):
                out.add(f"Process started at time {self.env.get_now()}. Request resource")
                self.resource.on_request(self.use_resource)

            def use_resource(self):
                out.add(f"Resource acquired at time {self.env.get_now()}. Released in 5min.")
                self.env.timeout(5, self.resource.on_release, tuple([self.use_resource, ()]))
    ...
    def test_store(self):
        ...
        class Producer:
            ...
            def on_initialize(self, env):
                self.env = env
                self.env.timeout(self.cycle_time, self.on_yield, tuple())  # launch the producing loop

            def on_yield(self):
                self.store.on_put(f"item{self.item_id}")
                self.env.timeout(self.cycle_time, self.on_yield, tuple())  # loop on itself every 5
                self.item_id += 1
        ...
        class Consumer(Process):
            ...
            def on_initialize(self, env):
                self.env = env
                self.store.on_get(self.on_yield)  # the consumer is immediately ready to receive a new item

            def on_yield(self, msg):
                # s.add(f'Consumer receives {msg} at {self.env.now}')
                # self.store.on_get(self.on_yield)
                self.env.timeout(self.cycle_time, self.store.on_get, tuple([self.on_yield]))
    ...
    def test_conditional_events(self):
        ...
        def cond3_callback():
            logs.append(f"The expression `(ev1 & ev4) | ev3` is true at {env.get_now()}")
    ...
    def test_container(self):
        ...
        def consumer(amount):
            logs.append(f"[{env.get_now()}] Consumed {amount} units (level={container.level})")

        def producer_done(amount):
            logs.append(f"[{env.get_now()}] Producer added {amount} units (level={container.level})")
        ...
        env.timeout(2, container.get, (5, consumer))  # fifo, so it would not be scheduled
    ...
    def test_preemptive_resource(self):
        ...
        pA = ("pA", lambda: logs.append("pA got"), (), 0, True, lambda: logs.append("pA preempted"))
        pB = ("pB", lambda: logs.append("pB got"), (), 0, True, lambda: logs.append("pB preempted"))
        pC = ("pC", lambda: logs.append("pC got"), (), -1, True, lambda: logs.append("pC preempted"))
    ...
    def test_filter_store(self):
        ...
        logs = []
        expected_log = ['user 1 got Machine(memory=2, cpu=1)  at 1',
        ...
        env.timeout(0, machine_shop.put, (
            m1,
            lambda x: logs.append(f"returned {x}  at {env.get_now()}")
        ))

        # 5 users request machines at time 1
        for user_i in range(5):
            env.timeout(1, machine_shop.get, (
                lambda m: m.memory >= 1,
                lambda m, ui=user_i: logs.append(f"user {ui} got {m}  at {env.get_now()}")
            ))

        # Return a machine at time 2
        env.timeout(2, machine_shop.put, (
            m1,
            lambda x: logs.append(f"returned {x} at {env.get_now()}")
        ))
    ...
    def test_priority_store(self):
        ...
        logs = []
        expected_log = ["add low", "add high", "add medium", "get high",
                        "add urgent", "get urgent", "get medium", "get low"]
