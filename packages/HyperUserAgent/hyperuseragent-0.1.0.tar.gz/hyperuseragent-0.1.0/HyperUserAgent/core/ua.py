...
class UA:
    ...
    def create(
        ...
        """
        b = browser or self._pick_browser(platform)
        info = browsers()[b]
        version = self._pick_version(info, min_version, channel)
        os_string = self._pick_os(platform)
        ua = self._format(b, version, os_string)
        return ua
    ...
    def chrome(
        ...
        """
        return self.create(platform, self.chrome.__name__, channel, min_version)
    ...
    def firefox(
        ...
        """
        return self.create(platform, self.firefox.__name__, channel, min_version)
    ...
    def safari(
        ...
        """
        return self.create(platform, self.safari.__name__, channel, min_version)
    ...
    def edge(
        ...
        return self.create(platform, self.edge.__name__, channel, min_version)
    ...
    def opera(
        ...
        """
        return self.create(platform, self.opera.__name__, channel, min_version)
    ...
    def brave(
        ...
        return self.create(platform, self.brave.__name__, channel, min_version)
    ...
    def yandex(
        ...
        """
        return self.create(platform, self.yandex.__name__, channel, min_version)

    def _pick_browser(self, platform: Platform) -> str:
        pool = {k: v for k, v in weights()[platform].items() if k in browsers()}
        return self.rng.choices(list(pool), weights=list(pool.values()))[0]

    def _pick_version(
        self, info: Dict[str, any], min_v: str | None, channel: str | None
    ) -> str:
        ...
        major = self.rng.randint(lo, hi)
        minor = self.rng.randint(0, 99)
        build = self.rng.randint(0, 9999)
        patch = self.rng.randint(0, 99)
        return f"{major}.{minor}.{build}.{patch}"

    def _pick_os(self, platform: Platform) -> str:
        """Return fully-formatted OS substrate."""
        os_name = self.rng.choice(list(os_cfg()))
        cfg = os_cfg()[os_name]
        if platform == "mobile" and os_name not in ("android", "ios"):
            os_name = "android"
            cfg = os_cfg()["android"]
        ver = self.rng.choice(cfg.get("versions", [["", ""]]))
        arch = self.rng.choice(cfg.get("arch", ["x64"]))
        ...
        if os_name == "linux":
            distro = self.rng.choice(cfg["distros"])
            return f"X11; {distro} {arch}"
        if os_name == "android":
            dev = self.rng.choice(cfg["devices"])
    ...
    def _format(self, b: str, ver: str, os_str: str) -> str:
