...
from ..commands.base import command_registry, CommandScope
...
class ReplCommandExecutor:
    ...
    def _execute_repl_command(self, parsed_cmd: ParsedCommand, context: ReplContext) -> Any:
        ...
        if not cmd_meta:
            # Check aliases
            for cmd_name, meta in command_registry._commands.items():
        ...
        if hasattr(cmd_meta.command_class, '__call__') and not hasattr(cmd_meta.command_class, '__init__'):
    ...
    def _switch_to_context(self, context_type: ContextType, name: str, context: ReplContext) -> str:
        ...
        if context_type == ContextType.CLIENT and context.current_gateway:
            ...
            return_msg = "Exited gateway context. " + self._switch_to_context(
                context_type, name, context
            )
            return return_msg
        elif context_type == ContextType.GATEWAY and context.current_client:
            context.reset_context()
            return_msg = "Exited client context. " + self._switch_to_context(
                context_type, name, context
            )
            return return_msg
...
"""
        ...
        if context.current_frame:
            if context.current_frame.type == ContextType.GATEWAY:
                ...
                help_text += "  get                  Get current gateway details\n"
                help_text += "  trigger-scan         Trigger BACnet scan on current gateway\n"
                help_text += "  deploy-points        Deploy point config to current gateway\n"
                help_text += "  enable-bacnet        Enable BACnet on current gateway\n"
                help_text += "  disable-bacnet       Disable BACnet on current gateway\n"
                help_text += "  bacnet-status        Show BACnet configuration status\n"
                help_text += "\nBACnet Examples:\n"
                help_text += "  trigger-scan --scan-address 192.168.1.0/24\n"
                help_text += "  bacnet-status\n"
                help_text += "  enable-bacnet\n"
                ...
                help_text += "  sites get            Get current site details\n"
                help_text += "  points list          List points for current site\n"
                help_text += "  gateways list        List gateways for current site\n"
            elif context.current_frame.type == ContextType.CLIENT:
                help_text += "\n\nClient Context Commands:\n"
                help_text += "  clients get          Get current client details\n"
                help_text += "  sites list           List sites for current client\n"
