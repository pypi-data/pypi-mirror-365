...
class ReplCommandExecutor:
    ...
    def _execute_repl_command(self, parsed_cmd: ParsedCommand, context: ReplContext) -> Any:
        ...
        elif command in ["trigger-scan", "deploy-points", "enable-bacnet", "disable-bacnet", "bacnet-status"]:
    ...
    def _execute_cli_command(self, parsed_cmd: ParsedCommand, context: ReplContext) -> Any:
        ...
        if parsed_cmd.command_path in bacnet_gateway_commands:
            # If no gateway argument provided and we're in gateway context
            if len(parsed_cmd.arguments) == 0 and context.current_gateway:
                parsed_cmd.arguments = [context.current_gateway]
        ...
        if parsed_cmd.command_path in volttron_gateway_commands:
            if parsed_cmd.command_path == ["volttron", "upload-agent"]:
                ...
                elif len(parsed_cmd.arguments) == 1:  # Only path provided
                    if context.current_gateway:
                ...
                elif len(parsed_cmd.arguments) == 1:  # Only file provided
                    if context.current_gateway:
                        parsed_cmd.arguments.append(context.current_gateway)
                    else:
                        raise click.ClickException(
                ...
                elif len(parsed_cmd.arguments) == 2:  # Path and config provided
                    if context.current_gateway:
                        parsed_cmd.arguments.append(context.current_gateway)
                    else:
                        raise click.ClickException(
    ...
    def _handle_get_command(self, args: list[str], parsed_cmd: ParsedCommand, context: ReplContext) -> Any:
            ...
            try:
                ...
                if save_file and output_format == "json":
                    try:
                        ...
                        return f"Error: Failed to parse JSON output"
                    except IOError as e:
