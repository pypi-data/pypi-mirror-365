...
class FileGolem:
    ...
    def _handle_defaults(self,config, ancestry=[]):
        ...
        #TODO: Move functionality from ml_golem to file_golem
        if 'origin_config' in config:
            print('ORIGIN_CONFIG_FOUND:', config['origin_config'])
            origin_config_full = self.load_config(config['origin_config'], ancestry)
            module_name = config.get('origin_module', None)
            print('MODULE_NAME:', module_name)

            def _create_origin_subconfig_keys(module_name):
                subconfig_keys = ['architecture']
                if module_name is not None:
                    subconfig_keys.append(module_name)
                return subconfig_keys
            
            subconfig_keys = _create_origin_subconfig_keys(module_name)
