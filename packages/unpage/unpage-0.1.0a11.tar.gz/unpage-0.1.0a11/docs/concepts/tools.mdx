...
unpage tools call metrics_get_metrics_for_node --node-id i-0abc123def456 --metric cpu_utilization

# See detailed output structure
unpage tools call graph_get_resource_details --resource-id i-0abc123def456 | jq
```
...
CPU_USAGE=$(unpage tools call metrics_get_metrics_for_node --node-id i-0abc123def456 --metric cpu_utilization --last 5m | jq '.avg')
...
2. **Test tools independently**: Use `unpage tools call` to understand behavior
