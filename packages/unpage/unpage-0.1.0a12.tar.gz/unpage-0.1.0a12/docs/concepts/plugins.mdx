...
<AccordionGroup>
  ...
  <Accordion title="LLM Tool Exposure" icon="toolbox">
    ...
    These tools become available when running `unpage start` to launch the MCP server.
  </Accordion>
</AccordionGroup>
...
When you start the MCP server with `unpage start`, plugins register tools that become available to LLMs:
