...
from auto_teacher_process.db.services.db_insert_dedup import DedupInsertDBProcessor


# 测试参数缺失场景
def test_run_missing_parameters(mocker):
    # 创建处理器实例并模拟数据库初始化
    mocker.patch.object(DedupInsertDBProcessor, "_setup_db_engine")  # 避免实际数据库连接
    processor = DedupInsertDBProcessor()
...
def test_run_file_processing(mock_read_csv, mock_walk, mocker):
    ...
    mock_df2 = pd.DataFrame(
        {"teacher_id": [3], "is_repeat": [1], "related_teacher_id": [20], "status": ["active"], "is_valid": [1]}
    )
    ...
    assert len(call_args["df"]) == 2  # 验证合并后的数据量
