...
class OutcarParser:
    ...
    def read_zpe_tds(path: Path, calc_tds: bool = True) -> tuple[float, float]:
        ...
        freqs = u.Quantity(freqs_cm, u.cm**-1) * const.c
        ...
        freqs_floor = u.Quantity([max(nu, 50.0) for nu in freqs_cm], u.cm**-1) * const.c
    ...
    def read_converged(path: Path) -> bool:
        ...
        for line in lines[:200]:  # scan up to the last 200 lines
    ...
    def auto_read(
        ...
        *,
        calc_tds: bool = False,
        zpe_locator: Optional[Callable[[Path, str], Path]] = None,
        check_structure: bool = False,
    ) -> Tuple[List[float], List[float], List[float], Optional[List[bool]]]:
        """
        ...
        for d in subdirs:
            ...
            if zfile.exists():
                ...
                warnings.warn(
                    f"No ZPE/TdS OUTCAR found at {zfile!r}; "
                    "setting ZPE and TdS to 0",
                    UserWarning,
                )
                zpe, tds = 0.0, 0.0
            zpes.append(zpe)
