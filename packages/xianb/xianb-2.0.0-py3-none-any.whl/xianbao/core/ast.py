...
class ConditionParser:
    ...
    def parse_condition_dict(self, condition_dict: Dict[str, Any]) -> ASTNode:
        ...
        # 处理逻辑操作符
        for logical_op in self.logical_ops:
            if logical_op in condition_dict:
                return self._parse_logical_op(logical_op, condition_dict[logical_op])
    ...
    def _parse_field_conditions(self, conditions: Dict[str, Any]) -> ASTNode:
        ...
        for field, value in conditions.items():
            if isinstance(value, dict):
                ...
                for op, op_value in value.items():
                    if op in special_ops:
                        ...
                        if op == '&~':
                            ...
                            # (field op value) != 0 (对于位运算) 或 field op value (对于算术运算)
                            if op in {'&', '|', '^'}:  # 位运算
                                expr = f"({field} {op} ?) != 0"
                            else:  # 算术运算
                                expr = f"{field} {op} ?"
                ...
                node = ASTNode('EQ')
                node.add_child(ASTNode('FIELD', field))
                node.add_child(ASTNode('VALUE', value))
                root.add_child(node)
...
class DMLGenerator:
    ...
    def _generate_set_expression(self, field: str, expr_dict: Dict[str, Any]) -> Tuple[str, List[Any]]:
        ...
        # 默认处理，如果操作符不被支持
        expression = f"{field} = ?"
        params.append(list(expr_dict.values())[0])
        return expression, params
