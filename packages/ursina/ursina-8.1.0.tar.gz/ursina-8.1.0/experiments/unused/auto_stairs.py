...
class AutoStairs(Entity):
    def __init__(self, height=1, **kwargs):
        ...
        side_2 = duplicate(side, flipped_faces = True)
        side_2.x *= -1
        ...
        railing = Entity(model=Pipe(path=path, thicknesses=((.1,.05),)), color=color.orange, world_parent=self)
        railing_2 = Entity(model=Pipe(path=path, thicknesses=((.1,.05),)), color=color.orange, world_parent=self, flipped_faces=True)
        railing_2.scale_x *= -1

        ground_collider = Entity(model='cube', color=color.green,
            parent = self,
            origin = (0,.5,-.5),
            scale_y = .8,
        ...
        top_collider = Entity(model='cube', color=color.blue,
            parent = self,
            origin = (0,.5,-.5),
        ...
        rail_start = Entity(model='cube', color=color.red,
            x = .5 - (.1/2),
            origin = (0, -.5, -.5),
