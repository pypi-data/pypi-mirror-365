    ...
    for filetype in file_types:
        ...
        for filename in path.glob(f'**/{name}{filetype}'):
            ...
            if filetype == '.obj':
                ...
                if blend_to_obj(path=path, name=name):
...
def load_blender_scene(name, path=application.asset_folder, load=True, reload=False, skip_hidden=True, models_only=False, max_decimals=4, triangulate=True):
    ...
    if reload or not out_file_path.exists():
        ...
        args = [
            ...
            '--triangulate' if models_only else '',
            f'--decimals={max_decimals}',
        ]
...
def blend_to_obj(path=None, outpath=application.models_compressed_folder, name='*'):

    if not application.models_compressed_folder.exists():
        application.models_compressed_folder.mkdir()
...
def obj_to_ursinamesh(path=application.models_compressed_folder, outpath=application.models_compressed_folder, name='*', return_mesh=True, save_to_file=False, delete_obj=False):
...
def blend_to_obj_fast(model_name=None, write_to_disk=False):
    ...
    compressed_files = os.listdir(application.models_compressed_folder)
...
def ursina_mesh_to_obj(mesh, name='', out_path=application.models_compressed_folder, max_decimals=3):
...
def compress_internal():
    blend_to_obj(application.internal_models_folder)
    obj_to_ursinamesh(
