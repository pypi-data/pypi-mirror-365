from ursina import *; pbr_shader = Shader(language=Shader.GLSL, vertex = '''#version 120
...
void main() {
    ...
    float metallic = clamp(metallic * metal_rough.b, 0.0, 1.0);
    float perceptual_roughness = clamp(roughness * metal_rough.g,  0.0, 1.0);
    float alpha_roughness = perceptual_roughness * perceptual_roughness;
    vec4 base_color = baseColor * v_color * p3d_ColorScale * texture2D(p3d_TextureBaseColor, v_texcoord);
    ...
    vec3 emission = emission.rgb * texture2D(p3d_TextureEmission, v_texcoord).rgb;
#else
...
if __name__ == '__main__':
    ...
    # node = scene.attachNewNode(directionalLight)
