...
class ProjectBrowser(Entity):
    ...
    def __init__(self):
        ...
        # self.title = Text()
        self.title = Text()
        self.title.parent = application.ui
        ...
        self.list = Entity()
        self.list.parent = application.ui
...
class ProjectBrowserButton():
    ...
    def input(self, key):
        if self.entity.hovered:
            ...
            or key == 'enter'
            and self.index > 0):
                self.load_project(application.paths[self.index])

    def on_mouse_enter(self):
        application.index = self.index
