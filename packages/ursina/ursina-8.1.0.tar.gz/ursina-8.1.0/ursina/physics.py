...
if __name__ == '__main__':
    ...
    class Player(Entity):
        def __init__(self):
            super().__init__(model=Capsule(), color=color.orange, y=2, z=-4)
            self.rb = RigidBody(shape=CapsuleShape(), entity=self, mass=1, kinematic=0, friction=1)
        ...
        def update(self):
            ...
            self.rb.apply_impulse(self.direction*time.dt*200)
            limit = 10

            self.rb.friction = 10 if self.direction.length() <.1 else 1
            print(self.direction.length())
            vel = self.rb.getLinearVelocity()
            self.rb.setLinearVelocity(Vec3(*[clamp(e, -limit, limit) for e in vel]))


    Player()
    ground = Entity(model='plane', scale=10, collider='box', z=10, rotation_x=-30, y=2, color=color.gray, name='ground')
    ground.rb = RigidBody(entity=ground, shape=BoxShape(size=Vec3(1,0,1)))
