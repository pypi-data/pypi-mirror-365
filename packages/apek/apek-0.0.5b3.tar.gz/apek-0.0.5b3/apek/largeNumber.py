...
class LargeNumber(typing.Number):
    ...
    def __init__(
        ...
        *,
        ...
        self.base = self._toMpf(abs(base))
        self.exp = exp
        self.calibrate()
    ...
    def __eq__(self, other):
        other = self._tryParseLargeNumberOrReturnNone(other)
    ...
    def __add__(self, other):
        ...
        if other is None:
            return NotImplemented
        if self.getExp() == other.getExp():
            return self.fastNew(self.getBase() + other.getBase(), other.getExp())
        selfIsBigger = self.getExp() > other.getExp()
        if selfIsBigger:
            big, small = self, other
        else:
            big, small = other, self
        es = big.getExp() - small.getExp()
        if es > big.getConfig("realPrec"):
            return big.copy()
        bigb = big.getBase() * _mp.power(10, es)
        smallb = small.getBase()
        return self.fastNew(bigb + smallb, small.getExp())
