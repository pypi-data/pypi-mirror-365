...
class RtuTransport(BaseTransport):
    ...
    def open(self) -> None:
        ...
        try:
            ...
            if not self._serial.is_open:
                raise ConnectionError(f"无法打开串口 | Unable to open serial port {self.port}")
                
            self._logger.info(f"RTU连接已建立 | RTU connection established: {self.port} @ {self.baudrate}bps")
    ...
    def send_and_receive(self, slave_id: int, pdu: bytes) -> bytes:
        ...
        try:
            ...
            self._logger.debug(f"RTU接收 | RTU Receive: {response_adu.hex(' ').upper()}")
            ...
            if response_adu[0] != slave_id:
                raise InvalidResponseError(f"从站地址不匹配 | Slave address mismatch: 期望 | Expected {slave_id}, 收到 | Received {response_adu[0]}")
            ...
            raise TimeoutError(f"RTU通信超时 | RTU communication timeout ({self.timeout}秒 | seconds)")
        except serial.SerialException as e:
            raise ConnectionError(f"串口通信错误 | Serial port communication error: {e}")
    ...
    def _receive_response(self, expected_slave_id: int, function_code: int) -> bytes:
        ...
        if response[1] & 0x80:  # 异常响应 | Exception response
            ...
            if len(remaining) < 3:
                raise TimeoutError("接收异常响应超时 | Receive exception response timeout")
            return response + remaining
        
        # 正常响应，根据功能码确定剩余长度 | Normal response, determine remaining length based on function code
        if function_code in [0x01, 0x02]:  # 读取线圈/离散输入 | Read coils/discrete inputs
            ...
            remaining_data = self._serial.read(byte_count + 2)  # 数据 + CRC | Data + CRC
        ...
        elif function_code in [0x03, 0x04]:  # 读取保持寄存器/输入寄存器 | Read holding registers/input registers
            # 格式：地址 + 功能码 + 字节数 + 数据 + CRC | Format: address + function code + byte count + data + CRC
            byte_count_data = self._serial.read(1)
            if len(byte_count_data) < 1:
                raise TimeoutError("接收字节数超时 | Receive byte count timeout")
            ...
            remaining = self._serial.read(6)  # 起始地址 + 数量 + CRC | Starting address + quantity + CRC
            if len(remaining) < 6:
                raise TimeoutError("接收写响应超时 | Receive write response timeout")
            return response + remaining
            ...
            remaining = self._serial.read(10)  # 最多再读10字节 | Read at most 10 more bytes
            return response + remaining
