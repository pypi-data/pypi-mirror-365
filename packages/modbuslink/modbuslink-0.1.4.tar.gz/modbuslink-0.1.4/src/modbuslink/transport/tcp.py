...
class TcpTransport(BaseTransport):
    ...
    def open(self) -> None:
        ...
        try:
            ...
            self._logger.info(f"TCP连接已建立 | TCP connection established: {self.host}:{self.port}")
    ...
    def close(self) -> None:
        ...
        if self._socket:
            try:
                self._socket.close()
                self._logger.info(f"TCP连接已关闭 | TCP connection closed: {self.host}:{self.port}")
            except socket.error:
    ...
    def send_and_receive(self, slave_id: int, pdu: bytes) -> bytes:
        ...
        try:
            ...
            self._logger.debug(f"TCP接收 | TCP Receive: {(response_mbap + response_pdu).hex(' ').upper()}")
            ...
            raise TimeoutError(f"TCP通信超时 | TCP communication timeout ({self.timeout}秒 | seconds)")
        except socket.error as e:
    ...
    def _receive_exact(self, length: int) -> bytes:
        ...
        while len(data) < length:
            try:
                ...
                if not chunk:
                    raise ConnectionError("连接被远程主机关闭 | Connection closed by remote host")
                data += chunk
            except socket.timeout:
                raise TimeoutError(f"接收数据超时 | Data receive timeout，已接收 | Received {len(data)}/{length} 字节 | bytes")
            except socket.error as e:
