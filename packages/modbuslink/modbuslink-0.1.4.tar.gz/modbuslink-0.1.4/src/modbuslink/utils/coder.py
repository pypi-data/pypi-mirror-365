...
class PayloadCoder:
    ...
    @staticmethod
    def decode_float32(registers: List[int], byte_order: str = BIG_ENDIAN, word_order: str = HIGH_WORD_FIRST) -> float:
        ...
        """
        if len(registers) != 2:
            raise ValueError("需要恰好2个寄存器来解码float32 | Exactly 2 registers required for float32 decoding")
        ...
        return struct.unpack('>f' if byte_order == PayloadCoder.BIG_ENDIAN else '<f', combined_bytes)[0]
    
    @staticmethod
    def encode_float32(value: float, byte_order: str = BIG_ENDIAN, word_order: str = HIGH_WORD_FIRST) -> List[int]:
        ...
        packed_bytes = struct.pack('>f' if byte_order == PayloadCoder.BIG_ENDIAN else '<f', value)
    ...
    @staticmethod
    def decode_int32(registers: List[int], byte_order: str = BIG_ENDIAN, word_order: str = HIGH_WORD_FIRST, signed: bool = True) -> int:
        ...
        """
        if len(registers) != 2:
            raise ValueError("需要恰好2个寄存器来解码int32 | Exactly 2 registers required for int32 decoding")
    ...
    @staticmethod
    def encode_int32(value: int, byte_order: str = BIG_ENDIAN, word_order: str = HIGH_WORD_FIRST, signed: bool = True) -> List[int]:
    ...
    @staticmethod
    def decode_int64(registers: List[int], byte_order: str = BIG_ENDIAN, word_order: str = HIGH_WORD_FIRST, signed: bool = True) -> int:
        ...
        """
        if len(registers) != 4:
            raise ValueError("需要恰好4个寄存器来解码int64 | Exactly 4 registers required for int64 decoding")
    ...
    @staticmethod
    def encode_int64(value: int, byte_order: str = BIG_ENDIAN, word_order: str = HIGH_WORD_FIRST, signed: bool = True) -> List[int]:
    ...
    @staticmethod
    def decode_string(registers: List[int], byte_order: str = BIG_ENDIAN, encoding: str = 'utf-8') -> str:
        ...
        try:
            ...
            return decoded_string.rstrip('\x00')  # 去除尾部空字符 | Remove trailing null characters
        except UnicodeDecodeError as e:
            raise ValueError(f"字符串解码失败 | String decoding failed: {e}")
    
    @staticmethod
    def encode_string(value: str, register_count: int, byte_order: str = BIG_ENDIAN, encoding: str = 'utf-8') -> List[int]:
        ...
        if len(encoded_bytes) > max_bytes:
            raise ValueError(f"字符串太长，需要{len(encoded_bytes)}字节，但只有{max_bytes}字节可用 | String too long, needs {len(encoded_bytes)} bytes but only {max_bytes} bytes available")
    ...
    @staticmethod
    def decode_uint32(registers: List[int], byte_order: str = BIG_ENDIAN, word_order: str = HIGH_WORD_FIRST) -> int:
    ...
    @staticmethod
    def encode_uint32(value: int, byte_order: str = BIG_ENDIAN, word_order: str = HIGH_WORD_FIRST) -> List[int]:
    ...
    @staticmethod
    def decode_uint64(registers: List[int], byte_order: str = BIG_ENDIAN, word_order: str = HIGH_WORD_FIRST) -> int:
    ...
    @staticmethod
    def encode_uint64(value: int, byte_order: str = BIG_ENDIAN, word_order: str = HIGH_WORD_FIRST) -> List[int]:
