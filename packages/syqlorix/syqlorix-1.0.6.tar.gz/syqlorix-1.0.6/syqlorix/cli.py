...
from . import Syqlorix, Node, Request, head, body
...
def build(file, output, minify):
    ...
    for route_regex, methods, handler_func in doc_instance._routes:
        if route_regex.pattern == '/$':
            root_handler = handler_func
            break
    page_content = None
    if root_handler:
        click.echo(f"   {C.MUTED}Found root route handler: '{root_handler.__name__}'. Rendering...{C.END}")
        class MockRequestHandler:
            def __init__(self):
                ...
                self.headers = {}
                self.rfile = None
        mock_request = Request(MockRequestHandler())
        page_content = root_handler(mock_request)
    else:
        click.echo(f"   {C.MUTED}No root route found. Rendering the base 'doc' object.{C.END}")
        page_content = doc_instance
    html_content = ""
    if isinstance(page_content, Syqlorix):
        html_content = page_content.render(pretty=not minify)
    elif isinstance(page_content, Node):
        temp_doc = Syqlorix(head(), body(page_content))
        html_content = temp_doc.render(pretty=not minify)
    else:
