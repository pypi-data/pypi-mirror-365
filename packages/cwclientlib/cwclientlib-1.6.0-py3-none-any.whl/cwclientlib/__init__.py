...
def cwproxy_for(instance, **kw):
    ...
    if instance not in cfg:
        ...
        if len(instances) == 1:
            ...
            raise ValueError(
                f"Found {len(instances)!r} configuration entries for {instance!r}"
            )
    if instance not in cfg:
    ...
    proxy_class = CWApiProxy if request_mech == "api" else CWRqlControllerProxy
...
def get_config():
    ...
    if not os.path.isfile(cfgfile):
        raise OSError("Cannot find the configuration file %r" % cfgfile)
    if os.name == "posix":
        perms = os.stat(cfgfile).st_mode % (0o1000)
        if (perms & 0o600) != perms:
            raise OSError(f"{cfgfile!r} permissions should be 0600 or 0400 ({perms:o})")
