...
class AgentSDK:
    ...
    def _clean_content_for_display(self, markdown: str) -> str:
        ...
        for line in lines:
            ...
            # Skip JavaScript, CSS, and code patterns
            skip_patterns = [
                'window[', 'function(', 'console.', 'var ', 'let ', 'const ',
                'sl_tr_', 'tesla_cta', 'slides":', 'media_type', 'roundedCorners',
                'landscape_image', 'alt_text', 'source_type', '{"', '"}', 
                '{', '}', '();', 'pageTitle', '_start', '_end'
            ]
            
            if any(skip in line for skip in skip_patterns):
                continue
                
            # Skip lines that are mostly code/data
            if line.count('"') > 2 or line.count(':') > 1:
                continue
                
            # Skip lines with underscores (usually variable names)
            if '_' in line and not any(keep in line.lower() for keep in ['learn_more', 'tesla']):
                continue
                
            # Skip very long lines (usually data)
            if len(line) > 100:
                continue
            
            # Keep meaningful content lines
            if any(keep in line.lower() for keep in ['tesla', 'cars', 'energy', 'solar', 'cybertruck', 'learn more', 'browser', 'supported']):
                clean_lines.append(line)
            elif line.startswith('#') or line.startswith('-') or line.startswith('['):
                clean_lines.append(line)
            elif len(line) > 10 and not line.replace(' ', '').replace(',', '').replace('.', '').isdigit():
                clean_lines.append(line)
        
        # Join and limit length  
        clean_text = '\n'.join(clean_lines[:15])  # First 15 meaningful lines
        if len(clean_text) > 600:
            clean_text = clean_text[:600] + "..."
        
        return clean_text if clean_text else "Page content loaded successfully."
