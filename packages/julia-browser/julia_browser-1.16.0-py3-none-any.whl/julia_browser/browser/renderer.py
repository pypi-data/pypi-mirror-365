...
class HTMLRenderer:
    ...
    def _render_clean_content(self, soup: BeautifulSoup, css_rules: Dict = None) -> str:
        ...
        try:
            ...
            # Apply basic content filtering (remove only obvious technical noise)
            clean_markdown = self._basic_content_filter(markdown)
    ...
    def _basic_content_filter(self, markdown: str) -> str:
        """Apply basic content filtering to remove obvious technical noise"""
        try:
            # Split into lines and filter out technical content
            ...
            # Remove technical sections entirely
            clean_content = '\n'.join(clean_lines)
            ...
            clean_content = re.sub(r'^\s*\n+', '', clean_content)  # Remove leading empty lines
