...
class AgentSDK:
    ...
    def _clean_content_for_display(self, markdown: str) -> str:
        """Clean markdown content to show complete page like CLI browser (minimal filtering)"""
        ...
        for line in lines:
            ...
            # Only skip obvious JavaScript code blocks and debugging info
            skip_patterns = [
                'window[', 'function(', 'console.log', 'var ', 'let ', 'const ',
                '():', '();', 'sl_tr_', '$.', 'jQuery', 'addEventListener',
                'createElement', 'getElementById', 'getElementsBy'
            ]
            
            if any(skip in line_stripped for skip in skip_patterns):
                continue
                
            # Skip very obvious data structures (but allow some JSON-like content)
            if line_stripped.startswith('{"') and line_stripped.endswith('"}') and len(line_stripped) > 50:
                continue
                
            # Keep all other content including headers, text, links, etc.
            clean_lines.append(line)
        
        # Return complete cleaned content without truncation
        return '\n'.join(clean_lines)
