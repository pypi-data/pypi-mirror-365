        ...
        try:
            ...
            if 1 <= link_num <= len(self.page_links):
                ...
                if url.startswith('/') and self.current_url:
                    ...
                    console.print("[yellow]Anchor links are not supported in CLI mode[/yellow]")
                    return
                
                console.print(f"[dim]Following: {link['text']}[/dim]")
                self.browse_url_enhanced(url)
            ...
            if element['type'] == 'button':
                if element['subtype'] == 'submit' and element.get('form'):
                    ...
                    if self._should_attempt_form_submission(element):
                        ...
                        # For non-navigation buttons, try to execute JavaScript action or simulate
                        onclick = element['element'].get('onclick', '')
                        if onclick and 'javascript:' not in onclick:
                            console.print("[blue]🔄 Executing button JavaScript action...[/blue]")
                            # Try to extract and execute meaningful JavaScript
                            self._execute_button_javascript(onclick)
                        else:
                            # Try to execute real form submission with typed input
                            result = self._handle_button_form_submission(element)
                            if result:
                                if isinstance(result, dict) and result.get('type') == 'form_submission':
                                    # Execute real form submission
                                    self._execute_form_submission(result)
                                else:
