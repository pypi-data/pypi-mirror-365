...
class BrowserEngine:
    ...
    def fetch_page(self, url: str, timeout: int = 30) -> Tuple[bool, str, Optional[BeautifulSoup]]:
        ...
        try:
            ...
            response = self.session.get(url, timeout=timeout, allow_redirects=True, headers=enhanced_headers)
            ...
            if 'application/json' in content_type or url.endswith('.json'):
                ...
                if final_url and final_url != response.url:
                    # Follow the redirect
                    try:
                        redirect_response = self.session.get(final_url, timeout=timeout, allow_redirects=True, headers=enhanced_headers)
    ...
    def render_page(self, soup: BeautifulSoup, execute_js: bool = True) -> str:
        """
        Render HTML page to markdown format
        ...
        try:
            ...
            js_modifications = {}
            if execute_js:
                js_modifications = self._execute_page_javascript(soup)
                
            # Apply JavaScript modifications to soup
            if js_modifications:
                soup = self._apply_js_modifications(soup, js_modifications)
                
            # Execute JavaScript if present to get dynamic content
            js_context = self._execute_page_javascript(soup)
            
            # Render HTML to markdown with JavaScript context for Web Components
            markdown_content = self.renderer.render_to_markdown(soup, css_rules, js_context)
