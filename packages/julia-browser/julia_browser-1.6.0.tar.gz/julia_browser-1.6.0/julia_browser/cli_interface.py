...
class CLIBrowser:
    ...
    def _execute_real_button_javascript(self, element):
        ...
        try:
            ...
            # Build JavaScript to simulate button click
            js_code = f"""
            // Simulate button click event
            var button = document.querySelector('button[onclick*="{onclick[:20]}"]') || 
                        document.querySelector('button[data-action="{data_action}"]') ||
            ...
            // Try to trigger any data-action handlers
            if (`{data_action}`) {{
                // Look for common patterns
                if (`{data_action}`.includes('toggle')) {{
                ...
                if (`{data_action}`.includes('modal')) {{
                    // Handle modal actions
                    var modal = document.querySelector('.modal') || document.querySelector('[role="dialog"]');
                    if (modal) {{
                        modal.style.display = 'block';
                        modal.classList.add('show');
                    }}
                }}
                if (`{data_action}`.includes('menu')) {{
                    // Handle menu actions
                    var menu = document.querySelector('.menu') || document.querySelector('.dropdown');
                    if (menu) {{
            ...
            // Check for any navigation changes
