...
# 定义命令行参数 [^2]
option_list <- list(
  make_option(c("--sc_data"), type="character"),
  make_option(c("--sc_meta"), type="character"),
  make_option(c("--st_data"), type="character"),
  make_option(c("--st_meta"), type="character"),
...
# 数据读取和处理（使用绝对路径）[^4]
sc_data <- read.csv(normalizePath(args$sc_data), row.names = 1, check.names = FALSE)
sc_meta <- read.csv(normalizePath(args$sc_meta), row.names = 1, check.names = FALSE)
st_data <- read.csv(normalizePath(args$st_data), row.names = 1, check.names = FALSE)
st_meta <- read.csv(normalizePath(args$st_meta), row.names = 1, check.names = FALSE)

# Seurat工作流 [^5]
sobj1 <- CreateSeuratObject(counts = sc_data, meta.data = sc_meta)
sobj1$batch <- 'sc'
sobj2 <- CreateSeuratObject(counts = st_data, meta.data = st_meta)
sobj2$batch <- 'st'
sobj_list <- list(sc=sobj1, st=sobj2)

for (i in 1:length(sobj_list)) {{
  sobj_list[[i]] <- NormalizeData(sobj_list[[i]], verbose = FALSE)
  sobj_list[[i]] <- FindVariableFeatures(
    sobj_list[[i]], 
...
features <- SelectIntegrationFeatures(object.list = sobj_list, nfeatures = 4000)
sobj.anchors <- FindIntegrationAnchors(
  object.list = sobj_list,
  anchor.features = features,
  dims = 1:30
)
sobj <- IntegrateData(
  anchorset = sobj.anchors,
...
integrated <- as.matrix(sobj@assays$integrated@data)
new_sc <- integrated[, rownames(sobj@meta.data[sobj$batch=='sc',])]
new_st <- integrated[, rownames(sobj@meta.data[sobj$batch=='st',])]

write.csv(new_sc, file.path(normalizePath(args$output_dir), "CCA_sc.csv"), 
          row.names = TRUE, fileEncoding = "UTF-8")
write.csv(new_st, file.path(normalizePath(args$output_dir), "CCA_st.csv"), 
          row.names = TRUE, fileEncoding = "UTF-8")
"""
    ...
    cmd = [
        ...
        "--sc_meta", sc_meta_path,
        "--st_data", st_data_path,
        "--st_meta", st_meta_path,
        "--output_dir", output_dir
    ]
    subprocess.run(cmd, check=True)  
    # 清理并读取结果 [^8]
    os.unlink(tmp_script_path)
    sc_output = os.path.join(output_dir, "CCA_sc.csv")
    st_output = os.path.join(output_dir, "CCA_st.csv")


    print('Loading data...')
    sc_meta = pd.read_csv(sc_meta_path, index_col=0)
    sc_data = pd.read_csv(sc_output, index_col=0)
    st_meta = pd.read_csv(st_meta_path, index_col=0)
    st_data = pd.read_csv(st_output, index_col=0)
...
def preporcess(
    ...
    """
    # 1. 清理 NaN 基因名
    valid_genes = ~pd.Series(st_adata.var_names).isna()
    st_adata = st_adata[:, valid_genes].copy()
    valid_genes = ~pd.Series(sc_adata.var_names).isna()
    sc_adata = sc_adata[:, valid_genes].copy()
    # 2. 过滤零表达基因（Seurat v3 要求非零表达）
    sc_nonzero = np.array(sc_adata.X.sum(axis=0) > 0).flatten()
    st_nonzero = np.array(st_adata.X.sum(axis=0) > 0).flatten()
    sc_adata = sc_adata[:, sc_nonzero].copy()
    st_adata = st_adata[:, st_nonzero].copy()
    ...
    if normalize:
        ...
        sc_adata.X = np.clip(sc_adata.X, 0, None) 
        sc.pp.log1p(sc_adata)
        # st_adata
        sc.pp.normalize_total(st_adata, target_sum=1e4)
        st_adata.X = np.clip(st_adata.X, 0, None) 
