...
def preporcess(
    ...
    counts = sc.get.obs_df(st_adata, keys=list(st_adata.var_names), use_raw=True)
    st_adata.obs['total_counts']=counts.T.sum(axis=0)
    total_counts = sc.get.obs_df(st_adata, keys=["total_counts"])
    norm_expr = NaiveDE.stabilize(counts.T).T
    resid_expr = NaiveDE.regress_out(total_counts, norm_expr.T, "np.log(total_counts)").T
    results = SpatialDE.run(st_adata.obsm["spatial"], resid_expr)
    top2000 = results.sort_values("qval").head(2000)[["g", "l", "qval"]]
    st_hvg = pd.Index(top2000.g)
...
def train(
    ...
    # 新增调度器：总 epoch 数设为 100，最小学习率设为 1e-5
    scheduler = CosineAnnealingLR(
        optimizer,
        T_max=100,    # 对应总训练 epoch 数（需与后续训练循环中的 epoch_num 一致）
        eta_min=1e-5  # 初始学习率的 1/100，避免震荡
        )
