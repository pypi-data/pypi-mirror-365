...
def load_data(
    ...
    common_genes = list(set(sc_adata.var_names).intersection(st_adata.var_names))
    sc_matrix = sc_adata[:, common_genes].X  
    st_matrix = st_adata[:, common_genes].X

    corrected, genes = scanorama.correct([sc_matrix,st_matrix], [common_genes,common_genes])
    corrected_matrix = corrected[0].toarray()
    corrected_sc = pd.DataFrame(
        corrected_matrix,
        index=sc_adata.obs.index,
        columns=genes)
    corrected_sc=corrected_sc.T
    cleaned_sc = corrected_sc.dropna()

    corrected_matrix = corrected[1].toarray()
    corrected_st = pd.DataFrame(
        corrected_matrix,
        index=st_adata.obs.index,
        columns=genes)
    corrected_st=corrected_st.T
    cleaned_st = corrected_st.dropna()

    sc_adata2 = ad.AnnData(cleaned_sc.T)
    sc_adata2.obs = sc_meta

    st_adata2 = ad.AnnData(cleaned_st.T)
    st_adata2.obs = st_meta
    st_adata2.obsm['spatial'] = np.array(st_meta[spatial_key])


    return sc_adata2, st_adata2
