...
from flexscspace.v2.models import *
from flexscspace.v2.utils import *
...
def construct_pseudo_space(
    ...
    mlp = init_model(net=INR_Encoder(input_size=dim, common_size=common_size, hidden_size=hidden_size, coord_dim=2))  # coord_dim为隐式坐标维度
    ...
    dummy_coords = torch.zeros(len(sc_tca), 2)
    predict = encoder(make_cuda(torch.tensor(sc_tca).to(torch.float32)), make_cuda(torch.tensor(dummy_coords.to(torch.float32))))
