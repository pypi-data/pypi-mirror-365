...
class MyDataset(Dataset):  # 需要继承data.Dataset
    def __init__(self, data, label):
        ...
        self.features = data[0]  # tca_features
        self.coords = data[1]
        self.label = label

    def __getitem__(self, idx):
        # TODO
        return (self.features[idx], self.coords[idx]), self.label[idx]
...
def get_data_loader(st_adata, batch_size=16):
    tca_features = torch.tensor(st_adata.obsm['TCA']).float()
    spatial_coords = torch.tensor(st_adata.obsm['spatial']).float()
    # 坐标归一化到[-1,1][^5]
    spatial_coords = (spatial_coords - spatial_coords.min(0)[0]) / (spatial_coords.max(0)[0] - spatial_coords.min(0)[0]) * 2 - 1
    
    dataset = MyDataset(data=(tca_features, spatial_coords), label=spatial_coords)  # label为坐标自身[^6]
    return DataLoader(dataset, batch_size=batch_size, shuffle=True)



def train(encoder, data_loader, losses, lr, epoch_num, log_epoch):
    optimizer = torch.optim.Adam(encoder.parameters(), lr=lr)
    criterion = nn.MSELoss()
    
    for epoch in tqdm(range(epoch_num)):
        batch_loss = []
        for step, (inputs, labels) in enumerate(data_loader):  # 解包为(inputs, labels)
            features, coords = inputs  # 分离特征和坐标 [^7]
            features = make_cuda(features)
            coords = make_cuda(coords)
            labels = make_cuda(labels)
            
            optimizer.zero_grad()
            pred = encoder(features, coords)  # 双输入模型 [^8]
        ...
        if (epoch + 1) % log_epoch == 0:
            ...
            print(f"Epoch [{epoch+1}/{epoch_num}]: Loss={np.mean(batch_loss):.4f}")
    return encoder
