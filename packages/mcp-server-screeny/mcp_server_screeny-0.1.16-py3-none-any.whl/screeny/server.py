...
def get_current_approved_windows() -> Dict[str, Dict[str, Any]]:
    ...
    logger.info("Checking approved windows and updating titles...")
    approved = load_approved_windows()
    if not approved:
        logger.info("No approved windows found")
        return {}

    current_windows = get_all_windows()
    current_window_map = {w.id: w for w in current_windows}
    
    logger.info(f"Found {len(current_windows)} current windows, {len(approved)} approved windows")

    still_open_approved = {}
    titles_updated = False

    for window_id, window_info in approved.items():
        logger.info(f"Processing window {window_id}: approved={window_info.get('approved')}, still_open={window_id in current_window_map}")
        ...
        if window_info.get('approved') and window_id in current_window_map:
            ...
            logger.info(f"Window {window_id} title check: stored='{old_title}' vs current='{new_title}'")
            
            if old_title != new_title:
                logger.info(f"🔄 UPDATING title for window {window_id}: '{old_title}' -> '{new_title}'")
                window_info['title'] = new_title
                titles_updated = True
            else:
                logger.info(f"✅ Title unchanged for window {window_id}")

            still_open_approved[window_id] = window_info
            
    # ALWAYS save to ensure titles are persistent
    logger.info(f"Saving approved windows - removed: {len(approved) - len(still_open_approved)}, titles_updated: {titles_updated}")
    ...
    if len(still_open_approved) < len(approved):
        ...
        logger.info(f"Removed {removed_count} closed windows from approved list")

    if titles_updated:
        logger.info("Updated window titles for approved windows")
    else:
        logger.info("No title updates needed")
...
def take_screenshot(request: ScreenshotRequest) -> list[ImageContent | TextContent]:
    ...
    try:
        ...
        # Target raw size = 1MB / 1.50 (50% overhead for base64 encoding and safety margin)
        mcp_limit_in_bytes = 1024 * 1024
        target_raw_size = int(mcp_limit_in_bytes / 1.50)
