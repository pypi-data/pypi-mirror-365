...
class PowerManager:
    ...
    def _initiate_shutdown(self) -> None:
        ...
        try:
            ...
            shutdown_methods = [
                # Method 1: Direct poweroff (if running as root/service)
                ["shutdown", "-P", "now"],
                # Method 2: Systemctl (modern systemd approach)
                ["systemctl", "poweroff"],
                # Method 3: sudo poweroff without password (if NOPASSWD configured)
                ["sudo", "shutdown", "-P", "now"],
                # Method 4: alternative poweroff command
                ["sudo", "poweroff"]
            ]
