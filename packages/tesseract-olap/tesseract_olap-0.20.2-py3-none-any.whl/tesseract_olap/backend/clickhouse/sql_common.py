...
from tesseract_olap.schema import InlineTable, Measure

from .dialect import ArrayElement, ClickhouseDataType, Power, Quantile, TopK
...
def debug_sql(query: QueryBuilder, meta: ParamManager):
    if meta.tables:
        tables = ", ".join(
            f"{table.name} AS ({inlinetable_to_sql(table)})" for table in meta.tables
        )
        print("WITH", tables, query)
    else:
        print(query)
