...
class Board:
    ...
    def wall_blocks(self, a: Tuple[int,int], b: Tuple[int,int]) -> bool:
        ...
        # 1) must be orthogonal neighbors
        if abs(dx) + abs(dy) != 1:
            return False

        # 2) both on the board
        if not (0 <= x1 < self.size and 0 <= y1 < self.size
                and 0 <= x2 < self.size and 0 <= y2 < self.size):
            return False

        # 3) vertical move? check horiz_walls[x1, min(y1,y2)]
        if dy != 0:
            xi = x1
            yi = min(y1, y2)
        ...
        # 4) horizontal move? check vert_walls[min(x1,x2), y1]
        xi = min(x1, x2)
        yi = y1
...
#        if abs(dx) + abs(dy) != 1:
#            return False  # Only adjacent squares

#        # Check bounds of positions
#        if not all(0 <= x < self.size for x in [ax, bx]) or not all(0 <= y < self.size for y in [ay, by]):
#            return False

#        if dx == 1:  # moving right
#            if ax >= self.size - 1 or ay >= self.size - 1:
#                return False
#            return self.vert_walls[ax, ay]
#        if dx == -1:  # moving left
#            if bx >= self.size - 1 or by >= self.size - 1:
#                return False
#            return self.vert_walls[bx, by]
#        if dy == 1:  # moving up
#            if ay >= self.size - 1 or ax >= self.size - 1:
#                return False
#            return self.horiz_walls[ax, ay]
#        if dy == -1:  # moving down
#            if ay <= 0 or ax >= self.size - 1:  # ay <= 0 since we check ay-1
#                return False
#            return self.horiz_walls[ax, ay - 1]  # Check wall above destination
#
#        return False
