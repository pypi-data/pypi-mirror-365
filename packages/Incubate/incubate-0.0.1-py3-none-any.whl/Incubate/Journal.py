    ...
    def Save(Source:str , Config:dict) -> None :
        try :
            Path = Common.Os.path.realpath('/'.join([
                ...
                Config.get('Path') ,
                Config.get('Type') ,
                Common.Function.Time(Method = '%Y-%m-%d') ,
                Common.Function.Time(Method = '%H') ,
            ]));
            if Common.Os.path.exists(Path) is False : Common.Os.makedirs(Path , exist_ok = True);
            Route = Common.Os.path.normpath(f'{Path}/{Common.Function.Time(Method = "%M")}.00.log');
            if Common.Os.path.exists(Route) is True and Common.Os.stat(Route).st_size > Config.get('Size') :
                Common.Shutil.copyfile(Route , f'{Path}/{Common.Function.Time(Method = "%M.%S")}.log');
                Common.Os.remove(Route);
            Log = '\n'.join([
                f'|## {"##" * 50}' ,
            ...
            with open(Route , 'a' , encoding = 'utf-8') as File : Common.Function.Log(Log , File = File);
