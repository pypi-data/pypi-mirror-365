  ...
  <script>
    window.onload = function() {
      if(window.innerHeight < document.body.scrollHeight) {
        let scrollInterval, isScrollingDown = true;

        function startAutoScroll() {
          const speed = 20; // pixels per scroll
          const delay = 100; // milliseconds between scrolls

          scrollInterval = setInterval(() => {
            // Check if we've reached the bottom
            if (isScrollingDown &&
                window.innerHeight + window.pageYOffset + 1 >= document.documentElement.scrollHeight) {
            ...
            window.scrollBy({top: isScrollingDown ? speed : -speed, left:0, behavior:'smooth'});
          }, delay);
        }
