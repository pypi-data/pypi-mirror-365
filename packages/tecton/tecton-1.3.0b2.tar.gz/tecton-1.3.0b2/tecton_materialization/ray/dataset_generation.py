...
def run_dataset_generation(
    ...
    if not use_legacy_spine_split(dataset_generation_params):
        ...
        fdw_list = _extract_fdw_list(materialization_task_params)
        for fdw in fdw_list:
            if fdw.has_iceberg_offline_store:
                error = "Iceberg offline store is not supported for dataset generation with legacy spine splitting. Set 'use_legacy_spine_split' to 'false' in `extra_config`."
...
def use_legacy_spine_split(dataset_generation_params: DatasetGenerationParameters) -> bool:
    ...
    use_spine_split = str(dataset_generation_params.extra_config.get("use_legacy_spine_split", "false")).lower()
...
def run_dataset_generation_with_qt_parallelization(
    ...
    # Ensure QT parallelization is enabled
