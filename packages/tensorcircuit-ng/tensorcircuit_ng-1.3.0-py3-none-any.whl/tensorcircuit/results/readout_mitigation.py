...
class ReadoutMit:
    ...
    def _form_cals(self, qubits):  # type: ignore
        """
        Form calibration data for specified qubits.
        
        :param qubits: Qubit indices to form calibration data for
        :return: Flattened calibration data array
        """
    ...
    def local_miti_readout_circ_by_mask(self, bsl: List[str]) -> List[Circuit]:
        """
        Generate local mitigation readout circuits based on bitstring list.
        
        :param bsl: List of bitstrings to generate circuits for
        :type bsl: List[str]
    ...
    def _apply_correction(  # type: ignore
        ...
        """
        Apply correction using M3 methods.
        
        :param counts: Input counts dictionary
        :param qubits: Qubit indices
        :param distance: Maximum distance for calibration matrix reduction
        :param method: Mitigation method to use
        :param max_iter: Maximum iterations for iterative methods
        :param tol: Tolerance for iterative methods
        :param return_mitigation_overhead: Whether to return mitigation overhead
        :param details: Whether to return detailed information
        :return: Mitigated quasi-probabilities
        """
        if self.local is False:
    ...
    def reduced_cal_matrix(self, counts, qubits, distance=None):  # type: ignore
        """
        Generate a reduced calibration matrix.
        
        :param counts: Input counts dictionary
        :param qubits: Qubit indices
        :param distance: Maximum distance for calibration matrix reduction
        :return: Reduced calibration matrix and counts
        """
        counts = dict(counts)
        # If distance is None, then assume max distance.
        num_bits = len(qubits)
    ...
    def _direct_solver(  # type: ignore
        ...
        """
        Direct solver for readout mitigation.
        
        :param counts: Input counts dictionary
        :param qubits: Qubit indices
        :param distance: Maximum distance for calibration matrix reduction
        :param return_mitigation_overhead: Whether to return mitigation overhead
        :return: Mitigated quasi-probabilities, column norms, and gamma factor
        """
        cals = self._form_cals(qubits)
        num_bits = len(qubits)
    ...
    def _matvec_solver(  # type: ignore
        ...
        """
        Matrix-vector solver for readout mitigation using iterative methods.
        ...
        :param details: Whether to return detailed information
        :param callback: Callback function for iterations
        :param return_mitigation_overhead: Whether to return mitigation overhead
        :return: Mitigated quasi-probabilities and gamma factor
        """
        cals = self._form_cals(qubits)
