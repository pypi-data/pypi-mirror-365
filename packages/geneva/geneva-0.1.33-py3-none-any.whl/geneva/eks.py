...
def build_api_client(
    ...
    if config_method == K8sConfigMethod.EKS_AUTH:
        ca_data, endpoint, token = _eks_auth(cluster_name, region, role_name)
        cafile = _write_cafile(ca_data)
        api_client_obj = _api_client(endpoint, token, cafile.name)
        _KUBE_API_CLIENT = api_client_obj
        return api_client_obj

    # default config method (non-EKS)
    return None
