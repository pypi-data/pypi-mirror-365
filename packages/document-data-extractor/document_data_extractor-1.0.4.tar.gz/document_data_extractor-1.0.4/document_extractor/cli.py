"""Command-line interface for document-data-extractor."""
...
def print_version():
    ...
    print(f"document-data-extractor v{__version__}")
...
Examples:
  ...
  document-data-extractor document.pdf --output csv  # Extract tables as CSV

  # Use specific model for cloud processing
  document-data-extractor document.pdf --api-key YOUR_KEY --model gemini
  document-data-extractor document.pdf --model openapi --output json
  ...
  document-data-extractor file1.pdf file2.docx file3.xlsx --output markdown

  # Extract specific fields using Ollama (local) or cloud
  document-data-extractor invoice.pdf --output json --extract-fields invoice_number total_amount vendor_name

  # Extract using JSON schema (local Ollama or cloud)
  document-data-extractor document.pdf --output json --json-schema schema.json

  # Save output to file
  document-data-extractor document.pdf --output-file output.md
  ...
  document-data-extractor --list-formats

  # Show version
  document-data-extractor --version
        """
    )
