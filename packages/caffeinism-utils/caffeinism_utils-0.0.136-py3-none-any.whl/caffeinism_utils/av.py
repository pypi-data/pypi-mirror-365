...
class AlphaMerger(BaseAlphaMerger):
    def __init__(self):
        self.alpha_mergers = defaultdict(_AlphaMerger)

    def push_image(self, frame: av.VideoFrame):
        self.alpha_mergers[(frame.width, frame.height, frame.format.name)].push_image(
            frame
        )

    def push_alpha(self, frame: av.VideoFrame):
        self.alpha_mergers[(frame.width, frame.height, frame.format.name)].push_alpha(
            frame
        )

    def pull(self) -> av.VideoFrame:
        for key in list(self.alpha_mergers):
            ret = self.alpha_mergers[key].pull()
            if ret is not None:
                return ret

            del self.alpha_mergers[ret]
