...
class PyAVWriter(PyAVInterface):
    ...
    def create_codec_context(self, stream: av.VideoStream):
        cc = stream.codec.create()
        cc.width = stream.width
        cc.height = stream.height
        cc.pix_fmt = stream.pix_fmt
        cc.bit_rate = stream.bit_rate
        cc.time_base = stream.time_base
        cc.options = stream.options
        return cc

    def _encode_video_frame(self, stream: av.VideoStream, frame: av.VideoFrame):
        cc = self._codex_contexts.get(stream)
        if cc is None:
            stream.width = frame.width
            stream.height = frame.height
            cc = self.create_codec_context(stream)
            self._codex_contexts[stream] = cc

        if (stream.width, stream.height) != (frame.width, frame.height):
