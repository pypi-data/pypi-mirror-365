...
class SimpleMultiHourBiddingStrategy(SatelliteModel):
    def __init__(
        ...
        **kwargs,
        ...
        -----
            ...
            electricity_price_forecast: 2D numpy array of price forecast information,
                each being used (in parallel) to determine the bid curves. If
            ...
            bid_margin: [0, ...) Amount of margin as a fraction to bid over the
        ...
        if not isinstance(demands, pd.DataFrame):
            demands = pd.read_csv(
                demands, index_col=0, parse_dates=True, date_parser=lambda x: pd.to_datetime(x, utc=True)
            )
        ...
        assert max(parse_flex_amount(d) for d in demands.columns) + rolling_horizon_step <= self.horizon_size
    ...
    def determine_bids(self) -> pd.DataFrame:
        ...
        Returns:
        --------
            bids: A collection of bids covering the next bid window.
        """
    ...
    def _get_horizon(self, length: int = None) -> pd.Index:
        ...
        Returns:
        --------
            horizon: Pandas Index object of the selected horizon values.
        """
        if length is None:
    ...
    def _make_bid(self, time_idx: int, flexibility: int, prices_forecast: pd.DataFrame) -> tuple[pd.Timestamp, float]:
        ...
        Returns:
        --------
            bid_time: expected best timestamp for the bid, always within current
                bidding window.
            bid_price: price for the bid.
        """
