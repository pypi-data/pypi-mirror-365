...
def open_excel_with_columns(path, password, address_col=None, key_col=None):
    ...
    try:
        ...
        print(f"成功转换 {len(result_list)} 条记录")
        return result_list

    except msoffcrypto.exceptions.DecryptionError:
        print(f"错误: 密码错误或文件未加密")
        return None
    except IndexError as e:
        print(f"错误: 列索引超出范围 - {str(e)}")
        return None
    except Exception as e:
        print(f"错误: 读取文件时出现问题 - {str(e)}")
        return None


def open_excel_single_text(path, password):
    """
    ...
    try:
        ...
        if not df.empty and len(df.columns) > 0:
            text_content = str(df.iloc[0, 0]) if pd.notna(df.iloc[0, 0]) else ''
            print(f"读取到文本长度: {len(text_content)} 字符")
            return text_content
        else:
            print("错误: Excel文件为空")
            return None
