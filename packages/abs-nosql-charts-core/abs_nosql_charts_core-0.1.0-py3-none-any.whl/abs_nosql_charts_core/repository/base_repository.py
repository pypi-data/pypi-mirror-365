...
class BaseChartRepository(BaseRepository):
    ...
    async def get_chart_data_pipeline(self, chart_schema: BaseChartSchema) -> Any:
        ...
        if chart_type_str in ["pie", "donut", "gauge"]:
            unwind_pipeline, aggregation, post_group_project = self.build_pie_chart_aggregation(chart_schema.data)
        elif chart_type_str == "number":
            unwind_pipeline, aggregation, post_group_project = self.build_number_chart_aggregation(chart_schema.data)
        elif chart_type_str in ["line", "area"]:
            unwind_pipeline, aggregation, post_group_project = self.build_line_chart_aggregation(chart_schema.data)
        elif chart_type_str in ["bar", "stacked_bar", "stacked_bar_100"]:
            unwind_pipeline, aggregation, post_group_project = self.build_bar_chart_aggregation(chart_schema.data)
        elif chart_type_str in ["column", "stacked_column", "stacked_column_100"]:
            unwind_pipeline, aggregation, post_group_project = self.build_column_chart_aggregation(chart_schema.data)
        elif chart_type_str == "scatter":
            unwind_pipeline, aggregation, post_group_project = self.build_scatter_chart_aggregation(chart_schema.data)
        elif chart_type_str == "candlestick":
            unwind_pipeline, aggregation, post_group_project = self.build_candlestick_chart_aggregation(chart_schema.data)
        elif chart_type_str == "table":
            unwind_pipeline, aggregation, post_group_project = self.build_table_chart_aggregation(chart_schema.data)
        else:
            unwind_pipeline, aggregation = self.build_number_chart_aggregation(chart_schema.data)
            post_group_project = None
        pipeline.extend(unwind_pipeline)
        
        aggregation_stages = None
        if aggregation is not None:
            aggregation_stages = self._build_aggregation_stage(aggregation, chart_type_str)
    ...
    def _build_label_stage(self, aggregation: ChartAggregation) -> dict | None:
        ...
        if aggregation.binning_specs and len(aggregation.binning_specs) > 0:
            ...
            if isinstance(bin_value, DateValueOptions):
                granularity = bin_value.granularity or "month"
                ...
                add_fields["label"] = {"$dateToString": {"format": format_str, "date": f"$_id.{alias}"}}
            else:
    ...
    def _build_aggregation_stage(self, aggregation: ChartAggregation, widget_type: str) -> List[Dict[str, Any]]:
        """Build MongoDB aggregation stage from ChartAggregation"""

        pipeline = []
        
        if aggregation.binning_specs:
        ...
        if aggregation.aggregation_fields:
            project_fields = {"_id": 1, "count": 1}
            for agg in aggregation.aggregation_fields:
                ...
                if agg_type == "distinct":
                    project_fields[sanitized_alias] = {
                        "$cond": {
                            "if": {"$isArray": f"${sanitized_alias}"},
                            "then": {"field": field_name, "type": agg_type, "value": {"$size": f"${sanitized_alias}"}},
                            "else": {"field": field_name, "type": agg_type, "value": 0}
                        }
                    }
                else:
                    project_fields[sanitized_alias] = {
        ...
        label_stage = self._build_label_stage(aggregation) if widget_type != "table" else None
        if label_stage:
        ...
        if aggregation.top_n:
            pipeline.extend(self.build_top_n_pipeline(aggregation.top_n))
                ...
                {"$facet": {
                    ...
                    "others": [
                        ...
                        {"$group": {
                            "_id": "Others",
                            # "label": "Others",
                            f"{top_n.field}": {"$sum": f"${top_n.field}"},
                            # "docs": {"$push": "$$ROOT"}
                        }}
                    ]
                }},
    ...
    def build_unwind_and_group_path_old(self, field_path: str, type_path: str):
        """
        Given a field path (e.g., 'data.details.name') and a type path (e.g., 'array.object.string'),
        returns a list of $unwind stages and the final group_by path.
        """
        path_parts = field_path.split('.')
        type_parts = type_path.split('.')
        pipeline = []
        current_path = []
        for part, typ in zip(path_parts, type_parts):
            current_path.append(part)
            if typ == 'array':
                pipeline.append({"$unwind": f"${'.'.join(current_path)}"})
        group_by_path = '.'.join(current_path)
        return pipeline, group_by_path
    

    def build_unwind_and_group_path(self, field_path: str, type_path: str):
        """
    ...
    def build_number_chart_aggregation(self, data: NumberChartDataSchema ):
        ...
        """
        number = data.number
        type_path = getattr(number, 'type_path', None)
        if not type_path:
            type_path = number.field_type or 'string'
            if hasattr(number, 'element_type') and number.element_type:
                type_path = f"{number.field_type}.{number.element_type}"
        pipeline, group_by_field = self.build_unwind_and_group_path(number.field, type_path)

        agg_spec = AggregationSpec(
            field=number.field, 
            type=AggregationType(number.aggregation),
            alias=getattr(number, "label", number.field)
        )
        ...
        post_group_project = None
        return pipeline, chart_agg, post_group_project

    def build_pie_chart_aggregation(self, data):
        """
        ...
        type_path = getattr(group_by, 'type_path', None)
        if not type_path:
            type_path = group_by.field_type or 'string'
            if hasattr(group_by, 'element_type') and group_by.element_type:
                type_path = f"{group_by.field_type}.{group_by.element_type}"
        pipeline, group_by_field = self.build_unwind_and_group_path(group_by.field, type_path)

        binning_specs = [group_by.binning] if getattr(group_by, 'binning', None) and getattr(group_by.binning, 'enabled', False) else None
        agg_spec = AggregationSpec(
            field=value.field,
            type=AggregationType(value.aggregation),
            alias=getattr(value, "label", value.field)
        )
        sort_by = [{ "field": agg_spec.alias, "direction": group_by.sort_by.value or SortDirection.DESC}]  # or -1 for descending
    ...
    def build_line_chart_aggregation(self, data):
        """
        ...
        type_path = getattr(x, 'type_path', None) or x.field_type or 'string'
        pipeline, group_by_field = self.build_unwind_and_group_path(x.field, type_path)
        ...
        if group_by:
            group_by_type_path = getattr(group_by, 'type_path', None) or group_by.field_type or 'string'
            group_by_pipeline, group_by_group_by_field = self.build_unwind_and_group_path(group_by.field, group_by_type_path)
        ...
        if x.field_type in ['date', 'datetime', 'timestamp',"int","number"]:
            ...
            for agg_spec in agg_specs:
                sort_by.append({
                    ...
                    "direction": x.sort_by.value or SortDirection.ASC
                })
    ...
    def build_bar_chart_aggregation(self, data):
        """
        ...
        type_path = getattr(y, 'type_path', None) or y.field_type or 'string'
        pipeline, group_by_field = self.build_unwind_and_group_path(y.field, type_path)
        group_by_list = [group_by_field]
        ...
        if group_by:
            group_by_type_path = getattr(group_by, 'field_type', None) or 'string'
            group_by_pipeline, group_by_group_by_field = self.build_unwind_and_group_path(group_by.field, group_by_type_path)
            pipeline += group_by_pipeline
            group_by_list.append(group_by_group_by_field)
        ...
        if y.field_type in ['date', 'datetime', 'timestamp', "int", "number"]:
            sort_by = [{"field": "label", "direction": y.sort_by.value or SortDirection.ASC}]
        elif getattr(y, 'sort_by', None):
            sort_by = []
            for agg_spec in agg_specs:
                sort_by.append({
                    "field": agg_spec.alias,
                    "direction": y.sort_by.value or SortDirection.ASC
                })
    ...
    def build_column_chart_aggregation(self, data):
        """
    ...
    def build_scatter_chart_aggregation(self, data):
        """
        ...
        type_path = getattr(x, 'type_path', None) or x.field_type or 'string'
        pipeline, x_group_by_field = self.build_unwind_and_group_path(x.field, type_path)
        
        y_type_path = getattr(y, 'type_path', None) or y.field_type or 'string'
        y_pipeline, y_group_by_field = self.build_unwind_and_group_path(y.field, y_type_path)
        pipeline += y_pipeline
        ...
        return pipeline, None, post_group_project

    def build_candlestick_chart_aggregation(self, data):
        """
        ...
        type_path = getattr(x, 'type_path', None) or x_field_type or 'string'
        pipeline, group_by_field = self.build_unwind_and_group_path(x.field, type_path)
        group_by = [group_by_field]
    ...
    def build_table_chart_aggregation(self, data):
        """
        ...
        project_fields = {}
        for group in groups:
            type_path = getattr(group, 'type_path', None) or group.field_type or 'string'
            unwind_stages, group_by_field = self.build_unwind_and_group_path(group.field, type_path)
            project_fields[group.label or group.field] = { "field": group_by_field, "type": type_path }
        ...
        agg_specs = []
        for value in values:
            agg_type = value.aggregation or 'sum'
            agg_specs.append(
                AggregationSpec(
                    field=value.field,
                    type=AggregationType(agg_type),
    ...
    def _build_table_groups_project(self, project_fields, agg_specs, limit):
        ...
        for label, field_info in project_fields.items():
            ...
            if 'binning' in field_info:
                ...
                if hasattr(bin_value, 'granularity') and bin_value.granularity:
                    granularity = bin_value.granularity
                    timezone = getattr(bin_value, 'timezone', 'UTC')
                    granularity_to_format = {
                        "year": "%Y",
                        "month": "%Y-%m",
                        "week": "%G-W%V",
