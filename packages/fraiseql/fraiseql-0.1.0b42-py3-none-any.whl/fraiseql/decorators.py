...
def query(fn: F | None = None) -> F | Callable[[F], F]:
    ...
    def decorator(func: F) -> F:
        ...
        # Wrap with JSON passthrough capability for production mode
        from fraiseql.gql.json_executor import create_json_passthrough_resolver
        import inspect
        
        # Get the return type annotation for proper typing
        sig = inspect.signature(func)
        return_type = sig.return_annotation
        
        # Create a wrapped resolver that can use JSON passthrough
        wrapped_func = create_json_passthrough_resolver(
            func,
            field_name=func.__name__,
            return_type=return_type,
            arg_name_mapping=None  # We don't need mapping for @query functions
        )
        ...
        wrapped_func.__wrapped__ = func  # Store original for introspection
        
        registry.register_query(wrapped_func)
