...
class JSONPassthrough:
    ...
    __slots__ = ('_data', '_type_name', '_type_hint', '_wrapped_cache', '_config')
    ...
    def __init__(
        ...
        """
        self._data = data
        self._type_name = type_name or data.get("__typename", "Unknown")
    ...
    def __getattr__(self, name: str) -> Any:
                ...
                # Handle lists
                    ...
                    if value and isinstance(value[0], dict):
                        ...
                        wrapped_list = [
                            JSONPassthrough(
                                item, 
                                item.get("__typename", self._get_nested_type_name(item_type_hint, item)),
                                item_type_hint
                            )
                            for item in value
                        ]
        ...
        available = sorted(self._data.keys())
        raise AttributeError(
            f"'{self._type_name}' object has no attribute '{name}'. "
    ...
    def __repr__(self):
        ...
        return f"JSONPassthrough({self._type_name}, fields={list(self._data.keys())})"
