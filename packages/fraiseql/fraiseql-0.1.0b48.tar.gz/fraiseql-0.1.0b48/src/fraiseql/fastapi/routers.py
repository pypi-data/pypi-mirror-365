...
from fraiseql.fastapi.custom_response import create_json_response
            ...
            if result.data and isinstance(result.data, dict):
                for key, value in result.data.items():
                    if isinstance(value, RawJSONResult):
                        ...
                        return create_json_response(value)
...
def create_production_router(
    ...
    async def graphql_endpoint(
        ...
        try:
            ...
            # Enable JSON passthrough based on configuration
            if config.json_passthrough_enabled and config.json_passthrough_in_production:
                context["json_passthrough"] = True
            context["mode"] = "production"
            
            # Try raw JSON execution for simple queries
            from fraiseql.fastapi.raw_json_handler import try_raw_json_execution
            
            raw_result = await try_raw_json_execution(
                schema,
                request.query,
                request.variables,
                context,
            ...
            if raw_result is not None:
                ...
                return create_json_response(raw_result)
