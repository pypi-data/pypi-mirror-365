...
class FraiseQLRepository:
    ...
    async def find(self, view_name: str, **kwargs) -> Union[list[dict[str, Any]], Any, RawJSONResult]:
        """Find records with mode-appropriate return type.
        
        In production mode, always uses raw JSON for consistent field mapping
        and maximum performance.
        """
        # Production mode: Always use raw JSON for field mapping and performance
        if self.mode == "production":
            # Get GraphQL info from context if available
            info = self.context.get("graphql_info")
            # Use the query field name from context, or default to view name
            field_name = self.context.get("graphql_field_name", view_name)
            
            # Always use raw JSON in production for consistent field mapping
            return await self.find_raw_json(view_name, field_name, info, **kwargs)
    ...
    async def find_one(self, view_name: str, **kwargs) -> Union[Optional[dict[str, Any]], Any, RawJSONResult]:
        ...
        if self.mode == "production":
            ...
            # Always use raw JSON in production for consistent field mapping
            return await self.find_one_raw_json(view_name, field_name, info, **kwargs)
    ...
    async def find_raw_json(
        ...
        bypassing all Python object creation and dict parsing.
    ...
    def _build_find_query(
        ...
        **kwargs,
        ...
        if raw_json:
            # For raw JSON without field paths, select the data column as JSON text
    ...
    def _build_find_one_query(
        ...
        **kwargs,
        ...
        return self._build_find_query(
            view_name, raw_json=raw_json, field_paths=field_paths, info=info, **kwargs
        )
