...
class QueryTypeBuilder:
    ...
    def _create_gql_resolver(self, fn, arg_name_mapping: dict[str, str] | None = None, field_name: str | None = None):
        ...
        """
        # Import JSON passthrough utilities
        from fraiseql.gql.json_executor import (
            execute_json_query,
            execute_sync_json_query,
            should_use_json_passthrough,
        )
        from fraiseql.gql.raw_json_wrapper import create_raw_json_resolver
        
        # Get the actual field name (camelCase if configured)
        config = SchemaConfig.get_instance()
        graphql_field_name = snake_to_camel(field_name) if config.camel_case_fields and field_name else field_name
        
        # Use raw JSON wrapper for production optimization
        return create_raw_json_resolver(fn, graphql_field_name or field_name or fn.__name__, arg_name_mapping)
