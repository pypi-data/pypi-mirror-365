...
class FraiseQLRepository:
    ...
    async def find_raw_json(self, view_name: str, field_name: str, **kwargs) -> RawJSONResult:
        ...
        query = self._build_find_query(view_name, raw_json=True, **kwargs)
    ...
    async def find_one_raw_json(self, view_name: str, field_name: str, **kwargs) -> RawJSONResult:
        ...
        """
        # Build query with raw JSON output
        query = self._build_find_one_query(view_name, raw_json=True, **kwargs)
    ...
    def _build_find_query(self, view_name: str, raw_json: bool = False, **kwargs) -> DatabaseQuery:
        ...
        if raw_json:
            # For raw JSON, select the data column as JSON text
    ...
    def _build_find_one_query(
        self, view_name: str, raw_json: bool = False, **kwargs
    ) -> DatabaseQuery:
        """Build a SELECT query for finding a single record."""
        # Force limit=1 for find_one
        kwargs["limit"] = 1
        return self._build_find_query(view_name, raw_json=raw_json, **kwargs)
