...
def start_tcp_dummy(port, symmetric_key, response_plaintext):
    ...
    def tcp_handler():
        ...
        try:
            ...
            encrypted_response = encrypt_response(symmetric_key, response_plaintext)
...
def start_udp_dummy(port, symmetric_key, response_plaintext):
    ...
    def udp_handler():
        ...
        try:
            ...
            encrypted_response = encrypt_response(symmetric_key, response_plaintext)
            sock.sendto(encrypted_response.encode('utf-8'), addr)
...
async def _gather_devices_and_initialize(
    ...
    async with aiohttp.ClientSession(raise_for_status=True) as session:
        ...
        async with session.post(url, headers={"Authorization": token}, json=body) as resp:
            ...
            new_iv, response_cipher = encrypt_response(symmetric_key, sym_key["response_plaintext"])
            logger.success(
