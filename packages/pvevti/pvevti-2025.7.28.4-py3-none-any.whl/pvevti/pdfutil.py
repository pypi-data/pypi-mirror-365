...
power_steering_config = {
    "docTitle": "Power Steering", "docSubTitle": "",
    "dataRange": [0, -1],
    "pages": [
        {
            "pageName": "Power Steering Analysis",
            "plots": [
                {
                    "plotTitle": "Steer Angle/Temperature",
                    "plotType" : "dualline",
                    "xData"    : "cumulative distance",
                    "yData"    : ["SteerWheelAngle", "STEER_RES_PSFLD_T"],
                    "filterLength": 40
                },
                {
                    "plotTitle": "Coolant and Oil Temperature",
                    "plotType" : "dualline",
                    "xData"    : "cumulative distance",
                    "yData"    : ["EngCoolantTemp", "EngOilTemp_Cval"],
                    "filterLength": 40
                }
            ]
        }
...
class Plot:
    def __init__(self, type="line", data=[], legend = [], xlabel = "", 
                 ...
                 title = "Unnamed Plot", filterLength = 0, tableItems = []):
        """
    ...
    def render(self):
        ...
        match self.type:
            ...
            case "dual" | "dualline":
                ...
                ax1.grid(True, 'major', 'y', color=getCol(0))
                ax2.grid(True, 'major', 'y', color=getCol(1), linestyle="--")
                ax1.set_xlabel(self.xlabel)
                # ax1.set_xticks(range(int(min(self.data[0])), int(max(self.data[0])), int((max(self.data[0])-min(self.data[0]))/8)))
                pass
            case "map":
                ax = plt.gca()
                geometry = [Point(xy) for xy in zip(pd.Series(self.data[1]), pd.Series(self.data[2]))]
                gdf = gpd.GeoDataFrame(geometry=geometry, crs='EPSG:4326')

                # print(self.data[0], self.data[1], self.data[2])

                gdf.plot(
                    ax=ax, markersize=4, column=pd.Series(self.data[0]),
                    cmap='coolwarm', marker='o', legend=True, label='GPS Track',
                    legend_kwds={'label': 'Vehicle Speed (mph)', 'location': 'bottom'}
                )
                ...
                try: 
                    ...
                    ax.text(0.5, 0.5, 'Basemap not available - Connection Timeout to OpenStreetMap Server',
        ...
        if self.type not in ['dual', 'dualline']:
...
def fixConfig(config_old: dict):
    ...
    for page in config["pages"]:
        ...
        for plot in page["plots"]:
            ...
            if "tableData" not in plot:
                plot["tableData"] = []
    return config

def createDocument(df: pd.DataFrame, config: dict, save_path: str=os.path.expanduser("~")+"\\"):
    """
    ...
    for (i, page) in enumerate(cfg['pages']):
        ...
        for (x, plot) in enumerate(page['plots']):
            ...
            cd_plot = Plot(type=plot["plotType"], data=toPlot, 
                           ...
                           title=plot['plotTitle'], filterLength=plot['filterLength'])
            cd_plot.infer_names()
            cd_plot.set_grid('y')
