...
def build_heatmap_rgba(
    ...
    """
    Renvoie une image RGBA (H, W, 4) :
      – RGB codent la classe,
      – canal A = alpha*255 pour les pixels de classe, 0 sinon.
    Idéal pour Napari en tant que calque semi‑transparent.
    """
    print("[DBG] build_heatmap_rgba  → shape:", class_map.shape,
          "dtype:", class_map.dtype, "uniq:", np.unique(class_map))
    if colors is None:
...
def process_folder(
    ...
    """
    Parcourt un dossier d’images, effectue l’inférence et écrit :
        • predictions.csv      (output_dir)
        • <nom>_map.tif (RGBA) (map_dir)
    Renvoie l’index de progression à chaque image puis, à la fin, le DataFrame.
    """
    ...
    for idx, img_path in enumerate(images, start=1):
        ...
        print("[DBG] write", f"{img_path.stem}_map.tif",
              "| shape", heatmap_rgba.shape,
              "alpha uniq", np.unique(heatmap_rgba[..., 3]))
