...
from qtpy.QtWidgets import QCheckBox, QFileDialog, QFormLayout, QProgressBar, QPushButton, QVBoxLayout, QWidget, QSpinBox, QComboBox  
...
class MitoclassWidget(QWidget):
        ...
        if self.master_path.exists():
            try:
                self.csv_results = pd.read_csv(
                    self.master_path, sep=";", decimal=",", encoding="utf-8-sig"
                )
            except Exception as e:
                self._status(f"Error reading master CSV: {e}")
                self.csv_results = None

        self.paths = {"input": None, "output": None, "map": None, "model": None}
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        self.view_heatmaps_btn = QPushButton("Show heatmaps"); self.view_heatmaps_btn.setEnabled(False)
        self.graph_source_combo = QComboBox(); self.graph_source_combo.addItems(["Master Data", "Last Session"])
        self.view_3d_btn = QPushButton("Display 3D graph"); self.view_3d_btn.setEnabled(False)
    ...
    def _choose_model(self):
        f,_ = QFileDialog.getOpenFileName(self, "Select a model .h5", filter="*.h5")
        if f:
    ...
    def _run_inference(self):
        missing = [k for k in ("input","output","map","model") if self.paths.get(k) is None]
        if missing:
        ...
        images = [
            p for p in sorted(self.paths["input"].iterdir())
    ...
    def _handle_finished(self, df_new: pd.DataFrame):
        ...
        if self.add_to_master_cb.isChecked():
            if self.csv_results is not None:
                master = pd.concat([self.csv_results, df_new], ignore_index=True)
            else:
                master = df_new.copy()
            master = master.drop_duplicates(subset="image", keep="last")
            master.to_csv(self.master_path, sep=";", decimal=",", encoding="utf-8-sig", index=False)
    ...
    def _handle_error(self, error):
        ...
        if "OOM" in msg or isinstance(error, tf.errors.ResourceExhaustedError):
            status("Inference error: GPU memory exhausted. Try smaller batch.", msecs=10000, viewer=self.viewer)
        else:
            status(f"Inference error: {error}", msecs=10000, viewer=self.viewer)
        self.progress.setVisible(False)
        self.run_btn.setEnabled(True)

    def _infer_active_layer(self):
        # ——— vérifications de base ————————————————————————————
        needed = [k for k in ("output", "map", "model") if self.paths.get(k) is None]
        if needed:
        ...
        worker_obj.finished.connect(lambda: (
            self.progress.setVisible(False),
    ...
    def _show_heatmaps(self):
        ...
        input_dir = self.paths.get("input")  # might be None for layer‑only runs
        ...
        for stem in sorted(stems):
            ...
            if stem in self.viewer.layers:
                ...
                candidates = [
                    p for p in Path(input_dir).glob(f"{stem}.*")
                    if p.suffix.lower() in {".tif", ".tiff", ".stk", ".png"}
                ]
                if candidates:
    ...
    def _show_3d(self):
        ...
        df_plot=self.csv_results.copy() if source=="Master Data" and self.csv_results is not None else self.df_new.copy()
        label_map={1:"Connected",2:"Fragmented",3:"Intermediate"}
        df_plot["Class"] = df_plot["global_class"].map(label_map).fillna("background")
        fig=px.scatter_3d(df_plot, x="pct_connected", y="pct_fragmented", z="pct_intermediate",
                          hover_name="image", color="Class",
                          color_discrete_map={"Connected":"red","Fragmented":"green","Intermediate":"blue","background":"gray"},
