...
pyautogui_screen_width = config.CONFIG["app"].get("browser_width", 1712)
pyautogui_screen_height = config.CONFIG["app"].get("browser_height", 1072)
logger.info(f"Screen resolution: {pyautogui_screen_width}x{pyautogui_screen_height}")
...
class ChromiumManager:
    ...
    async def stop(self) -> None:
        self.logger.info("Stopping ChromiumManager...")
    ...
    def _build_chrome_args(self, executable_path: str) -> list[str]:
        args = [
            ...
            "--window-position=0,0",
            f"--window-size={pyautogui_screen_width},{pyautogui_screen_height}",
            "--log-level=3",
    ...
    async def _connect_over_cdp(self) -> None:
        for attempt in range(1, 4):
            try:
                ...
                self.browser_context.pyautogui_screen_width = pyautogui_screen_width
                self.browser_context.pyautogui_screen_height = pyautogui_screen_height
    ...
    async def _shutdown_browser(self) -> None:
        if self.chrome_process:
            self.logger.info("Terminating browser process...")
            try:
