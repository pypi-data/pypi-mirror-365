...
class SiteCredentialManager:
    ...
    def load_all_secrets(self) -> dict:
        ...
        with db_service.get_db_connection_sync() as conn:
            cursor = conn.execute("""
                SELECT site_id, encrypted_username, encrypted_password
                FROM credentials
            """)
            rows = cursor.fetchall()

        for row in rows:
            site_id, enc_user, enc_pass = row
            try:
                username = self.decrypt(enc_user)
                password = self.decrypt(enc_pass)
                secret_dict[site_id] = {
                    "UsernameAssistant": username,
                    "UsernameNotShown": username,
                    "PasswordAssistant": password,
                    "PasswordNotShown": password,
                }
            except Exception as e:
    ...
    def find_site_info_by_name(self, query_name: str, cutoff: float = 0.6):
        """
