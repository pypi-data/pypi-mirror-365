...
class RMLoginEnvSelector:
    ...
    def select_environment(self, ambiente: str, produto: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            lista_ambientes_str = parametros.get_parametro("LISTA_AMBIENTES")
            
            if not lista_ambientes_str:
                raise ValueError("Parâmetro 'lista_ambientes_env' não encontrado")
            
            lista_ambientes = [item.strip() for item in lista_ambientes_str.split(',')]
