...
try:
    ...
    ERROR = ImportError("please install openai and diskcache to use the centricube-pyautogen.oai subpackage.")
...
class Completion(openai_Completion):
    ...
    async def _aget_resonse_llm(cls,
        ...
        config = RunnableConfig(callbacks=callbacks, tags=['centricube-pyautogen'], run_name="centricube-pyautogen")
        response = await llm.ainvoke(content, config=config)
        await run_manage.on_text(text="", sender="centricube-pyautogen")
