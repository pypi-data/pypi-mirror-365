...
class LiteLLMDatabase:
    ...
    def __init__(self, connection_string: str):
        ...
        if self.db_type == 'sqlite':
            ...
            if not self.sqlite_path:
                ...
                self.sqlite_path = connection_string.replace('sqlite:///', '')
                if not self.sqlite_path:
                    self.sqlite_path = connection_string.replace('sqlite://', '')
    ...
    def _quote_identifier(self, identifier: str) -> str:
        ...
        if self.db_type == 'sqlite':
            return identifier  # SQLite doesn't require quotes for identifiers
        else:
            return f'"{identifier}"'  # PostgreSQL uses double quotes
    ...
    def get_usage_data(self, limit: Optional[int] = None) -> pl.DataFrame:
        ...
        if self.db_type == 'sqlite':
            # SQLite query (no double quotes around table names)
            query = """
            SELECT
                s.id,
                s.date,
            ...
            # PostgreSQL query (with double quotes)
            query = """
            SELECT
                s.id,
                s.date,
        ...
        try:
            ...
            if self.db_type == 'sqlite':
                # SQLite connections don't have a close method in the same way
                pass
            else:
                conn.close()
