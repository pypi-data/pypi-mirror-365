...
class TestPortLinkClient:
    ...
    def test_print_tunnel_info_no_tunnel(self, mock_print):
        """测试打印隧道信息 - 无隧道"""
        client = PortLinkClient()
        client.print_tunnel_info()
        mock_print.assert_called_once_with("隧道尚未建立")
    
    @patch('builtins.print')
    def test_print_tunnel_info_with_tunnel(self, mock_print):
        """测试打印隧道信息 - 有隧道"""
        client = PortLinkClient()
        client.tunnel_addr = "localhost:12345"
        client.print_tunnel_info()
        ...
        assert len(calls) == 4
        assert "PortLink 隧道信息" in calls[0][0][0]
        assert "localhost:12345" in calls[1][0][0]
        assert "已就绪" in calls[2][0][0]
...
class TestPortLinkClientAsync:
        ...
        with patch('asyncio.open_connection') as mock_open_conn:
            ...
            with patch('builtins.print') as mock_print:
                ...
                error_calls = [call for call in mock_print.call_args_list 
                             if "Connection to localhost:8080 refused" in str(call)]
    ...
    async def test_receive_from_ws_heartbeat_response(self):
        """测试接收心跳响应"""
        client = PortLinkClient()
        mock_websocket = AsyncMock()
        client.websocket = mock_websocket
        ...
        # 模拟心跳响应消息
        heartbeat_response = json.dumps({"type": "heartbeat", "status": "ok"})
        mock_websocket.__aiter__.return_value = [heartbeat_response]
        
        with patch('builtins.print') as mock_print:
            await client._receive_from_ws(mock_writer)
            
            # 验证打印了心跳响应信息
            heartbeat_calls = [call for call in mock_print.call_args_list 
                             if "Received heartbeat response" in str(call)]
