...
class PortLinkClient:
    def __init__(self, local_port, server_uri="ws://106.75.139.203:8080/ws", debug=False, token=""):
    ...
    def print_tunnel_info(self):
        # 从 server_uri 中提取 IP 地址
        import re
        try:
            ip_match = re.search(r'://([^:/]+)', self.server_uri)
            server_ip = ip_match.group(1) if ip_match else 'unknown'
        except (AttributeError, IndexError, TypeError) as e:
            logging.warning(f"Failed to extract IP from server_uri '{self.server_uri}': {e}")
            server_ip = 'unknown'
        
        # 从 remote_addr 中提取端口号
        try:
            port_match = re.search(r':(\d+)$', self.remote_addr)
            remote_port = port_match.group(1) if port_match else 'unknown'
        except (AttributeError, IndexError, TypeError) as e:
            logging.warning(f"Failed to extract port from remote_addr '{self.remote_addr}': {e}")
            remote_port = 'unknown'
        
        # 拼接 server_ip + remote_port
        try:
            combined_addr = f"{server_ip}:{remote_port}"
        except Exception as e:
            logging.warning(f"Failed to create combined address: {e}")
            combined_addr = 'unknown:unknown'
        
        print("--------------------------------------------------")
        print(f"  Tunnel established:")
        print(f"    Local service:  127.0.0.1:{self.local_port}")
        print(f"    Remote address: {self.remote_addr}")
        print(f"    Public address: {combined_addr}")
        print("--------------------------------------------------")
