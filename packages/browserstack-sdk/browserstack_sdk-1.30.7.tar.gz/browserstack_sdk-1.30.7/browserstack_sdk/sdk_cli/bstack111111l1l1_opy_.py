...
class bstack1111111lll_opy_:
    ...
    bstack1111111ll1_opy_: Union[None, Callable]
    def __init__(self, timeout=1, bstack111111l11l_opy_=1, bstack1111111l1l_opy_=None, bstack1111111ll1_opy_=None):
    ...
    def enqueue(self, job: Callable):
        if not callable(job):
            raise ValueError(bstack1l11l11_opy_ (u"ࠤ࡬ࡲࡻࡧ࡬ࡪࡦࠣ࡮ࡴࡨ࠺ࠡࠤႍ") + type(job))
        self.queue.put(job)
    def start(self):
        if self.threads:
            return
        self.threads = [threading.Thread(target=self.worker, daemon=True) for _ in range(self.bstack111111l11l_opy_)]
    ...
    def worker(self):
        while not self.bstack111111l111_opy_.is_set():
            try:
                ...
                if callable(self.bstack1111111ll1_opy_):
                    self.bstack1111111ll1_opy_(e)