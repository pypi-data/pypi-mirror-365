...
from remote_kernel import PYTHON_BIN, REMOTE_CONN_DIR, KERNELS_DIR, log, usage, version, __version__
...
def start_kernel(endpoint, conn_file, jump=None):
    ...
    python_path, err = sshw.exec(f"which python3 || which python")
    
    cmd = f"{python_path} -m ipykernel_launcher -f {remote_conn_file}"
