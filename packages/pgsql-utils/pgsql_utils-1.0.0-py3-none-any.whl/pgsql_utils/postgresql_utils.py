...
class PostgreSQLUtils:
    ...
    def __init__(
        ...
        self._sequence_cache = {}  # 序列缓存，避免重复检查
        self._sequence_cache_timeout = 60  # 缓存超时时间（秒）
    ...
    def _reset_sequence_for_table(self, table_name: str, primary_key: str = "id"):
        ...
        """
        # 创建缓存键
        cache_key = f"{table_name}.{primary_key}"
        current_time = time.time()

        # 检查缓存是否有效
        if cache_key in self._sequence_cache:
            last_check_time, last_max_value = self._sequence_cache[cache_key]
            # 如果缓存未过期，则跳过检查
            if current_time - last_check_time < self._sequence_cache_timeout:
                return
        ...
        try:
            ...
            if sequence_exists:
                # 检查序列当前值是否小于表中最大ID值
                check_query = f"""
                    SELECT last_value, (SELECT COALESCE(MAX({primary_key}), 0) FROM public.{table_name}) 
                    FROM {sequence_name}
                """
                cursor.execute(check_query)
                last_value, max_value = cursor.fetchone()

                # 只有当序列值小于表中最大ID值时才重置序列
                if last_value < max_value:
                    query = f"SELECT setval(%s, %s)"
                    cursor.execute(query, (sequence_name, max_value))
                    self.connection.commit()
                    self.cp.green(f"{table_name}表{primary_key}序列已重置为{max_value}")
                    # 更新缓存
                    self._sequence_cache[cache_key] = (current_time, max_value)
                else:
                    # 即使没有重置，也更新缓存时间
                    self._sequence_cache[cache_key] = (current_time, max_value)
            else:
                ...
                if result and result[0]:
                    sequence_name = result[0].split(".")[-1]  # 获取序列名（去除模式名）
                    # 检查序列当前值是否小于表中最大ID值
                    check_query = f"""
                        SELECT last_value, (SELECT COALESCE(MAX({primary_key}), 0) FROM public.{table_name}) 
                        FROM {sequence_name}
                    ...
                    if last_value < max_value:
                        ...
                        self.connection.commit()
                        self.cp.green(
                            f"{table_name}表{primary_key}序列已重置为{max_value}"
                        )
                        # 更新缓存
                        self._sequence_cache[cache_key] = (current_time, max_value)
                    else:
    ...
    def add(self, table: str, data: Dict[str, Any], **kwargs) -> bool:
        ...
        # 重置表序列以防止主键冲突
        self._reset_sequence_for_table(table)
    ...
    def add_batch(self, table: str, data_list: List[Dict[str, Any]], **kwargs) -> bool:
        """
        批量插入数据，类似于feapder中MysqlDB的add_batch方法
    ...
    def add_smart(self, table: str, data: Dict[str, Any], **kwargs) -> bool:
        """
        智能插入数据，如果主键冲突则更新，类似于feapder中MysqlDB的add_smart方法

        Args:
            table (str): 表名
            data (Dict[str, Any]): 要插入或更新的数据字典
            **kwargs: 其他参数，如primary_key指定主键字段
        ...
        try:
            ...
            # logging.error(f"数据插入或更新失败: {e}")
            # logging.warning(f"错误数据: {processed_data}")
            self.connection.rollback()
            return False
