...
class ApiConnectorComponent(BaseComponent):
        ...
        if self.config is None:
            self.config = ComponentConfig(
                ...
                repository=GitRepositoryModel(
                    remote_url="https://codebase.helmholtz.cloud/hmc/hmc-public/unhide/development/apiconnector.git",
                ),
                env_vars=ApiConnectorEnvVars(),
            )
        else:
            self.config = config
    ...
    def validate_config(self, config: TOMLDocument) -> None:
        ...
        if "env_vars" in component_config:
            self.config.env_vars = ApiConnectorEnvVars(**component_config["env_vars"])
    ...
    def create_env_file(self) -> None:
        ...
        if self.config.env_vars:
            ...
            component_dir = self.name.replace("-", "").replace("_", "")
