...
class InternalDataModelComponent(BaseComponent):
    """Internal Data Model component."""
        ...
        if self.config is None:
            self.config = ComponentConfig(
                ...
                repository=GitRepositoryModel(
                    remote_url="https://codebase.helmholtz.cloud/hmc/hmc-public/unhide/development/semantics/internal-data-model.git",
                ),
                env_vars=InternalDataModelEnvVars(),
            )
        else:
            self.config = config
