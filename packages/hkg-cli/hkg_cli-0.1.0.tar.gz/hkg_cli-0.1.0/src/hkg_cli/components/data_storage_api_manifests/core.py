...
class DataStorageApiManifestsComponent(BaseComponent):
    """Data Storage API Manifests component."""

    name = "data-storage-api-manifests"
    is_private = True
    is_devops = True
        ...
        if self.config is None:
            self.config = ComponentConfig(
                ...
                repository=GitRepositoryModel(
                    remote_url="git@codebase.helmholtz.cloud:hmc/hmc-public/unhide/development/devops/data-storage-api-manifests.git",
                ),
                env_vars=DataStorageApiManifestsEnvVars(),
            )
        else:
            self.config = config
