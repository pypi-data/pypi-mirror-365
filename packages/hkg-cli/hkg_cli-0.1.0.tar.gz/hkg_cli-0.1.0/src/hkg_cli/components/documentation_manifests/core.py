...
class DocumentationManifestsComponent(BaseComponent):
    """Documentation Manifests component."""
        ...
        if self.config is None:
            self.config = ComponentConfig(
                ...
                repository=GitRepositoryModel(
                    remote_url="git@codebase.helmholtz.cloud:hmc/hmc-public/unhide/development/devops/documentation-manifests.git",
                ),
                env_vars=DocumentationManifestsEnvVars(),
            )
        else:
            self.config = config
