...
class MappingComponent(BaseComponent):
        ...
        if self.config is None:
            self.config = ComponentConfig(
                ...
                repository=GitRepositoryModel(
                    remote_url="https://codebase.helmholtz.cloud/hmc/hmc-public/unhide/development/mapping.git",
                ),
                env_vars=MappingEnvVars(),
            )
        else:
            self.config = config
    ...
    def validate_config(self, config: TOMLDocument) -> None:
        ...
        if "env_vars" in component_config:
            self.config.env_vars = MappingEnvVars(**component_config["env_vars"])
