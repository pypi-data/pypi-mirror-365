...
# This would normally be: from ..base import BaseTool
class BaseTool:
...
# Test the framework-compliant tool
if __name__ == "__main__":
    tool = CodingTool()
    
    print("Framework-Compliant CodingTool Test")
    print("=" * 50)
    
    # Test can_handle - should REJECT generation requests
    generation_requests = [
    ...
    print("\n--- Testing Generation Requests (Should be REJECTED) ---")
    for request in generation_requests:
        result = tool.can_handle(request)
        print(f"can_handle('{request}'): {result} {'' if not result else '❌'}")
    
    # Test can_handle - should ACCEPT processing requests
    processing_requests = [
        "validate this python code: def hello(): print('hi')",
        "format this code",
        "debug this function",
    ...
    print("\n--- Testing Processing Requests (Should be ACCEPTED) ---")
    for request in processing_requests:
        result = tool.can_handle(request)
        print(f"can_handle('{request}'): {result} {'' if result else '❌'}")
    
    # Test execute with code processing
...
"""
    ...
    result = tool.execute("validate this python code", code=test_code)
    ...
    if result['success']:
        ...
        print(f"Syntax valid: {result['result']['syntax_valid']}")
