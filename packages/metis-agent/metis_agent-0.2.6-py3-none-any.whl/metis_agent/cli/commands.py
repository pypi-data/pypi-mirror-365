...
def _start_interactive_mode(agent: SingleAgent, config: AgentConfig, session_id: str = None):
    """Start interactive chat mode with enhanced memory support."""
    ...
    try:
        session_history = agent.memory.get_session_history(current_session)
        if session_history:
            click.echo(f"Session has {len(session_history)} previous messages")
        else:
    ...
    click.echo("  - 'session <name>' - Switch to session")
    click.echo("  - 'sessions' - List all sessions")
    click.echo("  - 'history' - Show current session history")
    click.echo("  - 'clear' - Clear screen")
    click.echo("  - 'clear-session' - Clear current session memory")
            ...
            if user_input.lower() in ['exit', 'quit', 'bye']:
                ...
                if new_session:
                    ...
                    try:
                        ...
                        if session_history:
                            ...
                            click.echo("Starting new conversation in this session")
                    except Exception:
                        click.echo("Starting new conversation in this session")
                continue
                ...
                try:
                    ...
                    if sessions:
                        ...
                        for session in sessions:
                            count = len(agent.memory.get_session_history(session))
                            marker = " (current)" if session == current_session else ""
                            click.echo(f"  - {session}: {count} messages{marker}")
                    else:
                ...
                try:
                    ...
                    if session_history:
                        ...
                        for i, (timestamp, query, response) in enumerate(session_history[-5:], 1):  # Show last 5
                            click.echo(f"\n{i}. [{timestamp}]")
                            click.echo(f"   You: {query[:100]}{'...' if len(query) > 100 else ''}")
                            if response:
                                click.echo(f"   Metis: {response[:100]}{'...' if len(response) > 100 else ''}")
                        if len(session_history) > 5:
                            click.echo(f"\n... and {len(session_history) - 5} more messages")
                    else:
                        click.echo(f"No history found for session '{current_session}'")
                except Exception as e:
                    click.echo(f"Error retrieving history: {e}")
                continue
            
            elif user_input.lower() == 'clear-session':
                if click.confirm(f"Clear all memory for session '{current_session}'?"):
                    try:
                        agent.memory.clear_session(current_session)
                        click.echo(f"Session '{current_session}' memory cleared")
                    except Exception as e:
                        click.echo(f"Error clearing session: {e}")
                continue
...
def show_config():
    ...
    click.echo("=" * 30)
    click.echo(f"LLM Provider: {config.get_llm_provider()}")
    click.echo(f"LLM Model: {config.get_llm_model() or 'default'}")
    click.echo(f"Memory Enabled: {config.is_memory_enabled()}")
    click.echo(f"Titans Memory: {config.is_titans_memory_enabled()}")
    click.echo(f"Session Timeout: {config.get_session_timeout()}s")
    click.echo(f"Max Context Length: {config.get_max_context_length()}")
    click.echo(f"Agent ID: {config.get_agent_id()}")
    click.echo(f"Agent Name: {config.get_agent_name()}")
