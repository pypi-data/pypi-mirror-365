...
class SQLiteMemory(MemoryInterface):
    ...
    def get_context(self, user_id: str, query: Optional[str] = None, limit: int = 5) -> str:
        ...
        # Format as context string with length limits
        context = ""
        for result_type, content, timestamp in results:
            ...
            if result_type == "input":
                # Limit user inputs to 200 characters
                truncated_content = content[:200] + "..." if len(content) > 200 else content
                context += f"User ({time_str}): {truncated_content}\n"
            else:
                ...
                # Limit agent outputs to 300 characters
                truncated_content = content[:300] + "..." if len(content) > 300 else content
                context += f"Agent ({time_str}): {truncated_content}\n"
    ...
    def get_session_history(self, session_id: str, limit: int = 50) -> List[tuple]:
        """
        ...
        Returns:
            List of tuples (timestamp, query, response)
        """
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        
        # Get inputs and outputs for this session, ordered by timestamp
        cursor.execute("""
            SELECT 'input' as type, content, timestamp FROM user_inputs WHERE user_id = ?
            UNION ALL
            SELECT 'output' as type, content, timestamp FROM agent_outputs WHERE user_id = ?
            ORDER BY timestamp
            LIMIT ?
        """, (session_id, session_id, limit * 2))  # *2 because we have both inputs and outputs
        
        results = cursor.fetchall()
        conn.close()
        
        # Group inputs and outputs into conversation pairs
        history = []
        current_input = None
        
        for msg_type, content, timestamp in results:
            if msg_type == 'input':
                if current_input:  # Previous input without response
                    history.append((current_input[1], current_input[0], None))
                current_input = (content, timestamp)
            else:  # output
                if current_input:
                    # Try to parse JSON content
                    try:
                    ...
                    history.append((current_input[1], current_input[0], content))
                    current_input = None
        
        # Handle any remaining input without response
        if current_input:
            history.append((current_input[1], current_input[0], None))
    ...
    def get_all_sessions(self) -> List[str]:
        """
        Get all session IDs that have stored data.
        ...
        cursor.execute("""
            SELECT DISTINCT user_id FROM user_inputs
            UNION
            SELECT DISTINCT user_id FROM agent_outputs
            ORDER BY user_id
        """)
    ...
    def clear_session(self, session_id: str) -> None:
        ...
        cursor.execute("DELETE FROM user_inputs WHERE user_id = ?", (session_id,))
        cursor.execute("DELETE FROM agent_outputs WHERE user_id = ?", (session_id,))
        cursor.execute("DELETE FROM clarification_context WHERE user_id = ?", (session_id,))
