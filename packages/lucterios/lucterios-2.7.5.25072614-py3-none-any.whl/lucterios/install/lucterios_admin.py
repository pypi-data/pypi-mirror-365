...
def get_package_requires(name):
    ...
    if isinstance(requires, list):
        return [req.split(';')[0].strip().split(' ')[0].split('~')[0].split('=')[0] for req in requires]
    else:
        return []
    ...
    def _clear_before_restore(self):
        ...
        if "sqlite3" in self.databases['default']['ENGINE']:
            try:
                ...
                getLogger('lucterios.admin').exception('not deleted')
                self.clear(False)
                setup_from_none()
            self.read(True)
        else:
