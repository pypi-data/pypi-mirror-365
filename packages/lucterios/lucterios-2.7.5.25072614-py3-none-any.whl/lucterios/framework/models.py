...
from lucterios.framework.model_fields import get_obj_contains, PrintFieldsPlugIn, is_simple_field, LucteriosVirtualField
...
class LucteriosQuerySet(QuerySet):
    ...
    def order_by(self, *field_names):
        def getattr_from_field(item, field_names):
            value = getattr(item, field_names[0], None)
            if len(field_names) > 1:
                return getattr_from_field(value, field_names[1:])
            else:
                return value
        ...
        for fieldname in fieldnames:
            if fieldname[0] == '-':
                new_query._result_cache.sort(key=lambda item: getattr_from_field(item, fieldname[1:].split("__")), reverse=True)
            else:
                new_query._result_cache.sort(key=lambda item: getattr_from_field(item, fieldname.split("__")), reverse=False)
        return new_query
...
class LucteriosModel(models.Model):
    ...
    @staticmethod
    def have_class_item(itemclass):
        ident = "%s_nb" % itemclass.__name__
        value = cache.get(ident)
        if value is None:
            value = itemclass.objects.count() > 0
            cache.set(ident, value, timeout=30)
        return value

    @staticmethod
    def delete_class_item(itemclass):
        ident = "%s_nb" % itemclass.__name__
        if ident in cache:
            cache.delete(ident)

    @classmethod
    def get_cache_text(cls, identifier, fieldname=None):
        ident = '%s_%s' % (cls.__name__, identifier.id if isinstance(identifier, cls) else identifier)
        value = cache.get(ident)
        if value is None:
            if isinstance(identifier, cls):
                item = identifier
            else:
                item = cls.objects.get(id=identifier)
    ...
    @classmethod
    def delete_cache_text(cls, objid):
        if objid is None:
            return
        ident = '%s_%s' % (cls.__name__, objid)
        if ident in cache:
            cache.delete(ident)
...
"""
    ...
    @classmethod
    def get_query_for_duplicate(cls, match_fields=None, subfilter=Q()):
        if not isinstance(match_fields, list):
            match_fields = cls._meta.ordering
        query = Q()
        for ident in cls.objects.filter(subfilter).values(*match_fields).annotate(Count('id')).values(*match_fields).order_by().filter(id__count__gt=1):
            query |= Q(**ident)
        if len(query) == 0:
            empty_val = {}
    ...
    def get_final_child(self, level=-1):
        final_child = self
        ...
        if level != 0:
            for rel_obj in rel_objs:
                if hasattr(rel_obj, 'field') and (rel_obj.field.name == (self.__class__.__name__.lower() + '_ptr')):
                    try:
                        final_child = getattr(self, rel_obj.get_accessor_name()).get_final_child(level - 1 if level > 0 else -1)
    ...
    def save(self, force_insert=False, force_update=False, using=None, update_fields=None):
        self.delete_cache_text(self.id)
