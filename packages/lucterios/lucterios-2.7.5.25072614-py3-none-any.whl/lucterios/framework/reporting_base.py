...
from lucterios.framework.filetools import is_image_base64, open_from_base64, open_from_mdi
...
class LucteriosBase(object):
    ...
    def open_image(self, xmlimage):
        ...
        if img_content.startswith('mdi:'):
            img_file = open_from_mdi(img_content)
        elif is_image_base64(img_content):
    ...
    def _parse_comp(self, comp, y_offset):
        ...
        for child in comp:
            ...
            if hasattr(self, mtd):
                ...
                # getLogger('lucterios.core.print').debug("print: %s (x=%f,y=%f,w=%f,h=%f) ", child.tag, current_x / mm, current_y / mm, current_w / mm, current_h / mm)
                fct(child, current_x, current_y, current_w, current_h)
            else:
                getLogger('lucterios.core.print').warning("Unsupported method: " + mtd)
