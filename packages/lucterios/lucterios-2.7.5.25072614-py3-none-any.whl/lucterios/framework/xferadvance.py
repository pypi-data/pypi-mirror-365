...
from lucterios.framework.tools import ifplural, WrapAction, ActionsManage, FORMTYPE_MODAL, CLOSE_NO
from lucterios.framework.xfercomponents import XferCompImage, XferCompLabelForm, XferCompGrid, XferCompButton
...
class XferAddEditor(XferContainerCustom):
    ...
    def fillresponse(self):
        ...
        if len(self.actions) == 0:
            self.add_action(self.return_action(TITLE_OK, short_icon='mdi:mdi-check'), params={"SAVE": "YES"})
    ...
    def run_save(self, request, *args, **kwargs):
        ...
        save.short_icon = self.short_icon
...
class XferSave(XferContainerAcknowledge):
    ...
    def fillresponse(self):
        ...
        if self.getparam('URL_TO_REDIRECT') is not None:
            ...
            self.redirect_action(WrapAction('', short_icon=self.short_icon, url_text=url_text), params={self.field_id: self.item.id})
