...
from typing_extensions import TYPE_CHECKING, Optional, Tuple
...
class InvalidParameterError(ValueError):
    def __init__(self, parameter: str, value: object, message: Optional[str] = None) -> None:
        if message:
...
class Bytes(DatabaseEntity):
    ...
    def find_bytes_between(
        ...
        Raises:
            InvalidParameterError: If pattern is not bytes or is empty.
        ...
        elif not self.m_database.is_valid_ea(start_ea):
            raise InvalidEAError(start_ea)

        if end_ea is None:
            end_ea = self.m_database.maximum_ea
        elif not self.m_database.is_valid_ea(end_ea):
    ...
    def find_text_between(
        ...
        Raises:
            InvalidParameterError: If text is not string or is empty.
            InvalidEAError: If start_ea or end_ea are specified but invalid.
        """
    ...
    def get_next_head(self, ea: ea_t, max_ea: ea_t = None) -> Optional[ea_t]:
        ...
        elif not self.m_database.is_valid_ea(max_ea):
    ...
    def get_prev_head(self, ea: ea_t, min_ea: ea_t = None) -> Optional[ea_t]:
        ...
        elif not self.m_database.is_valid_ea(min_ea):
