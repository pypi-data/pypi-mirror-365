...
class TestBundleFileSystemErrorHandling:
    ...
    def test_bundle_manager_recovery_from_partial_file_corruption(self):
        """Should recover from partial file corruption by backing up corrupted files."""
        bundle_manager = BundleManager(self.project_root)
        ...
        corrupted_file.write_text("""# Generated by LakehousePlumber - Bundle Resource for test
resources:
  pipelines:
    test:
      libraries:
...
invalid yaml content""")
        ...
        # Should handle corruption gracefully by backing up and recreating
        bundle_manager.sync_resources_with_generated_files(generated_dir, "dev")
        
        # Should backup corrupted file and create new one
        backup_file = resources_lhp_dir / "test.pipeline.yml.bkup"
        assert backup_file.exists(), "Corrupted file should be backed up"
        assert corrupted_file.exists(), "New file should be created"
        
        # New file should be valid and use template format
        new_content = corrupted_file.read_text()
...
class TestBundleErrorRecoveryMechanisms:
    ...
    def test_bundle_manager_graceful_degradation_on_yaml_errors(self):
        """Should handle YAML corruption gracefully with backup and regeneration."""
        bundle_manager = BundleManager(self.project_root)
        ...
        corrupted_file.write_text("# Generated by LakehousePlumber - Bundle Resource for test_pipeline\ninvalid: yaml: content: [[[")
        
        # Should handle gracefully without raising errors (backup and regenerate)
        updated_count = bundle_manager.sync_resources_with_generated_files(generated_dir, "dev")
        
        # Should complete successfully
        assert updated_count >= 0
        
        # Should backup corrupted file and create new valid file
        backup_file = resources_lhp_dir / "test_pipeline.pipeline.yml.bkup"
        assert backup_file.exists(), "Corrupted file should be backed up"
        
        # New file should be valid YAML with glob pattern
        new_content = corrupted_file.read_text()
        assert "Generated by LakehousePlumber" in new_content
        assert "- glob:" in new_content
    ...
    def test_error_context_preservation_across_components(self):
        ...
        # Create existing LHP-generated resource file to trigger YAML processing
        resources_lhp_dir = self.project_root / "resources" / "lhp"
        resources_lhp_dir.mkdir(parents=True)
        resource_file = resources_lhp_dir / "test_pipeline.pipeline.yml"
        resource_file.write_text("""# Generated by LakehousePlumber - Bundle Resource for test_pipeline
resources:
  pipelines:
    test_pipeline:
      libraries:
...
""")
        ...
        # Test error context preservation with template rendering failure
        original_render = bundle_manager.render_template
        def failing_render(template_name, context):
            if "test_pipeline" in str(context):
                raise Exception("Template rendering failed for test_pipeline")
        ...
        with patch.object(bundle_manager, 'render_template', side_effect=failing_render):
            ...
            # Should preserve error context and mention the failed pipeline
            error_msg = str(exc_info.value)
            assert "test_pipeline" in error_msg  # Pipeline context
            assert ("template" in error_msg.lower() or "failed" in error_msg.lower())  # Error details
