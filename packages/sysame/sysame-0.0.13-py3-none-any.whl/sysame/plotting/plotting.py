...
def plot_scatter_subplots(
    ...
    shared_axis_limits : bool, optional
        If True, all subplots will share the same axis limits, by default False
    ...
    if aggregation_data_list is not None:
        for i, aggregation_data in enumerate(aggregation_data_list):
            if aggregation_data is not None:
                aggregation_array = np.asarray(aggregation_data).flatten()
                if aggregation_array.shape != x_arrays[i].shape:
                    raise ValueError(
                        f"Aggregation array for subplot {i} must have the same shape as x and y arrays."
                    )
                aggregation_arrays.append(aggregation_array)
            else:
    ...
    # Calculate shared axis limits if requested
    if shared_axis_limits:
        all_x_values = np.concatenate(x_arrays)
        all_y_values = np.concatenate(y_arrays)
        global_min_val = min(np.min(all_x_values), np.min(all_y_values))
        global_max_val = max(np.max(all_x_values), np.max(all_y_values))
        global_padding = 0.05 * (global_max_val - global_min_val)
        shared_xlim = (global_min_val - global_padding, global_max_val + global_padding)
        shared_ylim = (global_min_val - global_padding, global_max_val + global_padding)
    ...
    for i in range(n_plots):
        ...
        # Calculate individual data range for axis limits (if not using shared limits)
        if not shared_axis_limits:
            ...
            extent = (shared_xlim[0], shared_xlim[1], shared_ylim[0], shared_ylim[1])
        ...
        if aggregation_array is not None:
            ...
            if group_colors_str is None:
                ...
                if n_colors <= 12:
                    ...
                    base_palette = sns.color_palette("Spectral", n_colors=n_colors)
                group_colors_str = {
                    group: to_hex(base_palette[j % len(base_palette)])
            ...
            for j, group in enumerate(sorted_groups):
                mask = groups_array_str == group
                color = group_colors_str.get(
                    str(group), sns.color_palette("colorblind")[j]
                )

                # Get display label for the group
                display_label = (
                    group_labels.get(str(group), str(group))
                    if group_labels
                    else str(group)
                )
        ...
        # X label
        if check_for_single_ref:
            if np.all(np.equal(x_array, x_array.flat[0])):
    ...
    if main_title:
        fig.suptitle(
            main_title, fontsize=18, fontweight="bold", color="#2D3033", y=0.98
        )
