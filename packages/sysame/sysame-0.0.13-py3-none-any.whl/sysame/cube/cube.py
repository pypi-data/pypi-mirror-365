...
def csv_to_mat(
    ...
    """
    for csv in csvs:
        if not csv[1].is_file():  # type: ignore
    ...
    for n, csv in enumerate(csvs, start=1):
        script_text.append(f'FILEI MATI[{n}]="{Path(csv[1]).resolve()}",')
    ...
    script_text += [f"mw[{n}]=mi.{n}.1" for n in range(1, len(csvs) + 1)]
    script_text += ["", "ENDRUN"]
    ...
    _ = subprocess.run([str(a) for a in args], capture_output=True, check=False)
