...
async def get_store() -> GuidelineStore:
    """Get or initialize the guideline store."""
    global store
    if store is None:
        ...
        store = GuidelineStore(guidelines_path)
...
async def get_guidelines_path() -> str:
    """
    Get the path where the server is looking for guidelines.
    
    Returns:
        The current guidelines path being used by the server
    """
    guideline_store = await get_store()
    guidelines_path = str(guideline_store.filesystem.guidelines_path)
    ...
    try:
        if os.path.exists(guidelines_path):
            contents = []
            for item in os.listdir(guidelines_path):
                item_path = os.path.join(guidelines_path, item)
                if os.path.isdir(item_path):
                    try:
                        md_files = [f for f in os.listdir(item_path) if f.endswith('.md')]
                        contents.append(f"  - {item}/ ({len(md_files)} .md files)")
                    except Exception:
                        contents.append(f"  - {item}/ (error reading)")
                else:
                    contents.append(f"  - {item} (file)")
            
            directory_listing = "\n".join(contents) if contents else "  (empty)"
        else:
            directory_listing = "  (directory does not exist)"
    except Exception as e:
        directory_listing = f"  (error reading directory: {e})"
    
    # Also show the search attempts made during initialization
    guidelines_path_env = os.getenv("GUIDELINES_PATH", "not set")
    local_path_exists = os.path.exists("./guidelines")
    
    return f"""Guidelines Path Information:
- Environment GUIDELINES_PATH: {guidelines_path_env}
- Local ./guidelines exists: {local_path_exists}
- Current path being used: {guidelines_path}
- Path exists: {os.path.exists(guidelines_path)}
- Total guidelines loaded: {len(guideline_store.guidelines)}

Contents of guidelines directory:
{directory_listing}"""
...
async def list_categories_detailed() -> str:
    ...
    try:
        guideline_store = await get_store()
        category_info = guideline_store.get_category_details()
        ...
        for category, info in sorted(category_info.items()):
            lines.append(f"## {category}")
            lines.append(f"- **Guidelines**: {info['count']}")
            if info.get('description'):
                lines.append(f"- **Description**: {info['description']}")
            if info.get('subcategories'):
                lines.append(f"- **Subcategories**: {', '.join(info['subcategories'])}")
            lines.append("")
