...
def clean_double_brackets(smiles):
    # Replace any double brackets [[NH+]] with [NH+]
    return re.sub(r'\[{2}([A-Za-z0-9@+\-\(\)]+)\]{2}', r'[\1]', smiles)
...
def generate_datasets(filename, train_or_test, args):
    ...
    for i, mol in tqdm(data.iterrows(), total=data.shape[0]):
        ...
        smiles_A = clean_double_brackets(smiles_A)
...
def generate_infersets(small_mol, i, initial, ionized_smiles, ionization_states, args):
    ...
    smiles = clean_double_brackets(smiles)
