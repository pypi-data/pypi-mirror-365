...
def parse_gatk4_mutect2_vcf(
        df_vcf: pd.DataFrame,
        source_id: str,
        platform: str
) -> pd.DataFrame:
    """
    Parse a GATK4-Mutect2 VCF DataFrame and return a Pandas DataFrame.

    Parameters:
        df_vcf      :   Pandas DataFrame of rows from a GATK4-Mutect2 VCF file.
    ...
    for row in df_vcf.to_dict('records'):
        ...
        for i, alternate_allele in enumerate(alternate_allele_.split(',')):
            for sample_id in sample_ids:
                ...
                data['method'].append(VariantCallingMethod.GATK4_MUTECT2)
                ...
                if 'AF' in format_dict:
                    position_1_alternate_allele_fraction = float(format_dict['AF'].split(',')[i])
                    position_2_alternate_allele_fraction = float(format_dict['AF'].split(',')[i])
                else:
    ...
    logger.info('Converted %i GATK4-Mutect2 variants.' % len(df_variants['id'].unique()))
    return df_variants
