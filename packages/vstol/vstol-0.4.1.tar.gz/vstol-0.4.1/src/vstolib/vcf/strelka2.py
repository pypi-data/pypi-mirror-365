...
def parse_strelka2_vcf(
        df_vcf: pd.DataFrame,
        source_id: str,
        platform: str
) -> pd.DataFrame:
    """
    Parse a Strelka2 VCF DataFrame and return a Pandas DataFrame.

    Parameters:
        df_vcf      :   Pandas DataFrame of rows from a Strelka2 VCF file.
    ...
    for row in df_vcf.to_dict('records'):
        ...
        alternate_allele_ = retrieve_from_dict(dct=row, key='ALT', default_value='', type=str)
        ...
        for i, alternate_allele in enumerate(alternate_allele_.split(',')):
            for sample_id in sample_ids:
                ...
                data['method'].append(VariantCallingMethod.STRELKA2_SOMATIC)
                ...
                if len(reference_allele) == len(alternate_allele):
                    ...
                    if len(alternate_allele) == 1:
                        ...
                        variant_type = VariantType.SINGLE_NUCLEOTIDE_VARIANT
                    else:
                        data['variant_type'].append(VariantType.MULTI_NUCLEOTIDE_VARIANT)
                        variant_type = VariantType.MULTI_NUCLEOTIDE_VARIANT
                ...
                if variant_type == VariantType.SINGLE_NUCLEOTIDE_VARIANT:
                    if alternate_allele == 'A' or alternate_allele == 'a':
                        position_1_read_count_alternate_allele = int(format_dict['AU'].split(',')[0])
                        position_2_read_count_alternate_allele = int(format_dict['AU'].split(',')[0])
                    elif alternate_allele == 'C' or alternate_allele == 'c':
                        position_1_read_count_alternate_allele = int(format_dict['CU'].split(',')[0])
                        position_2_read_count_alternate_allele = int(format_dict['CU'].split(',')[0])
                    elif alternate_allele == 'G' or alternate_allele == 'g':
                        position_1_read_count_alternate_allele = int(format_dict['GU'].split(',')[0])
                        position_2_read_count_alternate_allele = int(format_dict['GU'].split(',')[0])
                    elif alternate_allele == 'T' or alternate_allele == 't':
                        position_1_read_count_alternate_allele = int(format_dict['TU'].split(',')[0])
                        position_2_read_count_alternate_allele = int(format_dict['TU'].split(',')[0])
                    else:
                        raise Exception('Unexpected alternate allele: %s.' % alternate_allele)
                elif variant_type == VariantType.INSERTION or variant_type == VariantType.DELETION:
                    position_1_read_count_alternate_allele = int(format_dict['TIR'].split(',')[0])
                    position_2_read_count_alternate_allele = int(format_dict['TIR'].split(',')[0])
                else:
                    raise Exception('Unexpected variant type: %s.' % variant_type)
                if 'DP' in format_dict:
                    position_1_read_count_total = int(format_dict['DP'])
                    position_2_read_count_total = int(format_dict['DP'])
                    position_1_read_count_reference_allele = position_1_read_count_total - position_1_read_count_alternate_allele
                    position_2_read_count_reference_allele = position_2_read_count_total - position_2_read_count_alternate_allele
                else:
                ...
                if position_1_read_count_total > 0 and position_1_read_count_alternate_allele != -1:
                ...
                if position_2_read_count_total > 0 and position_2_read_count_alternate_allele != -1:
    ...
    logger.info('Converted %i Strelka2 variants.' % len(df_variants['id'].unique()))
    return df_variants
