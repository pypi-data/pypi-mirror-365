...
def parse_sniffles2_vcf(
        df_vcf: pd.DataFrame,
        source_id: str,
        platform: str
) -> pd.DataFrame:
    """
    Parse a Sniffles2 VCF DataFrame and return a Pandas DataFrame.

    Parameters:
        df_vcf      :   Pandas DataFrame of rows from a Sniffles2 VCF file.
    ...
    for row in df_vcf.to_dict('records'):
        ...
        for sample_id in sample_ids:
            ...
            af = float(info_dict['AF']) if info_dict.get('AF', '.') != '.' else -1.0
            rnames = info_dict.get('RNAMES', '')
            ...
            def append_variant(
                ...
                data['method'].append(VariantCallingMethod.SNIFFLES2)
                ...
                data['read_ids_alternate_allele'].append(rnames)
    ...
    logger.info('Converted %i Sniffles2 variants.' % len(df_variants['id'].unique()))
    return df_variants
