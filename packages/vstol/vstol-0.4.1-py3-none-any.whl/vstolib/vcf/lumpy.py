...
def parse_lumpy_somatic_vcf(
        df_vcf: pd.DataFrame,
        source_id: str,
        platform: str
) -> pd.DataFrame:
    """
    Parse a Lumpy VCF DataFrame and return a Pandas DataFrame.

    Parameters:
        df_vcf      :   Pandas DataFrame of rows from a Lumpy VCF file.
    ...
    for row in df_vcf.to_dict('records'):
        ...
        for sample_id in sample_ids:
            ...
            num_alt_reads = max([int(format_dict['SU']), int(format_dict['PE']), int(format_dict['SR'])])
            ...
            def append_variant(
                ...
                data['method'].append(VariantCallingMethod.LUMPY_SOMATIC)
                ...
                data['position_1_read_count_reference_allele'].append(-1)
                data['position_2_read_count_reference_allele'].append(-1)
                ...
                data['position_1_read_count_total'].append(-1)
                data['position_2_read_count_total'].append(-1)
                data['position_1_alternate_allele_fraction'].append(-1.0)
                data['position_2_alternate_allele_fraction'].append(-1.0)
    ...
    logger.info('Converted %i Lumpy-Somatic variants.' % len(df_variants['id'].unique()))
    return df_variants
