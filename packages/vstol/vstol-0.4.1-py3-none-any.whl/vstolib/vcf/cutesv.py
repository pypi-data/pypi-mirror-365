...
def parse_cutesv_vcf(
        df_vcf: pd.DataFrame,
        source_id: str,
        platform: str
) -> pd.DataFrame:
    """
    Parse a CuteSV VCF DataFrame and return a Pandas DataFrame.

    Parameters:
        df_vcf      :   Pandas DataFrame of rows from a CuteSV VCF file.
    ...
    for row in df_vcf.to_dict('records'):
        ...
        for sample_id in sample_ids:
            ...
            def append_variant(
                ...
                data['method'].append(VariantCallingMethod.CUTESV)
    ...
    logger.info('Converted %i CuteSV variants.' % len(df_variants['id'].unique()))
    return df_variants
