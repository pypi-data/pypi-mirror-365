...
def test_folder_operations():
    """
    Test folder operations: upload to folder, verify via my_videos, and delete.
    
    Run with: pytest -m calls_api -k test_folder_operations
    """
    client = get_client()
    
    # Create a unique folder name to avoid conflicts
    folder_name = f"integration-test-folder"
    ...
    expected_ids = []
    # 1. Upload two videos to the folder
    for _ in range(2):
        response = client.video.upload(video_url, folder=folder_name)
        expected_ids.append(response['video_id'])
    
    # 2. Verify both videos are in the folder via my_videos
    folder_videos = client.video.my_videos(folder_id=folder_name)
    folder_video_ids = [v['video_id'] for v in folder_videos]
    
    for expected_id in expected_ids:
        assert expected_id in folder_video_ids, f"Video {expected_id} not found in folder"
    assert len(folder_videos) >= len(expected_ids), f"Expected >={len(expected_ids)} videos in folder, found {len(folder_videos)}"
...
def test_rapid_review():
    ...
    try:
        ...
        search_results = client.search_videos('parked bus', folder=folder_name)
        print("Search results:")
        print(search_results)
        assert len(search_results['matches']) >= 4, f"Expected at least 4 matches but got {search_results}"
