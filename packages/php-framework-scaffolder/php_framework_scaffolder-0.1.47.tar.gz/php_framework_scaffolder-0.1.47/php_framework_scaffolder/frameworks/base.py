...
class BaseFrameworkSetup(ABC):
    ...
    def extract_openapi(self, openapi_json_path: Path, target_folder: Path) -> None:
        ...
        openapi_command = self.get_cat_openapi_command()
        logger.info(f"Executing cat openapi command: {openapi_command}")
        _, openapi_output, _ = run_docker_compose_command(openapi_command, target_folder)
    ...
    def get_openapi_command(self) -> List[str]:
        return [
            ...
            "--exclude", ",".join(["vendor", "node_modules", "storage", "public"]),
            "--format", "json",
            "/app"
        ]
