...
class OptimizationSuggester:
    ...
    def _suggest_from_complexity(self, metrics: ComplexityMetrics, parsed: exp.Expression) -> List[OptimizationSuggestion]:
    ...
    def _suggest_from_antipatterns(self, antipatterns: List[AntipatternIssue]) -> List[OptimizationSuggestion]:
    ...
    def _suggest_query_rewriting(self, parsed: exp.Expression) -> List[OptimizationSuggestion]:
        ...
        for union in parsed.find_all(exp.Union):
            if not union.args.get("distinct") is False:  # Default UNION is DISTINCT
                suggestions.append(OptimizationSuggestion(
                    category="rewriting",
                    priority="low",
