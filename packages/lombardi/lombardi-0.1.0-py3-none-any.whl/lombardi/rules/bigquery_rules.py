...
class BigQueryRules:
    ...
    def _check_partitioning_opportunities(self, parsed: exp.Expression) -> List[OptimizationSuggestion]:
            ...
            for comparison in where.find_all((exp.GT, exp.GTE, exp.LT, exp.LTE, exp.EQ)):
                ...
                if isinstance(left, exp.Column):
                    ...
                    if isinstance(right, exp.Literal):
                        ...
                        if any(pattern in value.lower() for pattern in ['date', 'timestamp', '-', '/']):
    ...
    def _check_standard_sql_usage(self, parsed: exp.Expression) -> List[OptimizationSuggestion]:
        ...
        has_structs = any(
            "struct" in str(node).lower()
            for node in parsed.find_all(exp.DataType)
        )
