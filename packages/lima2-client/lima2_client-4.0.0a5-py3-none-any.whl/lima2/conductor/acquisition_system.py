...
from lima2.conductor.tango.utils import DeviceError, TangoDevice
...
class NoCurrentPipeline(RuntimeError):
    """The current pipeline is requested but prepare() hasn't been called yet."""


class PipelineNotFound(RuntimeError):
    """A pipeline with the specified UUID cannot be found."""


class CommandInProgress(RuntimeError):
    """Another user command is being processed."""


class InvalidCommandInState(RuntimeError):
    """The requested command is forbidden in the current state."""


class CommandFailed(RuntimeError):
    """An exception was thrown by a device during a user command."""
...
class AcquisitionSystem:
        ...
        def decorator(method: DecoratedFunc) -> DecoratedFunc:
            async def wrapper(self, *args, **kwargs):  # type: ignore
                if self.user_lock.locked():
                    raise CommandInProgress("Another command is in progress.")
    ...
    async def prepare(
        ...
        Raises:
          ValidationError: a set of params does not fit its schema.
          CommandFailed: an error occurred on a device during the tango command.
        """
    ...
    async def abort(self) -> None:
        ...
        """
        logger.info("Stop all running devices and enter FAULT state")
    ...
    async def get_pipeline(self, uuid: Literal["current"] | str | UUID) -> Pipeline:
        ...
        Raises:
          NoCurrentPipeline: uuid == "current" but conductor has no current pipeline
          PipelineNotFound: pipeline uuid not found in list of pipelines
        """
        ...
        if uuid == "current":
            if self.current_pipeline is not None:
                ...
                raise NoCurrentPipeline("No current pipeline: call prepare first")
