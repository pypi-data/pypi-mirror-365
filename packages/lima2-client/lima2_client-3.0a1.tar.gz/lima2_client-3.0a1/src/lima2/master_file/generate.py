...
class MasterFileTask:
    def __init__(self, task, end_event):
        self.task = task
...
def check_generation(
    ctrl_params: dict, recv_params: dict, proc_params: dict
) -> list[dict]:
...
def enable_generation(
    ...
    def master_file_task(src, params) -> None:
        try:
            ...
            logger.error("Error generating %s master file: %s", src, e)
            traceback.print_exception(*sys.exc_info())

    tasks = {}

    for src, params in generate_params.items():
        task = gevent.spawn(master_file_task, src, params)
        tasks[src] = MasterFileTask(task, end_event)

        def cleanup(task):
            if all([t.ready() for t in tasks.values()]):
                active_tasks.pop(uuid)

        task.link(cleanup)

    active_tasks[uuid] = tasks
