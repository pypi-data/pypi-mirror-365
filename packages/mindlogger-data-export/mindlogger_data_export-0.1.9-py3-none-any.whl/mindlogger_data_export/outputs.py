...
class WideFormat(Output):
    ...
    def _pivot_text(df: pl.DataFrame, option_scores: pl.DataFrame) -> pl.DataFrame:
        ...
        return (
            ...
            .rename(
                lambda s: s.removeprefix("response_response__") + "__response"
                if s.startswith("response_response__")
                else s.removeprefix("response_value__")
                if s.startswith("response_value__")
                else s
            )
        )
