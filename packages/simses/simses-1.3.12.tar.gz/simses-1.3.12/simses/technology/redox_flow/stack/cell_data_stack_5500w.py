...
class CellDataStack5500W(StackModule):
    ...
    def __init__(self, electrolyte_system: ElectrolyteSystem, voltage: float, power: float,
        ...
        SHUNT_FILE = redox_flow_data_config.cell_stack_shunt_current
        self.__shunt_current = pd.read_csv(SHUNT_FILE, header=None)  # A
        shunt_current_mat = self.__shunt_current.iloc[1:, 1:]
        soc_arr = self.__shunt_current.iloc[1:, 0]
        current_arr = self.__shunt_current.iloc[0, 1:]
        self.__shunt_interp2d = RegularGridInterpolator((soc_arr, current_arr), np.array(shunt_current_mat))
        internal_resistance = pd.read_csv(self.__RINT_FILE)  # Ohm
        soc_rint_arr = internal_resistance.iloc[:, self.__SOC_IDX]
        temp_rint_arr = internal_resistance.iloc[:3, self.__TEMP_IDX]
        if self.__2D_RINT:
            rint_mat_ch = internal_resistance.iloc[:, 2:5]
            rint_mat_dch = internal_resistance.iloc[:, 5:]
            self.__rint_ch_inter2d = RegularGridInterpolator((soc_rint_arr, temp_rint_arr), np.array(rint_mat_ch))
            self.__rint_dis_inter2d = RegularGridInterpolator((soc_rint_arr, temp_rint_arr), np.array(rint_mat_dch))
        else:
            rint_mat_ch = internal_resistance.iloc[:, 4]
            rint_mat_dch = internal_resistance.iloc[:, 7]
            self.__rint_ch_inter1d = scipy.interpolate.interp1d(soc_rint_arr, rint_mat_ch, kind='linear',
                                                                fill_value='extrapolate')
            self.__rint_dis_inter1d = scipy.interpolate.interp1d(soc_rint_arr, rint_mat_dch, kind='linear',
                                                                 fill_value='extrapolate')
    ...
    def get_internal_resistance(self, redox_flow_state: RedoxFlowState) -> float:
        ...
        if redox_flow_state.is_charge:
            ...
            if self.__2D_RINT:
                resistance = self.__rint_ch_inter2d((soc, temp))
            else:
            ...
            if self.__2D_RINT:
                resistance = self.__rint_dis_inter2d((soc, temp))
            else:
