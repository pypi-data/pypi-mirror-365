    ...
    def export(self, spans: list[ReadableSpan]) -> SpanExportResult:
        ...
        try:
            ...
            for span in spans:
                ...
                logger.info(f"{baggage.get_all()}")
                try:
                    logger.info(f"workflow_id: {baggage.get_baggage('workflow_id')}")
                    if workflow_id := baggage.get_baggage("workflow_id"):
                        current_baggage["workflow_id"] = workflow_id

                    logger.info(f"spinal_user_context: {baggage.get_baggage('spinal_user_context')}")
                    if user_context := baggage.get_baggage("spinal_user_context"):
                        current_baggage["spinal_user_context"] = user_context

                    logger.info(f"aggregation_id: {baggage.get_baggage('aggregation_id')}")
                    if aggregation_id := baggage.get_baggage("aggregation_id"):
                        current_baggage["aggregation_id"] = aggregation_id

                except Exception as e:
                    logger.debug(f"Could not extract baggage context: {e}")

                logger.info(f"current_baggage: {current_baggage}")
                ...
                logger.info(f"combined_attributes: {combined_attributes}")
                span_dict = {
                    "name": span.name,
...
def spinal_add_context(*, workflow_id: typing.Union[int, str, uuid.UUID], user_id: typing.Union[int, str, uuid.UUID], aggregation_id: typing.Union[int, str, uuid.UUID] = None):
    """
    ...
    if aggregation_id:
        baggage_to_add["aggregation_id"] = str(aggregation_id)
