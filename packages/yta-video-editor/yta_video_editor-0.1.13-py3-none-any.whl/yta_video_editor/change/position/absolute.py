...
class PositionAbsoluteDefault(PositionAbsoluteChange):
    ...
    def positions(
        ...
        if not hasattr(self, '_positions'):
            self._positions = [(DEFAULT_SCENE_WIDTH / 2, DEFAULT_SCENE_HEIGHT / 2)] * get_number_of_frames(self.duration, self.fps)
...
class PositionAbsoluteFromAtoB(PositionAbsoluteChange):
    ...
    def positions(
        ...
        if not hasattr(self, '_positions'):
            ...
            x_values = Progression(self.origin[0], self.destination[0], get_number_of_frames(self.duration, self.fps)).values
            y_values = Progression(self.origin[1], self.destination[1], get_number_of_frames(self.duration, self.fps)).values
