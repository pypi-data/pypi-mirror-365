...
class VideoEditor:
    ...
    def process_test(
        ...
        # This 'positions' array should be modified by
        # all the effects we apply
        positions = [(0, 0)]
        ...
        from yta_video_editor.change.position.offset import PositionOffsetShake

        video_position = PositionChange(
            position_absolute = PositionAbsoluteFromAtoB(self.video.duration, self.video.fps, (0, 0), (0, 0)),
            position_offsets = [
                PositionOffsetShake(self.video.duration, self.video.fps, 8)
            ]
        )
        ...
        from yta_video_editor.change.rotation.offset import RotationOffsetFlipRandomly

        video_rotation = RotationChange(
            rotation_absolute = RotationAbsoluteSpinXTimes(self.video.duration, self.video.fps, 2),
            rotation_offsets = [
                RotationOffsetFlipRandomly(self.video.duration, self.video.fps)
            ]
        )

        # I can precalculate the positions and then
        # pass to the method that applies the array
        positions = [
            video_position.get_position(t)
            for t in T.get_frame_time_moments(self.video.duration, self.video.fps)
        ]

        rotations = [
            video_rotation.get_rotation(t)
            for t in T.get_frame_time_moments(self.video.duration, self.video.fps)
        ]

        self._video = self.video.rotated(lambda t: rotations[T.frame_time_to_frame_index(t)])
        # Put black background to be able to position
        # or it will not work
        # I can calculate the values in the moment or
        # precalculate them and pass the array
        put_video_over_black_background(self.video, lambda t: positions[T.frame_time_to_frame_index(t)]).write_videofile(filename)
        #put_video_over_black_background(self.video, lambda t: video_position.get_position(t)).write_videofile(filename)
