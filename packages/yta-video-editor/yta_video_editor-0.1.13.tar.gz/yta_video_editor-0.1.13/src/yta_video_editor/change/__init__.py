...
class Changes:
    ...
    def add(
        ...
        change: Union[PositionAbsoluteChange, PositionOffsetChange, ResizeAbsoluteChange, ResizeOffsetChange, RotationAbsoluteChange, RotationOffsetChange]
    ):
        """
        ...
        """
        ParameterValidator.validate_mandatory_subclass_of('change', change, [PositionAbsoluteChange, PositionOffsetChange, ResizeAbsoluteChange, ResizeOffsetChange, RotationAbsoluteChange, RotationOffsetChange])
    ...
    def apply(
        ...
        # after resize
        video_sizes = [
            (video.w * resizes[i], video.h * resizes[i])
        ...
        for i in range(len(self.rotations)):
            video_sizes[i] = get_rotated_image_size(video_sizes[i], rotations[i])[0]
        ...
        positions = [
            (
                self.positions[i][0] - video_sizes[0] / 2,
                self.positions[i][1] - video_sizes[1] / 2
            )
            for i in range(len(self.positions))
        ]

        video = video.resized(lambda t: resizes[T.frame_time_to_frame_index(t, video.fps)])
        video = video.rotated(lambda t: rotations[T.frame_time_to_frame_index(t, video.fps)])
