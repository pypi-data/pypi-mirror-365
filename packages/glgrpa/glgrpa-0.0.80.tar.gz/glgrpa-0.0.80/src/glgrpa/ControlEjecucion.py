...
class ControlEjecucion(Windows, Terminal):
    ...
    def __init__(self, 
        ...
        # Ruta del archivo de estado en el directorio de trabajo actual
        self.archivo_estado = os.path.join(os.getcwd(), "estado_ejecucion.json")
    ...
    def ejecutar_con_reintentos(self, funcion_principal: Callable, *args, **kwargs) -> bool:
        """
        Ejecuta una función con control de reintentos y gestión de estado.
        ...
        for intento in range(1, self.intentos_maximos + 1):
            self.mostrar(f"🔄 Intento {intento}/{self.intentos_maximos}")
            
            try:
                # Actualizar estado antes del intento
                estado["intentos_realizados"] = intento
                estado["intentos_diarios"] = intento
                ...
                if resultado:
                    ...
                    self.mostrar(f"✅ Ejecución exitosa en intento {intento}")
                    ...
                    error_msg = f"Función principal retornó False en intento {intento}"
                ...
                try:
                    ...
                    screenshot_nombre = f"error_{self.nombre_script}_intento_{intento}_{timestamp}"
            ...
            # Guardar estado después del intento fallido
            self.guardar_estado_ejecucion(estado)
            
            # Delay entre intentos (más corto en modo dev)
            if intento < self.intentos_maximos:
                delay = 2 if self.dev else 10
                self.mostrar(f"⏱️  Esperando {delay}s antes del siguiente intento...")
                import time
                time.sleep(delay)
        
        # Se agotaron todos los intentos
        self.mostrar(f"❌ Todos los intentos fallaron ({self.intentos_maximos}/{self.intentos_maximos})")
        
        # Enviar email de fallo final
        self.enviar_notificacion_email(
            exitoso=False,
            error_msg=estado.get("ultimo_error"),
            intentos_realizados=self.intentos_maximos
        )
    ...
    def decorador_ejecucion_controlada(self, 
        ...
        def decorador(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                return control.ejecutar_con_reintentos(func, *args, **kwargs)
