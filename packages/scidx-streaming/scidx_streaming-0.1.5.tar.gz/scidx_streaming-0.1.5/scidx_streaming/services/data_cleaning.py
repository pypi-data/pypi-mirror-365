...
def eval_condition(condition, df):
    ...
    # Detect if value is a column or a literal
    value_clean = value.strip("'\"")
    if value_clean in df.columns:
        comparison_target = df[value_clean]
    else:
        try:
            comparison_target = pd.to_numeric(value, errors='raise')
        except ValueError:
            comparison_target = value.strip("'\"")
            df[field] = df[field].astype(str)
    
    comparison_map = {
        '>=': df[field] >= comparison_target,
        '>': df[field] > comparison_target,
        '<=': df[field] <= comparison_target,
        '<': df[field] < comparison_target,
        '!=': df[field] != comparison_target,
