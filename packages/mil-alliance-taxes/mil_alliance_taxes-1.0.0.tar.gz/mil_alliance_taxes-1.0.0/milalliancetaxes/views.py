...
@login_required
@permission_required("basic_access.basic_access")
...
def view_tax(request, year_month):
    ...
    for corporation_id in corporation_ids:
        ...
        logger.info(f"Retrieving the monthly tax value for the corp {corp.name} under the alliance {MILALLIANCETAXES_ALLIANCE_ID}")
...
def untrack_corporation(request, corporation_id):
    ...
    messages.success(
        request,
        format_html(
            "Taxes for <strong>{}</strong> will no longer be tracked.",
            corporation_id,
        ),
    )
    return redirect('milalliancetaxes:admin_panel')
