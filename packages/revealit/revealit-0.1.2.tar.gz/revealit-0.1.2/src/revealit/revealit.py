...
def print_candidates(
    candidates=None, num_peaks_array=None, spatial_bin_size=None, how_many=5
):
    for candidate, num_peaks in zip(candidates[:how_many], num_peaks_array[:how_many]):
        candidate_coo = (candidate + 0.5) * spatial_bin_size
        candidate_coo = candidate_coo.astype(int)
        print(
            f"Candidate at {candidate} bin ({candidate_coo} pixel), number of peaks meeting criteria = {num_peaks}"
        )
