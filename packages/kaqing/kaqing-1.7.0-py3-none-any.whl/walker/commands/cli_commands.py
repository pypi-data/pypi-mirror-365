...
class CliCommands:
    cached_values_by_sts_pod = {}

    def values(state: ReplState, collapse = False):
        key = f'{state.sts}:{state.pod}:{collapse}'
        if key in CliCommands.cached_values_by_sts_pod:
            return CliCommands.cached_values_by_sts_pod[key]
        ...
        CliCommands.cached_values_by_sts_pod[key] = d
