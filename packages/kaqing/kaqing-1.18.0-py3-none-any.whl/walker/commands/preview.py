...
class Preview(Command):
    ...
    def __new__(cls, *args, **kwargs):
        if not hasattr(cls, 'instance'): cls.instance = super(Preview, cls).__new__(cls)
    ...
    def completion(self, state: ReplState):
        if state.device == ReplState.P:
            if tables := Preview.pg_tables(state.namespace, state.pg_path):
                return {Preview.COMMAND: {db["name"]: None for db in tables if db["schema"] == PostgresSession.default_schema()}}
        else:
            if state.pod:
                tables = Preview.cql_tables(state)
                return {Preview.COMMAND: {f'{k}.{t}': None for k, ts in tables.items() for t in ts}}

        return {}

    def help(self, _: ReplState):
        return f'{Preview.COMMAND} TABLE\t preview table'
