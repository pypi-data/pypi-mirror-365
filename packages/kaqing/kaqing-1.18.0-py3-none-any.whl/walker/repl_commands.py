...
from walker.commands.preview import Preview
...
class ReplCommands:
    ...
    def navigation() -> list[Command]:
        return [Ls(), Preview(), Cd(), Pwd(), ClipboardCopy(), GetParam(), SetParam()] + Show.cmd_list()

    def cassandra_check() -> list[Command]:
        return [Check(), Issues(), NodeTool(), Processes(), Report(), Status(), Storage()]

    def cassandra_ops() -> list[Command]:
        return [Restart(), RollOut(), Watch()] + Reaper.cmd_list() + Repair.cmd_list()
