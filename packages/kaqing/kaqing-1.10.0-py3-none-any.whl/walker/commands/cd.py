...
from walker.k8s_utils.sts import StatefulSets
    ...
    def run(self, cmd: str, state: ReplState):
        ...
        for dir in arg.split('/'):
            if state.device == ReplState.P:
                state.pg_path = PostgresSession(state.namespace, state.pg_path).directory(arg)
            else:
                if dir == '..':
                    if state.pod:
                        state.pod = None
                    ...
                    if not state.sts:
                        ...
                        if p:
                            ...
                            names = StatefulSets.pod_names_by_host_id(state.sts, state.namespace);
                            if dir in names:
                                state.pod = names[dir]
                            else:
    ...
    def completion(self, state: ReplState):
        if state.device == ReplState.P:
            ...
            if pg and pg.db:
                ...
                return {Cd.COMMAND: {p: None for p in PostgresSession.hosts(state)}}
        else:
            if state.pod:
                return {Cd.COMMAND: {'..': None}}
            elif state.sts:
