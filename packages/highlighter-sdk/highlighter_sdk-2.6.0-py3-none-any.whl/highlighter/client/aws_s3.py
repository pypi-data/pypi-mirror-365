...
def upload_large_file_threaded_mem(
    ...
    def upload_worker(part_n: int, data: IO):
        with data:
            ...
            resp = requests.put(signed_url, data=data.read(part_size), timeout=300)
        if not resp.ok:
            return None
...
def upload_file_to_s3_io(
    ...
    response = requests.post(presigned_url.url, files=files, data=presigned_url.fields, timeout=300)
