...
class TableProcessor(BaseProcessor):
    ...
    def _process_xl_table(cls, xml: str) -> str:
        def process_table(match):
            ...
            # 提取 cols 属性
            cols_match = re.search(r'cols="([^"]*)"', attrs)
            if cols_match:
    ...
    def decompile_tbl(cls, xml: str) -> str:
        def process_word_table(match):
            ...
            if grid_match:
                ...
                if col_widths:
                    cols_str = '/'.join(col_widths)
                    styles['cols'] = cols_str # 将列宽设置添加到styles字典
            ...
            cols_attr = f' cols="{cols_str}"' if cols_str else ""
            return f'<xl-table style="{style_str}"{cols_attr}>{content}</xl-table>'
    ...
    def decompile_tblgrid(cls, xml: str) -> str:
        ...
        def process_tblgrid(match):
            ...
            if col_widths:
                cols_str = '/'.join(col_widths)
                return f'<xl-table cols="{cols_str}"/>'
