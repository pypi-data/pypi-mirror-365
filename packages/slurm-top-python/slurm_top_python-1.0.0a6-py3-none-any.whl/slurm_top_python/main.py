...
from slurm_top_python.constants import SENSOR_CONFIG_PATH, GENERAL_CONFIG_PATH
from huepy import *
...
def _update():
    ...
    '''
    try:
        if GENERAL_CONFIG["check-for-updates"]:
...
def main():
    try:
        ...
        csrt = (1000 if SENSOR_CONFIG["csrt"] >
                1000 else SENSOR_CONFIG["csrt"])
        dsrt = (1000 if SENSOR_CONFIG["dsrt"] >
                1000 else SENSOR_CONFIG["dsrt"])
        msrt = (1000 if SENSOR_CONFIG["msrt"] >
                1000 else SENSOR_CONFIG["msrt"])
        psrt = (1000 if SENSOR_CONFIG["psrt"] >
                1000 else SENSOR_CONFIG["psrt"])
        ssrt = (1000 if SENSOR_CONFIG["ssrt"] >
                1000 else SENSOR_CONFIG["ssrt"])
        ...
        # theme = getattr(sys.modules[npyscreen.Themes], GENERAL_CONFIG["theming"]["current-theme"])
    ...
    # catch the kill signals here and perform the clean up
        ...
        if GENERAL_CONFIG["logging"]["clear-log-on-error"]:
            with open(_log_file, 'w'):
                pass
