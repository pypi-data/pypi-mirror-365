...
%%
function [obj,Gamma,Omega,G,Theta,X,Y,info,runhist] = DNNLasso_old(Q,P,lamt,lams,OPTIONS,Gamma0,Omega0,G0,Theta0,X0,Y0)
if nargin < 5
    OPTIONS = [];
end
maxiter = 10000;
...
for iter = 1:maxiter
    ...
    obj(1) = - tmp + sum(sum(Omega.*P)) + sum(sum(Gamma.*Q)) + ...
        lamt*(sum(sum(abs(Gamma)))-sum(abs(diag(Gamma)))) + lams*(sum(sum(abs(Omega)))-sum(abs(diag(Omega))));
