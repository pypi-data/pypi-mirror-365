...
if sys.version_info < (3, 8):
    ...
    def tuplehash(collection):
        # type: (Collection) -> int

        remaining_length = PY_SSIZE_T(len(collection) - 1)  # type: PY_SSIZE_T
        ...
        return PY_HASH_T(x).promoted_to_int
else:
    # Pre-initialized magic numbers
    # To prevent repetitive construction of immutable objects
    ...
    def tuplehash(collection):
        # type: (Collection) -> int
        length = PY_SSIZE_T(len(collection))  # type: PY_SSIZE_T
        ...
        return PY_HASH_T(acc).promoted_to_int
