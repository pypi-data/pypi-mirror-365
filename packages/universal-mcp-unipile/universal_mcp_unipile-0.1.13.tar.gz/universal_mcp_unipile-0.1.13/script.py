...
integration = AgentRIntegration(name="unipile", client=AgentrClient(api_key="sk_416e4f88-3beb-4a79-a0ef-fb1d2c095aee", base_url="https://api.agentr.dev"))
app_instance = UnipileApp(integration=integration)
tool_manager = ToolManager()

tool_manager.add_tool(app_instance.search)
tool_manager.add_tool(app_instance.retrieve_profile)
...
async def main():
    ...
    result = await tool_manager.call_tool(
        name="search",
        arguments={
            "account_id": "MwY0Hzs-RT-zFNHwJAislQ",
            "keywords": "Rishabh Raj",
            "category": "people",
            "past_company": ["100723307"]
        }
    )
    
    # Test retrieve_profile tool
    profile_result = await tool_manager.call_tool(
