...
class SamudraAI:
    def __init__(self, time_seq: int = 9, lstm_units: int = 64, learning_rate: float = 1e-4, output_shape: tuple = None):
    ...
    def fit(self, x_data_hist: xr.DataArray, y_data_obs: xr.DataArray, epochs: int = 100, batch_size: int = 8, test_size: float = 0.2, seed: int = 42):
        ...
        if self.output_shape is None:
            self.output_shape = y_train.shape[2:4]  # ambil dari obs_ref

        self.model = build_cnn_bilstm(input_shape, self.output_shape, self.lstm_units)
    ...
    def correction(self, data_to_correct: xr.DataArray, save_path: str = None) -> xr.DataArray:
        ...
        if flat_pred.shape[1] != self.scaler_y.scale_.shape[0]:
            raise ValueError(f"Mismatch antara shape pred ({flat_pred.shape}) dan scaler_y ({self.scaler_y.scale_.shape})")

        pred_original = self.scaler_y.inverse_transform(flat_pred).reshape((pred_scaled.shape[0], *self.output_shape))
