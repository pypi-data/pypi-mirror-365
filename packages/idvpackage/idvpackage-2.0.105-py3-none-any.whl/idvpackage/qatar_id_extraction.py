...
def verify_qatar_id_info(name='', name_ar='', nationality='',id_number='', dob='', expiry_date='',occupation='',occupation_en=''):
    if occupation_en=='':
...
def qatar_id_info_chain(ocr_text, openai_key):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "Extract the relevant information, if not explicitly provided do not guess, leave empty string. Extract partial info. Translate where explicity stated."
         ),
        ("user", "{ocr_text}")
    ])
