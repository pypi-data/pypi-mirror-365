...
def verify_lbn_passport(ocr_text):
    function_spec = convert_pydantic_to_openai_function2(Verify_LBN_Passport)
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "user",
                "content": f"Verify if the following text represents a Lebanese Passport: {ocr_text}. Return True if it is a Lebanese Passport, return False otherwise."
            }
        ],
    ...
    if "function_call" in message:
        ...
        try:
            return Verify_LBN_Passport(**args)
...
# x = verify_lbn_passport(ocr_text)
# print(x)
