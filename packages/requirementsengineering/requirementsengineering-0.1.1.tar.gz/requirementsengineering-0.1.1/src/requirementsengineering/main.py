...
def prompt_for_multiple_subfolders(area_desc):
    subfolders = {}
    print(f"\nAdd subfolders for {area_desc}. Type 'q' to stop adding.")
    while True:
...
def prompt_resources_folders():
    selected = {}
    print("\nAdd resource folders. Type 'q' to stop adding.")
    while True:
        answer = input("Do you want to add a resource folder? (y/n/q): ").strip().lower()
        if answer == "q":
            break
        if answer == "y":
