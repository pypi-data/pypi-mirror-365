...
from resource_manager import ID, NAME, ConfigManager, DBView, ResourceDB, ResourceDBFactory, ResourceRecord
...
class MMDetectionCommand:  
    train_code_file_path:Path = env_config.train_code_path
    test_code_file_path:Path = env_config.test_code_path
    ...
    def get_command(self, task_type:Literal["train", "eval", "test"], relative:bool = True)->Path:
        if task_type == "train":
            ...
            path = self.test_code_file_path
        if relative:
            return path.relative_to(env_config.project_root)
        else:
            return path
...
class TaskRecord(ResourceRecord):
    ...
    def make_run_command(self, relative:bool = True)->str:
        ...
        if relative:
            main_config_file_path = main_config_file_path.relative_to(env_config.project_root)
    ...
    def get_dataset_config(self, dataset_id:ID)->tuple[Path, Path]:
        from .dataset_resource import DatasetResourceFactory
