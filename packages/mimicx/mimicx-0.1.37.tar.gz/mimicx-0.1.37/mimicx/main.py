...
class Model:
        ...
        if self.model is None and not self.model_loaded:
            ...
            if IS_PYODIDE:
                ...
                try:
                    ...
                    loop = asyncio.get_running_loop()
                    # If we're in an async context, we should still try sync loading
                    # as a fallback for .py files
                    print("Event loop detected - trying synchronous fallback for .py files")
                    try:
                        # Try sync loading with Python files only
                        success = self._load_python_only_sync(self.algorithm)
                        if success and self.model is not None:
                            self.model_loaded = True
                        else:
                            # If sync loading failed, suggest async loading
                            raise AttributeError(f"Model not loaded and attribute '{attr}' not found. "
                                               f"Sync fallback failed. Please use 'await model.load_async(\"{self.algorithm}\")' "
                                               f"or 'await model.ensure_loaded()' for full WASM support.")
                    except Exception as e:
                    ...
                    # No event loop running, we can try full sync loading
                    try:
                        self.load_model_file_sync(self.algorithm)
                        if self.model is not None:
                            self.model_loaded = True
    ...
    def _load_python_only_sync(self, model):
        """Synchronous loading that only tries Python files (skip WASM)"""
        ...
        # Only try .py extension for sync fallback in Pyodide
        extensions = ['.py']
        print(f"Trying Python-only sync loading with extensions: {extensions}")
