...
class Model:
    ...
    def __getattr__(self, attr):
        ...
        if self.model is None and not self.model_loaded:
            ...
            # Strategy 1: Force Python-only loading in Pyodide
            if IS_PYODIDE:
                print("Pyodide detected - attempting Python-only sync loading")
                try:
                    success = self._force_python_sync_load(self.algorithm)
                    if success and self.model is not None:
                        self.model_loaded = True
                        print(f"Model '{self.algorithm}' loaded successfully via Python-only sync")
                except Exception as e:
                    print(f"Python-only sync loading failed: {e}")
                    last_error = e
                    success = False
            
            # Strategy 2: Try standard sync loading if Python-only failed or not in Pyodide
            if not success:
                print("Attempting standard sync loading")
                try:
                    success = self.load_model_file_sync(self.algorithm)
                    if success and self.model is not None:
                        self.model_loaded = True
                        print(f"Model '{self.algorithm}' loaded successfully via standard sync")
                except Exception as e:
                    print(f"Standard sync loading failed: {e}")
                    last_error = e
                    success = False
            
            # Strategy 3: Try direct model setting (if file already exists)
            if not success:
                print("Attempting direct model setting")
                try:
                    success = self.set_model(self.algorithm)
                    if success and self.model is not None:
                        self.model_loaded = True
                        print(f"Model '{self.algorithm}' loaded successfully via direct setting")
                except Exception as e:
                    print(f"Direct model setting failed: {e}")
                    last_error = e
                    success = False
            
            # If all strategies failed
            if not success or self.model is None:
                error_msg = f"Failed to load model '{self.algorithm}' for attribute '{attr}'. "
                if last_error:
                    error_msg += f"Last error: {str(last_error)}. "
                if IS_PYODIDE:
                    error_msg += f"Try 'await model.load_async(\"{self.algorithm}\")' for full WASM support."
                else:
                    error_msg += "Please check that the algorithm exists and is properly named."
                raise AttributeError(error_msg)
        
        # If model is still None after loading attempt, raise clear error
        if self.model is None:
            raise AttributeError(f"Model '{self.algorithm}' failed to load completely. "
                               f"Cannot access attribute '{attr}'. "
                               f"Please verify the algorithm name and availability.")
        
        # Check if the attribute exists on the model
        if hasattr(self.model, attr):
            return getattr(self.model, attr)
        else:
            raise AttributeError(f"Model '{type(self.model).__name__}' loaded successfully "
                               f"but has no attribute '{attr}'")
    ...
    def _download_file_pyodide_sync(self, url, file_path):
        ...
        try:
            # In Pyodide, we can try using the built-in open function with URL
            # This is a fallback method that sometimes works
            print("Attempting Pyodide-compatible download...")
            
            # Try using requests-like functionality if available
            if hasattr(pyodide, 'http'):
                print("Trying pyodide.http for synchronous download...")
                # This is a simplified approach - real implementation would need async
                # For now, we'll fall back to suggesting async loading
                raise Exception("Pyodide synchronous HTTP download not supported. Use async loading.")
            
            # Alternative: Try using the browser's fetch in a blocking way
            ...
            print(f"Pyodide download method failed: {e}")
            print("Falling back to suggesting async loading...")
            return False
