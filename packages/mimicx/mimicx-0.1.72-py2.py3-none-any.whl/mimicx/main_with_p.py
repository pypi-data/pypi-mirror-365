    ...
    def __getattr__(self, attr):
        ...
        if IS_PYODIDE and self.api_client:
            ...
            def api_method(*args, **kwargs):
                # Create an async wrapper that can be called synchronously
                try:
                    # Check if we're already in an async context
                    loop = asyncio.get_running_loop()
                    # If we're in an async context, we need to handle this differently
                    if loop.is_running():
                        # Create a task that will be awaited by the caller
                        return self.api_client.call_api(attr, *args, **kwargs)
                    else:
                        # Run the async call in a new loop
                        return asyncio.run(self.api_client.call_api(attr, *args, **kwargs))
                except RuntimeError:
                    # No event loop, create one
                    return asyncio.run(self.api_client.call_api(attr, *args, **kwargs))
            
            # For async methods, provide both sync and async versions
            if attr.endswith('_async'):
                async def async_api_method(*args, **kwargs):
                    return await self.api_client.call_api(attr, *args, **kwargs)
                return async_api_method
            else:
                # Also provide an async version
                async def async_version(*args, **kwargs):
                    return await self.api_client.call_api(attr, *args, **kwargs)
                
                # Add async version as an attribute
                setattr(self, f"{attr}_async", async_version)
...
# Example usage:
"""
# Usage in Pyodide environment:
model = Model("nlp/sentiment_analysis")

# These calls will be redirected to API endpoints:
# Synchronous call (will be converted to async internally)
result = model.predict("This is a great product!")

# Explicit async call
result = await model.predict_async("This is a great product!")

# Or using the generic async caller
result = await model.call_async("predict", "This is a great product!")

# All method calls like train, evaluate, preprocess, etc. will work the same way
training_result = model.train(data, epochs=10)
evaluation = model.evaluate(test_data)
preprocessed = model.preprocess(raw_data)
"""