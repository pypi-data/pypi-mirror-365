...
class Model:
            ...
            if IS_PYODIDE:
                ...
                try:
                    ...
                    if 'localhost' in current_origin or '127.0.0.1' in current_origin:
                        ...
                        self.api_base_url = 'https://api.mimicx.ai'
                    else:
    ...
    def _sync_api_call(self, method_name: str, *args, **kwargs):
        ...
        try:
            ...
            try:
                ...
                # Try alternative approach with fetch API converted to sync
                return self._fallback_fetch_sync(endpoint, request_data, method_name)
    ...
    def _fallback_fetch_sync(self, endpoint: str, request_data: dict, method_name: str):
        """Fallback method using fetch API with promise conversion"""
        try:
            import js
            import json
            from pyodide.ffi import to_js
            
            #print(f"Trying fallback fetch for: {endpoint}")
            ...
            # This is still async, so we'll return a message
            return f"Fallback attempted for {method_name}. Use async version: await model.{method_name}_async(...)"
            
        except Exception as e:
            print(f"Fallback fetch failed: {e}")
            return f"Connection Error: Both XHR and fetch failed for {method_name}. Server may not be properly configured for CORS."
