...
class Model:
    ...
    def __init__(self, domainAlgorithm, api_base_url=None):
        ...
        if api_base_url:
            ...
            print(f"Using custom API URL: {self.api_base_url}")
        else:
            # Default API URLs based on environment
            if IS_PYODIDE:
                # For development/testing, try to detect from the server logs you showed
                # Since your server shows IP 95.178.109.242, it seems like it's accessible from internet
                # Let's try to auto-detect or provide a sensible default
                try:
                    import js
                    current_origin = js.window.location.origin
                    print(f"Current page origin: {current_origin}")
                    ...
                    if 'localhost' in current_origin or '127.0.0.1' in current_origin:
                        ...
                        # Try to use the same host but different port
                        hostname = js.window.location.hostname
                        # Based on your server logs, you might need to use the external IP
                        # For now, let's try localhost first, then provide override option
                        self.api_base_url = 'http://localhost:5000'
                    
                    print(f"Auto-detected API URL: {self.api_base_url}")
                except Exception as e:
                    print(f"Auto-detection failed: {e}")
                    # Fallback - use localhost for development
                    self.api_base_url = 'http://localhost:5000'
                    print(f"Using fallback API URL: {self.api_base_url}")
            else:
                self.api_base_url = 'http://localhost:5000'
    ...
    def __getattr__(self, attr):
        ...
        if IS_PYODIDE and self.api_client:
            ...
            def sync_api_method(*args, **kwargs):
                ...
                # Try sync first, but if it fails due to CORS, provide async alternative
                result = self._sync_api_call(attr, *args, **kwargs)
                
                # If sync failed due to CORS, suggest async alternative
                if isinstance(result, str) and ("CORS Error" in result or "Connection Error" in result):
    ...
    def _sync_api_call(self, method_name: str, *args, **kwargs):
        ...
        try:
            ...
            # Try to reduce payload size for large requests
            payload = json.dumps(request_data)
            payload_size = len(payload)
            print(f"Payload size: {payload_size} bytes")
            
            # For large payloads, use a more robust approach
            if payload_size > 100000:  # 100KB threshold
                print("Large payload detected, using enhanced sync method...")
                return self._enhanced_sync_call(endpoint, request_data, method_name)
            
            # Create synchronous XMLHttpRequest with proper settings
            ...
            # Set headers properly for CORS and large data
            ...
            if xhr.status == 200:
                try:
                    ...
                    if isinstance(result, dict) and 'success' in result and result['success']:
                        ...
                        if 'result' in result:
                            ...
                            print(f"✅ API call successful: {type(deserialized_result)}")
                ...
                print("CORS or network error - trying enhanced method...")
                return self._enhanced_sync_call(endpoint, request_data, method_name)
            else:
                error_msg = f"HTTP {xhr.status}: {xhr.responseText[:200]}..."
    ...
    def _enhanced_sync_call(self, endpoint: str, request_data: dict, method_name: str):
        """Enhanced synchronous call for large payloads using fetch with setTimeout trick"""
        try:
            import js
            import json
            
            print("Using enhanced sync method for large data...")
            
            # Use a JavaScript-based synchronous approach
            js_code = f"""
            (function() {{
                var result = null;
                var error = null;
                ...
                // Make async fetch but wait for it synchronously
                fetch('{endpoint}', {{
                    method: 'POST',
                    headers: {{
                        'Content-Type': 'application/json',
                    ...
                    body: {json.dumps(json.dumps(request_data))},
                    mode: 'cors'
                }})
                .then(response => {{
                    if (response.ok) {{
            ...
            if result:
                ...
                # Check if it's a successful API response
                if isinstance(python_result, dict) and python_result.get('success'):
                    if 'result' in python_result:
                        deserialized_result = self.api_client._deserialize_result(python_result['result'])
                        print(f"✅ Enhanced sync call successful")
                        return deserialized_result
                    else:
                        return python_result
                else:
                    error_msg = python_result.get('error', 'Unknown API error')
                    return f"API Error: {error_msg}"
            else:
                return "No response from enhanced sync call"
                
        except Exception as e:
            print(f"Enhanced sync call failed: {e}")
            return f"Enhanced Sync Error: Could not complete {method_name}. {str(e)}"
