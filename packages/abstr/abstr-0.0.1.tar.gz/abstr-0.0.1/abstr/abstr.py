...
class Abstr(MasterPiece):
    ...
    def init_mqtt(self, name: str) -> None:
        ...
        if Abstr.mqtt_class_id == "":
            self.warning(
                f"Suscpicious configuration: no mqtt_class_id set for {self.name}:{self.get_class_id()}"
            )
        elif not Abstr.find_class(Abstr.mqtt_class_id):
            self.error(
                f"Couldn't create mqtt broker {Abstr.mqtt_class_id},  class not imported"
            )
        else:
            self.mqtt_client = cast(
                Optional[Mqtt], MasterPiece.instantiate(Abstr.mqtt_class_id, name)
            )
            if self.mqtt_client is not None:
                ...
                self.error(f"Couldn't create mqtt broker {Abstr.mqtt_class_id}")
    ...
    def log_message(self, type: str, msg: str, details: str = "") -> None:
        ...
        ::
            ...
            # publish info message to the Abstr's 'log' topic
