...
Run with: python -m smoke_tests.test5_direct_audio_player
"""
...
try:
    from voxstream.io.capture import DirectAudioPlayer
    from voxstream.config.types import (
        StreamConfig, AudioFormat, AudioConstants, AudioBytes
    )
...
def test_different_audio_formats():
    ...
    try:
        ...
        for rate in sample_rates:
            config = StreamConfig(sample_rate=rate)
            player = DirectAudioPlayer(config=config)
