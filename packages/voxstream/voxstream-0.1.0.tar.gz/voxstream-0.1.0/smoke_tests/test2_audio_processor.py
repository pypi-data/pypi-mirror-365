"""
Smoke tests for stream processor functionality 

Run with: python -m smoke_tests.test2_audio_processor
"""
...
try:
    from voxstream.core.processor import (
        StreamProcessor, create_processor, validate_realtime_audio, chunk_for_streaming
    )
    from voxstream.core.buffer import BufferPool, StreamBuffer
    from voxstream.config.types import (
        StreamConfig, AudioFormat, ProcessingMode, BufferConfig,
        AudioConstants
    )
    from voxstream.exceptions import AudioError
except ImportError:
    print("ERROR: Could not import stream processor. Make sure voxstream is in PYTHONPATH")
    sys.exit(1)
...
def test_processor_creation():
    ...
    tests_total += 1
    try:
        config = StreamConfig(pre_allocate_buffers=True)
        processor = StreamProcessor(config=config, mode=ProcessingMode.REALTIME)
        passed = (
    ...
    tests_total += 1
    try:
        processor = StreamProcessor(mode=ProcessingMode.QUALITY)
...
def test_audio_chunking():
    ...
    try:
        config = StreamConfig(pre_allocate_buffers=True)
        fast_processor = StreamProcessor(config=config, mode=ProcessingMode.REALTIME)
