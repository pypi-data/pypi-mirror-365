"""
VoxStream Voice Activity Detection

Optimized VAD for real-time voice streaming with minimal latency.
Pre-allocated buffers and energy-based detection for <10ms latency.
"""
...
from voxstream.config.types import AudioBytes, StreamConfig, VADConfig, VADType
import collections
...
class VADetector:
    ...
    def process_chunk(self, audio_chunk: AudioBytes) -> VoiceState:
        ...
        if energy > self.config.energy_threshold:
            ...
            if self.state == VoiceState.SILENCE:
                self.state = VoiceState.SPEECH_STARTING
            ...
            elif self.state == VoiceState.SPEECH:
                self.state_duration_ms += chunk_duration_ms
            elif self.state == VoiceState.SPEECH_ENDING:
                # Speech resumed
                self.state = VoiceState.SPEECH
                self.state_duration_ms = chunk_duration_ms
                self.transitions += 1
            ...
            if self.state in [VoiceState.SPEECH, VoiceState.SPEECH_STARTING]:
                self.state = VoiceState.SPEECH_ENDING
                self.state_duration_ms = chunk_duration_ms
                self.transitions += 1
                self.metrics.transitions += 1
                self.metrics.state_changes += 1
    ...
    def old_process_chunk(self, audio_chunk: AudioBytes) -> VoiceState:
        """
        Process audio chunk and return current state.
    ...
    def _update_state(self, is_speech: bool) -> VoiceState:
        """
        ...
        if old_state != self.state:
            ...
            # Track durations
            if old_state == VoiceState.SPEECH:
...
class AdaptiveVAD(VADetector):
    ...
    def process_chunk(self, audio_chunk: AudioBytes) -> VoiceState:
        ...
        if state == VoiceState.SILENCE and not self.is_calibrating:
