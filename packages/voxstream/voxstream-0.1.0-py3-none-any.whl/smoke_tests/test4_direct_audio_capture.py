"""
Smoke tests for audio capture functionality.

Run with: python -m smoke_tests.test4_direct_audio_capture
"""
...
try:
    ...
    from voxstream.config.types import (
        StreamConfig, AudioFormat, AudioConstants
    )
    from voxstream.exceptions import AudioError
    import sounddevice as sd
except ImportError as e:
    print(f"ERROR: Could not import required modules: {e}")
    print("Make sure voxstream is in PYTHONPATH and sounddevice is installed")
    sys.exit(1)
...
def test_capture_creation():
    ...
    tests_total += 1
    try:
        config = StreamConfig(sample_rate=24000, chunk_duration_ms=100)
...
def test_capture_start_stop():
    ...
    async def test_basic_start_stop():
        try:
            config = StreamConfig(chunk_duration_ms=50)
            capture = DirectAudioCapture(config=config)
...
def test_audio_chunk_generation():
    ...
    async def test_chunk_timing():
        try:
            chunk_ms = 50
            config = StreamConfig(chunk_duration_ms=chunk_ms)
            capture = DirectAudioCapture(config=config)
...
def test_queue_operations():
    ...
    async def test_queue_monitoring():
        try:
            capture = DirectAudioCapture(config=StreamConfig(chunk_duration_ms=10))
            audio_queue = await capture.start_async_capture()
    ...
    async def test_continuous_stream():
        try:
            capture = DirectAudioCapture(config=StreamConfig(chunk_duration_ms=20))
            audio_queue = await capture.start_async_capture()
