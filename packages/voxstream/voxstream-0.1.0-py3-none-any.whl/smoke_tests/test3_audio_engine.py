"""
Smoke tests for VoxStream engine functionality.

Run with: python -m smoke_tests.test3_audio_engine
"""
...
# Import voxstream engine
try:
    from voxstream.core.stream import (
        VoxStream, ProcessingStrategy,
        create_fast_lane_engine, create_big_lane_engine, create_adaptive_engine
    )
    from voxstream.config.types import (
        StreamConfig, StreamMetrics, ProcessingMode, BufferConfig, AudioConstants
    )
    from voxstream.exceptions import AudioError
except ImportError:
    print("ERROR: Could not import voxstream engine. Make sure voxstream is in PYTHONPATH")
    sys.exit(1)
...
def test_engine_creation():
    ...
    tests_total += 1
    try:
        engine = VoxStream(mode=ProcessingMode.REALTIME)
        passed = (
