...
class RichLoggerMiddleware(BaseHTTPMiddleware):
    def __init__(self, app: ASGIApp, get_ocr_logs: Callable[[], list]):
        """
        Middleware for dual-column logging in FastAPI using rich.

        :param app: ASGI application (FastAPI instance)
        :param get_ocr_logs: Callable returning list of tuples [(LEVEL, MESSAGE), ...]
                             Example: [("INFO", "Started OCR"), ("ERROR", "Failed to parse")]
        """
        super().__init__(app)
