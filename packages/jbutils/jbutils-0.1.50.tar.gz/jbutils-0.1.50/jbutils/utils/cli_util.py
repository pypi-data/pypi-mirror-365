...
def input_choice(*args, choices: list[str]) -> str:
    ...
    while not choice or 0 < choice or choice >= len(choices):
        print(f"Invalid selection ({choice}), enter valid selection:")
        choice = input_int(*args)
...
def input_choice_dict(prompt: str, choices: dict) -> Any:
    ...
    choice = input_int(prompt) - 1

    while not choice or choice < 0 or choice >= len(choices):
        print(f"Invalid selection ({choice}), enter valid selection:")
