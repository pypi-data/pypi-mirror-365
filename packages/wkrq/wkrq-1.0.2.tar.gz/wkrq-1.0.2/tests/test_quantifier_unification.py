...
class TestUniversalQuantifierUnification:
    ...
    def test_universal_with_multiple_constants(self):
        ...
        inference1 = parse_inference('[∀X Human(X)]Mortal(X), Human(socrates), Human(plato) |- Mortal(socrates)')
        result1 = check_inference(inference1)
        assert result1.valid, "Universal should work with first constant"

        inference2 = parse_inference('[∀X Human(X)]Mortal(X), Human(socrates), Human(plato) |- Mortal(plato)')
    ...
    def test_universal_with_negation(self):
        ...
        inference = parse_inference('[∀X Human(X)](~Immortal(X)), Human(socrates) |- ~Immortal(socrates)')
        result = check_inference(inference)
    ...
    def test_universal_contrapositive_reasoning(self):
        ...
        inference = parse_inference('[∀X Human(X)]Mortal(X), ~Mortal(socrates) |- ~Human(socrates)')
        result = check_inference(inference)
...
class TestExistentialQuantifierUnification:
    ...
    def test_basic_existential_instantiation(self):
        ...
        inference = parse_inference('[∃X Human(X)]Mortal(X), Human(socrates) |- Mortal(socrates)')
        result = check_inference(inference)
        assert not result.valid, "Existential about some human doesn't imply all humans have the property"
    ...
    def test_existential_with_multiple_witnesses(self):
        ...
        inference1 = parse_inference('[∃X Human(X)]Wise(X), Human(socrates), Human(plato) |- Wise(socrates) | Wise(plato)')
    ...
    def test_existential_witness_selection(self):
        ...
        inference = parse_inference('[∃X Human(X)]Mortal(X), Human(socrates) |- Mortal(socrates)')
        result = check_inference(inference)
        assert not result.valid, "Existential doesn't guarantee specific instance has property"
    ...
    def test_existential_contradiction(self):
        ...
        inference = parse_inference('[∀X Human(X)](~Mortal(X)) |- ~([∃Y Human(Y)]Mortal(Y))')
        result = check_inference(inference)
...
class TestQuantifierEdgeCases:
    ...
    def test_empty_domain_existential(self):
        ...
        formula = parse('[∃X Unicorn(X)]Magical(X)')
        result = solve(formula, T)
        assert result.satisfiable, "Existential should be satisfiable with fresh witness"
    ...
    def test_tautological_quantifiers(self):
        ...
        inference = parse_inference('[∀X Human(X)]Human(X), Human(socrates) |- Human(socrates)')
        result = check_inference(inference)
    ...
    def test_multiple_constant_introduction(self):
        ...
        result = check_inference(inference)
        assert result.valid, "Universal should apply to all dynamically introduced constants"
...
class TestQuantifierSemantics:
    ...
    def test_restricted_quantification_semantics(self):
        ...
        result = check_inference(inference)
        assert result.valid, "Contrapositive reasoning should work with restricted quantification"
