...
class AppStore(object):
    ...
    def create_profile(self, is_dev:bool, bundle_id:str, name:str):
        ...
        profile_name = f'{name}_{title}'
        profile_list = self.agent.list_profiles(filters={"name": profile_name})
        ...
        attrs = ProfileCreateReqAttrs(profile_name, type)
    ...
    def upload_screenshot(self, bundle_id:str, appstore_version: str, screenshots:dict[str:dict[str:str]]):
        ...
        all_locale = map(lambda item: item.locale, locale_list)
        # 用逗号拼接所有本地语言
        all_locale = ','.join(all_locale)
        print(f"所有本地化语言：{all_locale}")
        for (local,screenshotDic) in screenshots.items():
            ...
            for (type, dir) in screenshotDic.items():
                try:
                    ...
                    die(f"截图类型{type}不存在")
                # 先查找图集，如果没有就创建图集
                print(f'正在获取截图集：{screenshotType.name}')
                sets = self.agent.list_app_screenshot_set(select_local.id, filters={"screenshotDisplayType":screenshotType.name})
                if sets:
                    print(f'获取 {screenshotType.name} 成功')
                    set_id = sets[0].id
                    # 获取截图集中所有的截图
                    print(f'正在获取 {screenshotType.name} 中的所有截图,并删除')
