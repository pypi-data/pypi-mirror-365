...
Provides unified storage interface with pluggable backends for scalable data persistence.
"""
...
import sqlite3
import hashlib
from typing import Any, Dict, Optional, List, Protocol, runtime_checkable, Union, Tuple
...
class StorageManager:
    """Unified storage manager with multiple backends and namespacing."""
    
    def __init__(self, default_backend: Optional[StorageBackend] = None):
        self.backends: Dict[str, StorageBackend] = {}
        self.default_backend = default_backend or MemoryStorage()
        self.backends["default"] = self.default_backend
            ...
            try:
                for name, backend in self.backends.items():
                    if hasattr(backend, 'cleanup_expired'):
                        ...
                        if removed > 0:
                            print(f"Storage cleanup: {name} backend removed {removed} expired items")
    ...
    def get_backend(self, name: str = "default") -> Optional[StorageBackend]:
        """Get storage backend by name."""
        return self.backends.get(name)
    
    def get(self, key: str, backend: str = "default", namespace: Optional[str] = None) -> Optional[Any]:
        """Retrieve value by key from specified backend."""
    ...
    def set(self, key: str, value: Any, ttl_seconds: Optional[int] = None,
            backend: str = "default", namespace: Optional[str] = None) -> bool:
        """Store value in specified backend."""
        storage = self.get_backend(backend)
        if not storage:
            return False
        
        full_key = f"{namespace}:{key}" if namespace else key
        return storage.set(full_key, value, ttl_seconds)
    
    def delete(self, key: str, backend: str = "default", namespace: Optional[str] = None) -> bool:
        """Delete value from specified backend."""
        storage = self.get_backend(backend)
        if not storage:
            return False
        
        full_key = f"{namespace}:{key}" if namespace else key
        return storage.delete(full_key)
    
    def exists(self, key: str, backend: str = "default", namespace: Optional[str] = None) -> bool:
        """Check if key exists in specified backend."""
        storage = self.get_backend(backend)
        if not storage:
            return False
        
        full_key = f"{namespace}:{key}" if namespace else key
        return storage.exists(full_key)
    
    def keys(self, pattern: Optional[str] = None, backend: str = "default", 
             namespace: Optional[str] = None) -> List[str]:
        """List keys from specified backend."""
        storage = self.get_backend(backend)
        if not storage:
            return []
        ...
        if namespace:
            ...
            keys = storage.keys(namespace_pattern)
    ...
    def clear(self, backend: str = "default", namespace: Optional[str] = None) -> int:
        """Clear data from specified backend."""
        storage = self.get_backend(backend)
        if not storage:
            return 0
        ...
        if namespace:
            ...
            keys = self.keys(backend=backend, namespace=namespace)
            count = 0
            for key in keys:
                if self.delete(key, backend=backend, namespace=namespace):
                    count += 1
            return count
        else:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'backend_types': {name: type(backend).__name__ for name, backend in self.backends.items()}
        }
        ...
        for name, backend in self.backends.items():
            ...
            if hasattr(backend, '_data'):
                backend_stats['items'] = len(backend._data)
            
            if hasattr(backend, '_expiry'):
                now = time.time()
                expired_count = sum(1 for exp in backend._expiry.values() if now > exp)
                backend_stats['expired_items'] = expired_count
            
            if backend_stats:
                stats[f'{name}_stats'] = backend_stats
        
        return stats


# Convenience factory functions
def create_memory_storage_manager() -> StorageManager:
    """Create storage manager with memory backend."""
    return StorageManager(MemoryStorage())


def create_file_storage_manager(storage_dir: str = "./storage", 
                              use_pickle: bool = False) -> StorageManager:
    """Create storage manager with file backend."""
    return StorageManager(FileStorage(storage_dir, use_pickle))


def create_sqlite_storage_manager(db_path: str = "./storage.db") -> StorageManager:
    """Create storage manager with SQLite backend."""
    return StorageManager(SQLiteStorage(db_path))


def create_hybrid_storage_manager() -> StorageManager:
    """Create storage manager with multiple backends for different use cases."""
    manager = StorageManager(MemoryStorage())  # Fast default
    manager.add_backend("file", FileStorage("./file_storage"))  # Persistent
    manager.add_backend("sqlite", SQLiteStorage("./data.db"))  # Queryable
    return manager


# Git storage backend (optional import)
try:
    from .git import GitStorage
    
    def create_git_storage_manager(repo_path: str, **kwargs) -> StorageManager:
        """Create storage manager with Git backend."""
        git_backend = GitStorage(repo_path, **kwargs)
        return StorageManager(git_backend)
        
except ImportError:
    # Git backend not available (missing dependencies)
    def create_git_storage_manager(*args, **kwargs):
        print("Error: Git storage backend requires git to be installed")
        return None
    GitStorage = None
