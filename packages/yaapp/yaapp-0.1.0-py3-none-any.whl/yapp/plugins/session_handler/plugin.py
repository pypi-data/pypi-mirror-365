"""
Session handling plugin for YAPP framework.
...
class SessionStorage(Protocol):
    ...
    def set(self, session_id: str, data: Dict[str, Any], ttl_seconds: Optional[int] = None) -> bool:
        ...
        with self._lock:
            ...
            for session_file in self.storage_dir.glob("*.json"):
                try:
                    ...
                    if 'expires_at' in session_data and now > session_data['expires_at']:
                        session_file.unlink()
                        removed_count += 1
...
class SessionHandler:
    """HTTP header-based session handler for YAPP framework."""
    
    def __init__(self, 
                 storage: Optional[SessionStorage] = None,
                 session_header: str = "yapp-session-id",
    ...
    def create_session(self, initial_data: Optional[Dict[str, Any]] = None, 
    ...
    def inject_session_id_to_headers(self, headers: Dict[str, str], 
    ...
    def middleware_extract_session(self, request_headers: Dict[str, str]) -> Optional[Dict[str, Any]]:
        """
    ...
    def middleware_create_response_headers(self, session_id: str, 
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        if hasattr(self.storage, '_expiry'):
            now = time.time()
            expired_count = sum(1 for expiry in self.storage._expiry.values() if now > expiry)
...
def create_memory_session_handler(session_header: str = "yapp-session-id",
                                default_ttl: int = 3600) -> SessionHandler:
...
def create_file_session_handler(storage_dir: str = "./sessions",
                              session_header: str = "yapp-session-id", 
                              default_ttl: int = 3600) -> SessionHandler:
