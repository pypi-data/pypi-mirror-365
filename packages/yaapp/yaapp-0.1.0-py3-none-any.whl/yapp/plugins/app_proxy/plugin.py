...
class AppProxy:
    ...
    def __init__(self, target_url: str, timeout: float = 30.0, max_retries: int = 3):
        """Initialize AppProxy with target YApp server URL."""
    ...
    def _check_circuit_breaker(self) -> None:
        ...
        if self._circuit_open:
            ...
            if time.time() - self._last_failure_time > self._circuit_timeout:
                ...
                raise Exception(f"Circuit breaker open - service unavailable. Retry in {self._circuit_timeout - (time.time() - self._last_failure_time):.1f}s")
    ...
    def expose_to_registry(self, name: str, exposer) -> None:
        ...
        try:
            ...
            raise Exception(f"Failed to expose proxy: {e}")
    ...
    async def _async_expose_to_registry(self, name: str, exposer) -> None:
        ...
        if not discovery_result.is_ok():
            raise Exception(f"Failed to discover remote API: {discovery_result.as_error}")
    ...
    def execute_call(self, function_name: str, **kwargs) -> Any:
        ...
        if function_name not in self._discovered_functions:
            ...
            raise Exception(f"Function '{function_name}' not found in remote API. Available: {available}")
    ...
    async def execute_call_async(self, function_name: str, **kwargs) -> Any:
        ...
        if function_name not in self._discovered_functions:
            raise Exception(f"Function '{function_name}' not found in remote API")
            ...
            def make_proxy(name):
                def proxy_func(**kwargs):
                    ...
                    try:
                        ...
                        raise Exception(f"Proxy call failed: {e}")
                return proxy_func
    ...
    async def _make_rpc_call_async(self, function_name: str, arguments: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            self._record_failure()
            raise Exception(f"RPC call timeout after {self.timeout}s")
        except aiohttp.ClientError as e:
            self._record_failure()
            ...
            if "Remote error:" in str(e):
                raise  # Re-raise application errors as-is (don't record as failure)
            if "Circuit breaker" in str(e):
                raise  # Re-raise circuit breaker exceptions
            self._record_failure()
