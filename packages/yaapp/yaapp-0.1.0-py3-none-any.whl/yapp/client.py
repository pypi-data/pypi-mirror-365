...
def create_client(target_url: str, name: str = "proxy") -> YApp:
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python -m yapp.client --target http://localhost:8000 tui
  python -m yapp.client --target http://api.example.com:9000 server --port 8080
  python -m yapp.client --target http://localhost:8000 --name remote run remote.greet --name Alice
...
Infinite chaining example:
  ...
  python -m yapp.client --target http://localhost:8000 server --port 8001
  
  # Client C proxies to B (which proxies to A), exposes on port 8002
  python -m yapp.client --target http://localhost:8001 server --port 8002
        """
    )
