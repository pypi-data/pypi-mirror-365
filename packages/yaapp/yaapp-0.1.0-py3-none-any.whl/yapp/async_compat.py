"""
Async compatibility utilities for YAPP framework.
...
def async_compatible(func: Callable) -> Callable:
    ...
    if asyncio.iscoroutinefunction(func):
        ...
        try:
            ...
            raise ValueError(f"Cannot add sync compatibility to {type(func).__name__}: {e}")
        ...
        try:
            ...
            raise ValueError(f"Cannot add async compatibility to {type(func).__name__}: {e}")
...
def smart_call(func: Callable, *args, **kwargs) -> Any:
    ...
    if asyncio.iscoroutinefunction(func):
        ...
        try:
            ...
            raise RuntimeError("Cannot call async function directly from smart_call in async context. Use await.")
        except RuntimeError:
