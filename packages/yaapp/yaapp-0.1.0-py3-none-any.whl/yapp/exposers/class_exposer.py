...
class ClassExposer(BaseExposer):
    ...
    def __init__(self):
        """Initialize with instance cache."""
        self._instance_cache = {}  # Cache instances per class
    ...
    def run(self, item: Any, **kwargs) -> Result[Any]:
        ...
        if inspect.isclass(item):
            try:
                instance = self._get_cached_instance(item)
                return Ok(instance)
            except Exception as e:
    ...
    def _get_cached_instance(self, cls):
        """Get or create cached instance for class execution."""
        if cls not in self._instance_cache:
            self._instance_cache[cls] = cls()  # Only instantiate once per class
