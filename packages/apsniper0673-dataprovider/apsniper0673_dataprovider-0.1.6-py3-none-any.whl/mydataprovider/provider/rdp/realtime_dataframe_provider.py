...
class RDP_fetch(_RDP):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._cache_df:pd.DataFrame = pd.DataFrame(columns=['일자', '종목코드']).set_index(['일자', '종목코드'], drop=False)  # 캐시용 DataFrame 초기화
...
if __name__ == '__main__':  
    ...
    df = rdp.fetch_from_to(start_date=pd.Timestamp('2024-12-01'), end_date=pd.Timestamp('2025-07-30'))
    cdf = rdp._cache_df
    df = rdp.fetch_from_to(start_date=pd.Timestamp('2024-12-01'), end_date=pd.Timestamp('2025-07-30'))
    df = rdp.fetch_from_to(start_date=pd.Timestamp('2024-12-01'), end_date=pd.Timestamp('2025-07-30'))
    df = rdp.fetch_from_to(start_date=pd.Timestamp('2024-12-01'), end_date=pd.Timestamp('2025-07-30'))
