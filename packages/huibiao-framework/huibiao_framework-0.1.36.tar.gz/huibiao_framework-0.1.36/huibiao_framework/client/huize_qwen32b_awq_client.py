...
from huibiao_framework.execption.vllm import Qwen32bAwqException
...
class HuiZeQwen32bQwqClient(HuibiaoAbstractClient):
    ...
    async def post(
        ...
        try:
            # 发送异步POST请求
            async with self.session.post(self.url, json=dto.model_dump()) as resp:
                sp_time = time.time() - start_time
                logger.debug(f"{session_tag},resp-{resp.status},cost {sp_time:.2f}s")
                resp.raise_for_status()
                response_data = await resp.json()
                response_data = HuizeQwen32bAwqVo(**response_data)
                return response_data
        except Qwen32bAwqException as e:
