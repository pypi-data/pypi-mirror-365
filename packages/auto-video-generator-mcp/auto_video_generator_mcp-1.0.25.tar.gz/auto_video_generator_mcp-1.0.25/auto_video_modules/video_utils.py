...
from moviepy import VideoFileClip, concatenate_videoclips, CompositeVideoClip, ImageClip, TextClip, AudioFileClip
...
def parse_video_segments(segments, video_duration, segments_mode='keep'):
    ...
    # 根据模式计算最终保留区间
    if segments_mode == 'keep' or segments_mode == 'audio':
...
async def create_video_with_subtitles_tool(video_path: str, audio_path: str, subtitle_segments: str, output_path: str) -> str:
    ...
    Args:
        video_path: 视频文件路径
        audio_path: 音频文件路径
        subtitle_segments: 字幕片段，格式：文本,开始时间,结束时间
...
def detect_audio_active_segments(video_path, threshold=0.01, min_duration=1.0, step=0.5):
    """
    检测视频中有音频的区间，返回segments列表
    Args:
        video_path: 视频文件路径
        ...
        step: 检测步长（秒）
    Returns:
        list: [{"start": xx, "end": xx}, ...]
    """
    from moviepy.editor import VideoFileClip
    import numpy as np
    print(f"[音频检测] 正在分析音频活动: {video_path}")
    video = VideoFileClip(video_path)
    audio = video.audio
    ...
    while t < duration:
        ...
        try:
            arr = audio.subclip(t, t_end).to_soundarray(fps=22050)
        ...
        t += step
    # 结尾处理
    if active and seg_start is not None and duration - seg_start >= min_duration:
        segments.append({"start": seg_start, "end": duration})
    video.close()
    print(f"[音频检测] 检测到有音频区间: {segments}")
    return segments 

def clip_video_by_srt(video_path, srt_path, output_path):
    """
    根据srt字幕文件剪辑视频，只保留有字幕的区间，输出新视频。
    Args:
        video_path: 原视频路径
        srt_path: 字幕文件路径（SRT）
        output_path: 输出视频路径
    ...
    for match in pattern.finditer(srt):
        ...
        if text.strip():
            intervals.append((srt_time_to_seconds(start), srt_time_to_seconds(end)))
    # 加载视频并剪辑
    video = VideoFileClip(video_path)
    clips = [video.subclip(s, e) for s, e in intervals if e > s]
    if not clips:
        raise ValueError('没有可保留的字幕区间，输出空视频')
    final_clip = concatenate_videoclips(clips)
