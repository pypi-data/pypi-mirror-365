...
* gen, to generate the classes required for evathon to run or docs
...
API_REF_URL = 'https://tl.evathon.dev/'

GENERATOR_DIR = Path('evathon_generator')
LIBRARY_DIR = Path('evathon')
...
def generate(which, action='gen'):
    from evathon_generator.parsers import\
        parse_errors, parse_methods, parse_tl, find_layer

    from evathon_generator.generators import\
...
def main(argv):
    if len(argv) >= 2 and argv[1] in ('gen', 'clean'):
        ...
        # Make sure tl.evathon.dev is up-to-date first
        with urllib.request.urlopen(API_REF_URL) as resp:
            ...
            from evathon_generator.parsers import find_layer
            layer = next(filter(None, map(find_layer, TLOBJECT_IN_TLS)))
        ...
        # Try importing the evathon module to assert it has no errors
        try:
            import evathon
        ...
        remove_dirs = ['__pycache__', 'build', 'dist', 'evathon.egg-info']
        ...
        for x in ('build', 'dist', 'evathon.egg-info'):
            shutil.rmtree(x, ignore_errors=True)
        ...
        with open('evathon/version.py', 'r', encoding='utf-8') as f:
        ...
        setup(
            ...
            url='https://github.com/LonamiWebs/evathon',
            download_url='https://github.com/LonamiWebs/evathon/releases',
