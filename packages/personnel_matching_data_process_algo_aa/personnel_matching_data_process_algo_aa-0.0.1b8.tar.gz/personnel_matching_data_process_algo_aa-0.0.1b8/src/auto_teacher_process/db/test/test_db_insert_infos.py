...
def test_run_missing_parameters(mocker):
    # 创建处理器实例并模拟数据库初始化
    mocker.patch.object(InfoInsertDBProcessor, "_setup_db_engine")  # 避免实际数据库连接
    processor = InfoInsertDBProcessor()
...
def test_update_db_call(mocker):
    ...
    assert mock_insert_db.call_count == 7
    assert mock_update_db.call_count == 6
