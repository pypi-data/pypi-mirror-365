...
class GooglePageMatchProcessor(BaseRunProcessor):
    def __init__(self, args):
        ...
        self.task_type = "new_teacher_google_page_match"  # 任务类型
        ...
        self.school_cn_en_dict = self.get_school_name_dict()
    ...
    async def process_row(self, row: pd.Series) -> dict | None:
        ...
        en_name = row_dict["derived_teacher_name"]
        school_name_cn = row_dict["school_name"]
        teacher_id = row_dict["teacher_id"]

        if pd.isna(en_name) or pd.isna(school_name_cn):
            return None

        if contains_chinese(en_name):
            try:
                result = await self.llm.run({"en_name": en_name})
                row_dict["english_name"] = result["english_name"]
                row_dict["chinese_name"] = result["chinese_name"]
                en_name = result["english_name"] or ""
        ...
        google_page = await self.db.fetch_google_page_async(
            teacher_name=teacher_name_variants, school_name=self.school_cn_en_dict[school_name_cn]
        )
        ...
        for _, teacher_row in google_page.iterrows():
            google_id = teacher_row["u_id"]
    ...
    async def run(self) -> None:
        ...
        df = self.db.get_teacher_data_from_db(id_start=self.task_args["id_start"], id_end=self.task_args["id_end"])
