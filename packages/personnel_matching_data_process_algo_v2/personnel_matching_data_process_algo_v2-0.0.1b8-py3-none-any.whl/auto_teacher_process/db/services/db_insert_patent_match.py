...
class PatentInsertDBProcessor(BaseDBProcessor):
    ...
    def get_patent_teacher_data_from_db(self, id_start, id_end):
        query = f"""
    ...
    # TODO:
    def get_raw_teacher_patent_from_db(self, id_start, id_end):
        query = f"""
                SELECT application_date, applicant, inventor, invention_name, id, patent_abstract, Technology_neighborhood
                FROM raw_teacher_patent
                WHERE (id>={id_start} AND id<={id_end});
                """
        return self.get_db(query)

    def get_new_company_info_from_db(self, id_start, id_end):
        query = f"""
                SELECT company_id, company_name, clean_former_name
                FROM raw_company_info
                WHERE (id>={id_start} AND id<={id_end});
                """
        return self.get_db(query)
    ...
    def get_all_school_info(self):
        query = """
        SELECT school_name, school_name_en
    ...
    def process(self, input_data):
        ...
        df_unique = merged_df.drop_duplicates(subset=["teacher_id", "patent_id"])

        # 更新数据
        self.logger.debug("开始插入数据库...")
        self.insert_db(
            df=df_unique,
            table_name="product_teacher_patent_relation",
            batch_size=1000,
            progress_file="patent_match_relation_insert_progress.txt",
        )
