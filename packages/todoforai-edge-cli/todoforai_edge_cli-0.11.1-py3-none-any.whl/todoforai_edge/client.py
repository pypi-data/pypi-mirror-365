...
class TODOforAIEdge:
    ...
    async def _on_config_change(self, changes: Dict[str, Any]) -> None:
        ...
        if self.edge_id and self.connected:
            sync_data = {k: v for k, v in changes.items() if k in {"workspacepaths", "MCPs", "name", "isShellEnabled", "isFileSystemEnabled"}}
    ...
    async def _handle_message(self, message):
        ...
        try:
            ...
            elif msg_type == FE.SHUTDOWN_REQUEST:
                asyncio.create_task(self.handle_shutdown_request(payload))
    ...
    async def handle_shutdown_request(self, payload: Dict[str, Any]):
        """Handle shutdown request from frontend"""
        try:
            logger.info("Received shutdown request from frontend")
            
            # Stop all file syncs
            from .handlers.file_sync import stop_all_syncs
            await stop_all_syncs()
            
            # Update edge status to OFFLINE
            ...
            # Exit the process after a brief delay to allow cleanup
            import os
            await asyncio.sleep(0.5)
            logger.info("Exiting process due to shutdown request")
            os._exit(0)
