class Relator:
    def __init__(self, name, relations=None, children=None, parents=None):
        ...
        for i in self.relations:
            if self not in i.relations:
                i.relations.append(self)
        for i in self.children:
            if self not in i.parents:
                i.parents.append(self)
        for i in self.parents:
            if self not in i.children:
    ...
    def color(self):
        has_relations = len(self.relations) > 0
        has_children = len(self.children) > 0
        has_parents = len(self.parents) > 0
        if has_relations and has_children and has_parents:
            return "SkyBlue"
        elif has_relations and has_children:
            return "Khaki"
        elif has_relations and has_parents:
            return "LightGreen"
