...
def create_graph(relator, net=None, visited=None, drawn_edges=None, depth=None, ancestors=True, descendants=True, relations=True, self_node=True):
    ...
    def node_id(obj):
        return str(id(obj))
    print(f"Creating graph for {relator}")
    if self_node:
        net.add_node(node_id(relator), label=str(relator), shape="box", color=relator.color())
    ...
    if descendants:
        for child in relator.children:
            net.add_node(node_id(child), label=str(child), shape="box", color=child.color())
            draw_edge(relator, child, arrows="to")
            create_graph(child, net, visited, drawn_edges, depth)

    if relations:
        for relation in relator.relations:
            net.add_node(node_id(relation), label=str(relation), shape="ellipse", color=relation.color())
            draw_edge(relator, relation, arrows="", dashes=True)
            create_graph(relation, net, visited, drawn_edges, depth)

    if ancestors:
        for parent in relator.parents:
            net.add_node(node_id(parent), label=str(parent), shape="box", color=parent.color())
            draw_edge(parent, relator, arrows="to")
            create_graph(parent, net, visited, drawn_edges, depth)
