...
Functions:
    get_winners: Determines the winning player(s) from TablePlayer input
    get_winners_from_board_players: Determines the winning player(s) from BoardPlayer input
    evaluate: Evaluates hands and equity for TablePlayer input
    evaluate_board_players: Evaluates hands and equity for BoardPlayer input
    get_winners_multi_board: Multi-board winner determination for TablePlayer input
"""
...
def evaluate_board_players(
    board_cards_keys: List[int],
    players: List[BoardPlayer],
    use_exhaustive_equity: bool = True
) -> List[BoardPlayer]:
    """
    Evaluate poker hands and set hand/equity values for all BoardPlayer instances across multiple boards.
    ...
    Returns:
        A list of BoardPlayer instances with updated hand and equity values.
    """
    if not board_cards_keys or not players:
        return players
    
    # Calculate best hand for each player across all boards
    for board_player in players:
        best_hand = get_player_best_hand(board_player, board_cards_keys)
        board_player.hand = best_hand
    ...
    if use_exhaustive_equity and all(board_key.bit_count() == 3 for board_key in board_cards_keys):
        # Use exhaustive equity calculation for flop scenarios
        calculate_exhaustive_equity(players, board_cards_keys)
    else:
        # Use simple equity calculation based on hand strength across boards
        _calculate_multi_board_simple_equity(players, board_cards_keys)
...
def get_winners_from_board_players(
    board_cards_key: int,
    players: List[BoardPlayer]
) -> List[BoardPlayer]:
    """
    Determine the winning player(s) based on their best 5-card hand 
    evaluated against the community cards. Accepts BoardPlayer objects directly.
    ...
    # Find winners for this board
    board_winners = _determine_winners_from_hands(players)
...
def _get_player_best_weight(player: Union[TablePlayer, BoardPlayer], board_cards_key: int) -> int:
...
def _get_holdem_best_weight(player: Union[TablePlayer, BoardPlayer], board_cards_key: int) -> int:
...
def _get_omaha_best_weight(player: Union[TablePlayer, BoardPlayer], board_cards_key: int) -> int:
...
def _determine_winners_from_weights(
    player_weights: List[Tuple[Union[TablePlayer, BoardPlayer], int]]
) -> List[Union[TablePlayer, BoardPlayer]]:
...
def calculate_exhaustive_equity(players: List[BoardPlayer], board_cards_keys: List[int]) -> None:
    ...
    Note:
        ...
        - Preserves original player hands after equity calculation
    """
    ...
    # Save original hands before equity calculation
    original_hands = {player.code: player.hand for player in players}
    ...
    # Restore original hands after equity calculation
    for player in players:
        player.hand = original_hands[player.code]
