...
"""
References:
A compact 2-D full-wave finite-difference frequency-domain method for general guided wave structures.pdf
A Compact 2-D FDFD Method for Modeling Microstrip Structures With Nonuniform Grids and Perfectly Matched Layer.pdf
Lecture 12 -- Finite-Difference Analysis of Waveguides (CEM Lectures) (Used to reduce the matrix size)
"""
import os
...
def getindiceordered(cc, cumsumarray, pml=0):
    ...
    """
    return np.searchsorted(cumsumarray[pml:], cc* (1.0 - 1.0e-4), 'right')+1
...
class fdfd_2d:
    ...
    def fdfd_mesh(self, xres, yres):
        ...
        for part in parts:
            if part[0] == "dielectric":
                ...
                xi1 = getindiceordered(part[2] - (part[4]/ 2.0), dxcumsum)
                xi2 = getindiceordered(part[2] + (part[4]/ 2.0), dxcumsum)
                yi1 = getindiceordered(part[3] - (part[5]/ 2.0), dycumsum)
                yi2 = getindiceordered(part[3] + (part[5]/ 2.0), dycumsum)
                # kritikx.append(xi1)
                # kritikx.append(xi2)
                # kritiky.append(yi1)
                # kritiky.append(yi2)
            ...
            if part[0] == "metal":
                ...
                cc = getindiceordered((part[2]), dxcumsum)
    ...
    def addpml(self, pmllayers):
        ...
        self.dx = newdx
        self.dy = newdy
        self.dxcumsum = np.cumsum(self.dx)
        self.dycumsum = np.cumsum(self.dy)
        self.er = newer
        self.erxx = newerxx
        self.eryy = neweryy
        self.erzz = newerzz
    ...
    def sigma(self, i,dx,nlayer):
        """
        Returns conductivity value at a grid index. Returns conductivity associated with PML layer in PML regions. Returns 0 for indices outside PML region.

        Args:
            i(float): Index of point to get the conductivity at. Either full integer of half integer.
            dx(list): List of grid cell lengths including the PML region
            nlayer(list or tuple): 2-element container. First element is number of cells in PML region at the beginning of grid array. Second element if the number of cells in PML region at the end of grid array.

        Returns:
            Float: Conductivity value.
        """
        Rth=1.0e-7
        n=2.0
        n1=nlayer[0]
    ...
    def sx(self,i):
        """
        Returns conductivity value at a dx grid index. Returns conductivity associated with PML layer in PML regions. Returns 0 for indices outside PML region.
    ...
    def sy(self,i):
        """
        Returns conductivity value at a dy grid index. Returns conductivity associated with PML layer in PML regions. Returns 0 for indices outside PML region.
    ...
    def waveguide(cls, boxwidth, boxheight, dielthickness, epsilon, freq, pmllayers, xres, yres):
        ...
        cc = getindiceordered((boxwidth/ 2.0), obj.dxcumsum, pmllayers[0])
        ...
        obj.vlines = [("eyn",cc, 0, getindiceordered(boxheight, obj.dycumsum, pmllayers[2])+1)]
        return obj
        # print("cloops1= ")
        # print(cloops)
        # return (dx, dy, er, erxx, eryy, erzz, metalnodesx, metalnodesy, metalnodesz, cloops, vlines, parts)
