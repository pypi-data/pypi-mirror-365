...
async def interactive_mode() -> int:
    ...
    try:
        ...
        while True:
            ...
            # Run discussion
            if include_human:
                display_message = selected_pattern.value.replace('_', ' ') + " discussion with human participation"
                if file_attachments:
                ...
                if file_attachments:
                    human_input_func = create_file_attachment_input_func(file_attachments, console)
                    console.print("[dim]Note: Attached files will be available in your responses[/dim]\n")
                ...
                display_message = selected_pattern.value.replace('_', ' ') + " discussion"
                if file_attachments:
                    display_message += f" ({len(file_attachments)} file(s) attached)"
                ...
                result = await panel.discuss(
                    topic=enhanced_topic, pattern=selected_pattern, max_rounds=max_rounds
                )
    ...
    async def run_discussion():
        ...
        try:
            ...
            if files:
                try:
                    ...
                    if file_attachments:
                        ...
                        enhanced_topic = attached_msg.get_full_content()
                        console.print(f"[green]✓ Processed {len(file_attachments)} file(s)[/green]")
                        
                        # Display attachment summary
                        file_cli._display_attachment_summary(file_attachments)
                    else:
                        console.print("[yellow]No files were successfully processed[/yellow]")
            ...
            if with_human:
                ...
                console.print(f"[green]Starting discussion on: {display_topic}[/green]\n")
