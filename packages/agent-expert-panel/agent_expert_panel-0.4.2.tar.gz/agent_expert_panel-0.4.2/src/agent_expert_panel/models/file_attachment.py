...
class FileAttachment(BaseModel):
    ...
    content: Optional[str] = Field(None, description="Processed text content of the file")
    ...
    def get_content_preview(self, max_chars: int = 500) -> str:
        ...
        return self.content[:max_chars] + f"... [truncated, full content: {len(self.content)} chars]"
...
class AttachedMessage(BaseModel):
    ...
    attachments: list[FileAttachment] = Field(default_factory=list, description="List of attached files")
    source: str = Field(..., description="Source of the message (user, agent name, etc.)")
...
class FileProcessingError(Exception):
    ...
    def __init__(self, file_path: Path, message: str, original_error: Optional[Exception] = None):
