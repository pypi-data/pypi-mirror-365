...
CoefC = (1/np.cos(2*theta_eta_etap)*((-(np.sqrt(2)*gS_f0etaetapr) + 2*gS_sigmaetaetapr)*np.cos(theta_S) + 2*CoefA*np.cos(2*theta_eta_etap) + 2*gS_f0etaetapr*np.sin(theta_S) + np.sqrt(2)*gS_sigmaetaetapr*np.sin(theta_S) + np.sqrt(2)*CoefA*np.sin(2*theta_eta_etap)))/3
CoefD = -1/72*(1/np.sin(theta_eta_etap)*1/np.cos(theta_eta_etap)*1/np.cos(2*theta_eta_etap)*(4*CoefA*np.cos(theta_S) - 12*CoefA*np.cos(theta_S - 4*theta_eta_etap) - 12*gS_sigmaetaetapr*np.cos(2*theta_eta_etap) - 8*np.sqrt(2)*gS_f0etaetapr*np.cos(2*(theta_S + theta_eta_etap)) + 
