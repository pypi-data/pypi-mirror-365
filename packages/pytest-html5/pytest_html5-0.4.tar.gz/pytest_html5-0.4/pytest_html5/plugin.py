...
class HTMLReport:
    def __init__(self, logfile, config):
        ...
        case_path = report_path[:report_path.rindex('test_case')]
        self.output_dir = os.path.join(case_path, "output")
        report_path = os.path.join(self.output_dir, 'report')
        self.html_report_path = os.path.join(report_path, "report.html")
        assets_path = os.path.join(report_path, "assets")
        self.pfs_json = os.path.join(assets_path, "pfs.json")
        self.e1_html_path = os.path.join(assets_path, "e1.html")
        self.e2_html_path = os.path.join(assets_path, "e2.html")
        ...
        replaced_js_path = os.path.join(self.report_path, "assets", "echarts.min.js")
        for _ in [self.report_path, self.screenshot_dir, self.log_dir, assets_path]:
            if not os.path.exists(_):
                os.makedirs(_)
        if not os.path.exists(replaced_js_path):
            download_js(replaced_js_path)
        self.replace_js = replaced_js_path if os.path.exists(replaced_js_path) else None
