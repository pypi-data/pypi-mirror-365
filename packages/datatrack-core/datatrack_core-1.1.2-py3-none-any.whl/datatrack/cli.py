...
def main(
    ...
    if help:
        ...
        typer.echo(" --parallel          Use parallel processing for large schemas (default: True).")
        typer.echo(" --max-workers <int> Maximum number of parallel workers (default: 4).")
        typer.echo(" --batch-size <int>  Batch size for memory-efficient processing (default: 100).")
        ...
        typer.echo("  datatrack snapshot --include-data --max-rows 50 --parallel --max-workers 8")
        ...
        typer.echo("NOTES:")
        typer.echo("  • Datatrack supports PostgreSQL, MySQL, and SQLite (via SQLAlchemy URIs).")
        typer.echo(
