...
def analyze_performance_trends() -> dict:
    ...
    avg_tables_per_second = sum(op['tables_per_second'] for op in recent_operations) / len(recent_operations)
    
    parallel_usage = sum(1 for op in recent_operations if op['parallel_enabled']) / len(recent_operations)
    ...
    return {
        ...
        "recommendations": [
            "Enable parallel processing more often" if parallel_usage < 0.5 else "Good parallel processing usage",
            "Consider optimizing for speed" if avg_tables_per_second < 10 else "Good processing speed"
        ]
    }
