...
class TestPerformanceOptimizations:
    ...
    def test_introspect_tables_parallel_small_set(self):
        ...
        with patch('datatrack.tracker.introspect_single_table') as mock_introspect:
            mock_introspect.return_value = {"name": "test", "columns": [], "indexes": [], "foreign_keys": [], "constraints": [], "triggers": []}
            
            result = introspect_tables_parallel(mock_inspector, table_names, max_workers=4)
    ...
    def test_introspect_tables_parallel_large_set(self):
        ...
        with patch('datatrack.tracker.introspect_single_table') as mock_introspect:
            ...
            result = introspect_tables_parallel(mock_inspector, table_names, max_workers=2)  # Reduced workers
    ...
    def test_introspect_tables_paginated(self):
        ...
        with patch('datatrack.tracker.introspect_tables_parallel') as mock_parallel:
            mock_parallel.return_value = [{"name": f"table_{i}", "columns": []} for i in range(5)]
            
            batches = list(introspect_tables_paginated(mock_inspector, table_names, batch_size=batch_size))
    ...
    def test_performance_monitoring_integration(self):
        ...
        with patch('datatrack.tracker.get_saved_connection') as mock_conn, \
             patch('datatrack.tracker.get_connected_db_name') as mock_db_name, \
             patch('datatrack.tracker.create_engine') as mock_engine, \
             patch('datatrack.tracker.save_schema_snapshot') as mock_save:
    ...
    def test_parallel_processing_parameters(self):
        ...
        mock_inspector = MagicMock()
        table_names = [f"table_{i}" for i in range(8)]  # 8 tables to trigger parallel processing
        ...
        with patch('datatrack.tracker.introspect_single_table') as mock_introspect:
            ...
            result = introspect_tables_parallel(mock_inspector, table_names, max_workers=2)
    ...
    def test_parallel_vs_sequential_threshold(self):
        ...
        with patch('datatrack.tracker.introspect_single_table') as mock_introspect:
            ...
            result = introspect_tables_parallel(mock_inspector, small_tables, max_workers=2)
            assert len(result) == 3
            
            # Large set should also work
            large_tables = [f"table_{i}" for i in range(10)]
            result = introspect_tables_parallel(mock_inspector, large_tables, max_workers=2)
    ...
    def test_error_handling_in_parallel_processing(self):
        ...
        table_names = [f"table_{i}" for i in range(7)] + ["error_table"]  # 8 tables total
        ...
        with patch('datatrack.tracker.introspect_single_table', side_effect=mock_introspect_side_effect):
            result = introspect_tables_parallel(mock_inspector, table_names, max_workers=2)
    ...
    def test_batch_size_optimization(self):
        ...
        with patch('datatrack.tracker.introspect_tables_parallel') as mock_parallel:
            ...
            list(introspect_tables_paginated(mock_inspector, table_names, batch_size=10))
            assert mock_parallel.call_count == 3  # 30/10 = 3 batches
            
            mock_parallel.reset_mock()
            
            list(introspect_tables_paginated(mock_inspector, table_names, batch_size=15))
    ...
    def test_performance_benchmark(self):
        ...
        with patch('datatrack.tracker.introspect_single_table', side_effect=fast_introspect):
            sequential_result = [fast_introspect(mock_inspector, name) for name in table_names]
        sequential_time = time.time() - start_time
        
        # Test parallel processing (should complete without errors)
        start_time = time.time()
        with patch('datatrack.tracker.introspect_single_table', side_effect=fast_introspect):
            parallel_result = introspect_tables_parallel(mock_inspector, table_names, max_workers=2)
