...
import logging
from typing import Any, TypeVar, Generic, Optional, List, Dict, Iterable
...
class Node(BaseNode[T_PrepRes, T_ExecRes]):
    ...
    """
    def __init__(self, max_retries: int = 1, wait: int = 0, node_id: Optional[str] = None):
        """
...
class Graph(BaseNode):
    ...
    def get_next_node(self, curr: BaseNode, action: T_Action) -> Optional[BaseNode]:
        """
        ...
        return next_node
    def _orch(self, shared: T_Shared, params: Optional[T_Params] = None) -> T_Action:
...
class AsyncGraph(Graph, AsyncNode):
    ...
    """
    async def _orch_async(self, shared: T_Shared, params: Optional[T_Params] = None) -> T_Action:
        current_node = copy.copy(self.start_node)
        node_params = params or {**self.params}
        last_action: T_Action = None
        while current_node:
