...
async def serve() -> Server:
    ...
    async def handle_list_tools() -> list[types.Tool]:
        ...
        return [
            ...
            types.Tool(
                name="retrieve",
                title="检索知识库",
                description="从RAGFlow指定的数据集检索与问题相关的内容",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "dataset_id": {
    ...
    async def handle_call_tool(
        ...
        if name == "list_datasets":
            ...
            try:
                chunk = ragflow.retrieve(dataset_ids=dataset_id, question=question)
                return [types.TextContent(type="text", text=json.dumps(chunk, ensure_ascii=False, indent=2))]
            except Exception as e:
                return [types.TextContent(type="text", text=f"Error: 检索会话失败 - {str(e)}")]
