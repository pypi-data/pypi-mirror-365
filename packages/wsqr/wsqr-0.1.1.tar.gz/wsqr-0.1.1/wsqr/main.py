...
def log(string, style=None):
    if DEBUG:
        ...
        console.print(f'[dim cyan][{datetime.now().strftime("%H:%M:%S")}][/]', string, style=style)
...
class QReaderDecoder(Decoder):
    ...
    def _test(cls):
        ...
        local_model_file_md5 = local_model_file.parent/(local_model_file.name + ".md5")
        ...
        if local_model_file.exists():
            use_local = True
            if md5(open(local_model_file,'rb').read()).hexdigest() != local_model_file_md5:
