    ...
    def _test(cls):
        ...
        try:
            if src_model_file.exists():
                console.log(f"[dim]测试QReader模型文件[/]")
                QReader("l", 0)
                return True
        except Exception as e:
            console.log(f"[dim]QReader模型文件不存在或损坏")
            # raise e
        except ConnectionError:
            pass

        local_model_dir.mkdir(exist_ok=True)
        local_model_file_md5 = local_model_file + ".md5"
        if os.path.isfile(local_model_file_md5):
            local_model_file_md5 = open(local_model_file + "md5", "r").read().strip()
        if local_model_file.exists():
            use_local = True
            if md5(local_model_file) != local_model_file_md5:
            ...
            if use_local:
                console.log(
                    f"[dim]复制本地模型文件: {local_model_file} 到 {src_model_file}[/]"
                )
                shutil.copy(local_model_file, src_model_file)
                return True

        console.log(f"[dim]正在下载QReader模型文件: {url} 到 {src_model_file}[/]")
        ...
        try:
            ...
            for dns in ["223.5.5.5", "8.8.8.8", "1.1.1.1"]:
                ...
                try:
                    console.log(f"[dim]使用DNS服务器: {dns}[/]")
                    session = DNSClientSession(dns)
                    ...
                    console.log(f"[dim]QReader模型文件下载完成: {src_model_file}[/]")
                    return True
                except ConnectionError:
                    break
                except Exception as e:
...
def process_worker(decoder_cls, img, share_manager):
    """工作进程任务函数"""
    if share_manager.stop_event.is_set():
        console.log(f"[dim]停止任务: {decoder_cls.name} {img}[/]")
        return []
    console.log(
    ...
    try:
        ...
        console.log(f"{decoder_cls.name}解码 {img} 发生错误:{e}")
    finally:
        img.release()
    if not data:
        console.log(f"[dim]{decoder_cls().name}解码 {img} 完成, 未解析到内容:(")
...
def result_callback(results, result_manager, stop_event, pool, stop_immediately):
    ...
    if results:
        if stop_immediately and not stop_event.is_set():
            ...
            stop_event.set()
            console.log("[bold]检测到有效结果，终止所有进程![/]")
...
def main(
    ...
    parallel: int = typer.Option(
        os.cpu_count(), "--parallel", "-p", help="并行工作线程数"
    ),
    ...
    """
    MAX_QREADER_COUNT = (
        psutil.virtual_memory().available // (1024 * 1024 * 1024 * 2) - 1
    )
    ...
    if "qreader" in engines:
        if MAX_QREADER_COUNT == 0:
            console.log(f"[red]内存不足使用QReader, 并发数应为1![/]")
            MAX_QREADER_COUNT = 1
        elif parallel > MAX_QREADER_COUNT:
            if parallel != os.cpu_count():
                console.log(
                    f"[red]当使用QReader时, 并发数应小于等于{MAX_QREADER_COUNT}![/]"
                )
            else:
                console.log(
                    f"当使用QReader时, 并发数应小于等于{MAX_QREADER_COUNT}, 已修改"
                )
    ...
    with multiprocessing.Manager() as manager:
        ...
        console.log(f"[dim]创建临时目录: {temp_dir}[/]")
        ...
        for e in engines:
            ...
            if not d:
                console.log(f"引擎{e}不存在", style="bold red")
                exit(1)
            decoders.append(d)
            if d == QReaderDecoder:
                qreader_enable = True
            ...
            for decoder in decoders:
                ...
                for img, skip_qreader in processor.process():
                    ...
                    async_results.append(
                        pool.apply_async(
                            process_worker,
                            args=(decoder, img, share_manager),
                            callback=partial(
                                result_callback,
    ...
    if result_manager.results:
        ...
        console.log("[bold yellow]⚠ 未检测到有效二维码[/]")
    # 清理临时文件
    if debug:
        console.log(f"[yellow]调试模式保留临时目录: {temp_dir}[/]")
    else:
        console.log(f"[dim]清理临时目录: {temp_dir}[/]")
