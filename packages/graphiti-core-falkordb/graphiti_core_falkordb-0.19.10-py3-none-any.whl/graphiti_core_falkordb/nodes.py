...
from graphiti_core_falkordb.helpers import parse_db_date
from graphiti_core_falkordb.models.nodes.node_db_queries import (
    COMMUNITY_NODE_SAVE,
    ENTITY_NODE_SAVE,
    EPISODIC_NODE_SAVE,
)
from graphiti_core_falkordb.graph_queries import get_community_node_save_query, get_entity_node_save_query
...
class EntityNode(Node):
    ...
    async def save(self, driver: GraphDriver):
        ...
        # Remove duplicates and ensure Entity is included
        labels_set = set(self.labels + ['Entity'])
        labels = list(labels_set)
        save_query = get_entity_node_save_query(driver.provider, labels)
...
class CommunityNode(Node):
    ...
    async def save(self, driver: GraphDriver):
        query = get_community_node_save_query(driver.provider)
        result = await driver.execute_query(
            query,
            uuid=self.uuid,
            name=self.name,
