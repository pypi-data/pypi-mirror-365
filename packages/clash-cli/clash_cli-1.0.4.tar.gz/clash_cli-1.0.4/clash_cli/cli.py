...
def check_sudo_requirement():
    ...
    if os.geteuid() != 0:
        ...
        if current_clash_cli and not os.path.exists(system_clash_cli):
            info_message("🔧 检测到 sudo 访问问题，正在自动配置...")
            try:
                ...
                if result.returncode == 0:
                    success_message("✅ 自动配置成功！请重新运行命令:")
                    info_message(f"   sudo clash-cli {' '.join(sys.argv[1:])}")
                else:
                    info_message(f"💡 请手动配置: sudo ln -s {current_clash_cli} {system_clash_cli}")
                    info_message(f"   然后重新运行: sudo clash-cli {' '.join(sys.argv[1:])}")
            except Exception:
                info_message(f"💡 请手动配置: sudo ln -s {current_clash_cli} {system_clash_cli}")
                info_message(f"   然后重新运行: sudo clash-cli {' '.join(sys.argv[1:])}")
