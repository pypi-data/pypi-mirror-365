...
class K8sClient:
    ...
    def wait_for_job_completion(self, job_name: str, namespace: str, config: ScannerConfig, timeout: int | None = None) -> bool:
        """Wait for job completion or early readiness after scanner-init is finished.

        Will return success (True) when:
        ...
        If the pod/job fails before that, returns False.
        If the bundle is not found or init container fails, handles/report as before.
        """
        ...
        self._handle_job_timeout(job_name, namespace, timeout, discovered_pod_name, remote_bundle_path)

    def _handle_job_timeout(self, job_name: str, namespace: str, timeout: int, discovered_pod_name: str | None, remote_bundle_path: str) -> NoReturn:
        """Handle job timeout scenarios with appropriate error messages."""
        if not discovered_pod_name:
            raise TimeoutError(f"Job {job_name} timed out after {timeout} seconds. Could not find associated pod.")

        if not self._check_init_container_completion(discovered_pod_name, namespace, remote_bundle_path):
            raise TimeoutError(f"Job {job_name} timed out after {timeout} seconds. Init container did not complete successfully.")
        else:
            raise TimeoutError(f"Job {job_name} timed out after {timeout} seconds. Bundle file was not found at {remote_bundle_path}.")
