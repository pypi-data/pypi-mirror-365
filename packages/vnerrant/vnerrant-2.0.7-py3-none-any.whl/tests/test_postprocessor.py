...
@pytest.mark.parametrize(
    ...
    [
        ...
        # ── ① Wrong polarity ────────────────────────────────────────────
        ("She is coming , is she ?", "She is coming , isn't she ?"),
        ("He hasn't arrived yet , hasn't he ?", "He hasn't arrived yet , has he ?"),
        # ── ② Wrong auxiliary/modal ────────────────────────────────────
        (
            "She can play the guitar , does she ?",
            "She can play the guitar , can't she ?",
        ),
        ("They will call us , do they ?", "They will call us , won't they ?"),
        # ── ③ Wrong pronoun ────────────────────────────────────────────
        (
        ...
        ("Anna is late again , isn't Anna ?", "Anna is late again , isn't she ?"),
        ...
        ("Close the door , can you ?", "Close the door , will you ?"),
        # ── ⑦ Tag after let’s (should be 'shall we') ───────────────────
        ("Let's start , will you ?", "Let's start , shall we ?"),
        # ── ⑧ Irregular 'I am' tag (should be 'aren’t I') ──────────────
        ("I'm next , am I ?", "I'm next , aren't I ?"),
        # ── ⑨ Missing contraction (did not you → didn’t you) ───────────
        ("You cleaned the room , did not you ?", "You cleaned the room , didn't you ?"),
    ],
)
def test_postprocess_question_tag(original, corrected):
    edits = annotator.annotate_raw(original, corrected)
    for edit in edits:
        print(edit)
