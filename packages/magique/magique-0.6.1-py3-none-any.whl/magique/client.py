...
class ServiceProxy:
    ...
    async def ensure_connection(self):
        ws, info = await self.server_proxy.ensure_connection(
...
class MultiConnectionServerProxy(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.client_id = client_id or str(uuid.uuid4())
        self.servers = [
            ServerProxy(url, client_id=self.client_id, **kwargs) for url in urls
        ]
    ...
    async def ensure_connection(
        ...
        if self.active_server is not None:
            if await self.active_server.is_connected():
                if service_id_or_name is not None:
                    info = await self.active_server.get_service_info(service_id_or_name)
                    return self.active_server._connection, info
                else:
                    return self.active_server._connection
            else:
                self.active_server = None

        for server in self.servers:
            try:
                await server.ensure_connection(retry_count, retry_delay)
                self.active_server = server
                if service_id_or_name is not None:
                    info = await server.get_service_info(service_id_or_name)
                    return server._connection, info
                else:
                    return server._connection
            except Exception as e:
                continue
        raise MagiqueError("Failed to connect to any server")
