...
class TestUtilityMethods:
    ...
    def test_to_dict(self):
        table = Table.from_rows(["a", "bb", "ccc"])
        result = table.to_dict(lambda x: len(x))
        assert result == {1: "a", 2: "bb", 3: "ccc"}
    ...
    def test_parallel_map(self):
        ...
        start = time.time()
        parallel_result = numbers.pselect(slow_square, parallelism=4).to_list()
