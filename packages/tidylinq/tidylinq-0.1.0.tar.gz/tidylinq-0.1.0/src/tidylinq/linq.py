...
class Enumerable(ABC, Generic[T]):
    ...
    def pselect(self, selector: Callable[[T], U], parallelism: int = 1) -> "Enumerable[U]":
        """Transform each element using selector in parallel.

        Example: from_iterable([1, 2, 3]).pselect(lambda x: x * 2, 2) -> [2, 4, 6]
        """
    ...
    def window(
        ...
        """Group elements into sliding windows based on either a fixed size or a predicate function.
    ...
    def to_dict(
        ...
        """Convert to dictionary using key and value selectors.

        Example: from_iterable(['a', 'bb']).to_dict(lambda x: x, lambda x: len(x)) -> {'a': 1, 'bb': 2}
        """
    ...
    def try_select(
        ...
        for item in self:
            try:
                result = selector(item)  # type: ignore
