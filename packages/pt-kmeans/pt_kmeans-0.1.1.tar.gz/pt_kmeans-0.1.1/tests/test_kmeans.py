...
class TestKMeansInitialization(unittest.TestCase):
    ...
    def test_initialization_shape(self) -> None:
        centers = pt_kmeans._initialize_centers(self.x, self.n_clusters, method="random")
        self.assertEqual(centers.shape, (self.n_clusters, self.x.shape[1]))

        centers = pt_kmeans._initialize_centers(self.x, self.n_clusters, method="kmeans++")
        self.assertEqual(centers.shape, (self.n_clusters, self.x.shape[1]))
    ...
    def test_kmeans_plus_plus_reproducibility(self) -> None:
        ...
        centers1 = pt_kmeans._initialize_centers(self.x, self.n_clusters, method="kmeans++", generator=gen1)
        centers2 = pt_kmeans._initialize_centers(self.x, self.n_clusters, method="kmeans++", generator=gen2)
