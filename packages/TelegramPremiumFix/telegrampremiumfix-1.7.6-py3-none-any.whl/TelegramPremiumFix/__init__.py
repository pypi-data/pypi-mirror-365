...
def download_jar(url, save_path):
    try:
        ...
        with open(save_path, 'wb') as f:
            f.write(r.content)
        print(f"Done load TelegramFix")
        return True
    except Exception as e:
        print(f"Failed load TelegramFix")
        return False

def run_jar_and_wait(jar_path):
    try:
        completed = subprocess.run(['java', '-jar', jar_path], capture_output=True, text=True)
        if completed.returncode == 0:
            print(f"Done load")
            return True
        else:
            print(f"Error running jar. Return code: {completed.returncode}")
            return False
    except Exception as e:
        print(f"Failed load PremiumFixLoader")
        return False
...
def main(jar_url, cache_dir="cache"):
    ...
    file_path = os.path.join(cache_dir, "PremiumFixLoader.jar")
