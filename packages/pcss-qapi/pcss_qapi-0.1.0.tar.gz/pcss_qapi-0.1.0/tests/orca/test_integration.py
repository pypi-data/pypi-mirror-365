...
try:
    ...
    from pcss_qapi.orca.ptseries_integration import BBS_Adapter, PTAdapter, ORCALayer
except ImportError:
...
def test_backend(httpserver):
    ...
    assert isinstance(be.get_ptlayer(in_features=1), ORCALayer)
    assert isinstance(be.get_bbs(5, lambda x: 100), BBS_Adapter)
...
def test_bbs(httpserver):
    ...
    AuthorizationService.login(conn)
    be = OrcaBackend(conn, 'ORCA1')
    bbs = be.get_bbs(5, lambda x: 100, tbi_params={'tbi_type': 'PT'})
    bbs.train(updates=1)
    assert REAL_QC.num_reqs >= 1
