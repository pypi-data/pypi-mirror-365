...
class HomelabClient:
    ...
    def _build_config(
        ...
        config = HomelabSettings()

        if consul_urls:
            logger.debug("Setting consul URLs in configuration")
            config.consul_urls = consul_urls

        if consul_token:
            logger.debug("Setting consul token in configuration")
            config.consul_token = consul_token if isinstance(consul_token, SecretStr) else SecretStr(consul_token)
    ...
    def __call__(self, key: str) -> str | None:
        ...
        Returns:
            Optional[str]: Configuration value if exists, None otherwise
        ...
        try:
            ...
            logger.error("Configuration key error: {}", str(e))
            raise
...
if __name__ == "__main__":
    ...
    try:
        ...
        default_config = client("cfg/tmp/tmp")
        print(f"Default environment config: {default_config}")

        # 2. Using specific environments
        dev_config = client("cfg/tmp/tmp,dev")
        staging_config = client("cfg/tmp/tmp,staging")
        prod_config = client("cfg/tmp/tmp,prod")

        print(f"Dev config: {dev_config}")
        print(f"Staging config: {staging_config}")
        print(f"Prod config: {prod_config}")

        # 3. Using invalid environment (will raise ValueError)
        invalid_config = client("cfg/tmp/tmp,invalid")
    except ValueError as e:
        print(f"Error: {e}")
