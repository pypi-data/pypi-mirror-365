...
from .core import pull_models, pull_models_for_pydantic
...
@app.command("pull:schema")
def pull_models_for_response(output: str = "models_response.py"):
    """
    Gera schemas Pydantic para documentação e validação.
    """
    console.print("[cyan]Gerando schemas Pydantic...[/cyan]")
    success = pull_models_for_pydantic(output)
    if success:
        console.print(f"[green]Schemas salvos em: {output}[/green]")
    else:
        console.print("[red]Erro ao gerar schemas.[/red]")
...
def show_docs(terminal: bool = typer.Option(False, "--terminal", help="Mostra no terminal ao invés do navegador")):
    """
    Exibe documentação das rotas documentadas via @documentate.
    """
    ...
    if terminal:
        ...
        table.add_column("Request", style="yellow")
        ...
        for route in DOCUMENTED_ROUTES:
            table.add_row(
                route["method"],
                route["path"],
                route["request_model"].__name__ if route["request_model"] else "-",
                route["response_model"].__name__ if route["response_model"] else "-"
            )
...
#             route["method"],
#             route["path"],
#             getattr(route["request_model"], "__name__", "-") if route["request_model"] else "-",
#             getattr(route["response_model"], "__name__", "-") if route["response_model"] else "-"
#         )
