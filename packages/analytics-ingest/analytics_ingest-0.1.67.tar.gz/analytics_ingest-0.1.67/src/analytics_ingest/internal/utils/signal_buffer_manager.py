...
class SignalBufferManager:
    ...
    def add_signal(self, signal: dict):
        if not signal or not isinstance(signal, dict):
            raise ValueError("[add_signal] Signal must be a valid dict")
        ...
        print("signal appended in buffer", self.buffer)
        print("max signal count", self.max_signal_count)
        print("length of buffer", len(self.buffer))

        if len(self.buffer) >= self.max_signal_count:
            print("flushing", len(self.buffer) >= self.max_signal_count)
            self.flush(force=True)
