...
from Osdental.ServicesBus.AuditQueue import AuditFlowQueue
...
from Osdental.Handlers.Instances import aes
...
async def enqueue_response(data: Any, batch: int, headers: Dict[str,str], audit_flow: AuditFlowQueue, msg_info: str = None):
    if data and isinstance(data, list):
        if batch > 0 and len(data) > batch:
            ...
            for idx, data_batch in enumerate(batches, start=1):
                ...
                await audit_flow.enqueue(custom_response.message_to_queue())
        else:
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                if request:
                    ...
                    await audit_flow.enqueue(custom_request.message_to_queue())
                ...
                await enqueue_response(data, batch, headers, audit_flow, msg_info)

                return response

            except OSDException as ex:
                logger.warning(f'Controlled server error: {str(ex.error)}')
                await audit_flow.enqueue(ex.message_to_queue())
