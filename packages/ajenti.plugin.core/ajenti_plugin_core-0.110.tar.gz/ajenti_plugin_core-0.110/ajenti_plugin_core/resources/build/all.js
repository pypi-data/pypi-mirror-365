...
angular.module('core').service('config', function ($http, $q, initialConfigContent) {
    ...
    this.getTfaConfig = function () {
        return $http.get("/api/core/tfa-config").then(function (response) {
            return response.data;
        });
    };

    this.deleteTfa = function (data) {
        return $http.post("/api/core/tfa-config", data).then(function (response) {
            return response.data;
        });
    };
