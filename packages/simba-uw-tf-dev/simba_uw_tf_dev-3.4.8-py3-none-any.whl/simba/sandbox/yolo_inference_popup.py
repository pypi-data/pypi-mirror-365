...
class YOLOPoseInferencePopUP(PopUpMixin):
    ...
    def run(self, multiple: bool):
        ...
        runner = YOLOPoseInference(weights_path=mdl_path, video_path=video_paths, verbose=verbose, save_dir=save_dir,device=device,format=format, batch_size=batch_size, torch_threads=workers, threshold=threshold, max_tracks=max_tracks, interpolate=interpolate, imgsz=img_size, iou=iou)
        runner.run()
