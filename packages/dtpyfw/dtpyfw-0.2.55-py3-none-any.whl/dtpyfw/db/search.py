...
def get_list(
    ...
    if unique:
        count_query = select(func.count(distinct_func(getattr(model, primary_column)))).select_from(model)
    else:
    ...
    if return_rows_data:
        rows = list(map(
            ...
            (
                db.execute(final_query).scalars().unique().all()
                if unique
                else db.execute(final_query).scalars().all()
            )
        ))
    else:
        rows = None
