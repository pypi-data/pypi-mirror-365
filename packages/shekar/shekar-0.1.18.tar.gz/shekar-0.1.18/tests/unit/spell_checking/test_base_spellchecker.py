...
def test_spellchecker_initialization_default_model():
    ...
    with patch("shekar.spell_checking.base.SPELL_CHECKING_REGISTRY", {"statistical": MagicMock()}) as fake_registry:
        spell = SpellChecker()
...
def test_spellchecker_fit_calls_underlying_model():
    ...
    with patch("shekar.spell_checking.base.SPELL_CHECKING_REGISTRY", {"statistical": lambda: fake_model}):
        spell = SpellChecker()
        X = ["متن تستی"]
        spell.fit(X)
