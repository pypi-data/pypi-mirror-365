...
### Data Enrichment
- **GeoIP**: MaxMind GeoLite2 country & city lookup with coordinates
...
All analysis commands (`logsentinelai-httpd-access`, `logsentinelai-apache-error`, `logsentinelai-linux-system`, `logsentinelai-tcpdump`) support the same CLI options and provide consistent GeoIP enrichment with coordinates:
...
LogSentinelAI automatically enriches IP addresses in analysis results with structured location information using MaxMind GeoLite2 database.
...
- **Comprehensive data**: Provides country, city, latitude, longitude in structured JSON
- **Elasticsearch-friendly**: Optimized for filtering, aggregation, and geolocation visualization
- **Private IP handling**: Marks internal IPs appropriately without database lookup
- **Statistics enrichment**: Enhanced IP statistics with full geographic context

### 🗺️ Data Structure

All IP addresses are enriched with comprehensive location data:
...
**Note**: If city/coordinates are not available for an IP, those fields will be `null` while country information is still provided.
...
# Database path (automatically uses comprehensive City database)
...
```bash
# Download GeoLite2-City database (includes country + city + coordinates)
logsentinelai-geoip-download
...
2. Download "GeoLite2 City" (.mmdb) - comprehensive location database
3. Extract and place as `GeoLite2-City.mmdb` in project directory
