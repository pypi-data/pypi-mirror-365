...
# Global user ID for associating traces with users
...
def set_user_id(user_id: str):
    """Set the user ID that will be used to tag all traces."""
...
def trace(name: Optional[str] = None, tags: Optional[List[str]] = None, **kwargs):
    ...
    Args:
        ...
        **kwargs: Additional keyword arguments that will be passed to Langfuse's observe decorator
                 after the middleware adds agent name tag, session ID, and user ID.
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **func_kwargs):
            # Add agent name as tag, session ID, and user ID if available
            ...
            if _CURRENT_USER_ID:
                trace_kwargs['user_id'] = _CURRENT_USER_ID
