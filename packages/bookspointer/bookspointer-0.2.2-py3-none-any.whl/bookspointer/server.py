...
class AuthorAPI(BaseAPI):
    ...
    def create(self, author_data):
        ...
        Note:
            The method includes comprehensive error handling and user feedback.
        """
        response = self._post("authors/", author_data)
        if isinstance(response, dict) and response.get("status") == "error":
        ...
        try:
            ...
            pass
        print(
            f"Author created successfully. Name: {author_data.get('name', author_data.get('author'))}")
        return {"success": True, "message": f"Author '{author_data.get('name', author_data.get('author'))}' created.", "id": obj_id}
    ...
    def get_unscraped_authors(self):
        ...
        Note:
            The method includes error handling and user feedback through logging.
            It looks for authors where 'is_scraped' equals "yes".
        """
        response = self._get("authors/")
        if response.status_code == 200:
            authors = response.json()
