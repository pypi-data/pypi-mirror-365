...
class TemperatureScaledDeepEnsembleRegressor(DeepEnsembleRegressor):
    ...
    def fit(self, X, y):
        ...
        self.tau = torch.nn.Parameter(torch.tensor(1.0, device=means.device))
        optimizer = Adam([self.tau], lr=self.lr)

        for epoch in range(self.temp_scaling_epochs):
            optimizer.zero_grad()
            scaled_var = raw_variances * self.tau**2
            precision = 1.0 / scaled_var
            nll = 0.5 * (torch.log(scaled_var) + precision * (val_y - means)**2)
            loss = nll.mean()
            loss.backward()
            optimizer.step()
