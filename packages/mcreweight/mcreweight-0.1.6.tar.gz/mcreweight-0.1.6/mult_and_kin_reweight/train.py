...
def gbreweight(mc_train, data_train, mc_test, data_test, w_mc_train, w_mc_test, w_data_train, w_data_test, columns, trials, weightsdir):
    """
    ...
    """
    study_path = f"{weightsdir}/mult_and_kin_optuna_study_" + "_".join(columns) + ".pkl"
    if joblib.os.path.exists(study_path):
        ...
        study = run_optuna(mc_train, data_train, mc_test, data_test, w_mc_train, w_mc_test, w_data_train, w_data_test, columns, trials)
    ...
    joblib.dump(gb, f"{weightsdir}/gb_model_" + "_".join(columns) + ".pkl")
    joblib.dump(new_mc_weights, f"{weightsdir}/gb_weights_" + "_".join(columns) + ".pkl")
...
def kfolding(gb, mc, data, w_mc, w_data, columns, n_folds, weightsdir):
    ...
    joblib.dump(gb_kfolding, f"{weightsdir}/gb_kfolding_model_" + "_".join(columns) + ".pkl")
    joblib.dump(gb_weights_kfolding, f"{weightsdir}/gb_weights_kfolding_" + "_".join(columns) + ".pkl")
...
def binning_reweight(mc_train, data_train, mc_test, w_mc_train, w_mc_test, w_data_train, w_data_test, columns, n_bins, n_neighs, weightsdir):
    ...
    joblib.dump(bins, f"{weightsdir}/binning_model_" + "_".join(columns) + ".pkl")
    joblib.dump(new_mc_weights, f"{weightsdir}/binning_weights_" + "_".join(columns) + ".pkl")
