...
def lookup_root_path(
    ...
    """
    Search for a path by traversing up the directory hierarchy.

    Recursively searches for the specified path starting from a root directory
    and moving up through parent directories for a limited number of iterations.
    Useful for finding project root directories or configuration files.
    ...
    >>> config_root = lookup_root_path('config.ini', remaining_iteration=5)
    """
    ...
    return lookup_root_path(
        path_to_search_string, remaining_iteration - 1, root_path.parent
    )
