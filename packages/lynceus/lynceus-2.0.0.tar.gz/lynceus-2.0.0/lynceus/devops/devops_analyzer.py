...
class DevOpsAnalyzer(LynceusClientClass):
    ...
    def uncompress_tarball(
        ...
        def custom_member_filter(tarinfo):
            # Convert tarinfo name to a Path object
            target_path = Path(tarinfo.name)

            # Discard files with directory traversal sequences
            if '..' in target_path.parts or not is_safe_path(Path('.'), target_path):
                return None

            # Discard special file types
            if tarinfo.islnk() or tarinfo.issym() or tarinfo.ischr() or tarinfo.isblk() or tarinfo.isfifo():
                return None
        ...
        try:
            with tarfile.open(tarball_path) as tarball:
                tarball.extractall(
                    tarball_uncompress_path,
                    members=(custom_member_filter(tarinfo) for tarinfo in tarball)
                )
