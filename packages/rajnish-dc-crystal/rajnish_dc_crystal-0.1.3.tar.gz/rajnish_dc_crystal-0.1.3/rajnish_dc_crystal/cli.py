...
def handle_punch(args: argparse.Namespace, config: Dict[str, Any]) -> int:
    ...
    delay_minutes = args.delay or config.get('behavior', {}).get('default_delay_minutes', 0)
    
    # Add random delay if configured
    random_delay = config.get('behavior', {}).get('random_delay_max_minutes', 0)
    # if random_delay > 0:
    #     logger.debug(f"Random delay chosen: {random_added} minutes")
    #     delay_minutes += random.randint(0, random_delay)
    if random_delay > 0:
        random_added = random.randint(0, random_delay)
        logger.debug(f"Random delay chosen: {random_added} minutes")
        delay_minutes += random_added
        
    if delay_minutes >= 0:
        logger.info(f"⏳ Waiting for {delay_minutes} minutes before {args.action}...")
