...
class DataCapsule():
    ...
    def row_exists(self, row_index) -> bool:
        if row_index >= 0:
            ...
            if len(self.result_set) >= abs(row_index) and self[row_index].row_exists():
                return True
        if self.get_flag("verbose"):
            logger.debug(f"Row index {row_index} didnt exists")
        return False
        
    def debug(self):
        logger.error("CAPSULE DEBUG")
        logger.error(f"success: {self.last_action_success}")
        logger.error(f"{self.sql_command.replace("\n", " ").replace(" "*4, " ").replace(" "*3, " ").replace(" "*2, " ")[0:80]}")
        logger.error(f"{len(self.result_set)} rows")
        if len(self.result_set) > 0:
            logger.error("First row")
            logger.error(f"{self.result_set[0]}")
        logger.error(f"in profile: {self.last_profile_name}")
        logger.error(f"with message: {self.last_error_message}")
        flags: dict = self.get_flags()
        for flag_key, flag_value in flags.items():
            logger.error(f"Flag {flag_key} = {flag_value}")
    ...
    def stream(self, keep: bool = False):
        if self.open_cursor and hasattr(self.open_cursor, "fetchone"):
            ...
            if self.get_flag("verbose"):
                logger.debug("Stream attempt on closed or non-standard cursor")
    ...
    def set_action_error(self, error_message: str):
        ...
        if error_message:
            ...
            if self.get_flag("verbose"):
                logger.debug("Error message is set")
        else:
    ...
    def open_stream(self, cursor): # anname kursori, mida saab kirjehaaval lugeda
        ...
        if cursor and hasattr(cursor, "fetchone"):
            ...
            if self.get_flag("verbose"):
                logger.debug("Assign non-standard cursor")
