...
@pytest.fixture
def crud_with_metadata() -> AutoCRUD:
...
@pytest.fixture
def sample_users(crud_with_metadata: AutoCRUD, base_time: datetime):
    """創建示例用戶數據"""
    users_data = [
        {
            "name": "Alice",
    ...
    for i, data in enumerate(users_data):
        # 模擬不同的創建時間
        mock_time = (
            base_time.replace(tzinfo=LOCAL_TZ) - timedelta(hours=i)
        ).isoformat()
...
def test_basic_list_all_backward_compatibility(crud_with_metadata: AutoCRUD):
...
@pytest.mark.usefixtures("sample_users")
def test_list_with_pagination(crud_with_metadata: AutoCRUD):
    """測試分頁功能"""
    # 第一頁，每頁2項
...
@pytest.mark.usefixtures("sample_users")
def test_filter_by_created_by(crud_with_metadata: AutoCRUD):
    """測試按創建者過濾"""
...
@pytest.mark.usefixtures("sample_users")
def test_filter_by_updated_by(crud_with_metadata: AutoCRUD):
    """測試按更新者過濾"""
...
@pytest.mark.usefixtures("sample_users")
def test_filter_by_multiple_users(crud_with_metadata: AutoCRUD):
    """測試按多個用戶過濾"""
...
@pytest.mark.usefixtures("sample_users")
def test_filter_by_time_range(crud_with_metadata: AutoCRUD):
    """測試按時間範圍過濾"""
    # 獲取中間時間點
...
@pytest.mark.usefixtures("sample_users")
def test_sorting_by_created_time(crud_with_metadata: AutoCRUD):
    """測試按創建時間排序"""
    # 降序排序（最新的在前）
...
@pytest.mark.usefixtures("sample_users")
def test_invalid_sort_field(crud_with_metadata: AutoCRUD):
    """測試無效的排序字段"""
...
@pytest.mark.usefixtures("sample_users")
def test_combined_filters_and_pagination(crud_with_metadata: AutoCRUD):
    """測試組合過濾器和分頁"""
...
@pytest.mark.usefixtures("sample_users")
def test_empty_results(crud_with_metadata: AutoCRUD):
    """測試空結果"""
...
def test_date_time_range_class():
    """測試 DateTimeRange 類"""
    start_time = datetime(2024, 1, 1, tzinfo=LOCAL_TZ)
    end_time = datetime(2024, 12, 31, tzinfo=LOCAL_TZ)

    date_range = DateTimeRange(start=start_time, end=end_time)

    # 在範圍內
    assert date_range.contains(datetime(2024, 6, 15, tzinfo=LOCAL_TZ)) is True
    assert date_range.contains(start_time + timedelta(seconds=1)) is True
    assert date_range.contains(end_time - timedelta(seconds=1)) is True

    # 在範圍外
    assert date_range.contains(datetime(2023, 12, 31, tzinfo=LOCAL_TZ)) is False
    assert date_range.contains(datetime(2025, 1, 1, tzinfo=LOCAL_TZ)) is False
    assert date_range.contains(start_time - timedelta(seconds=1)) is False
    assert date_range.contains(end_time + timedelta(seconds=1)) is False
...
@pytest.mark.usefixtures("sample_users")
def test_list_all_with_params_simple_mode(crud_with_metadata: AutoCRUD):
...
@pytest.mark.usefixtures("sample_users")
@pytest.mark.parametrize("timerange_type", ["created_time", "updated_time"])
def test_filter_time_range(
    crud_with_metadata: AutoCRUD, base_time: datetime, timerange_type: str
):
    timerange = DateTimeRange(
        start=base_time - timedelta(hours=1),
        end=base_time,
    )
    if timerange_type == "created_time":
        params = ListQueryParams(created_time_range=timerange)
    else:
        params = ListQueryParams(updated_time_range=timerange)
    items = crud_with_metadata.list_all("users", params)
    ...
    for item in items:
        assert (
            timerange.start
            <= datetime.fromisoformat(item[timerange_type])
            <= timerange.end
        )
