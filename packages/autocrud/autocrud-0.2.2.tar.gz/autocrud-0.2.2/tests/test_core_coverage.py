...
class TestSingleModelCRUDCoverage:
    ...
    def test_list_with_time_range_filters(self):
        ...
        old_key = crud.create({"name": "Old", "age": 30})
        recent_key = crud.create({"name": "Recent", "age": 25})

        # 手動設置時間戳（使用正確的鍵）
        old_data = crud.get(old_key)
        old_data["created_time"] = old_time.isoformat()
        assert crud.update(old_key, old_data)

        recent_data = crud.get(recent_key)
        recent_data["created_time"] = recent_time.isoformat()
        assert crud.update(recent_key, recent_data)

        # 測試創建時間範圍過濾
        time_range = DateTimeRange(
            start=old_time,  # 使用相同的 naive datetime
        ...
        assert result.items[0]["id"] == old_key
    ...
    def test_datetime_parsing_failures(self):
        ...
        # 創建測試數據
        key = crud.create({"id": "1", "name": "Test", "age": 30})

        # 測試情況1：字符串解析失敗
        data = crud.get(key)
        data["created_time"] = "invalid-datetime-format"
        crud.update(key, data)

        # 測試時間範圍過濾，無效格式應該被過濾掉
        time_range = DateTimeRange(
            start=datetime.now().replace(microsecond=0) - timedelta(hours=1),
            end=datetime.now().replace(microsecond=0) + timedelta(hours=1),
        )
        params = ListQueryParams(created_time_range=time_range)
