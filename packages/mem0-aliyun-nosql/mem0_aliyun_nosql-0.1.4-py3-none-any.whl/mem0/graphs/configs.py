...
class GremlinGraphConfig(BaseModel):
    ...
    message_serializer: Optional[str] = Field(
        description="Message serializer for the graph database",
        default=None
    )
...
class GraphStoreConfig(BaseModel):
    provider: str = Field(
        description="Provider of the data store (e.g., 'neo4j', 'memgraph', 'neptune', 'gremlin')",
        default="neo4j",
    )
    config: Union[Neo4jConfig, MemgraphConfig, NeptuneConfig, GremlinGraphConfig] = Field(
