...
class TestClaudeCodeIntegration:
    ...
    def test_safe_bash_commands_allowed(self) -> None:
        ...
        for command in safe_commands:
            ...
            assert exit_code == 0, f"Command '{command}' was blocked but should be allowed"
            
            # Parse JSON output to verify approval
            try:
                response = json.loads(stdout)
                hook_output = response["hookSpecificOutput"]
                assert hook_output["permissionDecision"] == "allow", f"Command '{command}' denied"
            except json.JSONDecodeError:
    ...
    def test_tool_enforcement_patterns(self) -> None:
        ...
        for command, suggested_tool in blocked_commands:
            ...
            assert exit_code == 2, f"Command '{command}' should be blocked by tool enforcement"
    ...
    def test_json_output_invalid_json_input(self) -> None:
        ...
        assert "allowing operation to proceed" in hook_output["permissionDecisionReason"]
