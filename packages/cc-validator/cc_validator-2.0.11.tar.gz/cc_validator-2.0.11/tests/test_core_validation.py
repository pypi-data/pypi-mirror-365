...
class TestSecurityValidation:
    ...
    def test_safe_bash_command(self, run_validation_factory: Any) -> None:
        ...
        assert (
            returncode == 0
        ), f"Expected safe command to be allowed, stdout: {stdout}"
        response = json.loads(stdout)
    ...
    def test_grep_instead_of_find(self, run_validation_factory: Any) -> None:
        ...
        assert "Use 'rg' (ripgrep)" in response["hookSpecificOutput"]["permissionDecisionReason"]
    ...
    def test_api_key_in_bash(self, run_validation_factory: Any) -> None:
        ...
        assert (
            "Security violation" in response["hookSpecificOutput"]["permissionDecisionReason"]
            or "secrets" in response["hookSpecificOutput"]["permissionDecisionReason"].lower()
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if stdout:
                try:
                    ...
                    assert any(
                        phrase in response.get("hookSpecificOutput", {}).get("permissionDecisionReason", "")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if stdout:
                try:
                    ...
                    assert any(
                        phrase in response.get("hookSpecificOutput", {}).get("permissionDecisionReason", "").lower()
...
class TestToolRouting:
    ...
    def test_unknown_tool_allowed(self, run_validation_factory: Any) -> None:
        ...
        if stdout:
            response = json.loads(stdout)
            assert response["hookSpecificOutput"]["permissionDecision"] == "deny"
            assert "Unhandled tool" in response["hookSpecificOutput"]["permissionDecisionReason"]
