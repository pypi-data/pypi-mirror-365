...
class TestHybridFactoryDemo:
    ...
    @pytest.mark.quick
    def test_composable_environment_fixtures(
        self, run_validation_factory: Any, remove_api_key_env: Any
    ) -> None:
        ...
        tool_data = {
            ...
            "input": {
                "file_path": "demo.txt",
                "content": "Test with composable fixtures",
            },
            "conversation_context": "",
        }

        # Environment is set up by the fixtures
        returncode, stdout, stderr = run_validation_factory(tool_data)
        assert returncode == 2, f"Expected failure without API key, stderr: {stderr}"
