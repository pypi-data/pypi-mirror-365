...
class TestSecurityValidation:
    ...
    def test_dangerous_bash_command(self, run_validation_factory: Any) -> None:
        ...
        assert (
            "Dangerous command pattern detected" in response["permissionDecisionReason"]
        )

    @pytest.mark.quick
    @pytest.mark.security
    def test_safe_bash_command(self, run_validation_factory: Any) -> None:
        """Test that bash commands are blocked by default (fail-closed)."""
        tool_data = {
            "tool": "Bash",
            "input": {"command": "ls -la"},
            "conversation_context": "",
        ...
        assert (
            returncode == 2
        ), f"Expected command to be blocked by fail-closed policy, stdout: {stdout}"
        response = json.loads(stdout)
        assert response["permissionDecision"] == "deny"
        assert (
            "Bash command not explicitly approved"
    ...
    def test_grep_instead_of_find(self, run_validation_factory: Any) -> None:
        ...
        response = json.loads(stdout)
        assert response["permissionDecision"] == "deny"
        assert "Use 'rg' (ripgrep)" in response["permissionDecisionReason"]
    ...
    def test_api_key_in_bash(self, run_validation_factory: Any) -> None:
        ...
        response = json.loads(stdout)
        assert response["permissionDecision"] == "deny"
        # Commands with secrets are blocked by fail-closed policy
        assert (
            "Bash command not explicitly approved"
            in response["permissionDecisionReason"]
        )
    ...
    def test_api_key_in_write(self, run_validation_factory: Any) -> None:
        ...
        assert response["permissionDecision"] == "deny"
        assert any(
            phrase in response["permissionDecisionReason"]
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if stdout:
                try:
                    response = json.loads(stdout)
                    assert (
                        response.get("permissionDecision") == "deny"
                    ), f"Expected operation to be blocked by TDD, got: {response}"
                    assert any(
                        phrase in response.get("permissionDecisionReason", "")
    ...
    def test_write_test_file_allowed(self, run_validation_factory: Any) -> None:
        ...
        if stdout:
            try:
                response = json.loads(stdout)
                assert (
                    response.get("permissionDecision") == "allow"
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if stdout:
                try:
                    ...
                    assert any(
                        phrase in response.get("permissionDecisionReason", "").lower()
