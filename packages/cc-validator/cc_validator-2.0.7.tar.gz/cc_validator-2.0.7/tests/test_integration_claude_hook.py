...
class TestClaudeCodeIntegration:
    ...
    def test_write_config_file(self) -> None:
        ...
        hook_input = self.env.create_hook_input(
            "Write", {"file_path": ".gitignore", "content": "*.pyc\n__pycache__/\n.venv/"}
        )
        exit_code, stdout, stderr, duration_ms = self.env.invoke_hook(
            hook_input, env_vars={"GEMINI_API_KEY": os.environ.get("GEMINI_API_KEY", "")}
        )
    ...
    def test_dangerous_bash_command(self) -> None:
        ...
        try:
            ...
            assert (
                "dangerous" in response["permissionDecisionReason"].lower()
                or "destructive" in response["permissionDecisionReason"].lower()
            )
        ...
        hook_input = self.env.create_hook_input(
            "MultiEdit",
            {
                ...
                "edits": [
                    ...
                    {"old_string": '"main": "index.js"', "new_string": '"main": "src/index.js"'},
                ],
            },
        )
        exit_code, stdout, stderr, duration_ms = self.env.invoke_hook(
    ...
    def test_write_with_production_secrets(self) -> None:
        ...
        try:
            ...
            assert response["permissionDecision"] == "deny"
            assert (
                "security" in response["permissionDecisionReason"].lower()
                or "secret" in response["permissionDecisionReason"].lower()
            )
        except json.JSONDecodeError:
            pytest.fail(f"Invalid JSON output: {stdout}")
    ...
    def test_json_output_allowed_operation(self) -> None:
        ...
        assert "permissionDecision" in response
        assert "permissionDecisionReason" in response
        assert response["permissionDecision"] == "allow"
        assert response["permissionDecisionReason"] == "Operation approved"
    ...
    def test_json_output_blocked_operation(self) -> None:
        ...
        assert "permissionDecisionReason" in response
        assert response["permissionDecision"] == "deny"
        assert "❌" in response["permissionDecisionReason"]
    ...
    def test_json_output_invalid_json_input(self) -> None:
        ...
        # Verify JSON structure
        assert response["permissionDecision"] == "allow"
        assert "Invalid JSON input" in response["permissionDecisionReason"]
        assert "allowing operation to proceed" in response["permissionDecisionReason"]
    ...
    def test_json_output_exception_handling(self) -> None:
        ...
        try:
            ...
            assert response["permissionDecision"] == "deny"
            assert "Unhandled tool ''" in response["permissionDecisionReason"]
