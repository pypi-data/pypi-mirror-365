...
class TDDValidator:
    ...
    async def categorize_file(
        ...
        """
        Smart categorization for TDD validation logic using hybrid approach.

        Uses fast path pattern matching first, then falls back to LLM for
        ambiguous cases. This provides both speed and accuracy.
        ...
        Returns dict with:
            ...
            reason: str
        """
        # Step 1: Try fast path categorization first
        fast_category = self._fast_path_categorize(file_path)
        if fast_category:
            return {
                "category": fast_category,
                "requires_tdd": fast_category == "implementation",
                "reason": f"Pattern-based categorization: {fast_category} file",
            }

        # Step 2: Fall back to LLM for ambiguous files
        if not self.api_key or not self.client:
            ...
            if any(
                ...
                return {
                    "category": "test",
                    "requires_tdd": False,
                    "reason": "Test files don't require TDD but need validation for multiple test rule",
                }
            ...
            if self._is_config_file(file_path):
                return {
                    "category": "config",
                    "requires_tdd": False,
                    "reason": "Configuration files don't require tests",
                }
            ...
            if ext in doc_extensions:
                return {
                    "category": "docs",
                    "requires_tdd": False,
                    "reason": "Documentation files don't require tests",
                }
            ...
            for doc_dir in doc_directories:
                if doc_dir in file_path or f"/{doc_dir[:-1]}/" in file_path:
                    return {
                        "category": "docs",
                        "requires_tdd": False,
                        "reason": f"File in documentation directory: {doc_dir}",
                    }
            ...
            if any(pattern in basename.lower() for pattern in generated_patterns):
                return {
                    "category": "data",
                    "requires_tdd": False,
                    "reason": "Generated files don't require tests",
                }
            ...
            if basename in lock_files:
                return {
                    "category": "data",
                    "requires_tdd": False,
                    "reason": "Lock files don't require tests",
                }
            ...
            if "migration" in file_path.lower() or "/migrate/" in file_path:
                return {
                    "category": "data",
                    "requires_tdd": False,
                    "reason": "Migration files don't require tests",
                }
            ...
            if ext in schema_extensions:
                return {
                    "category": "data",
                    "requires_tdd": False,
                    "reason": "Schema/definition files don't require tests",
                }
...
- "implementation": Code with business logic that MUST be tested (contains functions, classes, algorithms)
- "structural": Organizational code without logic (imports, constants, type definitions only)
- "config": Configuration files (JSON, YAML, TOML, etc.)
- "docs": Documentation files
- "data": Data files (CSV, SQL, etc.)

IMPORTANT: Any Python/JS/Go/etc file with function definitions (def/function/func) that actually DO something (not just pass/return constants) should be "implementation" and require TDD.

Examples:
- "def add(a, b): return a + b" → implementation (requires TDD)
- "class User: ..." with methods → implementation (requires TDD)  
- "__init__.py" with only imports → structural (no TDD)
- "constants.py" with only CONSTANT = value → structural (no TDD)
    ...
    async def execute_tdd_validation(
        ...
        try:
            ...
            if "error" not in result:
                # Validate that the response is about the correct files
                reason = result.get("reason", "")
                detailed_analysis = result.get("detailed_analysis", "")

                # Check for hallucinated file references
                # Only check for specific service/controller names that indicate hallucination
                specific_hallucination_patterns = [
                    "UserService",
                    "MyService",
                    "MyController",
                    "ExampleService",
                ...
                # Only check for hallucinations if we have actual file paths AND it's not a test file
                if affected_files and affected_files[0]:
                    # Skip hallucination check for test files
                    is_test_file = any(
                        self.detect_test_files(f, "") for f in affected_files
                    )
                    if not is_test_file:
                        # Check if response mentions specific hallucinated class/file names
                        combined_text = f"{reason} {detailed_analysis}"
                        for pattern in specific_hallucination_patterns:
                            if pattern in combined_text:
                                # Hallucination detected - retry with stronger prompt
                                if os.environ.get("DEBUG"):
                                    print(
                                        f"DEBUG: Hallucination detected - pattern '{pattern}' found in response",
                                        file=sys.stderr,
                                    )
                                    print(
                                        f"DEBUG: Combined text: {combined_text[:200]}...",
                                        file=sys.stderr,
                                    )
                                return {
                                    "approved": False,  # Fail-closed on hallucination
                                    "reason": "TDD validation blocked: LLM response validation failed",
                                    "tdd_phase": "unknown",
                                    "affected_files": affected_files,
                                    "detailed_analysis": f"LLM response validation failed - mentioned unrelated files. Original response: {reason}",
                                }
