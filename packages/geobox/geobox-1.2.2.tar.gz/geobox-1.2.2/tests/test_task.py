...
def test_published_asset_property(api, mock_publish_task_data, mock_vector_data, mock_raster_data, mock_model_data):
    # raster layer
    ...
    assert task.publised_asset.name == expected_layer.name
    assert task.publised_asset.uuid == expected_layer.uuid
    assert task.publised_asset.data == expected_layer.data
    assert type(task.publised_asset) == Raster
    # vector layer
    ...
    mock_publish_task_data['name'] = 'import:publish-file-to-vector-layer'
    task = Task(api, uuid=mock_publish_task_data['uuid'], data=mock_publish_task_data)
    ...
    assert task.publised_asset.name == expected_layer.name
    assert task.publised_asset.layer_type == expected_layer.layer_type
    assert task.publised_asset.uuid == expected_layer.uuid
    assert task.publised_asset.data == expected_layer.data
    assert type(task.publised_asset) == VectorLayer
    #model
    ...
    mock_publish_task_data['name'] = 'import:publish-file-to-3d-model'
    task = Task(api, uuid=mock_publish_task_data['uuid'], data=mock_publish_task_data)
