...
def list_remove_duplicates(target: list,
    ...
    For convenience, the modified imput list is returned.
    ...
    if is_sorted:
        write_index = 1
        # remove duplicates by comparing each item with the previous unique one
        for read_index in range(1, len(target)):
            if target[read_index] != target[write_index - 1]:
                # overwrite the beginning of the list with this unique element
                target[write_index] = target[read_index]
                write_index += 1
    else:
        write_index = 0
        for read_index in range(len(target)):
            item: Any = target[read_index]
            # verify if 'item' is already in the
            if item not in target[:write_index]:
                # overwrite the beginning of the list with this unique element
                target[write_index] = item
                write_index += 1
