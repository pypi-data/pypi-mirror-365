...
class JobEditForm(NetBoxModelForm):
    ...
    platforms = lazy(PlatformMultipleChoiceField.get_choices,
                     PlatformMultipleChoiceField)
    commands = forms.CharField(
        ...
        widget=forms.Textarea(attrs={
            'rows': 15,
            'cols': 80,
            'placeholder': 'Enter cli commands separated by new-line...',
...
def get_job_exec_form(job: models.Job, fixtures):
    ...
    qs = get_runnable_devices_query(job.platforms or None)
    dev_field = forms.ModelMultipleChoiceField(
        queryset=qs,
        help_text=('Devices known to Netpicker by IP address'
                   ' (and comply to platform(s) specified by the rule)'))
