...
def getDefineForDevice(device_id, familyDefines):
    ...
    deviceDefines = sorted(
        [define for define in familyDefines if define.startswith(devName)]
    )
...
stm32_flash_latency = {
    ...
    "l1": [
        ...
        {1800: [16, 32], 1500: [8, 16], 1200: [2.1, 4.2]},  # Cat 2,3,4,5,6
    ],
    "l4": [
        {  # L4+ devices
        ...
        {1200: [16, 32, 48, 64, 80], 1000: [6, 12, 18, 26]},  # L4 devices
    ],
    "l5": {
...
def getFlashLatencyForDevice(did):
    ...
    # Convert MHz to Hz and filter out string keys
    lconv = lambda l: {
        k: [int(f * 1e6) for f in v] for k, v in l.items() if isinstance(k, int)
    }
    if isinstance(lts, dict):
        return lconv(lts)
        # whole family uses same table
    for lt in lts:
        # check if all conditions match
...
def getMemoryForDevice(device_id, total_flash, total_ram):
    ...
    if main_sram is not None:
        main_sram_name = next(
            ram for ram in mem_start.keys() if main_sram.startswith(ram)
        )
        # compute the size from total ram
        mem_model[main_sram] = total_ram
        main_sram_index = (
            int(main_sram.split("sram")[-1]) if main_sram[-1].isdigit() else 0
        )
    ...
    for name, size in mem_model.items():
        ...
        if index > 1:
            ...
            for mem_name, mem_size in mem_model.items():
                mem_index = (
                    int(mem_name.split("sram")[-1]) if mem_name[-1].isdigit() else 0
                )
