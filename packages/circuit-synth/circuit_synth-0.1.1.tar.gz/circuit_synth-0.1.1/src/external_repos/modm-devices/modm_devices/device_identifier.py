...
class DeviceIdentifier:
    ...
    def _ustring(self):
        if self.__ustring is None:
            self.__ustring = "".join(
                [k + self._properties[k] for k in sorted(self._properties.keys())]
            )
    ...
    def __repr__(self):
        return (
            self.string if self.naming_schema else "DeviceId({})".format(self._ustring)
        )
            ...
            for clength in range(len(keys)):
                for kcomb in itertools.combinations(keys, clength):
                    filtered = complete.filter(
                        lambda i: any(i[k] in diff.getAttribute(k) for k in kcomb)
                    )
