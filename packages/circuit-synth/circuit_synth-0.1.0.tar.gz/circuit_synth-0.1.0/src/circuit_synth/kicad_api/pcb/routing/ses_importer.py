...
from ..types import Track, Via, Point
...
class SESParser:
    ...
    def parse(self) -> RoutingSession:
        ...
        logger.info(f"Parsed {len(self.session.wires)} wires and {len(self.session.vias)} vias")
        return self.session
    ...
    def _parse_routes(self):
        ...
        if network_out_start:
            ...
            if paren_count == 0:
                ...
                network_out_match = re.search(r'\(network_out\s*(.*)\)\s*\)', routes_content, re.DOTALL)
            ...
            while True:
                ...
                net_match = re.search(r'\(net\s+(?:"([^"]+)"|(\S+))', network_content[pos:])
                if not net_match:
                    break
                
                # Get the net name from whichever group matched
                net_name = net_match.group(1) if net_match.group(1) else net_match.group(2)
            ...
            net_count = 0
            while True:
                network_match = re.search(r'\(network\s+"([^"]+)"', routes_content[pos:])
            ...
            logger.info(f"=== Parsed {net_count} nets from individual network blocks ===")
    ...
    def _parse_network_routes(self, net_name: str, content: str):
        ...
        for match in re.finditer(path_pattern, content, re.DOTALL):
            ...
            if len(coords) >= 2:
                ...
                wire_count += 1
                logger.debug(f"  Added path wire {wire_count} on layer {layer}: {len(coords)} points, width={width}")
        ...
        for match in re.finditer(protect_pattern, content, re.DOTALL):
            ...
            if len(coords) >= 2:
                ...
                self.session.add_wire(wire)
                wire_count += 1
                logger.debug(f"  Added protected wire {wire_count} on layer {layer}: {len(coords)} points, width={width}")
...
def import_ses_to_pcb(pcb_file: str, ses_file: str, output_file: Optional[str] = None) -> str:
    """
