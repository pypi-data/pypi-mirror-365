...
class SpiralHierarchicalPlacer(HierarchicalPlacer):
    ...
    def __init__(self, 
                 ...
                 max_spiral_radius: float = 50.0): # mm - maximum search radius
        """
    ...
    def place(self, components: List[ComponentWrapper],
        ...
        return super().place(components, connections, board_width, board_height, **kwargs)
    ...
    def _build_connection_map(self, connections: List[Tuple[str, str]]):
        ...
        logger.debug(f"Built connection map with {len(self._connection_map)} connected components")
    ...
    def _find_best_placement_point(self, 
        ...
        if self._is_valid_position(item, ideal_x, ideal_y, placed_items):
            logger.debug(f"Placing {self._get_reference(item)} at ideal position ({ideal_x:.2f}, {ideal_y:.2f})")
        ...
        if best_position:
            logger.debug(f"Found position for {self._get_reference(item)} at ({best_position[0]:.2f}, {best_position[1]:.2f})")
            return best_position
        
        # Fallback to original algorithm if spiral search fails
        logger.debug(f"Spiral search failed for {self._get_reference(item)}, falling back to corner placement")
        return super()._find_best_placement_point(item, placed_items, placement_points)
    
    def _calculate_ideal_position(self, item, placed_items: List) -> Tuple[float, float]:
        """
        ...
        logger.debug(f"Ideal position for {ref}: ({avg_x:.2f}, {avg_y:.2f}) based on {len(placed_connected)} connected components")
        
        return (avg_x, avg_y)
    
    def _spiral_search(self, item, start_x: float, start_y: float, placed_items: List) -> Optional[Tuple[float, float]]:
        """
