...
class HierarchicalPlacer(PlacementAlgorithm):
    ...
    def _group_by_hierarchy(self, components: List[ComponentWrapper]) -> Dict[str, ComponentGroup]:
        ...
        for group in groups.values():
            for component in group:
                ...
                logger.debug(f"Component {component.reference}: original bbox {original_bbox}, inflated to {component._bbox_cache}")
        ...
        for item in sorted_items:
            ...
            if placement_points:
                ...
                if not placed_items:
                    ...
                    self._set_bottom_left(item, board_min_x + margin, board_min_y + margin)
            ...
            placement_points.extend([
                (orig_bbox.min_x - spacing, orig_bbox.min_y),  # Left side with spacing
                (orig_bbox.max_x + spacing, orig_bbox.min_y),  # Right side with spacing
                (orig_bbox.min_x, orig_bbox.min_y - spacing),  # Top side with spacing
                (orig_bbox.min_x, orig_bbox.max_y + spacing),  # Bottom side with spacing
            ])
    ...
    def _find_best_placement_point(self, 
        ...
        for point in placement_points:
            ...
            if not overlaps:
                ...
                width = bbox.width()
                height = bbox.height()
                size = bbox.area() + abs(width - height) * 0.1  # Prefer square-ish layouts
    ...
    def _bbox_within_board(self, bbox: BoundingBox) -> bool:
        ...
        return (bbox.min_x >= self.board_outline.min_x + margin and
                bbox.max_x <= self.board_outline.max_x - margin and
                bbox.min_y >= self.board_outline.min_y + margin and
                bbox.max_y <= self.board_outline.max_y - margin)