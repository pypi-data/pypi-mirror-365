...
from ..core.types import Label, Point, Schematic, LabelType, Wire
from .connection_utils import snap_to_grid, points_equal
...
class LabelManager:
    ...
    def add_label(
        ...
        if orientation not in [0, 90, 180, 270]:
            logger.error(f"Invalid orientation {orientation}, must be 0, 90, 180, or 270")
            return None
    ...
    def find_labels_at_point(self, point: Tuple[float, float], tolerance: float = 0.1) -> List[Label]:
        """
    ...
    def find_labels_by_text(self, pattern: str, exact_match: bool = False) -> List[Label]:
        """
    ...
    def _find_wires_near_point(self, point: Tuple[float, float], tolerance: float = 2.54) -> List[Wire]:
    ...
    def get_labels_by_type(self, label_type: LabelType) -> List[Label]:
        ...
        return [label for label in self.schematic.labels if label.label_type == label_type]
