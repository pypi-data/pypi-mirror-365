...
from ..core.types import SchematicSymbol, Point, Schematic
...
class ComponentManager:
    ...
    def find_components_by_library(self, library_pattern: str) -> List[SchematicSymbol]:
        ...
        return [
            comp for comp in self.schematic.components
            if comp.lib_id == library_pattern
        ]
        ...
        if check_collision:
            ...
            for comp in self.schematic.components:
                if comp.reference != reference:
                    if (abs(comp.position.x - new_position[0]) < 5.0 and
                        abs(comp.position.y - new_position[1]) < 5.0):
                        logger.warning(f"Position {new_position} would collide with {comp.reference}")
                        return False
    ...
    def validate_schematic(self) -> Tuple[bool, List[str]]:
        ...
        for i, comp1 in enumerate(self.schematic.components):
            for comp2 in self.schematic.components[i+1:]:
                if (abs(comp1.position.x - comp2.position.x) < 5.0 and
                    abs(comp1.position.y - comp2.position.y) < 5.0):
                    messages.append(f"Components {comp1.reference} and {comp2.reference} may overlap")
    ...
    def _snap_to_grid(self, position: Tuple[float, float], grid_size: float = 2.54) -> Tuple[float, float]:
        """
