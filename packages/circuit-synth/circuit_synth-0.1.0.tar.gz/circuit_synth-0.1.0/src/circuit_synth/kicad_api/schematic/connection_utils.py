...
from ..core.types import Point, SchematicSymbol, SchematicPin
...
def get_wire_segments(points: List[Point]) -> List[Tuple[Tuple[float, float], Tuple[float, float]]]:
    """
...
def simplify_points(points: List[Point], tolerance: float = 0.01) -> List[Point]:
    ...
    for i in range(1, len(points) - 1):
        ...
        cross = (curr.y - prev.y) * (next.x - curr.x) - (curr.x - prev.x) * (next.y - curr.y)
        if abs(cross) > tolerance:
