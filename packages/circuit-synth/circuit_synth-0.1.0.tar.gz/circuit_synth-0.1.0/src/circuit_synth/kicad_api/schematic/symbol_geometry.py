...
class SymbolGeometry:
    ...
    CHAR_WIDTH_FACTOR = 0.8  # Character width is ~80% of font height (increased for better accuracy)
    ...
    def get_symbol_bounds(self, lib_id: str) -> Tuple[float, float]:
        ...
        try:
            ...
            logger.info(f"Calculated bounds for {lib_id}: {bounds[0]:.2f} x {bounds[1]:.2f} mm")
            return bounds
        except Exception as e:
            logger.warning(f"Failed to parse symbol {lib_id}: {e}")
            # Fall back to defaults
            default_bounds = self._get_default_bounds(lib_id)
            logger.info(f"Using default bounds for {lib_id}: {default_bounds[0]:.2f} x {default_bounds[1]:.2f} mm")
    ...
    def _calculate_symbol_bounds(self, lib_id: str) -> Tuple[float, float]:
        ...
        if symbol_def.graphic_elements:
            for graphic in symbol_def.graphic_elements:
                bounds = self._update_bounds_from_graphic_element(graphic, min_x, min_y, max_x, max_y)
