...
from ..core.types import Schematic, Point, Label, LabelType, SchematicSymbol, Sheet
...
from .placement import PlacementStrategy, PlacementEngine
...
class ProjectGenerator:
    ...
    def _copy_blank_project_files(self):
        ...
        blank_project_dir = Path(__file__).parent.parent.parent / "kicad" / "sch_gen" / "kicad_blank_project"
        
        if not blank_project_dir.exists():
            raise FileNotFoundError(f"Blank project template not found at {blank_project_dir}")
    ...
    def _generate_simple_design(self, circuit, schematic_path: Path):
        ...
        self._generate_top_sheet(schematic_path, [(sub_sheet_name, sub_sheet_path.name, sheet_dims, sheet_uuid)], top_uuid)
        
        logger.info(f"Generated simple hierarchical schematic with top sheet: {schematic_path}")
    ...
    def _generate_hierarchical_design(self, circuit, schematic_path: Path):
        ...
        sheet_info.append((main_sheet_name, main_sheet_path.name, main_dims, main_sheet_uuid))
        ...
        for subcircuit in circuit._subcircuits:
            ...
            self._generate_sub_sheet(subcircuit, sub_sheet_path, top_uuid, sub_sheet_uuid)
    ...
    def _generate_root_with_unified_placement(self, circuit, schematic_path: Path):
        ...
        if hasattr(circuit, '_subcircuits') and circuit._subcircuits:
            for subcircuit in circuit._subcircuits:
                ...
                sheet_size = placement_engine._estimate_sheet_size(Sheet(
                    name=sub_name,
                ...
                sheet_info.append((sub_name, subcircuit, sub_sheet_path, sub_sheet_uuid, sheet))
        ...
        for sub_name, subcircuit, sub_sheet_path, sub_sheet_uuid, sheet in sheet_info:
            self._generate_sub_sheet(subcircuit, sub_sheet_path, root_uuid, sub_sheet_uuid)
        ...
        logger.info(f"Generated root schematic with {len(schematic.components)} components and {len(schematic.sheets)} sheets")
        
    def _generate_top_sheet(self, schematic_path: Path, sheet_info: list, top_uuid: str):
        ...
        for i, (sheet_name, sheet_filename, sheet_dims, sheet_uuid) in enumerate(sheet_info):
            ...
            sheet_symbol = [
                ...
                [
                    ...
                    [sexpdata.Symbol("color"), 0, 0, 0, 0.0000]
                ],
                [sexpdata.Symbol("uuid"), sheet_uuid],
                [
                    sexpdata.Symbol("property"), "Sheetname", sheet_name,
                ...
                [
                    sexpdata.Symbol("property"), "Sheetfile", sheet_filename,
                ...
                [
                    ...
                    [
                        sexpdata.Symbol("project"), self.project_name,
                        [
                            sexpdata.Symbol("path"), "/" + sheet_uuid,
    ...
    def _generate_sub_sheet(self, circuit, sub_sheet_path: Path, top_uuid: str, sheet_uuid: str):
                        ...
                        if pin_pos:
                            ...
                            if pos_tuple not in net_label_positions[net.name]:
                                ...
                                if pin_obj:
                                    ...
                                    label = GeometryUtils.create_dynamic_hierarchical_label(
                                        net.name, pin_obj, pin_pos,
                                        kicad_comp.rotation if hasattr(kicad_comp, 'rotation') else 0
                                    )
                                    schematic.add_label(label)
                                    # Mark this position as having a label for this net
                                    net_label_positions[net.name].add(pos_tuple)
                                    logger.debug(f"Created label for net '{net.name}' at position {label.position}")
                            else:
                                logger.debug(f"Skipping duplicate label for net '{net.name}' at position {pos_tuple}")
    ...
    def _get_pin_position(self, component: SchematicSymbol, pin_num: str) -> Optional[Point]:
        """
