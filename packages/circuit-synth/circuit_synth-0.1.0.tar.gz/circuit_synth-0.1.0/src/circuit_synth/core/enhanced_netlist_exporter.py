...
class NetlistMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'performance_ratios': {
                'components_per_second': self.component_count / (self.generation_time_ms / 1000) if self.generation_time_ms > 0 else 0,
                'bytes_per_component': self.file_size_bytes / max(self.component_count, 1),
                'cache_hit_rate': self.cache_hits / max(self.cache_hits + self.cache_misses, 1) * 100,
                'memory_efficiency': self.file_size_bytes / max(self.memory_usage_mb * 1024 * 1024, 1)
            }
        }
...
class EnhancedNetlistExporter:
    ...
    def export_netlist_with_analytics(self, circuit, output_path: str,
                                    enable_optimization: bool = True,
                                    force_rust_backend: bool = False) -> NetlistMetrics:
        """
        ...
        try:
            with generation_logger.stage_timer(GenerationStage.NETLIST_GENERATION):
                ...
                if rust_backend_used:
                    context_logger.info(
                        ...
                        reason="performance_optimization" if component_count > 100 else "forced"
                    )
                    self._generation_stats['rust_backend_usage_count'] += 1
                
                # Apply optimizations if enabled
                optimization_applied = enable_optimization and self._should_optimize(component_count)
            ...
            generation_logger.log_error_with_recovery(
                ...
                recovery_details={
                    'component_count': component_count if 'component_count' in locals() else 0,
                    'output_path': output_path,
                    'generation_time_ms': generation_time_ms
                }
            )
    ...
    def _export_with_rust_backend(self, circuit, output_path: str, 
                                optimization_applied: bool) -> tuple:
        """Export netlist using Rust backend with performance tracking."""
        context_logger.debug(
            "Exporting with Rust backend",
            component="NETLIST_RUST_BACKEND"
        )
        ...
        try:
            ...
            start_time = time.time()
            result = export_netlist_optimized(circuit, output_path, optimization_applied)
            ...
            return self._export_with_python_backend(circuit, output_path, optimization_applied)
    
    def _export_with_python_backend(self, circuit, output_path: str,
                                  optimization_applied: bool) -> tuple:
        """Export netlist using Python backend with performance tracking."""
        context_logger.debug(
            "Exporting with Python backend",
            component="NETLIST_PYTHON_BACKEND"
        )
    ...
    def _log_netlist_analytics(self, metrics: NetlistMetrics):
        ...
        performance_logger.log_metric("netlist_components_per_second", 
                                    performance_data['performance_ratios']['components_per_second'],
                                    component="NETLIST_THROUGHPUT")
        
        performance_logger.log_metric("netlist_memory_efficiency",
                                    performance_data['performance_ratios']['memory_efficiency'],
                                    component="NETLIST_EFFICIENCY")
        
        performance_logger.log_metric("netlist_cache_hit_rate",
...
def export_netlist_with_analytics(circuit, output_path: str,
                                 enable_optimization: bool = True,
                                 force_rust_backend: bool = False) -> NetlistMetrics:
