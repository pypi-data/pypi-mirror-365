...
class SimplifiedPinAccess:
    ...
    def __getitem__(self, pin_id: Union[int, str]) -> Union[Pin, PinGroup]:
        ...
        if not isinstance(pin_id, str):
            raise ComponentError(f"Pin identifier must be string or int, got {type(pin_id).__name__}")
        ...
        if hasattr(self, '_pin_names'):
            ...
            if available_names:
                available_pins.extend([f"'{name}'" for name in available_names if name and name != "~"])
    ...
    def __setitem__(self, pin_id: Union[int, str], new_pin: Pin):
        ...
        if not isinstance(new_pin, Pin):
            raise ComponentError(f"Can only assign Pin objects, got {type(new_pin).__name__}")
