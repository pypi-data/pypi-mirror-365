...
from .unified_logger import context_logger, ContextLogger
...
class BackwardCompatibilityLogger:
    ...
    def __init__(self, name: str = "compat", context_logger_instance: Optional[ContextLogger] = None):
        self.name = name
    ...
    def _log_with_compat(self, level: str, msg, *args, **kwargs):
        ...
        log_method = getattr(self.context_logger, level.lower(), self.context_logger.info)
...
class LoggingMigrationTool:
    ...
    def __init__(self):
        self.conversion_patterns = [
            ...
            (r'logging\.debug\((.*?)\)', r'context_logger.debug(\1, component="DEBUG")'),
            (r'logging\.warning\((.*?)\)', r'context_logger.warning(\1, component="WARNING")'),
            (r'logging\.error\((.*?)\)', r'context_logger.error(\1, component="ERROR")'),
            (r'logging\.critical\((.*?)\)', r'context_logger.error(\1, component="CRITICAL")'),
            ...
            (r'logger\.warning\((.*?)\)', r'context_logger.warning(\1, component="WARNING")'),
            (r'logger\.error\((.*?)\)', r'context_logger.error(\1, component="ERROR")'),
            (r'logger\.critical\((.*?)\)', r'context_logger.error(\1, component="CRITICAL")'),
            (r'logger\.exception\((.*?)\)', r'context_logger.error(\1, component="EXCEPTION", exc_info=True)'),
            
            # Print statements to logging
            (r'print\(f?"([^"]*)".*?\)', r'context_logger.info("\1", component="PRINT")'),
            (r"print\(f?'([^']*)'.*?\)", r'context_logger.info("\1", component="PRINT")'),
        ]
        ...
        try:
            ...
            for pattern, replacement in self.conversion_patterns:
                ...
                if matches:
                    ...
                    changes.extend([f"Converted {pattern} -> {replacement}" for _ in matches])
            
            # Add imports if logging calls were converted
            needs_import = any('context_logger' in change for change in changes)
            if needs_import and 'from circuit_synth.core.logging.unified import context_logger' not in content:
                # Find the best place to add import
                import_line = 'from circuit_synth.core.logging.unified import context_logger\n'
    ...
    def migrate_directory(self, directory: Path, recursive: bool = True) -> Dict[str, MigrationResult]:
        """
...
"""
        ...
        for file_path, result in results.items():
            report += f"\n{file_path}:\n"
            if result.success:
                report += f"  ✓ Successfully migrated ({len(result.changes_made)} changes)\n"
...
def patch_logging_module():
    ...
    logging.debug = lambda msg, *args, **kwargs: context_logger.debug(str(msg), component="LOGGING", **kwargs)
    logging.info = lambda msg, *args, **kwargs: context_logger.info(str(msg), component="LOGGING", **kwargs)
    logging.warning = lambda msg, *args, **kwargs: context_logger.warning(str(msg), component="LOGGING", **kwargs)
    logging.error = lambda msg, *args, **kwargs: context_logger.error(str(msg), component="LOGGING", **kwargs)
    logging.critical = lambda msg, *args, **kwargs: context_logger.error(str(msg), component="CRITICAL", **kwargs)
    logging.exception = lambda msg, *args, **kwargs: context_logger.error(str(msg), component="EXCEPTION", exc_info=True, **kwargs)
    
    context_logger.info("Standard logging module patched for unified logging", component="COMPAT")
...
def validate_migration(file_path: Path) -> List[str]:
    ...
    try:
        ...
        if 'context_logger' in content and 'from circuit_synth.core.logging.unified import context_logger' not in content:
