...
class PerformanceLogger:
    ...
    def _system_monitoring_loop(self):
        ...
        while self._monitoring_enabled:
            try:
                ...
                if self.logger:
                    self.logger.error(f"System monitoring error: {e}", component="PERF_MONITOR")
    ...
    def _collect_system_metrics(self):
        ...
        try:
            ...
            if self.logger:
                self.logger.error(f"Failed to collect system metrics: {e}", component="PERF_MONITOR")
    ...
    def log_counter(self, name: str, increment: int = 1, component: str = "METRICS", **metadata):
        """Log a counter metric."""
        self.log_metric(name, increment, "count", component, **metadata)
    
    def log_gauge(self, name: str, value: float, component: str = "METRICS", **metadata):
        """Log a gauge metric."""
        self.log_metric(name, value, "value", component, **metadata)
    
    def log_histogram(self, name: str, value: float, component: str = "METRICS", **metadata):
        """Log a histogram metric."""
        self.log_metric(name, value, "histogram", component, **metadata)
    
    def get_operation_stats(self, operation: str, hours: int = 24) -> Optional[OperationStats]:
        ...
        with self._lock:
            ...
            recent_metrics = [
                metric for metric in self._operation_history[operation]
            ...
            error_count = sum(1 for metric in recent_metrics 
                            if metric.metadata.get('error', False))
    ...
    def get_system_metrics_summary(self, hours: int = 1) -> Dict[str, Any]:
        ...
        with self._lock:
            recent_metrics = [
                metric for metric in self._system_metrics_history
                if datetime.fromisoformat(metric.timestamp) > cutoff_time
            ]
        ...
        return {
            ...
            'threads': {
                'current_count': recent_metrics[-1].active_threads
            }
        }
    ...
    def generate_performance_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        if operation_stats:
            top_by_calls = sorted(operation_stats.items(), 
                                key=lambda x: x[1].total_calls, reverse=True)[:10]
            top_by_duration = sorted(operation_stats.items(), 
                                   key=lambda x: x[1].total_duration_ms, reverse=True)[:10]
            slowest_avg = sorted(operation_stats.items(), 
                               key=lambda x: x[1].avg_duration_ms, reverse=True)[:10]
        else:
        ...
        return {
            ...
            'summary': {
                ...
                'total_duration_ms': sum(stats.total_duration_ms for stats in operation_stats.values()),
                'total_errors': sum(stats.error_count for stats in operation_stats.values())
            },
            'top_operations': {
                'by_call_count': [(op, stats.total_calls) for op, stats in top_by_calls],
                'by_total_duration': [(op, stats.total_duration_ms) for op, stats in top_by_duration],
                'by_avg_duration': [(op, stats.avg_duration_ms) for op, stats in slowest_avg]
            },
            'operation_details': {op: asdict(stats) for op, stats in operation_stats.items()},
