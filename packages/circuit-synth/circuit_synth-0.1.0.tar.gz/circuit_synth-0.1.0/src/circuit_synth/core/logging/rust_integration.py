...
class RustLogHandler(logging.Handler):
    ...
    def _parse_rust_message(self, message: str) -> Dict[str, Any]:
        ...
        elif message.startswith('COMPONENT_INIT:') or message.startswith('COMPONENT_SHUTDOWN:'):
            ...
            for part in parts[1:]:
                if '=' in part:
                    ...
                    if key == 'operation':
                        ...
                        context['clean_message'] = f"Rust error in {context.get('operation', 'unknown')}: {value}"
                    elif key == 'context':
            ...
            for part in parts[1:]:
                if '=' in part:
                    ...
                    if key == 'operation':
                        ...
                        context['clean_message'] = f"Rust memory usage for {context.get('operation', 'unknown')}: {value}MB"
        except Exception:
            pass
            ...
            for part in parts[1:]:
                if '=' in part:
                    ...
                    if key == 'cache':
                        ...
                        context['clean_message'] = f"Rust cache '{context.get('cache_name', 'unknown')}' hit rate: {value}"
        except Exception:
            pass
            ...
            for part in parts[1:]:
                if '=' in part:
                    ...
                    if key == 'operation':
                        ...
                        context['clean_message'] = f"Rust file {context.get('file_operation', 'operation')}: {value}MB/s"
        except Exception:
            pass
            ...
            for part in parts[1:]:
                if '=' in part:
                    ...
                    if key == 'query':
                        ...
                        context['duration_ms'] = float(value)
                        context['clean_message'] = f"Rust search found {context.get('results_found', 0)} results in {value}ms"
        except Exception:
            pass
            ...
            for part in parts[1:]:
                if '=' in part:
                    ...
                    if key == 'component':
                        ...
                        context['clean_message'] = f"Rust component '{context.get('rust_component', 'unknown')}' {context.get('lifecycle_event', 'event')}"
                    elif key == 'uptime_ms':
                        context['uptime_ms'] = float(value)
                        context['clean_message'] = f"Rust component '{context.get('rust_component', 'unknown')}' shutdown after {value}ms"
        except Exception:
            pass
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                'success_rate': (self._message_count - self._error_count) / max(1, self._message_count)
            }
...
def setup_rust_logging(context_logger_instance=None) -> RustLogHandler:
    ...
    logger_instance.info("Rust logging integration configured successfully", component="SYSTEM")
