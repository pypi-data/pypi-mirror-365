...
from .unified_logger import (
    ContextLogger, PerformanceLogger, LLMConversationLogger,
    UserContext, initialize_logging, context_logger
)
from .context_manager import (
    SessionManager, RequestContext, get_current_context,
    current_user, current_session, current_request, current_chat
)
from .performance_monitor import (
    PerformanceMetric, monitor_performance, performance_context
)
from .config_manager import LoggingConfig
from .migration_utils import (
    BackwardCompatibilityLogger, LoggingMigrationTool,
    create_compatibility_logger, validate_migration
)
...
class TestSessionManager:
    ...
    def test_session_activity_tracking(self):
        ...
        original_activity = self.session_manager.get_session_info("session1").last_activity
        time.sleep(0.01)  # Small delay
        
        self.session_manager.update_session_activity("session1")
        updated_activity = self.session_manager.get_session_info("session1").last_activity
...
'''
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert 'from circuit_synth.core.logging.unified import context_logger' in migrated_content
...
class TestConcurrency:
        ...
        def user_logging_task(user_id: str, message_count: int):
            with UserContext(user_id, f"{user_id}_session"):
                ...
                for i in range(message_count):
                    ...
                    context_logger.info(f"Message {i} from {user_id}", component="CONCURRENT")
    ...
    def test_concurrent_performance_logging(self):
        ...
        def performance_task(task_id: int):
            with perf_logger.timer(f"concurrent_task_{task_id}", component="CONCURRENT"):
                time.sleep(0.01)  # Small delay
                return task_id
    ...
    def test_concurrent_logging_throughput(self):
        ...
        def logging_worker(worker_id: int, message_count: int):
            ...
            with UserContext(f"worker_{worker_id}"):
                for i in range(message_count):
                    logger.info(f"Worker {worker_id} message {i}", component="BENCHMARK")
        ...
        assert throughput > 5000, f"Concurrent throughput too low: {throughput:.0f} msg/s"
