...
from .loggers import DashboardLogger, LLMLogger, FileOperationLogger
...
def setup_unified_logging(
    ...
    try:
        ...
        if enable_rust:
            try:
                ensure_rust_logging_setup()
                context_logger.info("Rust logging integration enabled", component="SYSTEM")
        ...
        if enable_migration_compat:
            try:
                patch_logging_module()
                context_logger.info("Backward compatibility patching enabled", component="SYSTEM")
            except Exception as e:
                context_logger.warning(
