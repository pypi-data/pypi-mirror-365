...
class ValidationResult:
    ...
    def error_count(self) -> int:
        return len([issue for issue in self.issues if issue.severity in [ValidationSeverity.ERROR, ValidationSeverity.CRITICAL]])
...
class ValidationLogger:
    ...
    def validate_with_logging(self, validation_type: ValidationType, stage: GenerationStage,
                            validator_func: callable, *args, **kwargs) -> ValidationResult:
        """
        ...
        try:
            ...
            success = len([issue for issue in issues if issue.severity in [ValidationSeverity.ERROR, ValidationSeverity.CRITICAL]]) == 0
    ...
    def validate_circuit_syntax(self, code: str, context: Optional[Dict[str, Any]] = None) -> ValidationResult:
    ...
    def validate_circuit_semantics(self, circuit_obj, context: Optional[Dict[str, Any]] = None) -> ValidationResult:
    ...
    def validate_netlist(self, netlist_path: str, context: Optional[Dict[str, Any]] = None) -> ValidationResult:
        ...
        def netlist_validator(netlist_path: str) -> Dict[str, Any]:
            ...
            try:
                ...
                if '(comp' not in netlist_content and '(component' not in netlist_content:
                    issues.append({
                        'severity': 'warning',
    ...
    def _parse_validation_output(self, output: Any, validation_type: ValidationType) -> List[ValidationIssue]:
    ...
    def get_recent_validation_summary(self, limit: int = 10) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_duration_ms': sum(v.duration_ms for v in recent_validations) / max(len(recent_validations), 1),
...
def validate_circuit_syntax(code: str, context: Optional[Dict[str, Any]] = None) -> ValidationResult:
    """Validate circuit code syntax."""
    return validation_logger.validate_circuit_syntax(code, context)

def validate_circuit_semantics(circuit_obj, context: Optional[Dict[str, Any]] = None) -> ValidationResult:
    """Validate circuit semantic correctness."""
    return validation_logger.validate_circuit_semantics(circuit_obj, context)

def validate_netlist(netlist_path: str, context: Optional[Dict[str, Any]] = None) -> ValidationResult:
    """Validate generated netlist."""
