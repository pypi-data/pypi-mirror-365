...
class CircuitSynthLogger:
    ...
    def _setup_loggers(self):
        ...
        # User-specific logs
        user_log_dir = base_log_dir / "users" / self.username / datetime.now().strftime('%Y-%m-%d')
        ...
        # Performance logs
        perf_log_dir = base_log_dir / "performance" / datetime.now().strftime('%Y-%m-%d')
        ...
        session_log_path = user_log_dir / f"session_{self.session_id.split('_', 1)[1]}.log"
        self._create_logger('session', session_log_path, detailed_formatter, microsecond_filter)
        
        # Daily summary logger
        daily_log_path = user_log_dir / "daily_summary.log"
        self._create_logger('daily', daily_log_path, detailed_formatter, microsecond_filter)
        
        # System loggers
        dashboard_log_path = system_log_dir / "dashboard.log"
        self._create_logger('dashboard', dashboard_log_path, detailed_formatter, microsecond_filter)
        
        orchestrator_log_path = system_log_dir / "orchestrator.log"
        self._create_logger('orchestrator', orchestrator_log_path, detailed_formatter, microsecond_filter)
        ...
        self._create_logger('errors', error_log_path, detailed_formatter, microsecond_filter)
        
        # Performance logger
        metrics_log_path = perf_log_dir / "metrics.log"
        self._create_logger('performance', metrics_log_path, detailed_formatter, microsecond_filter)
        
        # Master logger (everything)
        master_log_path = master_log_dir / "circuit_synth_all.log"
        self._create_logger('master', master_log_path, detailed_formatter, microsecond_filter)
        
        # Log session start
        self.log_session_start()
    
    def _create_logger(self, name: str, log_path: Path, formatter: logging.Formatter, filter: logging.Filter):
    ...
    def log_dash_callback(self, callback_id: str, action: str, duration_ms: float, 
    ...
    def log_llm_request(self, model: str, prompt_tokens: int, max_tokens: int, 
                       temperature: float, request_id: str, prompt_preview: str):
    ...
    def log_llm_response(self, model: str, request_id: str, completion_tokens: int,
                        total_tokens: int, duration_ms: float, cost_usd: float, 
    ...
    def log_circuit_generation(self, stage: str, circuit_type: str, 
                              components: list, duration_ms: float):
    ...
    def log_performance_metric(self, metric: str, value: float, unit: str, process: str):
