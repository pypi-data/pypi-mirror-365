...
try:
    from .unified_logger import context_logger, performance_logger, UserContext
except ImportError:
    from circuit_synth.core.logging.unified_logger import context_logger, performance_logger, UserContext
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'statistics': {
                ...
                'validation_errors': sum(len(vr.errors) for vr in self.validation_results),
                'validation_warnings': sum(len(vr.warnings) for vr in self.validation_results),
                'avg_component_selection_time': sum(cs.search_duration_ms for cs in self.component_selections) / max(len(self.component_selections), 1),
                'total_validation_time': sum(vr.duration_ms for vr in self.validation_results),
                'total_performance_time': sum(pm.duration_ms for pm in self.performance_metrics),
            }
        }
...
class GenerationLogger:
    ...
    def start_session(self, description: str, requirements: Dict[str, Any], 
    ...
    def end_session(self, success: bool, error_message: Optional[str] = None,
        ...
        if not self.current_session:
            context_logger.warning("No active session to end", component="GENERATION_SESSION")
            return
    ...
    def log_component_selection(self, component_type: str, selected_symbol: str,
                              library: str, reason: ComponentSelectionReason,
                              alternatives_considered: List[str],
                              selection_criteria: Dict[str, Any],
                              confidence_score: float,
                              search_duration_ms: float):
    ...
    def log_validation_result(self, stage: GenerationStage, validation_type: str,
                            success: bool, errors: List[str], warnings: List[str],
                            duration_ms: float, context: Dict[str, Any]):
    ...
    def log_performance_metrics(self, stage: GenerationStage, duration_ms: float,
                              memory_usage_mb: float, cpu_usage_percent: float,
                              io_operations: int, cache_hits: int, cache_misses: int):
        ...
        performance_logger.log_metric(f"{stage.value}_memory_usage", memory_usage_mb, unit="megabytes", component="MEMORY_USAGE")
        performance_logger.log_metric(f"{stage.value}_cpu_usage", cpu_usage_percent, unit="percent", component="CPU_USAGE")
        performance_logger.log_metric(f"{stage.value}_io_operations", io_operations, unit="count", component="IO_OPERATIONS")
        performance_logger.log_metric(f"{stage.value}_cache_hit_rate",
                                    cache_hits / max(cache_hits + cache_misses, 1) * 100,
                                    unit="percent",
                                    component="CACHE_PERFORMANCE")
    
    def log_error_with_recovery(self, stage: GenerationStage, error: Exception,
                              recovery_attempted: bool, recovery_successful: bool,
    ...
    def log_netlist_generation_analytics(self, component_count: int, net_count: int,
                                       generation_time_ms: float, file_size_bytes: int,
                                       optimization_applied: bool, rust_backend_used: bool):
        ...
        analytics_data = {
            ...
            'components_per_second': component_count / (generation_time_ms / 1000) if generation_time_ms > 0 else 0,
        ...
        performance_logger.log_metric("netlist_generation_time", generation_time_ms, component="NETLIST_PERFORMANCE")
        performance_logger.log_metric("netlist_components_per_second", analytics_data['components_per_second'], component="NETLIST_PERFORMANCE")
        performance_logger.log_metric("netlist_file_size", file_size_bytes, component="NETLIST_PERFORMANCE")
...
def start_generation_session(description: str, requirements: Dict[str, Any], 
                           user_id: Optional[str] = None) -> str:
    """Start a new generation session."""
    return generation_logger.start_session(description, requirements, user_id)

def end_generation_session(success: bool, error_message: Optional[str] = None,
                         files_generated: Optional[List[str]] = None):
    """End the current generation session."""
    generation_logger.end_session(success, error_message, files_generated)

def log_component_selection(component_type: str, selected_symbol: str, library: str,
                          reason: ComponentSelectionReason, alternatives: List[str],
                          criteria: Dict[str, Any], confidence: float, duration_ms: float):
    """Log component selection decision."""
    generation_logger.log_component_selection(
        component_type, selected_symbol, library, reason, alternatives,
        criteria, confidence, duration_ms
    )

def log_validation_result(stage: GenerationStage, validation_type: str, success: bool,
                        errors: List[str], warnings: List[str], duration_ms: float,
                        context: Dict[str, Any]):
...
def log_netlist_analytics(component_count: int, net_count: int, generation_time_ms: float,
                        file_size_bytes: int, optimization_applied: bool, rust_backend_used: bool):
    """Log netlist generation analytics."""
    generation_logger.log_netlist_generation_analytics(
        component_count, net_count, generation_time_ms, file_size_bytes,
        optimization_applied, rust_backend_used
    )