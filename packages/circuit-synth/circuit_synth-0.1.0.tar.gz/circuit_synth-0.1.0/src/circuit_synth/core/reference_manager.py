...
class ReferenceManager:
    ...
    def __init__(self, initial_counters: Optional[dict[str, int]] = None):
        ...
        self._prefix_counters: dict[str, int] = initial_counters.copy() if initial_counters else {}
    ...
    def register_reference(self, ref: str) -> None:
        ...
        if not self.validate_reference(ref):
            raise ValidationError(f"Reference {ref} already in use in circuit hierarchy")
            
        self._used_references.add(ref)
        context_logger.debug("Registered reference", component="REFERENCE_MANAGER", reference=ref)
    ...
    def set_initial_counters(self, counters: Dict[str, int]) -> None:
        ...
        for prefix, start_num in counters.items():
            if prefix not in self._prefix_counters or self._prefix_counters[prefix] < start_num:
                self._prefix_counters[prefix] = start_num
                context_logger.debug("Set initial counter for prefix", component="REFERENCE_MANAGER", prefix=prefix, start_num=start_num)
    ...
    def generate_next_unnamed_net_name(self) -> str:
        ...
        context_logger.debug("Generated unnamed net name", component="REFERENCE_MANAGER", net_name=name)
        return name
