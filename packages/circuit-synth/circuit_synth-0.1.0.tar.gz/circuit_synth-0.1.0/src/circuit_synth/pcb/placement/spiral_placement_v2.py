...
class SpiralPlacementAlgorithmV2:
    ...
    def place_components(
        ...
        sorted_components = sorted(
            components,
            key=lambda c: connection_graph.get(c.reference, ConnectionInfo(c.reference, set(), 0)).connection_count,
            reverse=True
        )
        ...
        if courtyard:
            print(f"[DEBUG] Component {first_comp.reference} has courtyard polygon with {len(courtyard.vertices)} vertices")
        else:
            print(f"[DEBUG] Component {first_comp.reference} has no courtyard, using bounding box")
        
        # Place remaining components
        for component in sorted_components[1:]:
            # Calculate ideal position based on connections
            ideal_x, ideal_y = self._calculate_ideal_position(component, connection_graph)
            ...
            if courtyard:
                print(f"[DEBUG] Component {component.reference} placed with courtyard at ({valid_pos[0]:.2f}, {valid_pos[1]:.2f})")
            
        return PlacementResult(success=True, message="Spiral placement with courtyard collision completed successfully")
    ...
    def _calculate_ideal_position(
        self,
        component: Footprint,
        connection_graph: Dict[str, ConnectionInfo]
    ) -> Tuple[float, float]:
        """
        ...
        if not conn_info or not conn_info.connected_refs:
            ...
            if self.placed_components:
                ...
                return sum_x / len(self.placed_components), sum_y / len(self.placed_components)
            else:
    ...
    def get_placement_info(self) -> Dict[str, Dict[str, any]]:
        ...
        for ref, footprint in self.placed_components.items():
            ...
            polygon = self.collision_detector.get_footprint_polygon(footprint, self.use_courtyard)
