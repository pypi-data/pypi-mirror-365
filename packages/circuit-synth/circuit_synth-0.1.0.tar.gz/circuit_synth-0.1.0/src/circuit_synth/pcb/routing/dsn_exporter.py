...
from ..types import Footprint, Pad, Point, Net, Layer, Line, Arc
...
class DSNExporter:
    ...
    def _extract_components(self) -> None:
        ...
        logger.debug(f"board.footprints keys: {list(self.board.footprints.keys()) if hasattr(self.board, 'footprints') else 'N/A'}")
        logger.debug(f"pcb_data footprints count: {len(self.board.pcb_data.get('footprints', []))}")
        ...
        if hasattr(self.board, 'footprints') and self.board.footprints:
            ...
            for ref, footprint in self.board.footprints.items():
                ...
                logger.debug(f"  Side: {side}, Position: ({footprint.position.x}, {footprint.position.y})")
                
                # Extract pads
                dsn_pads = []
                logger.debug(f"  Processing {len(footprint.pads)} pads")
                for pad in footprint.pads:
                    logger.debug(f"    Pad {pad.number}: net={pad.net}, type={pad.type}, layers={pad.layers}")
                    ...
                    dsn_pad = DSNPad(
                        ...
                        drill=pad.drill if isinstance(pad.drill, (int, float)) else None
                    )
    ...
    def _extract_nets(self) -> None:
        ...
        for component in self.components:
            ...
            for pad in component.pads:
                ...
                logger.debug(f"  Pad {pad.name} (component {component.name}, pad {pad_num}): net_name = {net_name}")
    ...
    def _get_pad_net(self, ref: str, pad_num: str) -> Optional[str]:
        ...
        if ref in self.board.footprints:
            ...
            # Find the pad
            for pad in footprint.pads:
                logger.debug(f"    Checking pad {pad.number} (net={pad.net}, net_name={getattr(pad, 'net_name', 'N/A')})")
                    ...
                    if pad.net is not None and pad.net > 0:
                        ...
                        for net in self.board.pcb_data.get('nets', []):
                            if net.number == pad.net:
                                logger.info(f"    Found net name for {ref}-{pad_num}: {net.name} (net {net.number})")
                                return net.name
                        logger.warning(f"    Could not find net {pad.net} in pcb_data nets!")
                    # Fallback to net_name if available
                    elif hasattr(pad, 'net_name') and pad.net_name:
                        logger.info(f"    Using net_name attribute for {ref}-{pad_num}: {pad.net_name}")
                        return pad.net_name
                    else:
                        logger.warning(f"    Pad {ref}-{pad_num} has no net assignment (net={pad.net})")
        else:
            logger.error(f"  Footprint {ref} not found in board.footprints!")
            logger.debug(f"  Available footprints: {list(self.board.footprints.keys())}")
    ...
    def _generate_dsn(self) -> str:
        ...
        # Via definitions
        lines.append("    (via")
        lines.append(f"      \"Via[0-{len(self.layers)-1}]_"
        ...
        for component in self.components:
            lines.append(f"    (component {component.footprint_name} (place {component.name} {component.x:.3f} {component.y:.3f} {component.side} {component.rotation:.1f}))")
        lines.append("  )")
        lines.append("")
        ...
        for net_name, pads in self.nets.items():
            ...
            if len(pads) > 1:  # Only include nets with multiple connections
                ...
                logger.info(f"  Writing net '{sanitized_name}' (original: '{net_name}') with pads: {pads}")
                ...
                logger.warning(f"  Skipping net '{net_name}' - only has {len(pads)} pad(s): {pads}")
    ...
    def _add_padstack_definitions(self, lines: List[str]) -> None:
        ...
        for layer in self.layers:
            lines.append(f"      (shape (circle {layer.name} {self.DEFAULT_VIA_SIZE:.3f} 0 0))")
        ...
        for pad_type in sorted(pad_types):
            ...
            if "Round[A]" in pad_type:
                ...
                lines.append(f"      (shape (rect front -{width/2:.3f} -{height/2:.3f} "
    ...
    def _add_footprint_definitions(self, lines: List[str]) -> None:
        ...
        for fp_id, components in footprint_types.items():
            ...
            if bbox:
                ...
                lines.append(f"        (rect front {bbox[0]:.3f} {bbox[1]:.3f} "
            ...
            for pad in ref_component.pads:
                ...
                lines.append(f"      (pin {padstack} {pad.number} "
                            f"{rel_x:.3f} {rel_y:.3f})")
    ...
    def _calculate_footprint_bbox(self, component: DSNComponent) -> Optional[Tuple[float, float, float, float]]:
