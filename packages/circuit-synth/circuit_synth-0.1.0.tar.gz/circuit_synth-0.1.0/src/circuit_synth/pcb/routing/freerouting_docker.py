...
class FreeroutingDocker:
    ...
    def route(
        ...
        cmd = [
            "docker", "run", "--rm",
            "-v", f"{work_dir}:/work",  # Mount the directory
            self.DOCKER_IMAGE,
            "java", "-jar", "/app/freerouting-executable.jar",
            "-de", f"/work/{dsn_name}",  # Input DSN
            "-do", f"/work/{output_name}",  # Output SES
            "-mp", str(optimization_passes),  # Max passes
        ]
        ...
        try:
            ...
            if pull_result.returncode != 0:
                logger.info("Pulling Docker image (this may take a moment on first run)...")
            ...
            if result.returncode == 0:
                ...
                if os.path.exists(output_file):
                    ...
                    logger.info(f"Routing complete. Output saved to: {output_file} (size: {output_size} bytes)")
                        ...
                        try:
                            with open(dsn_file, 'r') as f:
                                ...
                                if 'network' not in dsn_content.lower():
                                    logger.warning("DSN file appears to have no network/nets defined")
                                logger.debug(f"First 500 chars of DSN:\n{dsn_content[:500]}")
