...
class PCBFormatter:
    ...
    def format(self, sexp: Any) -> str:
        ...
        if isinstance(sexp, list):
            ...
            keywords = {
                'setup', 'general', 'layers', 'pcbplotparams', 'paper', 'net',
                'footprint', 'via', 'segment', 'zone', 'group', 'dimension',
                'gr_line', 'gr_rect', 'gr_circle', 'gr_arc', 'gr_text',
                'pad_to_mask_clearance', 'allow_soldermask_bridges_in_footprints',
                'tenting', 'layerselection', 'plot_on_all_layers_selection',
                ...
                'plotframeref', 'mode', 'useauxorigin', 'hpglpennumber',
                'hpglpenspeed', 'hpglpendiameter', 'pdf_front_fp_property_popups',
                'pdf_back_fp_property_popups', 'pdf_metadata', 'pdf_single_document',
                'dxfpolygonmode', 'dxfimperialunits', 'dxfusepcbnewfont',
                'psnegative', 'psa4output', 'plot_black_and_white',
                'plotinvisibletext', 'sketchpadsonfab', 'plotpadnumbers',
                'hidednponfab', 'sketchdnponfab', 'crossoutdnponfab',
                'subtractmaskfromsilk', 'outputformat', 'mirror', 'drillshape',
                'scaleselection', 'outputdirectory', 'thickness', 'legacy_teardrops',
                'front', 'back', 'yes', 'no', 'signal', 'user'
            }
    ...
    def _format_list(self, lst: List) -> str:
        ...
        if lst and (isinstance(lst[0], sexpdata.Symbol) or isinstance(lst[0], str)):
            ...
            if symbol_name in ["at", "size", "thickness", "width", "layer", "effects", 
                             "font", "justify", "uuid", "stroke", "fill", "net",
                             "drill", "layers", "roundrect_rratio", "thermal_bridge_angle",
    ...
    def format_pcb(self, sexp: List) -> str:
        ...
        Returns:
            Formatted string
        """
        if not sexp or not isinstance(sexp[0], sexpdata.Symbol) or str(sexp[0]) != "kicad_pcb":
