...
class ConnectionAnalyzer:
    ...
    def analyze_circuit(self, circuit) -> None:
        ...
        logger.info(f"  Total components with connections: {len(self.connection_counts)}")
        logger.info(f"  Total pairwise connections: {len(self.connection_matrix)}")
        
        # Log top connected components
        sorted_comps = sorted(self.connection_counts.items(), key=lambda x: x[1], reverse=True)
    ...
    def _is_power_net(self, net_name: str) -> bool:
        ...
        power_patterns = [
            'VCC', 'VDD', 'VSS', 'GND', 'GROUND', 'PWR',
            '+3V3', '+5V', '+12V', '-12V', '+1V8', '+2V5',
            'VBAT', 'VBUS', 'VIN', 'VOUT', 'AGND', 'DGND'
        ]
    ...
    def calculate_wire_length(self, comp_positions: Dict[str, Tuple[float, float]]) -> float:
        """
