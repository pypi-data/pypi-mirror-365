        ...
        if check_collision:
            ...
            if collision and collision != reference:
                raise PlacementError(
                    snapped_pos,
                    f"Collision with component {collision}"
                )
    ...
    def clone_component(
        ...
        if new_reference is None:
            if auto_increment:
                ...
                new_reference = self.manager.reference_manager.generate_reference(source.lib_id)
        ...
        if new_component:
            logger.info(f"Cloned {reference} as {new_reference} at position {new_position}")
