...
class ComponentSearch:
    ...
    def find_by_reference_pattern(
        ...
        if use_regex:
            try:
                ...
                return [comp for comp in self.schematic.components 
    ...
    def find_by_criteria(self, criteria: SearchCriteria) -> List[SchematicSymbol]:
        ...
        if criteria.reference_pattern:
            ref_matches = set(self.find_by_reference_pattern(
                criteria.reference_pattern,
                criteria.use_regex
            ))
            results &= ref_matches
        ...
        if criteria.value_pattern:
            value_matches = set(self.find_by_value(
                criteria.value_pattern,
                criteria.use_regex,
        ...
        if criteria.footprint_pattern:
            footprint_matches = set(self.find_by_footprint(
                criteria.footprint_pattern,
                criteria.use_regex
            ))
        ...
        for prop_name, prop_value in criteria.property_filters.items():
            prop_matches = set(self.find_by_property(
                prop_name,
                prop_value,
                criteria.use_regex
            ))
    ...
    def find_by_custom_filter(
        self,
        filter_func: Callable[[SchematicSymbol], bool]
    ) -> List[SchematicSymbol]:
        """
