...
from typing import Dict, List, Any, Optional, Set, Tuple, TextIO
...
def _extract_sheets_from_circuit(circuit_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def process_circuit(data: Dict[str, Any], parent_uuid: Optional[str] = None, is_root: bool = False, parent_path: str = ""):
...
def generate_components_section(circuit_data: Dict[str, Any]) -> List[Any]:
    ...
    def process_components(circ_data: Dict[str, Any], current_path: str):
        ...
        for ref, comp_data in components.items():
            ...
            sheet_file = "circuit.kicad_sch"  # Use generic name since we don't need actual files
        ...
        for subcircuit in circ_data.get("subcircuits", []):
            ...
            new_path = f"{current_path}{subname}/" if current_path != "/" else f"/{subname}/"
...
def generate_libparts_section(circuit_data: Dict[str, Any]) -> List[Any]:
    ...
    def _collect_components_recursive(data: Dict[str, Any], path: str = "/"):
        all_components.update({f"{path}{ref}" if path != "/" else ref: comp
    ...
    for ref, comp_data in all_components.items():
        ...
        if symbol and ":" in symbol:
            try:
                ...
                raise ValueError(f"Invalid symbol format '{symbol}' for component '{ref}'. Expected 'Library:Part' format.")
        else:
            # --- Fallback Logic (Revised as per plan v3.1) ---
            # Only use fallbacks if symbol is missing or invalid
            logger.debug(f"  Symbol missing or invalid ('{symbol}'). Attempting fallbacks for ref '{ref}'...")
            ...
            if ki_lib and ki_part:
                lib = ki_lib
                part = ki_part
                logger.debug(f"  Using fallback from properties: ki_lib='{lib}', ki_part='{part}'.")
            elif libsource and ":" in libsource:
                 try:
                     lib, part = libsource.split(":", 1)
                     logger.debug(f"  Using fallback from libsource property: '{libsource}' -> lib='{lib}', part='{part}'.")
                 except ValueError:
                     raise ValueError(f"Invalid libsource format '{libsource}' for component '{ref}'. Expected 'Library:Part' format.")
            else:
                 # No valid library found - raise error
                 raise ValueError(f"No library specified for component '{ref}'. Symbol must be in 'Library:Part' format.")
            # --- End Fallback Logic ---
        ...
        if libpart_entry: # Check if entry was generated successfully
             ...
             logger.warning(f"  Failed to generate libpart entry for '{libpart_key}' (ref: {ref}).")

    logger.debug(f"...generate_libparts_section finished. Found {len(unique_libparts)} unique libparts.")
    return libparts_section


def generate_libpart_entry(lib: str, part: str, comp_data: Dict[str, Any], circuit_data: Dict[str, Any] = None) -> Optional[List[Any]]:
    """
    ...
    logger.debug(f"Starting generate_libpart_entry for lib='{lib}', part='{part}' (from ref='{ref}')")
    # logger.debug(f"  Input comp_data: {comp_data}") # Optional: Log full comp_data if needed

    # --- Basic Info ---
    description = comp_data.get("description", "")
    properties = comp_data.get("properties", {})
    datasheet = comp_data.get("datasheet", "") or "~" # Use ~ if empty, as KiCad expects
    footprint = properties.get("ki_footprint", comp_data.get("footprint", "")) # Prioritize ki_footprint property
    footprint_filters_str = properties.get("ki_fp_filters", "")
    value = str(comp_data.get("value", part)) # Use part name as fallback value

    logger.debug(f"  Extracted Fields: description='{description}', datasheet='{datasheet}', footprint='{footprint}', fp_filters='{footprint_filters_str}', value='{value}'")
    ...
    if footprint_filters_str:
        ...
        if footprints_list:
            ...
            logger.debug(f"  Added footprints section with filters: {footprint_filters_str}")
        else:
            logger.debug("  Footprint filters string was present but resulted in no valid filters.")
    else:
    ...
    ref_prefix = ref[0] if ref and not ref.startswith("/") else "?" # Use '?' if ref is invalid/missing
    fields = [
        ["field", ["name", "Reference"], ref_prefix], # Use prefix like R, C, U
        ["field", ["name", "Value"], value],
        # Use the determined footprint value here
        ["field", ["name", "Footprint"], footprint if footprint else ""], # Use empty string if no footprint
        ["field", ["name", "Datasheet"], datasheet if datasheet != "~" else ""], # Use empty string if no datasheet
    ]
    logger.debug(f"  Added standard fields: Ref='{ref_prefix}', Value='{value}', Footprint='{footprint}', Datasheet='{datasheet}'")
    ...
    for prop_name, prop_value in properties.items():
        ...
        if prop_name.startswith("ki_") and prop_name not in ["ki_footprint", "ki_fp_filters", "ki_lib", "ki_part"]:
    ...
    if added_ki_fields:
        logger.debug(f"  Added KiCad-specific fields from properties: {', '.join(added_ki_fields)}")
    else:
    ...
    if not isinstance(pins_list, list):
        logger.error(f"  Pins data for {lib}:{part} (ref: {ref}) is not a list (type: {type(pins_list)}). Cannot process pins.", exc_info=True)
        pins_list = [] # Reset to empty list to avoid further errors

    if not pins_list:
         logger.warning(f"  No pins found in comp_data for {lib}:{part} (ref: {ref}). Libpart entry might be incomplete.")
    ...
    def sort_key(pin_dict):
        ...
        try:
            ...
            logger.debug(f"    Pin number '{pin_num_str}' is non-numeric, sorting alphabetically.")
    ...
    try:
        ...
        if len(valid_pin_items) != len(pins_list):
             logger.warning(f"  Some pin data items were not dictionaries for {lib}:{part} (ref: {ref}).")

        sorted_pins = sorted(valid_pin_items, key=sort_key)
    except Exception as e:
        logger.error(f"  Error sorting pins for {lib}:{part} (ref: {ref}): {e}. Pins might be out of order.", exc_info=True)
    ...
    for pin_info in sorted_pins:
        ...
        if not pin_type_str:
            ...
            logger.debug(f"  No pin type found for pin {pin_num}, defaulting to 'passive'")
        ...
        pin_entry = ["pin", ["num", str(pin_num)], ["name", pin_name], ["type", kicad_pin_type]]
    ...
    logger.debug(f"...generate_libpart_entry finished successfully for {lib}:{part} (ref: {ref}).")
...
def generate_libraries_section(circuit_data: Dict[str, Any]) -> List[Any]:
    ...
    for ref, comp_data in all_components.items():
        ...
        if not isinstance(comp_data, dict):
             logger.error(f"  Component data for ref '{ref}' is not a dictionary! Skipping library processing for this component.")
             continue
        properties = comp_data.get("properties", {})
        ...
        if lib is None:
            try:
                logger.debug(f"    [L2] Checking 'ki_lib' in properties ({'ki_lib' in properties})")
                if "ki_lib" in properties:
                    lib = properties["ki_lib"]
                    logger.debug(f"    [L3] Found ki_lib: {lib}")
                elif "libsource" in properties:
                    logger.debug(f"    [L4] Checking 'libsource' in properties ({'libsource' in properties})")
                    libsource = properties["libsource"]
                    logger.debug(f"    [L5] Found libsource: {libsource}")
                    if ":" in libsource:
                        logger.debug(f"    [L6] Checking ':' in libsource ({':' in libsource})")
                     ...
                     logger.debug(f"    [L9] No ki_lib or libsource found in properties.")
            except Exception as e:
        ...
        if lib is None:
            raise ValueError(f"No library found for component '{ref}'. Component must have a valid library specification.")
        unique_libs.add(lib)
...
def generate_nets_section(circuit_data: Dict[str, Any]) -> List[Any]:
        ...
        for net_name in sheet_data.get("nets", []):
            ...
            if '/' in net_name:
                ...
                if len(path_parts) >= 3 and path_parts[1] == "Project Architecture":
                    ...
                    logger.debug(f"  Extracted sheet '{sheet_name}' from hierarchical path: {net_name}")
    ...
    for comp_ref, comp_data in circuit_data.get("components", {}).items():
        ...
        if sheet_name:
            for net_name in comp_data.get("nets", []):
                ...
                if net_name not in original_net_sheets or len(sheet_name.split('/')) > len(original_net_sheets[net_name].split('/')):
                    original_net_sheets[net_name] = sheet_name
                    logger.debug(f"  Mapped net '{net_name}' to sheet '{sheet_name}' from component {comp_ref}")
    ...
    def process_net_nodes(circ, path="/"):
        for net_name, net_data in circ.get("nets", {}).items():
            ...
            if not validate_net_data(net_name, net_data):
                logger.warning(f"Skipping invalid net '{net_name}' during node collection")
                continue
            ...
            if '/' in original_name:
                ...
                if len(path_parts) >= 3 and path_parts[1] == "Project Architecture":
                    final_net_name = original_name
                    logger.debug(f"  Using existing hierarchical path: {final_net_name}")
            ...
            if (original_name.startswith("unconnected-") or
                original_name.startswith("Net-") or
                original_name.startswith("+") or
                original_name.startswith("-") or
                original_name.upper() in {"GND", "GNDA", "GNDD", "VCC", "VDD", "VSS", "VDDA"}):
                # Keep original name for special nets
                logger.debug(f"  Special case net - keeping original name: {original_name}")
                local_nets.add(original_name)
            # Keep original path if it has one
            elif original_name.startswith('/'):
                # Already has a path, use as-is
                logger.debug(f"  Net already has path - preserving exactly: {original_name}")
                final_net_name = original_name
                hierarchical_nets.add(final_net_name)
                    ...
                    if original_name.startswith('/'):
                        ...
                        if original_name in original_net_sheets:
                            ...
                            if sheet_path:
                                ...
                                logger.debug(f"  Using original sheet assignment: {existing_net}")
                        
                        # If no sheet assignment found, look in circuit data
                        if not existing_net:
                            for net_name in circuit_data.get("nets", {}):
                                if net_name.endswith('/' + original_name) and '/' in net_name:
                                    existing_net = net_name
                                    logger.debug(f"  Found existing path in circuit data: {existing_net}")
                                    break
                            ...
                            if not existing_net:
                                for net_name in all_nodes_by_net_name.keys():
                                    if net_name.endswith('/' + original_name) and '/' in net_name:
                                        existing_net = net_name
                                        logger.debug(f"  Found existing path in all_nodes: {existing_net}")
                                        break
            ...
            # Otherwise keep as local net
                ...
                logger.debug(f"  Treating as local net - keeping original name: {original_name}")
                final_net_name = original_name
                local_nets.add(original_name)
            ...
            for node in nodes:
                ...
                if "original_path" in node_copy:
                    ...
                    if path == "/" or comp_ref.startswith("/"):
                        ...
                        node_copy["component"] = normalize_hierarchical_path("", comp_ref)
                    else:
                        # Components in subcircuits - ensure proper path format
                        node_copy["component"] = normalize_hierarchical_path(path, comp_ref)
    ...
    for final_net_name, nodes in all_nodes_by_net_name.items():
        ...
        if final_net_name.startswith("unconnected-"):
            ...
            if len(parts) >= 3:
                        ...
                        if libpart.get("lib") == lib and libpart.get("part") == part:
                            for pin in libpart.get("pins", []):
                                ...
                                if pin.get("name") == pin_name:
                                    ...
                                    if pin_type:
                                        logger.debug(f"  Found pin type in libpart: {pin_type}")
                                        break
                ...
                node_entry = [
                    ...
                    ["pintype", pin_type if pin_type else "passive"]  # Use original pin type
                ]
        ...
        for node in nodes:
            ...
            pin_num = pin_info.get("number")
            if not pin_num:
                logger.warning(f"  Skipping node - missing pin number key ('number'): {pin_info}")
                continue
                ...
                for libpart in circuit_data.get("libparts", []):
                    if libpart.get("lib") == lib and libpart.get("part") == part:
                        ...
                        for pin in libpart.get("pins", []):
                            ...
                            if str(pin.get("num")) == str(pin_num) or pin.get("name") == pin_function:
            ...
            if not pin_type:
                pin_type = "passive"
                logger.debug(f"  No pin type found in component or libpart for {component_ref}:{pin_num}, defaulting to 'passive'")
            
            logger.debug(f"  Pin type for {component_ref}:{pin_num} before mapping: {pin_type}")
            ...
            if pin_info.get("unconnected", False):
                ...
                if unconnected_name not in unconnected_pins:
                    ...
                    unconnected_net_entry = ["net", ["code", str(net_code)], ["name", unconnected_name]]
                    net_code += 1
                    ...
                    if symbol and ":" in symbol:
                        ...
                        for libpart in circuit_data.get("libparts", []):
                            if libpart.get("lib") == lib and libpart.get("part") == part:
                                for pin in libpart.get("pins", []):
                                    # Match by pin number or name
                                    if str(pin.get("num")) == str(pin_num) or pin.get("name") == pin_info.get("name"):
                                        libpart_type = pin.get("type")
                                        pin_function = pin.get("name", "")
                                        if libpart_type:
                                            pin_type = libpart_type
                    ...
                    if normalized_ref.startswith("/"):
                        ...
                        logger.debug(f"  Normalized unconnected component ref from {component_ref} to {normalized_ref}")
                    
                    unconnected_node_entry = [
                        "node",
                        ["ref", normalized_ref],      # Use normalized reference without leading slash
            ...
            logger.debug(f"  Mapped pin type for {component_ref}:{pin_num}: {pin_type} -> {mapped_pin_type}")
            
            # Normalize component reference by removing leading slash
            normalized_ref = str(component_ref)
            if normalized_ref.startswith("/"):
                normalized_ref = normalized_ref[1:]
                logger.debug(f"  Normalized component ref from {component_ref} to {normalized_ref}")
        ...
        nets_section.append(net_entry)
        logger.debug(f"  Appended net_entry for '{final_net_name}'. Current nets_section length: {len(nets_section)}")
...
def generate_component_entry(comp_data: Dict[str, Any], sheet_name: str = "Root", sheet_file: str = "unknown.kicad_sch", sheet_path: str = "/") -> Optional[List[Any]]:
    """
    ...
    symbol = comp_data.get("symbol", "")
    logger.debug(f"  Component Entry: Determining libsource for ref='{ref}', symbol='{symbol}'")

    if symbol and ":" in symbol:
        # --- Primary Logic: Use symbol attribute ---
        try:
            lib, part = symbol.split(":", 1)
            logger.debug(f"  Component Entry: Extracted lib='{lib}', part='{part}' from symbol for libsource.")
        except ValueError:
            raise ValueError(f"Invalid symbol format '{symbol}' for component '{ref}'. Expected 'Library:Part' format.")
    else:
        # --- Fallback Logic (Mirroring generate_libparts_section) ---
        # Only use fallbacks if symbol is missing or invalid
        logger.debug(f"  Component Entry: Symbol missing or invalid ('{symbol}'). Attempting fallbacks for ref '{ref}' for libsource...")
        properties = comp_data.get("properties", {})
        ki_lib = properties.get("ki_lib")
        ki_part = properties.get("ki_part")
        ...
        if ki_lib and ki_part:
            ...
            lib = ki_lib
            part = ki_part
            logger.debug(f"  Component Entry: Using fallback from properties: ki_lib='{lib}', ki_part='{part}' for libsource.")
    ...
    libsource_entry = ["libsource", ["lib", lib], ["part", part], ["description", libsource_description]]
    ...
    if "ki_keywords" in properties:
         comp_entry.append(["property", ["name", "ki_keywords"], ["value", str(properties["ki_keywords"])]])
    if "ki_fp_filters" in properties:
         comp_entry.append(["property", ["name", "ki_fp_filters"], ["value", str(properties["ki_fp_filters"])]])
...
def generate_net_entry(net_name: str, nodes: List[Dict[str, Any]]) -> Optional[List[Any]]:
    """
    ...
    for node_connection in nodes:
        ...
        if not component_ref or not pin_details:
            logger.warning(f"Skipping invalid node connection in net '{net_name}': {node_connection}")
            continue
        ...
        if not pin_type:
            ...
            pin_type = "passive"
            logger.debug(f"No pin type specified for {component_ref}:{pin_num_str} in net '{net_name}', defaulting to 'passive'")
        logger.debug(f"Pin type for {component_ref}:{pin_num_str} in net '{net_name}': {pin_type}")

        # Check if we successfully obtained a pin number
        if not pin_num_str:
            logger.warning(f"Missing pin number for component '{component_ref}' in net '{net_name}'. Skipping node.")
            continue

        # Determine KiCad pin type, preserving original type
        pin_kicad_type = PinType.to_kicad(pin_type)
        logger.debug(f"Mapped pin type for {component_ref}:{pin_num_str}: {pin_type} -> {pin_kicad_type}")
...
def format_s_expr(expr: Any, indent: int = 0) -> str:
    ...
    """
    logger.debug(f"format_s_expr called with type={type(expr)}, value={repr(expr)}, indent={indent}")
    indent_str = "  " * indent
    ...
    if isinstance(expr, list):
        ...
        # Sheet section
            ...
            result = "(sheet"
            number = next((item for item in expr[1:] if isinstance(item, list) and item[0] == "number"), None)
            name = next((item for item in expr[1:] if isinstance(item, list) and item[0] == "name"), None)
            tstamps = next((item for item in expr[1:] if isinstance(item, list) and item[0] == "tstamps"), None)
            ...
            title_block = next((item for item in expr[1:] if isinstance(item, list) and item[0] == "title_block"), None)
            if title_block:
                result += "\n      (title_block"
        ...
        # Component entry - KiCad has very specific format
            ...
            ref_item = next((item for item in expr[1:] if isinstance(item, list) and item[0] == "ref"), None)
            ...
            remaining_items = [item for item in expr[1:] if isinstance(item, list) and item[0] != "ref"]
        ...
        # Libpart entry - KiCad specific format
            ...
            lib_item = next((item for item in expr[1:] if isinstance(item, list) and item[0] == "lib"), None)
            part_item = next((item for item in expr[1:] if isinstance(item, list) and item[0] == "part"), None)
            ...
            remaining_items = [item for item in expr[1:] if isinstance(item, list) and item[0] not in ["lib", "part"]]
        ...
        # Library entry
            ...
            logical_item = next((item for item in expr[1:] if isinstance(item, list) and item[0] == "logical"), None)
            uri_item = next((item for item in expr[1:] if isinstance(item, list) and item[0] == "uri"), None)
        ...
        # Net entry - KiCad specific format
            ...
            code = next((item for item in expr[1:] if isinstance(item, list) and item[0] == "code"), None)
            name = next((item for item in expr[1:] if isinstance(item, list) and item[0] == "name"), None)
            ...
            node_strings = []
            nodes = [item for item in expr[1:] if isinstance(item, list) and item[0] == "node"]
            for node in nodes:
...
def format_node(node_expr: List, indent: int = 0) -> str:
    ...
    ref = next((item for item in node_expr[1:] if isinstance(item, list) and item[0] == "ref"), None)
    pin = next((item for item in node_expr[1:] if isinstance(item, list) and item[0] == "pin"), None)
    pintype = next((item for item in node_expr[1:] if isinstance(item, list) and item[0] == "pintype"), None)
    ...
    pinfunc = next((item for item in node_expr[1:] if isinstance(item, list) and item[0] == "pinfunction"), None)
    if pinfunc:
