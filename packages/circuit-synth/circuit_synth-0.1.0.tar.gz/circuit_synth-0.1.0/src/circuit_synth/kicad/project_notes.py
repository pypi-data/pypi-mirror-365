...
class ProjectNotesManager:
    ...
    def save_datasheet(self, pdf_path: Union[str, Path], component_id: str, 
                      part_number: Optional[str] = None) -> Path:
        """
    ...
    def save_analysis_result(self, analysis_data: Dict[str, Any], 
                           analysis_type: str = "circuit_analysis") -> Path:
        """
    ...
    def add_user_note(self, note_title: str, note_content: str, 
                     component_id: Optional[str] = None) -> Path:
        """
    ...
    def export_cache_summary(self) -> Dict[str, Any]:
        ...
        # Count datasheets
        datasheets_path = self.notes_path / "datasheets"
        datasheet_count = len(list(datasheets_path.glob("*.pdf"))) if datasheets_path.exists() else 0
        
        # Count analyses
        history_path = self.notes_path / "analysis_history"
        analysis_count = len(list(history_path.glob("*.json"))) if history_path.exists() else 0
