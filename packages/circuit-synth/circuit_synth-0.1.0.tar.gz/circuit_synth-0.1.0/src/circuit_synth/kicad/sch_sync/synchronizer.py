...
from .schematic_updater import SchematicUpdater, ComponentUpdate, create_component_updates_from_sync_report
...
class SchematicSynchronizer:
    ...
    def __init__(self, project_path: str, match_criteria: List[str] = None, 
        ...
        logger.info(f"SchematicSynchronizer initialized for project: {self.project_path}")
    ...
    def sync_with_circuit(self, circuit) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Loaded {len(kicad_components)} components from KiCad schematic")
            
            # Step 2: Try canonical matching first
            canonical_matches = self._match_components_canonical(circuit, kicad_schematic)
            
            if canonical_matches:
                logger.info(f"Canonical matching successful: {len(canonical_matches)} matches found")
                ...
                for circuit_id, kicad_ref in canonical_matches.items():
                    if circuit_id in circuit_components and kicad_ref in kicad_components:
                        ...
                        connection_changed = self._check_connection_changes(
                            circuit, circuit_id, kicad_ref,
                            canonical_matches, circuit_components, kicad_components
                        )
                        
                        if needs_update or connection_changed:
                            updates = self._get_component_updates(circuit_comp, kicad_comp)
                ...
                logger.info("Canonical matching failed or returned no matches, falling back to traditional matching")
                return self.synchronize(circuit)
            
            # Log summary
            summary = sync_report['summary']
            logger.info(f"Sync complete - Matched: {summary['matched']}, "
    ...
    def _get_component_updates(self, circuit_comp: Dict, kicad_comp: Dict) -> Dict[str, Any]:
    ...
    def synchronize(self, circuit) -> Dict[str, Any]:
        ...
        try:
            ...
            circuit_components = self._extract_circuit_components(circuit)
            logger.info(f"Extracted {len(circuit_components)} components from circuit definition")
            ...
            match_result = self.matcher.match_components(circuit_components, kicad_components)
            report.matched = len(match_result.matched_pairs)
            
            # Step 4: Process matches and updates
            self._process_component_matches(match_result, circuit_components, kicad_components, report)
            
            # Step 5: Handle unmatched components
            self._process_unmatched_components(match_result, circuit_components, kicad_components, report)
    ...
    def _load_kicad_schematic(self):
        ...
        if cover_sheet_path.exists() and cover_sheet_path != schematic_path:
            ...
            if self._hierarchical_sheet_uuid:
                logger.info(f"Found hierarchical sheet UUID: {self._hierarchical_sheet_uuid}")
    ...
    def _find_main_schematic(self) -> Path:
        ...
        if schematic_path.exists():
            ...
            if self._schematic_has_components(schematic_path):
                ...
                logger.info(f"Main schematic {schematic_path} appears to be a cover sheet, looking for circuit schematics")
        ...
        if schematic_path.exists():
            logger.warning(f"No circuit schematics found with components, using main schematic: {schematic_path}")
            return schematic_path
        ...
        raise FileNotFoundError(f"No schematic files found in project directory: {self.project_path.parent}")
    
    def _find_sheet_uuid_for_schematic(self, cover_sheet_path: Path, schematic_filename: str) -> Optional[str]:
            ...
            for sheet in sheets:
                ...
                for item in sheet:
                    if (isinstance(item, list) and len(item) >= 3 and
                        isinstance(item[0], sexpdata.Symbol) and str(item[0]) == 'property' and
                        item[1] == 'Sheetfile' and item[2] == schematic_filename):
                        # Found the sheet, now get its UUID
                        for uuid_item in sheet:
                            if (isinstance(uuid_item, list) and len(uuid_item) >= 2 and
                                isinstance(uuid_item[0], sexpdata.Symbol) and str(uuid_item[0]) == 'uuid'):
                                return uuid_item[1]
                        break
        ...
        if isinstance(expr, list):
            for item in expr:
                if isinstance(item, list) and len(item) > 0:
                    ...
                    first_elem = item[0]
                    if isinstance(first_elem, sexpdata.Symbol) and str(first_elem) == name:
                        result.append(item)
    ...
    def _schematic_has_components(self, schematic_path: Path) -> bool:
        ...
        try:
            ...
            logger.debug(f"Schematic {schematic_path} has {len(symbols)} component symbols")
    ...
    def _extract_circuit_components(self, circuit) -> Dict[str, Any]:
        ...
        if hasattr(circuit, '_components'):
            ...
            for comp_id, component in circuit._components.items():
                components[comp_id] = {
                    'reference': getattr(component, 'ref', comp_id),  # Circuit uses 'ref' not 'reference'
                    'value': getattr(component, 'value', None),
                    'footprint': getattr(component, 'footprint', None),
                    'original_component': component
                }
    ...
    def _process_component_matches(self, match_result: MatchResult, 
                                 circuit_components: Dict, kicad_components: Dict, 
                                 report: SyncReport):
    ...
    def _process_unmatched_components(self, match_result: MatchResult,
                                    circuit_components: Dict, kicad_components: Dict,
                                    report: SyncReport):
    ...
    def _check_connection_changes(self, circuit, circuit_id: str, kicad_ref: str,
                                 canonical_matches: Dict[str, str],
                                 circuit_components: Dict, kicad_components: Dict) -> bool:
        """
        ...
        try:
            ...
            for comp in circuit._components.values():
                if (hasattr(comp, 'id') and comp.id == circuit_id) or comp.ref == circuit_id:
                    circuit_comp = comp
                    break
            ...
            if circuit_nets != kicad_nets:
                logger.info(f"Connection change detected for {kicad_ref}: "
            ...
            logger.warning(f"Error checking connection changes for {circuit_id}/{kicad_ref}: {e}")
    ...
    def _export_kicad_netlist(self) -> Path:
        ...
        try:
            ...
            cmd = [
                ...
                '--format', 'kicadsexpr',  # Try S-expression format first
                '-o', str(netlist_path),
                str(schematic_path)
            ]
            ...
            logger.error("KiCad CLI tools not found. Please ensure KiCad is installed and in PATH")
    ...
    def _match_components_canonical(self, circuit, kicad_schematic) -> Dict[str, str]:
        ...
        try:
            ...
            logger.info(f"Python circuit canonical form: {new_canonical.component_count} components")
            ...
            logger.info(f"KiCad circuit canonical form: {old_canonical.component_count} components")
            ...
            for old_idx, new_idx in matches.items():
                if new_idx != -1 and old_idx < len(kicad_components) and new_idx < len(circuit_components):
                    ...
                    circuit_id = circuit_comp.id if hasattr(circuit_comp, 'id') else circuit_comp.ref
                ...
                for item in data:
                    if isinstance(item, list) and len(item) > 0:
                        if str(item[0]) == 'symbol':
                            ...
                            # Skip power symbols
                            if lib_id and not any(pwr in lib_id for pwr in ['power:', 'Power:', '#PWR', '#FLG']):
                                symbols.append(item)
                        else:
    ...
    def _write_updated_schematic(self, schematic, report: SyncReport):
        ...
        try:
            ...
            actual_changes = [u for u in updates if u.action in ['add', 'modify', 'remove']]
