...
class ComponentMatcher:
    ...
    def __init__(self, match_criteria: List[str] = None):
        ...
        logger.debug(f"ComponentMatcher initialized with criteria: {self.match_criteria}")
    
    def match_components(self, circuit_components: Dict, kicad_components: Dict) -> MatchResult:
        """
        ...
        """
        logger.info(f"Matching {len(circuit_components)} circuit components with {len(kicad_components)} KiCad components")
        ...
        for circuit_id, circuit_comp in circuit_components.items():
            ...
            if circuit_ref and circuit_ref in kicad_components:
                ...
                if confidence > 0.7:  # High confidence threshold for reference matches
                    ...
                    logger.debug(f"Reference match: {circuit_id} -> {circuit_ref} (confidence: {confidence:.2f})")
        
        # Second pass: fuzzy matching for remaining components
        unmatched_circuit = [cid for cid in circuit_components.keys() 
                           if cid not in [pair[0] for pair in matched_pairs]]
        unmatched_kicad = [ref for ref in kicad_components.keys() 
                         if ref not in used_kicad_refs]
        ...
        for circuit_id in unmatched_circuit[:]:  # Copy list to modify during iteration
            ...
            if best_match:
                ...
                logger.debug(f"Fuzzy match: {circuit_id} -> {best_match} (confidence: {best_confidence:.2f})")
        ...
        logger.info(f"Matching complete: {len(matched_pairs)} matches, "
