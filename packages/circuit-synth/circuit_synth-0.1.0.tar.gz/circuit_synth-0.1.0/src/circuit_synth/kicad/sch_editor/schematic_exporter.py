...
from .schematic_reader import SchematicSymbol, SchematicSheet, SchematicNet, HierarchicalLabel
import logging
...
class SchematicExporter:
    ...
    def export_file(self, filename: str) -> None:
        ...
        for symbol in self.schematic.components:
            if symbol.lib_id == "Connector:TestPoint":
                logging.info(f"Exporting TestPoint symbol: {symbol.uuid}, Lib ID: {symbol.lib_id}, Properties: {symbol.properties}")
                # Log the S-expression of the lib_symbol if available
                lib_symbol_data = self.schematic.lib_symbols.get(symbol.lib_id)
                if lib_symbol_data:
                    logging.info(f"TestPoint lib_symbol S-expression: {lib_symbol_data}")
                else:
                    logging.warning(f"No lib_symbol data found for TestPoint: {symbol.lib_id}")
        ...
        hierarchical_path = "/"
        if hasattr(schematic_obj, 'hierarchical_path') and schematic_obj.hierarchical_path:
            hierarchical_path = schematic_obj.hierarchical_path
        elif hasattr(schematic_obj, 'sheet_instances') and schematic_obj.sheet_instances:
        ...
        if hierarchical_path != "/":
            ...
            sheet_instances.append([sexpdata.Symbol("path"), hierarchical_path, [sexpdata.Symbol("page"), "1"]])
            ...
            sheet_instances = [sexpdata.Symbol("sheet_instances")]
            sheet_instances.append([sexpdata.Symbol("path"), "/", [sexpdata.Symbol("page"), "1"]])
            inner_data.append(sheet_instances)
        ...
        for component in schematic_obj.components:
            ...
            component_path = f"{hierarchical_path}/{component.uuid}" if hierarchical_path != "/" else f"/{component.uuid}"
    ...
    def _validate_schematic(self, data: List[Any]) -> bool:
        ...
        try:
            ...
            for item in data:
                ...
                section_name = str(item[0]) if isinstance(item[0], sexpdata.Symbol) else str(item[0])
    ...
    def write_hierarchical_label(self, label: HierarchicalLabel) -> List[Any]:
        ...
        hlabel = [
            ...
            [sexpdata.Symbol("at"), label.position[0], label.position[1], 0]  # Add rotation angle (0)
        ]
        ...
        if label.effects:
            ...
            if "font" in label.effects:
                ...
                if "size" in font_data:
                    font.append([sexpdata.Symbol("size"), font_data["size"][0], font_data["size"][1]])
                effects.append(font)
            if "justify" in label.effects:
                effects.append([sexpdata.Symbol("justify"), sexpdata.Symbol(label.effects["justify"])])
    ...
    def write_component(self, component: SchematicSymbol) -> List[Any]:
        ...
        symbol = [
            ...
            [sexpdata.Symbol("unit"), int(component.unit) if component.unit else 1],
            [sexpdata.Symbol("in_bom"), sexpdata.Symbol("yes") if component.in_bom else sexpdata.Symbol("no")],
            [sexpdata.Symbol("on_board"), sexpdata.Symbol("yes") if component.on_board else sexpdata.Symbol("no")]
        ]
        ...
        symbol.append([
            sexpdata.Symbol("property"), "Reference", component.reference,
            [sexpdata.Symbol("id"), 0],
            [sexpdata.Symbol("at"), ref_pos[0], ref_pos[1], ref_pos[2]],
            [sexpdata.Symbol("effects"), [sexpdata.Symbol("font"), [sexpdata.Symbol("size"), 1.27, 1.27]], [sexpdata.Symbol("justify"), sexpdata.Symbol("left")]]
        ])

        symbol.append([
            sexpdata.Symbol("property"), "Value", component.value,
        ...
        if self.project_name and hasattr(self.schematic, 'uuid'):
            ...
            instance = [
                ...
                [
                    sexpdata.Symbol("path"), f"/{self.schematic.uuid}",
                    [sexpdata.Symbol("reference"), component.reference],
                    [sexpdata.Symbol("unit"), int(component.unit) if component.unit else 1]
                ]
            ]
    ...
    def write_sheet(self, sheet: SchematicSheet) -> List[Any]:
        ...
        sheet_expr = [
            ...
            [sexpdata.Symbol("stroke"), [sexpdata.Symbol("width"), 0.1524], [sexpdata.Symbol("type"), "solid"], [sexpdata.Symbol("color"), 0, 0, 0, 0]],
            [sexpdata.Symbol("fill"), [sexpdata.Symbol("type"), "none"]],
            [sexpdata.Symbol("uuid"), sheet.uuid],
            [
                sexpdata.Symbol("property"), "Sheet name", sheet.name,
                [sexpdata.Symbol("id"), 0],
                [sexpdata.Symbol("at"), 127, 87.1984, 0],
                [sexpdata.Symbol("effects"), [sexpdata.Symbol("font"), [sexpdata.Symbol("size"), 1.27, 1.27]]]
            ],
            [
                sexpdata.Symbol("property"), "Sheet file", sheet.path,
                [sexpdata.Symbol("id"), 1],
