...
from .types import Pad, Layer, Property, Line, Arc, Text
...
class FootprintLibraryCache:
    ...
    def __init__(self, cache_dir: Optional[Path] = None):
        ...
        self._cache_dir = cache_dir or Path.home() / ".cache" / "circuit_synth" / "footprints"
    ...
    def _discover_system_libraries(self):
        ...
        if system == "Darwin":  # macOS
            ...
            search_paths.extend([
                ...
                Path(os.environ.get("APPDATA", "")) / "kicad/footprints" if os.environ.get("APPDATA") else None,
            ])
        else:  # Linux
            search_paths.extend([
        ...
        if self._index_file.exists():
            try:
                ...
                if not needs_rebuild:
                    ...
                    logger.info(f"Loaded footprint index with {len(self._footprint_index)} footprints")
                    return
    ...
    def _build_index(self):
        ...
        logger.info(f"Indexed {total_footprints} footprints in {len(self._library_index)} libraries")
    ...
    def search_footprints(self, query: str = "", filters: Optional[Dict[str, Any]] = None) -> List[FootprintInfo]:
        ...
        for fp_id, info in self._footprint_index.items():
            ...
            if query and not any(query_lower in field.lower() for field in [
                info.name, info.description, info.tags, info.keywords, info.library
            ]):
                continue
