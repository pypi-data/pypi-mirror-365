...
from circuit_synth.kicad_api.pcb.placement.base import PlacementAlgorithm, ComponentWrapper
from circuit_synth.kicad_api.pcb.placement.bbox import BoundingBox
from circuit_synth.kicad_api.pcb.placement.courtyard_collision import CourtyardCollisionDetector, Polygon
...
class ForceDirectedPlacement(PlacementAlgorithm):
    ...
    def __init__(self,
        ...
        self.attraction_strength = attraction_strength * 1.5  # Increase attraction to keep connected components closer
        ...
        self.collision_detector = CourtyardCollisionDetector(spacing=component_spacing * 0.5)
        
    def place(self, components: List[ComponentWrapper],
              connections: List[Tuple[str, str]],
              board_width: float = 100.0,
              board_height: float = 100.0,
              **kwargs) -> Dict[str, Point]:
        """
        ...
        """
        logger.info(f"Starting force-directed placement for {len(components)} components")
        ...
        for group_path, group in groups.items():
            logger.debug(f"Optimizing group {group_path} with {len(group.components)} components")
    ...
    def _build_connection_graph(self, footprints: List[Footprint], 
                               connections: List[Tuple[str, str]]) -> Dict[str, Set[str]]:
    ...
    def _group_by_subcircuit(self, footprints: List[Footprint]) -> Dict[str, SubcircuitGroup]:
    ...
    def _optimize_subcircuit(self, group: SubcircuitGroup, 
        ...
        for iteration in range(self.iterations_per_level):
            ...
            for comp in components:
                ...
                for connected_ref in connection_graph.get(comp.reference, set()):
                    if connected_ref in comp_dict:
                        ...
                        force = force + self._calculate_attraction(comp, connected,
                                                                 is_internal=True)
            ...
            if total_displacement < convergence_threshold:
                ...
                if convergence_count >= convergence_iterations:
                    logger.debug(f"Subcircuit converged after {iteration + 1} iterations")
                    break
            else:
                convergence_count = 0
    ...
    def _optimize_rotations(self, components: List[Footprint], 
                          connection_graph: Dict[str, Set[str]]):
    ...
    def _count_inter_group_connections(self, groups: Dict[str, SubcircuitGroup],
                                     connection_graph: Dict[str, Set[str]]):
    ...
    def _optimize_group_positions(self, groups: Dict[str, SubcircuitGroup],
                                board_outline: BoundingBox):
        ...
        for iteration in range(self.iterations_per_level // 2):  # Fewer iterations for groups
            ...
            for i, group1 in enumerate(group_list):
                ...
                for connected_path, connection_count in group1.connections_to_other_groups.items():
            ...
            for group in group_list:
                ...
                for comp in group.components:
                    comp.position = Point(
                        comp.position.x + move_x,
                        comp.position.y + move_y
                    )
    ...
    def _calculate_attraction(self, comp1: Footprint, comp2: Footprint,
                            is_internal: bool = False) -> Force:
    ...
    def _calculate_repulsion(self, comp1: Footprint, comp2: Footprint) -> Force:
        ...
        magnitude = self.repulsion_strength * (self.component_spacing / effective_distance) ** 2
        
        # Repulsion is in opposite direction
        return Force(-magnitude * dx, -magnitude * dy)
    
    def _calculate_boundary_force(self, comp: Footprint, board_outline: BoundingBox) -> Force:
        ...
        if comp.position.x < board_outline.min_x + margin:
            force.fx += strength * (board_outline.min_x + margin - comp.position.x) / margin
        
        if comp.position.x > board_outline.max_x - margin:
            force.fx += strength * (board_outline.max_x - margin - comp.position.x) / margin
        
        if comp.position.y < board_outline.min_y + margin:
            force.fy += strength * (board_outline.min_y + margin - comp.position.y) / margin
        
        if comp.position.y > board_outline.max_y - margin:
            force.fy += strength * (board_outline.max_y - margin - comp.position.y) / margin
        
        return force
    
    def _calculate_group_attraction(self, group1: SubcircuitGroup, 
                                  group2: SubcircuitGroup,
                                  connection_count: int) -> Force:
        ...
        # Stronger attraction for more connections
        magnitude = self.attraction_strength * math.log(connection_count + 1) * distance / 50
        
        return Force(magnitude * dx, magnitude * dy)
    
    def _calculate_group_repulsion(self, group1: SubcircuitGroup,
                                 group2: SubcircuitGroup) -> Force:
        ...
        size1 = max(group1.bbox.max_x - group1.bbox.min_x,
                   group1.bbox.max_y - group1.bbox.min_y)
        size2 = max(group2.bbox.max_x - group2.bbox.min_x,
    ...
    def _calculate_group_boundary_force(self, group: SubcircuitGroup, 
                                      board_outline: BoundingBox) -> Force:
    ...
    def _enforce_minimum_spacing(self, components: List[Footprint], max_iterations: int = 20):
        """
        ...
        self._connectivity_aware_collision_resolution(components, connectivity, max_iterations // 2)
        ...
        for iteration in range(max_iterations // 2):
            ...
            sorted_comps = sorted(components, key=lambda c: (c.position.x, c.position.y))
            ...
            if collision_count == 0:
                logger.info(f"All collisions resolved after {iteration + 1} strict iterations")
                break
            else:
                logger.warning(f"Strict iteration {iteration + 1}: Found {collision_count} collisions, separated {separation_count} component pairs")
                
                if iteration == max_iterations // 2 - 1:
                    logger.error(f"Failed to resolve all collisions! {collision_count} collisions remain.")
    
    def _build_connectivity_from_board(self, components: List[Footprint]) -> Dict[str, Set[str]]:
        """Build connectivity map from board nets."""
            ...
            if hasattr(board, 'nets'):
                for net in board.nets.values():
                    if net.name and net.name != "":  # Skip empty nets
                        connected_refs = []
                        for pad in net.pads:
                            if hasattr(pad, 'parent') and pad.parent.reference in connectivity:
    ...
    def _connectivity_aware_collision_resolution(self, components: List[Footprint], 
        ...
        for iteration in range(max_iterations):
            ...
            for i in range(len(components)):
                for j in range(i + 1, len(components)):
                    ...
                    if self.collision_detector.check_collision(comp1, comp2):
                        ...
                        are_connected = comp2.reference in connectivity.get(comp1.reference, set())
            ...
            if collision_count == 0:
                logger.info(f"Connectivity-aware resolution complete after {iteration + 1} iterations")
                break
            else:
                logger.debug(f"Connectivity iteration {iteration + 1}: {collision_count} collisions found")