...
class ConnectionCentricPlacement(PlacementAlgorithm):
    ...
    def place(self, components: List[ComponentWrapper],
        ...
        """
        logger.info(f"[ConnectionCentricPlacement] Starting placement for {len(components)} components")
        ...
        for subcircuit_name, subcircuit_components in subcircuits.items():
            logger.info(f"[ConnectionCentricPlacement] Placing subcircuit '{subcircuit_name}' "
        ...
        logger.info(f"[ConnectionCentricPlacement] Placement complete. "
                   f"Placed {len(self.placed_positions)} components")
        
        return self.placed_positions
    
    def _analyze_connections(self, components: List[ComponentWrapper], 
                           connections: List[Tuple[str, str]]) -> Dict[str, ConnectionInfo]:
        ...
        for ref, info in connection_info.items():
            logger.debug(f"[ConnectionAnalysis] {ref}: {info.total} connections, "
    ...
    def _group_by_subcircuit(self, components: List[ComponentWrapper], 
                           connection_info: Dict[str, ConnectionInfo]) -> Dict[str, List[ComponentWrapper]]:
        """Group components by subcircuit."""
        subcircuits = {}
    ...
    def _place_subcircuit(self, components: List[ComponentWrapper],
        ...
        for i, comp in enumerate(sorted_components):
            if i == 0:
                ...
                logger.debug(f"[Placement] Placing first component {comp.reference} at {position}")
            else:
                ...
                logger.debug(f"[Placement] Placing {comp.reference} at {position} "
    ...
    def _find_optimal_position(self, component: ComponentWrapper,
        ...
        connected_placed = [
            ref for ref in conn_info.connected_components
    ...
    def _find_nearest_valid_position(self, component: ComponentWrapper,
                                   ideal_pos: Point,
                                   board_width: float,
                                   board_height: float) -> Point:
        ...
        logger.warning(f"[Placement] Could not find valid position near {ideal_pos}, "
                      "placing at edge")
        return self._find_edge_position(component, board_width, board_height)
    
    def _is_valid_position(self, component: ComponentWrapper,
                          position: Point,
                          board_width: float,
        ...
        try:
            ...
            if hasattr(self.collision_detector, 'get_courtyard_polygon'):
                courtyard = self.collision_detector.get_courtyard_polygon(component.footprint)
                if courtyard:
                    logger.debug(f"[Collision] Component {component.reference} using courtyard for collision detection")
                else:
                    logger.debug(f"[Collision] Component {component.reference} has no courtyard, using bounding box")
            bbox = polygon.get_bounding_box()  # Returns tuple (min_x, min_y, max_x, max_y)
            
            # Check board boundaries
            min_x, min_y, max_x, max_y = bbox
            if (min_x < 0 or max_x > board_width or
                min_y < 0 or max_y > board_height):
                return False
    ...
    def _find_edge_position(self, component: ComponentWrapper,
                           board_width: float,
                           board_height: float) -> Point:
        ...
        for x in range(int(component.bbox.width() / 2),
                      int(board_width - component.bbox.width() / 2),
                      int(self.min_spacing)):
            for y in range(int(component.bbox.height() / 2),
