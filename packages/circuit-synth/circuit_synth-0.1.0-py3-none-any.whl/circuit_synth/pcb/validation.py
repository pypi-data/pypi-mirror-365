...
from .types import Point, Footprint, Pad, Track, Via, Zone
...
class ValidationIssue:
    ...
    def __str__(self) -> str:
        ...
        loc_str = f" at ({self.location.x:.2f}, {self.location.y:.2f})" if self.location else ""
        items_str = f" [{', '.join(self.affected_items)}]" if self.affected_items else ""
...
class ValidationResult:
    ...
    def add_error(self, category: str, message: str, location: Optional[Point] = None, 
                  affected_items: Optional[List[str]] = None):
        """Add an error-level issue."""
        self.issues.append(ValidationIssue(
            ValidationSeverity.ERROR, category, message, location, 
            affected_items or []
        ))
    
    def add_warning(self, category: str, message: str, location: Optional[Point] = None,
                    affected_items: Optional[List[str]] = None):
        """Add a warning-level issue."""
        self.issues.append(ValidationIssue(
            ValidationSeverity.WARNING, category, message, location,
            affected_items or []
        ))
    
    def add_info(self, category: str, message: str, location: Optional[Point] = None,
                 affected_items: Optional[List[str]] = None):
        """Add an info-level issue."""
        self.issues.append(ValidationIssue(
            ValidationSeverity.INFO, category, message, location,
            affected_items or []
        ))
    ...
    def print_summary(self):
        ...
        logger.info("VALIDATION", f"Errors: {self.error_count}", error_count=self.error_count)
        logger.info("VALIDATION", f"Warnings: {self.warning_count}", warning_count=self.warning_count)
        logger.info("VALIDATION", f"Info: {self.info_count}", info_count=self.info_count)
        logger.info("VALIDATION", f"Valid: {'Yes' if self.is_valid else 'No'}", is_valid=self.is_valid)
...
class PCBValidator:
    ...
    def __init__(self, min_trace_width: float = 0.15,
    ...
    def _validate_board_outline(self, pcb_board, result: ValidationResult):
        ...
        if 'polygon' in outline:
            ...
            if len(points) < 3:
                result.add_error("Board Outline", "Polygon outline has less than 3 points")
    ...
    def _validate_component_placement(self, pcb_board, result: ValidationResult):
        ...
        for footprint in pcb_board.footprints:
            ...
            if (bbox.min_x < board_min_x or bbox.max_x > board_max_x or
                bbox.min_y < board_min_y or bbox.max_y > board_max_y):
                result.add_error(
                    "Component Placement",
            ...
            if (bbox.min_x < board_min_x + edge_clearance or 
                bbox.max_x > board_max_x - edge_clearance or
                bbox.min_y < board_min_y + edge_clearance or 
                bbox.max_y > board_max_y - edge_clearance):
                result.add_warning(
                    "Component Placement",
            ...
            for fp2 in footprints[i+1:]:
                ...
                if bbox1.intersects(bbox2):
                    result.add_error(
                        ...
                        Point((fp1.position.x + fp2.position.x) / 2,
                ...
                if pad_ref not in connected_pads and pad.type != 'np_thru_hole':
                    ...
                    if not pad.number.startswith('MP') and pad.number != '':
                        result.add_warning(
                            ...
                            Point(footprint.position.x + pad.position.x,
        ...
        for track in tracks:
            ...
            if track.width < self.min_trace_width:
                result.add_error(
                    ...
                    Point((track.start.x + track.end.x) / 2,
