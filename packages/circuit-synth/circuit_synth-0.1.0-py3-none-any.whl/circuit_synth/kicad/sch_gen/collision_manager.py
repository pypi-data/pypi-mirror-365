...
from .collision_detection import CollisionDetector, BBox
...
class CollisionManager:
    ...
    def __init__(self, sheet_size: Tuple[float, float] = (210.0, 297.0), grid=2.54):
        ...
        """
        logger.debug("Initializing CollisionManager with sheet_size=%s, grid=%.2f", sheet_size, grid)
    ...
    def snap_to_grid(self, val: float) -> float:
        ...
        logger.debug("Snapped %.2f -> %.2f on grid=%.2f with offset=%.2f",
                    val, snapped, self.grid, offset)
        return snapped

    def place_symbol(self, symbol_width: float, symbol_height: float) -> Tuple[float, float]:
        """
        ...
        while attempt_count < max_attempts:
            ...
            if self.detector.add_bbox(test_bbox):
                logger.debug("Placed symbol at (%.2f, %.2f) with BBox=%s", test_x, test_y, test_bbox)
                ...
                if next_component_end > self.sheet_size[0] - RIGHT_EDGE_MARGIN:
                    ...
                    self.current_x = SHEET_MARGIN + grid_offset  # Reset to left margin with offset
                    
                    # If this is the first row, set a consistent Y position for the next row
                    # This prevents double-counting of component heights
                    if self.first_row:
                        self.current_y = SHEET_MARGIN + self.current_row_height + ROW_SPACING + grid_offset
                        self.first_row = False
                    else:
                        self.current_y += self.current_row_height + ROW_SPACING  # Move down by row height plus spacing
        ...
        logger.warning(f"Reached maximum placement attempts ({max_attempts}). Forcing placement at current position.")
        test_x = self.snap_to_grid(self.current_x)
        test_y = self.snap_to_grid(self.current_y)
    ...
    def register_existing_symbol(self, center_x: float, center_y: float,
                                symbol_width: float, symbol_height: float):
        """
        ...
        """
        logger.debug(f"Registering existing symbol at ({center_x}, {center_y}) "
