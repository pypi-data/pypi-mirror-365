...
from .collision_manager import CollisionManager, MIN_COMPONENT_SPACING
...
class ConnectionAwareCollisionManager(CollisionManager):
    ...
    def place_component_connection_aware(self, comp_ref: str, symbol_width: float, 
                                       symbol_height: float) -> Tuple[float, float]:
        """
        ...
        """
        logger.debug(f"Connection-aware placement for {comp_ref} ({symbol_width:.2f}x{symbol_height:.2f}mm)")
        ...
        placed_connections = [(ref, count) for ref, count in connected_comps 
                            if ref in self.placed_components]
        
        if not placed_connections:
            # No connected components placed yet, use regular placement
            logger.debug(f"No connected components placed yet for {comp_ref}, using default placement")
            position = self.place_symbol(symbol_width, symbol_height)
        else:
            # Calculate ideal position based on connected components
            ideal_x, ideal_y = self._calculate_ideal_position(comp_ref, placed_connections)
            logger.debug(f"Ideal position for {comp_ref}: ({ideal_x:.2f}, {ideal_y:.2f})")
            
            # Find nearest valid position using spiral search
            position = self._find_nearest_valid_position(ideal_x, ideal_y, symbol_width, symbol_height)
            
            if position is None:
                # Fallback to regular placement if no valid position found
                logger.warning(f"Could not find valid position near ideal for {comp_ref}, using fallback")
                position = self.place_symbol(symbol_width, symbol_height)
    ...
    def _calculate_ideal_position(self, comp_ref: str, 
                                placed_connections: List[Tuple[str, int]]) -> Tuple[float, float]:
        """
    ...
    def _find_nearest_valid_position(self, ideal_x: float, ideal_y: float,
                                   symbol_width: float, symbol_height: float) -> Optional[Tuple[float, float]]:
        """
    ...
    def _try_position(self, center_x: float, center_y: float,
                     symbol_width: float, symbol_height: float) -> bool:
        """
    ...
    def get_placement_metrics(self) -> Dict[str, float]:
        ...
        if self.placed_components:
            ...
            total_component_area = sum(w * h for w, h in self.component_dimensions.values())
