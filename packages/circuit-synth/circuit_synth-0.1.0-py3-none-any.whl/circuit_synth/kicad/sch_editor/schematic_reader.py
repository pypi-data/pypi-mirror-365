...
class SchematicReader:
    ...
    def read_file(self, filepath: str) -> Schematic:
        ...
        if not (len(data) > 0 and str(data[0]) == 'kicad_sch'):
            raise ValueError("Invalid KiCad schematic format: File must start with (kicad_sch ...)")
        ...
        if sheet_instances_sections:
            sheet_instances_raw = sheet_instances_sections[0]  # Store the raw S-expression
            
        symbol_instances_sections = self._find_sections(data, 'symbol_instances')
        if symbol_instances_sections:
            symbol_instances_raw = symbol_instances_sections[0]  # Store the raw S-expression
        ...
        for lib_section in lib_symbols:
            for symbol in lib_section[1:]:  # Skip 'lib_symbols' token
                if isinstance(symbol, list) and len(symbol) > 1:
                    ...
                    self._lib_symbols[lib_id] = symbol # Store the full S-expression for the symbol
    ...
    def _parse_symbol_section(self, section: List) -> Optional[SchematicSymbol]:
        ...
        try:
            ...
            fields_autoplaced = 'fields_autoplaced' in [str(x) for x in section if isinstance(x, sexpdata.Symbol)]
            ...
            for pin_def in self._find_sections(section, 'pin'):
                ...
                if pin_type not in ['input', 'output', 'bidirectional', 'tristate', 'passive', 'power_in', 'power_out', 'open_collector', 'open_emitter', 'unspecified']:
                    pin_type = 'passive'
    ...
    def _parse_nets(self, data: List, components: List[SchematicSymbol]) -> List[SchematicNet]:
            ...
            try:
                ...
                if effects_sections:
                    for effect_section in effects_sections:
                        ...
                        # Parse justify
                        justify_sections = self._find_sections(effect_section, 'justify')
