...
from ..sch_editor.schematic_reader import Schematic, SchematicSymbol, SchematicPin
import sexpdata
...
class ComponentManager:
    ...
    def __init__(self, schematic: Schematic):
        ...
        existing_refs = [comp.reference for comp in schematic.components 
        ...
        logger.info(f"Initialized ComponentManager with {len(schematic.components)} components")
    ...
    def add_component(
        ...
        """
        logger.debug(f"Adding component: lib_id={lib_id}, reference={reference}, value={value}")
    ...
    def _create_pins(
        ...
        logger.debug(f"Created {len(component.pins)} pins for component {component.reference}")
    ...
    def update_component_property(
        ...
        if property_name in component.properties:
            ...
            logger.debug(f"Updated property {property_name} of {reference} to {new_value}")
            return True
            
        elif create_if_missing:
            # Add new property
            component.properties[property_name] = new_value
            logger.debug(f"Added property {property_name} to {reference} with value {new_value}")
            return True
    ...
    def validate_schematic(self) -> List[str]:
        ...
        for comp in self.schematic.components:
            if comp.reference.endswith('?'):
                issues.append(f"Unassigned reference: {comp.reference} at {comp.position}")
        ...
        for i, comp1 in enumerate(self.schematic.components):
            ...
            for comp2 in self.schematic.components[i+1:]:
                ...
                if dx < 5.0 and dy < 5.0:  # Within 5mm
                    issues.append(f"Components {comp1.reference} and {comp2.reference} may overlap")
        
        return issues
    
    # Delegate methods to ComponentOperations
    def move_component(self, reference: str, new_position: Tuple[float, float],
    ...
    def clone_component(self, reference: str, new_reference: Optional[str] = None,
                       offset: Tuple[float, float] = (25.4, 0),
                       auto_increment: bool = True) -> Optional[SchematicSymbol]:
        """Clone an existing component."""
        return self.operations.clone_component(reference, new_reference, offset, auto_increment)