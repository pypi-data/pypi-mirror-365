...
class KiCadOperationMetrics:
    ...
    operation_type: str  # 'schematic_generation', 'pcb_generation', 'component_placement', etc.
    project_name: str
    component_count: int
    net_count: int
    duration_ms: float
...
class KiCadLogger:
    ...
    @contextmanager
    def operation_context(self, operation_name: str, project_name: str = "", **metadata):
        """
        ...
        try:
            with self.perf_logger.timer(operation_name, component=self.component) as timer_id:
    ...
    def log_schematic_generation(self, metrics: SchematicGenerationMetrics, project_name: str):
        """Log detailed schematic generation metrics."""
        total_time = (
            metrics.circuit_loading_ms + metrics.symbol_lookup_ms + 
            metrics.placement_ms + metrics.connection_analysis_ms + 
            metrics.file_writing_ms + metrics.validation_ms
        )
    ...
    def log_pcb_generation(self, metrics: PCBGenerationMetrics, project_name: str):
        ...
        total_time = (
            metrics.netlist_parsing_ms + metrics.component_loading_ms + 
            metrics.placement_ms + metrics.routing_ms + metrics.file_writing_ms
        )
    ...
    def log_component_placement(self, component_ref: str, position: Tuple[float, float], 
                              rotation: float = 0.0, placement_algorithm: str = "default"):
    ...
    def log_symbol_lookup(self, lib_id: str, lookup_time_ms: float, found: bool, 
    ...
    def log_collision_detection(self, component_ref: str, collision_count: int, 
    ...
    def log_routing_progress(self, nets_completed: int, total_nets: int, 
                           success_rate: float, current_net: str = ""):
    ...
    def log_file_generation(self, file_path: Path, file_type: str, size_bytes: int, 
    ...
    def log_validation_result(self, validation_type: str, passed: bool, 
                            issues: List[str] = None, validation_time_ms: float = 0):
    ...
    def log_performance_bottleneck(self, operation: str, duration_ms: float, 
                                 threshold_ms: float, details: Dict[str, Any] = None):
...
class KiCadOperationContext:
    ...
    def __init__(self, operation_id: str, operation_name: str, project_name: str, 
                 logger: KiCadLogger, timer_id: str):
...
def log_schematic_generation(project_name: str, **metadata):
    ...
    with kicad_logger.operation_context("schematic_generation", project_name, **metadata) as ctx:
        yield ctx

@contextmanager  
def log_pcb_generation(project_name: str, **metadata):
    """Context manager for PCB generation logging."""
    with kicad_logger.operation_context("pcb_generation", project_name, **metadata) as ctx:
        yield ctx
...
def log_component_placement(project_name: str, algorithm: str = "default", **metadata):
    ...
    with kicad_logger.operation_context("component_placement", project_name, 
                                       placement_algorithm=algorithm, **metadata) as ctx:
        yield ctx

def log_kicad_error(error: Exception, operation: str, project_name: str = "", **context):
