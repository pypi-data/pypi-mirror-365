...
from ..interfaces.kicad_interface import (
    ...
    GenerationResult, ComponentPlacement, NetConnection
)
...
from ..kicad_api.pcb.footprint_library import get_footprint_cache, FootprintLibraryCache
from ..kicad_api.core.symbol_cache import get_symbol_cache, SymbolLibraryCache
...
class UnifiedSchematicGenerator(ISchematicGenerator):
    ...
    def __init__(self, output_dir: str, project_name: str):
        ...
        self.project_generator = ProjectGenerator(str(self.output_dir), self.project_name)
        
        # Track components and nets for interface compliance
        self._component_count = 0
        self._net_count = 0
        
        logger.info(f"UnifiedSchematicGenerator initialized for project: {project_name}")
    ...
    def generate_from_circuit_data(self,
        ...
        with log_schematic_generation(self.project_name, config=config) as ctx:
            try:
                ...
                metrics.circuit_loading_ms = (time.perf_counter() - circuit_load_start) * 1000
                ctx.log_progress(f"Circuit loaded with {component_count} components", 20)
                ...
                metrics.symbol_lookup_ms = (time.perf_counter() - symbol_lookup_start) * 1000
                ...
                metrics.file_writing_ms = (time.perf_counter() - file_write_start) * 1000
    ...
    def add_component(self,
        ...
        try:
            ...
            logger.info(f"Added component {reference} ({library}:{symbol_name}) at {position}")
            return True
        except Exception as e:
    ...
    def add_wire(self,
        ...
        try:
            # This would be implemented by the underlying ProjectGenerator
            logger.info(f"Added wire from {start_point} to {end_point} (net: {net_name})")
            return True
        except Exception as e:
    ...
    def add_net_connection(self, connection: NetConnection) -> bool:
        ...
        try:
            # This would be implemented by the underlying ProjectGenerator
            self._net_count += 1
            logger.info(f"Added net connection: {connection.net_name} -> {connection.component_ref}.{connection.pin_number}")
            return True
        except Exception as e:
            logger.error(f"Failed to add net connection: {e}")
            return False
    
    def set_component_property(self,
                              reference: str,
                              property_name: str,
        ...
        with log_pcb_generation(self.project_name, config=config) as ctx:
            try:
                ...
                if config:
                    ...
                    placement_algorithm = getattr(config, 'placement_algorithm', placement_algorithm)
                ...
                metrics.netlist_parsing_ms = (time.perf_counter() - netlist_start) * 1000
                ...
                metrics.component_loading_ms = (time.perf_counter() - component_load_start) * 1000
                
                ctx.log_progress(f"Loaded {components_added} components", 60)
                
                # Component placement phase
                placement_start = time.perf_counter()
                ctx.log_progress(f"Applying {placement_algorithm} placement algorithm", 70)
    ...
    def _add_components_from_circuit_data(self, circuit_data: Dict[str, Any]) -> int:
        ...
        if hasattr(circuit, '_components'):
            for comp_id, component in circuit._components.items():
                try:
                    # Determine footprint
                    footprint = getattr(component, 'footprint', 'Device:R_0603_1608Metric')
...
class UnifiedSymbolLibrary(ISymbolLibrary):
    ...
    def list_libraries(self) -> List[str]:
        ...
        self.kicad_logger.logger.debug(
            "Listing all available symbol libraries",
            component="KICAD_SYMBOL_LIBRARY"
        )
...
class UnifiedFootprintLibrary(IFootprintLibrary):
    ...
    def list_footprints(self, library: str) -> List[str]:
        ...
        try:
            ...
            footprint_infos = self.footprint_cache.search_footprints("", filters={"library": library})
...
class UnifiedKiCadIntegration(IKiCadIntegration):
        ...
        try:
            ...
            if hasattr(circuit, '_components'):
                if not circuit._components:
                    validation_result['warnings'].append("No components found in circuit")
                else:
    ...
    def generate_project(self,
                        ...
                        draw_bounding_boxes: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"Using legacy generation logic for reliable KiCad project creation...")
...
def create_unified_kicad_integration(output_dir: str, 
                                   project_name: str,
                                   container: Optional[IDependencyContainer] = None) -> IKiCadIntegration:
    """Create a unified KiCad integration instance."""
    return UnifiedKiCadIntegration(output_dir, project_name, container)


def create_unified_schematic_generator(output_dir: str, project_name: str) -> ISchematicGenerator:
    """Create a unified schematic generator instance."""
