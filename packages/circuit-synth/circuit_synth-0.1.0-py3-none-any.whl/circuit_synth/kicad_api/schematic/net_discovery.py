...
class NetDiscovery:
    ...
    def _create_net_info(self, trace: NetTrace) -> NetInfo:
        ...
        component_pins = [(pin.component_ref, pin.pin_number) for pin in trace.component_pins]
    ...
    def _is_power_net(self, net_name: str) -> bool:
        ...
        power_patterns = [
            r"^VCC", r"^VDD", r"^V\+", r"^\+\d+V", r"^\+\d+\.\d+V",
            r"^VBAT", r"^VBUS", r"^VIN", r"^VOUT"
        ]
    ...
    def _is_ground_net(self, net_name: str) -> bool:
        ...
        ground_patterns = [
            r"^GND", r"^VSS", r"^AGND", r"^DGND", r"^PGND",
            r"^V-", r"^-\d+V", r"^0V"
        ]
    ...
    def merge_net_aliases(self, nets: List[NetInfo]) -> List[NetInfo]:
        ...
        for group in net_groups:
            if len(group) == 1:
                ...
                primary = sorted(group, key=lambda n: (n.name.startswith("Net_"), n.name))[0]
    ...
    def analyze_net_connectivity(self) -> ConnectivityReport:
        ...
        # Suggest net names
        suggested_names = self.suggest_net_names([n for n in merged_nets if n.name.startswith("Net_")])
        
        # Calculate connectivity score
        total_components = len(self.schematic.components)
        connected_components = total_components - len(unconnected_pins)
        connectivity_score = (connected_components / total_components * 100) if total_components > 0 else 0
        ...
        for net in unnamed_nets:
            ...
            if len(component_types) == 1:
                ...
                if "R" in types and "C" in types:
                    ...
                    suggestions[net.name] = "PULLUP" if len(component_types["R"]) == 1 else "RESISTOR_NET"
                elif "C" in types and "U" in types:
                    suggestions[net.name] = "BYPASS" if len(component_types["C"]) == 1 else "CAPACITOR_NET"
                else:
    ...
    def get_net_statistics(self) -> NetStatistics:
        ...
        # Find extremes
        most_connected = max(merged_nets, key=lambda n: len(n.component_pins), default=None)
        longest = max(merged_nets, key=lambda n: n.total_length, default=None)
        shortest = min(merged_nets, key=lambda n: n.total_length if n.total_length > 0 else float('inf'), default=None)
...
class HierarchicalNetDiscovery(NetDiscovery):
    ...
    def __init__(self, root_schematic: Schematic, sheet_schematics: Dict[str, Schematic]):
        """
