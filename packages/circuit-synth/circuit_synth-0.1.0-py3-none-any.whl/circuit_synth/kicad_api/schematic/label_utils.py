...
from ..core.types import Point, Label, Text, Wire, LabelType, SchematicSymbol
...
def get_wire_direction_at_point(wire: Wire, point: Tuple[float, float]) -> Optional[str]:
    """
    ...
    for i, wire_point in enumerate(wire.points):
        if abs(wire_point.x - test_point.x) < 0.1 and abs(wire_point.y - test_point.y) < 0.1:
    ...
    while to_check:
        ...
        for wire in wires:
            for point in wire.points:
                if (abs(point.x - current_label.position.x) < 0.1 and 
                    abs(point.y - current_label.position.y) < 0.1):
        ...
        for wire in label_wires:
            for label in all_labels:
                ...
                for point in wire.points:
                    if (abs(point.x - label.position.x) < 0.1 and 
                        abs(point.y - label.position.y) < 0.1):
