...
from ..types import Footprint, Point, Line, Rectangle
...
class CourtyardCollisionDetector:
    ...
    def get_courtyard_polygon(self, footprint: Footprint) -> Optional[Polygon]:
        ...
        courtyard_rects = [
            rect for rect in footprint.rectangles
            if rect.layer in ['F.CrtYd', 'B.CrtYd']
        ]
        
        if courtyard_rects:
            # If multiple rectangles, use the one with the largest area
            best_rect = max(courtyard_rects, key=lambda r: abs((r.end.x - r.start.x) * (r.end.y - r.start.y)))
        ...
        if hasattr(footprint, 'circles'):
            courtyard_circles = [
                circle for circle in footprint.circles
            ...
            if courtyard_circles:
                ...
                vertices = self._circle_to_polygon(circle.center.x, circle.center.y, circle.radius)
                return Polygon(vertices)
        
        # Check for courtyard arcs (if arc support exists)
        if hasattr(footprint, 'arcs'):
            courtyard_arcs = [
                arc for arc in footprint.arcs
                if arc.layer in ['F.CrtYd', 'B.CrtYd']
            ]
        ...
        courtyard_lines = [
            line for line in footprint.lines
            if line.layer in ['F.CrtYd', 'B.CrtYd']
        ]
        ...
        logger.error(f"No courtyard layer found for footprint {footprint.reference} ({footprint.name})")
        raise ValueError(f"Footprint {footprint.reference} ({footprint.name}) is missing required courtyard layer (F.CrtYd or B.CrtYd). All footprints must have courtyards for proper placement.")
    
    def _lines_to_polygon(self, lines: List[Line]) -> Optional[List[Tuple[float, float]]]:
        """
    ...
    def _circle_to_polygon(self, center_x: float, center_y: float, radius: float, segments: int = 32) -> List[Tuple[float, float]]:
        """
    ...
    def get_footprint_polygon(self, footprint: Footprint, use_courtyard: bool = True) -> Polygon:
        """
        ...
        if use_courtyard:
            ...
            logger.debug(f"Calculated bbox for {footprint.reference}: ({min_x:.2f}, {min_y:.2f}) to ({max_x:.2f}, {max_y:.2f})")
    ...
    def _calculate_footprint_bbox(self, footprint: Footprint) -> Tuple[float, float, float, float]:
    ...
    def check_collision(self, footprint1: Footprint, footprint2: Footprint) -> bool:
        ...
        logger.debug(f"Checking collision between {footprint1.reference} and {footprint2.reference}")
        logger.debug(f"  {footprint1.reference} bbox: ({bbox1[0]:.2f}, {bbox1[1]:.2f}) to ({bbox1[2]:.2f}, {bbox1[3]:.2f})")
        logger.debug(f"  {footprint2.reference} bbox: ({bbox2[0]:.2f}, {bbox2[1]:.2f}) to ({bbox2[2]:.2f}, {bbox2[3]:.2f})")
        ...
        logger.debug(f"  Bounding boxes overlap! Checking polygon collision with spacing={self.spacing}mm")
    ...
    def _bboxes_overlap(self, bbox1: Tuple[float, float, float, float],
        ...
        return not (bbox1[2] < bbox2[0] or  # bbox1 right < bbox2 left
                   ...
                   bbox2[3] < bbox1[1])     # bbox2 top < bbox1 bottom
    
    def check_collision_with_placed(self, footprint: Footprint,
                                   placed_footprints: List[Footprint]) -> bool:
        """
    ...
    def _polygons_intersect(self, poly1: Polygon, poly2: Polygon) -> bool:
        ...
        for i in range(len(poly1.vertices)):
            ...
            edge = (poly1.vertices[j][0] - poly1.vertices[i][0], 
        ...
        for i in range(len(poly2.vertices)):
            ...
            edge = (poly2.vertices[j][0] - poly2.vertices[i][0], 
    ...
    def _project_polygon(self, polygon: Polygon, axis: Tuple[float, float]) -> Tuple[float, float]:
    ...
    def _inflate_polygon(self, polygon: Polygon, distance: float) -> Polygon:
        ...
        for i in range(n):
            ...
            prev_edge = (vertices[i][0] - vertices[prev_i][0], 
                        vertices[i][1] - vertices[prev_i][1])
            next_edge = (vertices[next_i][0] - vertices[i][0],
            ...
            # Average the normals
            avg_normal = ((prev_normal[0] + next_normal[0]) / 2,
