...
class Polygon:
    ...
    def expand(self, distance: float) -> 'Polygon':
        ...
        for i in range(n):
            ...
            # Normalize average
            avg_len = math.sqrt(avg_normal.x * avg_normal.x + avg_normal.y * avg_normal.y)
            if avg_len > 0:
...
class CourtyardCollisionDetector:
    ...
    def detect_collisions(self, footprints: List[Footprint]) -> List[Tuple[Footprint, Footprint]]:
        """
            ...
            for j in range(i + 1, n):
                ...
                if self._bounding_boxes_overlap(fp1, fp2):
                    ...
                    if self.check_collision(fp1, fp2):
                        ...
                        logger.debug(f"Collision detected: {fp1.reference} <-> {fp2.reference}")
    ...
    def _bounding_boxes_overlap(self, fp1: Footprint, fp2: Footprint) -> bool:
        ...
        return not (bbox1[2] < bbox2[0] or  # box1 right < box2 left
                   ...
                   bbox1[3] < bbox2[1] or   # box1 bottom < box2 top (KiCad Y)
                   bbox2[3] < bbox1[1])     # box2 bottom < box1 top (KiCad Y)
    ...
    def _get_cached_polygon(self, footprint: Footprint) -> Polygon:
        ...
        cache_key = (footprint.reference, footprint.position.x, 
                    footprint.position.y, footprint.rotation)
    ...
    def _trace_polygon_from_lines(self, lines: List[Line]) -> Optional[List[Tuple[float, float]]]:
    ...
    def get_footprint_polygon(self, footprint: Footprint, use_courtyard: bool = True) -> Polygon:
        ...
        if use_courtyard:
            ...
            if courtyard:
                ...
                logger.debug(f"No courtyard found for {footprint.reference}, using bounding box")
        ...
        logger.debug(f"Bbox for {footprint.reference}: ({min_x:.2f}, {min_y:.2f}) to ({max_x:.2f}, {max_y:.2f})")
    ...
    def get_collision_vector(self, fp1: Footprint, fp2: Footprint) -> Optional[Tuple[float, float]]:
        """
