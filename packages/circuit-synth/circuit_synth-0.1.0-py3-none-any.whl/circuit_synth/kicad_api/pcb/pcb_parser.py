...
from .types import (
    Footprint, Pad, Property, Line, Arc, Text, Point,
    Net, Via, Track, Layer, Zone, Rectangle
)
...
class PCBParser:
    ...
    def parse_string(self, content: str) -> Dict[str, Any]:
        ...
        if not self._is_sexp_list(sexp) or self._get_symbol_name(sexp[0]) != "kicad_pcb":
    ...
    def _parse_general(self, sexp: List) -> Dict[str, Any]:
        ...
        general['legacy_teardrops'] = self._get_value(sexp, "legacy_teardrops", "no") == "yes"
        return general
    ...
    def _pcb_to_sexp(self, pcb_data: Dict[str, Any]) -> List:
        ...
        sexp = [
            ...
            [sexpdata.Symbol("generator_version"), pcb_data.get('generator_version', self.generator_version)]
        ]
        ...
        if 'general' in pcb_data:
            ...
            if 'thickness' in pcb_data['general']:
                general.append([sexpdata.Symbol("thickness"), pcb_data['general']['thickness']])
            if 'legacy_teardrops' in pcb_data['general']:
                val = sexpdata.Symbol("yes") if pcb_data['general']['legacy_teardrops'] else sexpdata.Symbol("no")
        ...
        if 'layers' in pcb_data:
            ...
            for layer in pcb_data['layers']:
                layer_elem = [layer['number'], layer['canonical_name'], sexpdata.Symbol(layer['type'])]
        ...
        if 'embedded_fonts' in pcb_data:
            val = sexpdata.Symbol("yes") if pcb_data['embedded_fonts'] else sexpdata.Symbol("no")
    ...
    def _zone_to_sexp(self, zone: Zone) -> List:
        ...
        fill.append([sexpdata.Symbol("thermal_bridge_width"), zone.thermal_relief_bridge])
        sexp.append(fill)
            ...
            if item_type == "net":
                ...
                if pts_elem:
                    for pt in pts_elem[1:]:
                        if self._is_sexp_list(pt) and self._get_symbol_name(pt[0]) == "xy":
    ...
    def _pad_to_sexp(self, pad: Pad) -> List:
        ...
        sexp = [sexpdata.Symbol("pad"), pad.number, sexpdata.Symbol(pad.type), sexpdata.Symbol(pad.shape)]
