...
from .base import PlacementAlgorithm, ComponentWrapper
...
class ForceDirectedPlacement(PlacementAlgorithm):
    ...
    def place(self, components: List[ComponentWrapper],
        ...
        logger.info(f"Board dimensions: {board_width}x{board_height}mm (Y-axis inverted)")
    ...
    def _update_group_properties(self, group: SubcircuitGroup):
        ...
        group.center = Point(
            sum_x / len(group.components),
            sum_y / len(group.components)
        )
    ...
    def _calculate_boundary_force(self, comp: Footprint, board_outline: BoundingBox) -> Force:
        ...
        if comp.position.x > board_outline.max_x - margin:
            force.fx -= strength * (comp.position.x - (board_outline.max_x - margin)) / margin
        ...
        if comp.position.y > board_outline.max_y - margin:
            force.fy -= strength * (comp.position.y - (board_outline.max_y - margin)) / margin
    ...
    def _calculate_group_attraction(self, group1: SubcircuitGroup, 
        ...
        # Stronger attraction for more connections
        magnitude = self.attraction_strength * math.log(connection_count + 1) * distance / 50.0
    ...
    def _calculate_group_repulsion(self, group1: SubcircuitGroup, 
        ...
        size1 = math.sqrt((group1.bbox.max_x - group1.bbox.min_x) * 
                         (group1.bbox.max_y - group1.bbox.min_y))
        size2 = math.sqrt((group2.bbox.max_x - group2.bbox.min_x) * 
        ...
        magnitude = self.repulsion_strength * 2 * (min_distance / effective_distance) ** 2
    ...
    def _enforce_minimum_spacing(self, footprints: List[Footprint],
                               connection_graph: Dict[str, Set[str]]):
        """
        ...
        for iteration in range(max_iterations // 2):
            ...
            logger.debug(f"Iteration {iteration + 1}: Found {len(collisions)} collisions")
        ...
        if remaining_collisions:
            logger.info(f"Second pass: Strict collision resolution for {len(remaining_collisions)} remaining collisions")
    ...
    def _build_connectivity_from_graph(self, footprints: List[Footprint],
                                     connection_graph: Dict[str, Set[str]]) -> Dict[str, Set[str]]:
    ...
    def _connectivity_aware_collision_resolution(self, footprints: List[Footprint],
                                               collisions: List[Tuple[Footprint, Footprint]],
                                               connectivity: Dict[str, Set[str]],
                                               gentle: bool = True) -> int:
        """
        ...
        for fp1, fp2 in collisions:
            ...
            are_connected = (fp2.reference in connectivity.get(fp1.reference, set()) or
                           fp1.reference in connectivity.get(fp2.reference, set()))
            ...
            fp1.position = Point(
                fp1.position.x - dx * push_distance,
                fp1.position.y - dy * push_distance
            )
            fp2.position = Point(
                fp2.position.x + dx * push_distance,
                fp2.position.y + dy * push_distance
            )
            ...
            # Log resolution
            if are_connected:
                logger.debug(f"Gently separating connected components {fp1.reference} and {fp2.reference}")
            else:
                logger.debug(f"Separating unconnected components {fp1.reference} and {fp2.reference}")
