...
class HierarchyNavigator:
    ...
    def _build_hierarchy_recursive(self, schematic: Schematic, 
    ...
    def _validate_sheet_connections(self, sheet: Sheet, 
                                   sheet_contents: Schematic,
                                   issues: List[str]):
    ...
    def analyze_hierarchy(self) -> HierarchyAnalysis:
        ...
        duplicate_instances = {
            filename: count 
            for filename, count in instance_counts.items() 
            if count > 1
        }
