...
from ..core.types import (
    Schematic, SchematicSymbol, Wire, Label, Junction,
    Point, BoundingBox, Net, LabelType
)
...
class SearchQuery:
    ...
    def add_criterion(self, field: str, pattern: str, 
                     match_type: MatchType = MatchType.CONTAINS,
                     case_sensitive: bool = False):
        """Add a search criterion."""
        self.criteria.append(SearchCriterion(
            field=field,
            pattern=pattern,
...
class SearchResults:
    ...
    def total_count(self) -> int:
        ...
        return (len(self.components) + len(self.nets) + 
                len(self.wires) + len(self.labels))
...
class SearchEngine:
    ...
    def search_components(self, query: Union[SearchQuery, str] = None,
                         reference: str = None,
                         value: str = None,
                         footprint: str = None,
                         lib_id: str = None,
                         use_regex: bool = False) -> List[SchematicSymbol]:
        """
        ...
        # Build query from individual parameters
            ...
            query = SearchQuery()
            if reference:
                query.add_criterion("reference", reference,
                                  MatchType.REGEX if use_regex else MatchType.CONTAINS)
            if value:
                query.add_criterion("value", value,
                                  MatchType.REGEX if use_regex else MatchType.CONTAINS)
            if footprint:
                query.add_criterion("footprint", footprint,
                                  MatchType.REGEX if use_regex else MatchType.CONTAINS)
            if lib_id:
                query.add_criterion("lib_id", lib_id,
                                  MatchType.REGEX if use_regex else MatchType.CONTAINS)
    ...
    def _matches_query(self, component: SchematicSymbol, query: SearchQuery) -> bool:
        ...
        for criterion in query.criteria:
            ...
            matches.append(self._matches_pattern(
                field_value, criterion.pattern,
                criterion.match_type, criterion.case_sensitive
            ))
    ...
    def _get_component_field(self, component: SchematicSymbol, field: str) -> Optional[str]:
    ...
    def _matches_pattern(self, text: str, pattern: str, 
                        match_type: MatchType, case_sensitive: bool) -> bool:
    ...
    def search_by_value(self, value_pattern: str, 
                       tolerance: float = None) -> List[SchematicSymbol]:
        """
    ...
    def find_power_nets(self) -> List[str]:
        ...
        power_patterns = [
            r"^VCC", r"^VDD", r"^VSS", r"^GND", r"^AGND", r"^DGND",
            r"^\+\d+V", r"^-\d+V", r"^V\+", r"^V-"
        ]
...
class SearchQueryBuilder:
    ...
    def with_property(self, name: str, pattern: str, 
                     match_type: MatchType = MatchType.CONTAINS):
