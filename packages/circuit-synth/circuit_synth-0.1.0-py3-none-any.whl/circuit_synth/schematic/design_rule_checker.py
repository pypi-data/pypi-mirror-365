...
from ..core.types import SchematicSymbol as Component, Wire, Junction, Label, Point
...
class DesignRules:
    ...
    required_properties: List[str] = field(default_factory=lambda: ["Reference", "Value"])
...
class DesignRuleChecker:
    ...
    def check_schematic(self, components: List[Component], 
                       wires: List[Wire],
                       junctions: List[Junction],
                       labels: List[Label],
                       no_connects: List[NoConnect]) -> List[RuleViolation]:
    ...
    def _check_unconnected_pins(self, components: List[Component],
                               wires: List[Wire],
        ...
        for comp in components:
            ...
            for pin in comp.pins:
                ...
                for node in graph.nodes.values():
                    if (abs(node.position.x - pin_pos[0]) < 0.01 and
                        abs(node.position.y - pin_pos[1]) < 0.01):
                ...
                if not connected:
                    ...
                    if hasattr(pin, 'electrical_type') and pin.electrical_type == 'power_in':
                    ...
                    self._add_violation(RuleViolation(
                        rule_name="unconnected_pin",
                        severity=severity,
                        category=RuleCategory.ELECTRICAL,
                        message=message,
                        elements=[comp],
                        location=pin.position if hasattr(pin, 'position') else comp.position,
                        suggestion="Connect pin or add no-connect symbol"
                    ))
    
    def _check_floating_nets(self, components: List[Component],
                            wires: List[Wire],
                            labels: List[Label]):
    ...
    def _check_missing_references(self, components: List[Component]):
        ...
        for comp in components:
            if not comp.reference or comp.reference == "?" or comp.reference.endswith("?"):
                self._add_violation(RuleViolation(
    ...
    def _check_reference_naming(self, components: List[Component]):
        ...
        for comp in components:
            if comp.reference and comp.reference != "?" and not pattern.match(comp.reference):
                self._add_violation(RuleViolation(
    ...
    def _check_overlapping_components(self, components: List[Component]):
        ...
        for i, comp1 in enumerate(components):
            for comp2 in components[i+1:]:
                ...
                if dx < self.rules.min_component_spacing and dy < self.rules.min_component_spacing:
                    self._add_violation(RuleViolation(
    ...
    def _check_off_grid(self, components: List[Component], 
                       wires: List[Wire],
                       junctions: List[Junction]):
    ...
    def _find_wire_intersection(self, wire1: Wire, wire2: Wire) -> Optional[Tuple[float, float]]:
    ...
    def _check_missing_footprints(self, components: List[Component]):
        ...
        for comp in components:
            ...
            if not footprint_prop or not footprint_prop.value or footprint_prop.value == "~":
                self._add_violation(RuleViolation(
    ...
    def _check_text_size(self, components: List[Component], labels: List[Label]):
        ...
        for comp in components:
            ...
            for prop in comp.properties:
                if hasattr(prop, 'effects') and prop.effects and hasattr(prop.effects, 'font'):
                    size = prop.effects.font.size.x if hasattr(prop.effects.font.size, 'x') else 1.27
                    ...
                    if size < min_size:
                        self._add_violation(RuleViolation(
                            ...
                            elements=[comp],
                            location=prop.at if hasattr(prop, 'at') and prop.at else comp.position,
    ...
    def filter_violations(self, severity: Optional[RuleSeverity] = None,
                         category: Optional[RuleCategory] = None) -> List[RuleViolation]:
