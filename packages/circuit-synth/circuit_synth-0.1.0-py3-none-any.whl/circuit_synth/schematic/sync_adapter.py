...
class SyncAdapter:
    ...
    def _find_schematic(self) -> Path:
        ...
        for sch_file in sch_files:
            if sch_file != main_schematic_path and self._schematic_has_components(sch_file):
                return sch_file
        
        # Fallback: use the main schematic even if it's a cover sheet
        if main_schematic_path.exists():
            print(f"[WARNING] No circuit schematics found with components, using main schematic: {main_schematic_path}")
            return main_schematic_path
    ...
    def _schematic_has_components(self, schematic_path: Path) -> bool:
        ...
        try:
            ...
            for comp in schematic.components:
                # Skip power symbols
                if comp.lib_id and not any(pwr in comp.lib_id for pwr in ['power:', 'Power:', '#PWR', '#FLG']):
                    component_count += 1
