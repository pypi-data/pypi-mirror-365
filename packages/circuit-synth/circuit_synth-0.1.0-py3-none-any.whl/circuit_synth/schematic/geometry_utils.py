...
import math
import logging
from ..core.types import Point, SchematicPin, SchematicSymbol, Label, LabelType
...
class GeometryUtils:
    ...
    def get_actual_pin_position(symbol: SchematicSymbol, pin_number: str) -> Optional[Point]:
        ...
        """
        logger.info(f"Getting actual position for pin {pin_number} on symbol {symbol.reference}")
        ...
        for pin in symbol.pins:
            if pin.number == pin_number:
                ...
                logger.info(f"  Pin {pin_number} world position: ({world_pos.x}, {world_pos.y})")
                return world_pos
    ...
    @staticmethod
    def transform_pin_to_world(pin: SchematicPin, symbol_pos: Point,
                              symbol_rotation: float) -> Point:
        """
    ...
    @staticmethod
    def get_pin_end_position(pin: SchematicPin, pin_world_pos: Point,
                           symbol_rotation: float) -> Point:
        """
        ...
        if world_orientation == 0:  # Pin points right, so end is to the right
        ...
        elif world_orientation == 90:  # Pin points up, so end is up (negative Y in KiCad)
            offset_x = 0
            offset_y = -pin.length
        ...
        elif world_orientation == 270:  # Pin points down, so end is down (positive Y in KiCad)
            offset_x = 0
            offset_y = pin.length
            ...
            logger.info(f"  Pin at {world_orientation}°: offset = ({offset_x}, {offset_y})")
    ...
    @staticmethod
    def calculate_label_position(pin: SchematicPin, pin_world_pos: Point,
                               symbol_rotation: float,
                               offset_distance: float = 0) -> Point:
        """
    ...
    @staticmethod
    def create_dynamic_hierarchical_label(net_name: str, pin: SchematicPin,
                                        pin_world_pos: Point,
                                        symbol_rotation: float,
                                        offset_distance: float = 0) -> Label:
        """
        ...
        logger.info(f"  Final label '{net_name}': position=({label_pos.x}, {label_pos.y}), orientation={label_orientation}°")
    ...
    @staticmethod
    def get_pins_at_position(symbol: SchematicSymbol,
                           tolerance: float = 0.01) -> Dict[Tuple[float, float], List[SchematicPin]]:
        """
        ...
        for pin in symbol.pins:
            ...
            for pos_key in pins_by_position:
                if (abs(pos_key[0] - world_pos.x) < tolerance and 
                    abs(pos_key[1] - world_pos.y) < tolerance):
