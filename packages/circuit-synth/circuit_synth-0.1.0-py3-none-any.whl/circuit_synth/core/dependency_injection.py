...
from ._interfaces import (
    ...
    ISymbolLibrary, IFootprintLibrary,
    
    # Circuit Models
    ICircuitModel, IComponentModel, IPinModel, INetModel,
    CircuitModelFactory, ComponentModelFactory, NetModelFactory, PinModelFactory,
    
    # Intelligence System
    ILLMProvider, ILLMManager, IAgent, IAgentManager,
    IKnowledgeBase, IPromptTemplate, IPromptManager,
    LLMProviderFactory, AgentFactory, KnowledgeBaseFactory, PromptManagerFactory
)
...
class DependencyRegistration:
    ...
    def __init__(
        ...
        if sum(bool(x) for x in [implementation, factory, instance]) > 1:
            raise ValueError("Can only provide one of implementation, factory, or instance")
...
class DependencyContainer(IDependencyContainer):
    ...
    def register(self, registration: DependencyRegistration) -> None:
        ...
        with self._lock:
            ...
            logger.debug(f"Registered {registration.interface.__name__} with {registration.lifetime} lifetime")
