...
from circuit_synth.core.logging.unified_logger import (
    logger, context_logger, performance_logger, llm_conversation_logger
)
import uuid
...
def example_user_flow():
    ...
    # User login
    username = "admin"
    session_id = f"{username}_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:8]}"
    
    context_logger.info("User login successful", component="AUTH", user=username, session=session_id, success=True)
    
    # Create a new chat
    chat_id = f"chat_{uuid.uuid4().hex[:12]}"
    log_chat(f'New chat created: "Voltage Regulator Design"', 
             user=username, chat=chat_id, session=session_id)
    
    # User sends a message
    context_logger.info("User message received", component="CHAT", user=username, chat=chat_id)
    ...
    context_logger.info("Circuit generation started", component="CIRCUIT",
        user=username,
        chat=chat_id,
    ...
    context_logger.info("Circuit generation completed", component="CIRCUIT",
        user=username,
        chat=chat_id,
        generation_id=generation_id,
        files=4,
    ...
    # User logout
    context_logger.info("User logout", component="AUTH", user=username, session=session_id, success=True)
...
def example_multi_user_activity():
    ...
    # User 1 activity
    context_logger.info("User login successful", component="AUTH", user="alice", session="alice_20250627_214530_abc123")
    context_logger.info('New chat: "Op-Amp Filter"', component="CHAT", user="alice", chat="chat_aaa111")
    
    # User 2 activity (interleaved)
    context_logger.info("User login successful", component="AUTH", user="bob", session="bob_20250627_214531_def456")
    context_logger.info('New chat: "Power Supply"', component="CHAT", user="bob", chat="chat_bbb222")
...
if __name__ == "__main__":
    ...
    print('grep "\\[LLM     \\]" logs/circuit_synth.log      # All LLM requests/responses')
