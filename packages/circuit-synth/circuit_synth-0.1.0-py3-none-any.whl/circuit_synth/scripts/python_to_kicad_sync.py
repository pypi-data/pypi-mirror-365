...
class PythonCircuitParser:
    ...
    def __init__(self, python_file_path: str):
        ...
        if not self.python_file_path.exists():
            raise FileNotFoundError(f"Python circuit file not found: {python_file_path}")
    ...
    def extract_circuit(self) -> Circuit:
        ...
        logger.info(f"Circuit has {len(circuit.components)} components and {len(circuit.nets)} nets")
        return circuit
    
    def _load_circuit_from_module(self) -> Optional[Circuit]:
        """Try to load Circuit object directly from module"""
        try:
            spec = importlib.util.spec_from_file_location("circuit_module", self.python_file_path)
            ...
            if len(circuit_objects) == 1:
                ...
                logger.warning(f"Found {len(circuit_objects)} Circuit objects, using first one")
                return circuit_objects[0]
            else:
...
class PythonToKiCadSyncer:
    ...
    def _create_backup(self):
        ...
        shutil.copytree(project_dir, backup_dir, ignore=shutil.ignore_patterns('*_backup*'))
    ...
    def _generate_final_report(self, sync_report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        report = {
            ...
            'details': {
                ...
                'components_preserved': len(sync_report.get('components_to_preserve', [])),
...
Examples:
  ...
  %(prog)s my_circuit.py my_project.kicad_pro --apply --report sync_report.json
    ...
    action_group.add_argument('--apply', action='store_true',
                             help='Apply changes to KiCad schematic')
    
    # Sync options
    parser.add_argument('--backup', action='store_true',
                       help='Create backup before applying changes')
    parser.add_argument('--no-preserve-user', action='store_true',
                       help='Do not preserve user-added components')
    parser.add_argument('--match-criteria', nargs='+',
    ...
    parser.add_argument('--report', metavar='FILE',
                       help='Save detailed sync report to file')
