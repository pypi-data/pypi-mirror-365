...
def calculate_features(song_path, sr=22050, chunk_size=256000):
    ...
    for i in range(num_chunks):
        ...
        if i == 0:
            ...
            chroma_result = librosa.feature.chroma_stft(S=np.abs(stft_result), sr=sr, n_fft=2048, hop_length=512)
    ...
    return merged_mel_spec, estimated_key
...
def process_song(access_token, song_path, song_title, artist_name, language, genre, date_released):
    ...
    merged_mel_spec, key = calculate_features(song_path)
    response2 = requests.post(
        'http://localhost:8000/api/save_key',
        json={'song_id': track_id, 'key': key},
        headers={
            'Content-Type': 'application/json',
            'Authorization': f'Bearer {access_token}'
        }
