...
class JobBase(BaseSchema):
    ...
    def sanitized_copy(self, logged_in=True, same_project=False) -> Self:
        ...
        return self.__class__(
            id=self.id,
            name=self.name
            if same_project or self.name == "JupyterHub-singleuser"
            else ("job-" + self.id[:6] if self.id else "job"),
            deployment_environment=self.deployment_environment,
            request=self.request.sanitized_copy(logged_in, same_project),  # pyright: ignore [reportCallIssue]
            description=self.description if logged_in and same_project else None,
            owner=self.owner.sanitized_copy(logged_in, same_project) if hasattr(self.owner, "sanitized_copy") else None,  # pyright: ignore [reportCallIssue,reportAttributeAccessIssue]
