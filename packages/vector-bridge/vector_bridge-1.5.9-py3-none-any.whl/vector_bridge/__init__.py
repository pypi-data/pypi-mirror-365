...
class BaseVectorBridgeClient:
    ...
    def _create_http_exception(self, status_code: int, error_data: Any) -> HTTPException:
...
╚██╗ ██╔╝██╔══╝  ██║        ██║   ██║   ██║██╔══██╗██╔══██╗██╔══██╗██║██║  ██║██║   ██║██╔══╝     ██╔══██║██║
 ╚████╔╝ ███████╗╚██████╗   ██║   ╚██████╔╝██║  ██║██████╔╝██║  ██║██║██████╔╝╚██████╔╝███████╗██╗██║  ██║██║
  ╚═══╝  ╚══════╝ ╚═════╝   ╚═╝    ╚═════╝ ╚═╝  ╚═╝╚═════╝ ╚═╝  ╚═╝╚═╝╚═════╝  ╚═════╝ ╚══════╝╚═╝╚═╝  ╚═╝╚═╝
DOCS: https://docs.vectorbridge.ai/                                                           version: 1.5.9
...
class VectorBridgeClient(BaseVectorBridgeClient):
    ...
    def _handle_response(self, response: requests.Response) -> Any:
        ...
        if 200 <= response.status_code < 300:
            ...
            exc = self._create_http_exception(response.status_code, error_data)
            raise exc
...
class AsyncVectorBridgeClient(BaseVectorBridgeClient):
    ...
    async def _handle_response(self, response: aiohttp.ClientResponse) -> Any:
        ...
        if 200 <= response.status < 300:
            ...
            exc = self._create_http_exception(response.status, error_data)
            raise exc
