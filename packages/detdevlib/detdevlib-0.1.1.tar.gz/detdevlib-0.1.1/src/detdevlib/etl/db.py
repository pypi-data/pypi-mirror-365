...
class MSSQLSettings(BaseSettings, DialectSettings):
    ...
    def get_connection_url(self) -> str:  # noqa: D102
        return (
            f"mssql+pyodbc://{self.username}:{self.password.get_secret_value()}@{self.hostname}/"
...
class PostgreSQLSettings(BaseSettings, DialectSettings):
    ...
    def get_connection_url(self) -> str:  # noqa: D102
        return f"postgresql+psycopg2://{self.username}:{self.password.get_secret_value()}@{self.hostname}:{self.port}/{self.database}"
...
class SQLiteSettings(BaseSettings, DialectSettings):
    ...
    def get_connection_url(self) -> str:  # noqa: D102
        return f"sqlite:///{self.database.resolve()}"
...
class MySQLSettings(BaseSettings, DialectSettings):
    ...
    def get_connection_url(self) -> str:  # noqa: D102
        return f"mysql+pymysql://{self.username}:{self.password.get_secret_value()}@{self.hostname}:{self.port}/{self.database}"
...
class SnowflakeSettings(BaseSettings, DialectSettings):
    ...
    def get_connection_url(self) -> str:  # noqa: D102
        base_url = f"snowflake://{self.user}:{self.password.get_secret_value()}@{self.account}/{self.database}/{self.schema_name}?warehouse={self.warehouse}"
        if self.role:
            base_url += f"&role={self.role}"
        return base_url
