...
class Chrome():
    ...
    def getDriverPath(self, system):
        ...
        if system == "Windows":
            driver_path = common.resourcePath(f"./drive/chrome/chromedriver/windows/{driver_filename}")
            # 如果本地没有驱动文件，则从远程下载
            if not os.path.exists(driver_path):
                self.download_driver(base_url_windows + chrome_major + ".exe", driver_path)

        elif system == "Linux":
            driver_path = common.resourcePath(f"./drive/chrome/chromedriver/linux/{chrome_major}/chromedriver")
            # 如果本地没有驱动文件，则从远程下载
            if not os.path.exists(driver_path):
