...
from exceptions import IpException, ChromeException, FingerprintException
...
class Chrome():
    ...
    def run_mitmproxy(self, data, listen_port):
        ...
        # 获取根目录
        root_dir = Path(os.getcwd())  # Convert to Path object
        print("root_dir:", root_dir)

        # 获取 mitmdump.py 的脚本路径
        script_path = root_dir / "mitmdump.py"
        print("script_path:", script_path)

        # 创建日志文件路径（使用端口号区分不同实例）
        log_dir = root_dir / "log" / "mitm_logs"
        log_dir.mkdir(parents=True, exist_ok=True)  # 使用 Path 的 mkdir 方法创建目录
        log_file_path = log_dir / f"mitmdump_{listen_port}.log"
    ...
    def download_driver(self, url, driver_path):
        '''
    ...
    def getDriverPath(self, system):
        ...
        base_url_windows = self.cdn_host + "/drive/chrome/chromedriver/windows/"
        base_url_linux = self.cdn_host + "/drive/chrome/chromedriver/linux/"

        if system == "Windows":
            # 转换为 Path 对象
            driver_path = Path(common.resourcePath(f"./drive/chrome/chromedriver/windows/{driver_filename}"))

            # 调试输出
            print(f"Windows driver_path 类型: {type(driver_path)}, 值: {driver_path}")

            # 如果本地没有驱动文件，则从远程下载
            if not driver_path.exists():
                self.download_driver(base_url_windows + chrome_major + ".exe", driver_path)

        elif system == "Linux":
            # 转换为 Path 对象
            driver_path = Path(common.resourcePath(f"./drive/chrome/chromedriver/linux/{chrome_major}/chromedriver"))

            # 调试输出
            print(f"Linux driver_path 类型: {type(driver_path)}, 值: {driver_path}")

            # 如果本地没有驱动文件，则从远程下载
            if not driver_path.exists():
                self.download_driver(base_url_linux + f"{chrome_major}/chromedriver", driver_path)
                # 设置 Linux 系统下的执行权限
        ...
        print(f"最终的 driver_path: {driver_path}")
    ...
    def getFingerprint(self, driver, shop_data, params):
        '''
        @Desc    : 获取指纹信息
        @Author  : 钟水洲
        @Time    : 2025/06/24 20:16:20
    ...
    def closeTimeoutProcess(self):
        ...
        '''
        # 定义10分钟前的时间戳
        time_threshold = time.time() - (10 * 60)  # 10分钟之前的时间戳
