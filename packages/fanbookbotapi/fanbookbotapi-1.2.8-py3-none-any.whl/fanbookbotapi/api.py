...
def getme(token:str,session=requests.Session()) ->requests.models.Response:
    """获取bot信息
    ...
    """
    r=session.get(apiurl+token+apilist['getme'])
    return r

def sendmessage(session=requests.Session(),token='',chatid=0,biaoti="标题",add_Key=False,ik=[[{"text":"下一页","callback_data":"{\"type\":\"next\",\"index\":2,\"msg\":\"114514\"}"}]],text='文本',biaoticolor='ffe4e4',type="text",shade=['ff764a','ffb39aff'],backgroundColor='ddeeff00',getjson=False) -> requests.models.Response|str:
    """发送消息

    Args:
        session (requests.Session, optional): requests会话对象. Defaults to requests.Session().
...
def getPrivateChat(token:str,userid:int,session=requests.Session()) ->requests.models.Response:
    """创建私聊频道
...
def sendCard(token:str,chatid:int,markdown:str,color=['#00afee','#f2f2f2'],bt='标题',textcolor='#f2f2f2',openbutton=False,btcolor='#00afee',burl='https://www.baidu.com',button='按钮',come_from_name=None,come_from_icon=None,session=requests.Session()) -> dict:
    """发送消息卡片
    ...
    r=sendmessage(session=session,token=token,chatid=chatid,type='fanbook',text=json.dumps({'width': None, 'height': None, 'data':json.dumps(data) , 'notification': None, 'come_from_icon': None, 'come_from_name': come_from_name, 'template': None, 'no_seat_toast': None, 'type': 'messageCard'})).text
    return json.loads(r)
