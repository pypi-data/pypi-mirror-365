...
class UUV_EKF:
    ...
    def __init__(
        ...
        H_attitude = np.zeros((4, self.state_dim))
        H_attitude[:, 3:7] = np.eye(4, dtype=np.float64)
        ...
        self._R_attitude = np.zeros((4, 4), dtype=np.float64)
        if isinstance(covariance_ahrs_quaternion, float):
            self._R_attitude[:3, :3] = (
                np.eye(3, dtype=np.float64) * covariance_ahrs_quaternion
            )
        else:
            self._R_attitude[:3, :3] = np.diag(covariance_ahrs_quaternion)
    ...
    def correction_attitude(
        ...
        z = np.vstack(quaternion)

        # Innovation or residual (y, shape=(4, 1))
        y = z - self._H_attitude @ np.vstack(self._current_state[1:])

        # Measurement noise covariance (R, shape=(4, 4))
        if measurement_covariance is None:
            R = self._R_attitude
        else:
            if measurement_covariance.shape[0] != z.shape[0]:
