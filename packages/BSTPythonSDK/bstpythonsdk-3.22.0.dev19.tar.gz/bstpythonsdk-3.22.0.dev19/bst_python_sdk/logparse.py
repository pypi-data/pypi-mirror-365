...
class Parser:
    ...
    def parse_log(self, filename: str) -> dict:
        ...
        for pkt in bst_packets:
            if (pkt.FROM & 0xFF000000) == 0x41000000 or not self.has_addr:
                ...
                # if pkt.TYPE == PacketTypes.ACTUATORS_VALUES.value:
                    # print(self.ac_sys_current_time)
    ...
    def add_packet(self, pkt, pkt_data):
        ...
        has_sys_time = hasattr(pkt_data, 'system_time') and pkt.TYPE != 33

        if has_sys_time and pkt_data.system_time != 0 and pkt_data.system_time > self.prev_pkt_time:
            self.prev_pkt_time = pkt_data.system_time
            # print(f'adding system time {self.prev_pkt_time} from {PacketTypes(pkt.TYPE).name}')
