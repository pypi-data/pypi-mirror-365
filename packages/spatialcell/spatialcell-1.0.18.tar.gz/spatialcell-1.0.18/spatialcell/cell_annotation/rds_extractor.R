...
# Define standard cell types for analysis
SuScs_celltypes <- c("Pre-osteoblast", "Immature osteoblast","Proliferating preosteoblast", "Axin2⁺-SuSC", "Pro-inflammatory dural fibroblast", "Chondrocyte","Ectocranial SM", "Outer dural fibroblast", "Ligament-like mesenchyme","Neural progenitor cell", "Gli1⁺-SuSC", "Capillary endothelial cell","Pericyte", "Mmp13⁺-Pre-osteoblast", "Neuron", "Glial cell", "Naive B cell", "M2 Macrophage", "M1 Macrophage", "Arachnoid fibroblast", "Proliferating B cell", "Inner dural fibroblast", "Mast cell", "Neutrophil","Osteoclast")

# Function to process cell types based on data composition
process_cell_selection <- function(counts, meta, celltype_col, cells) {
  ...
  if (has_axin2 && has_gli1) {
    ...
    return(list(counts = SuSCs_counts, meta = SuSCs_meta, cells = SuSCs_cells))
  } else {
    return(list(counts = counts, meta = meta, cells = cells))
  }
}
...
if (is.null(samples)) {
  ...
  processed_data <- process_cell_selection(counts, meta, celltype_col, all_cells)
  ...
  for (time_point in samples) {
    ...
    if (length(all_cells_time) == 0) {
      ...
      processed_data <- process_cell_selection(counts_time, meta_time, celltype_col, all_cells_time)
