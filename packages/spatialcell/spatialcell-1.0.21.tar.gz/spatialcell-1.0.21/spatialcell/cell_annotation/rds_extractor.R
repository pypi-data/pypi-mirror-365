...
SuScs_celltypes <- c("Immature osteoblast", "Axin2⁺-SuSC", "Pro-inflammatory dural fibroblast", "Chondrocyte","Ectocranial SM", "Pre-osteoblasts", "Outer dural fibroblast",  "Ligament-like mesenchyme","Neural progenitor cell", "Gli1⁺-SuSC", "Capillary endothelial cell","Pericyte", "Mature  osteoblast","Mmp13⁺-Pre-osteoblast",  "Neuron", "Glial cell", "Naive B cell", "M2 Macrophage", "M1 Macrophage", "Proliferating preosteoblast", "Arachnoid fibroblast", "Proliferating B cell", "Proliferating meningeal fibroblasts", "Inner dural fibroblast", "Mast cell", "Neutrophil","Osteoclast")

# Function to process cell types
process_cell <- function(counts, meta, celltype_col, cells) {
  read_celltypes <- function() {
    color_file <- "../utils/color_schemes.py"
    if (file.exists(color_file)) {
      lines <- readLines(color_file)
  ...
  cat("=== Cell Type Processing Validation ===\n")
  cat("Key cell types check - Axin2⁺-SuSC:", has_axin2, ", Gli1⁺-SuSC:", has_gli1, "\n")
  ...
  if (has_axin2 && has_gli1) {
    ...
    cat("Filtered cells:", length(SuSCs_cells), "out of", length(cells), "\n")
    ...
    cat("Original cell type counts (before sorting):\n")
    print(celltype_counts)
    
    ordered_celltypes <- names(sort(celltype_counts, decreasing = FALSE))
    cat("Cell types ordered by count (ascending):\n")
    for (i in 1:length(ordered_celltypes)) {
      cat(sprintf("%2d. %-35s (n=%d)\n", i, ordered_celltypes[i], celltype_counts[ordered_celltypes[i]]))
    }
    
    celltype_names <- read_celltypes()
    
    if (!is.null(celltype_names)) {
      cat("Standard cell type names loaded from color_schemes.py:\n")
      for (i in 1:min(length(celltype_names), 10)) {  # 只显示前10个
        cat(sprintf("%2d. %s\n", i, celltype_names[i]))
      }
      if (length(celltype_names) > 10) {
        cat("... and", length(celltype_names) - 10, "more\n")
      }
      cat("Total standard names:", length(celltype_names), "\n")
    } else {
      cat("Could not load standard cell type names from color_schemes.py\n")
    }
    
    if (!is.null(celltype_names) && length(celltype_names) == length(ordered_celltypes)) {
      cat("=== Applying Rename Mapping ===\n")
      celltype_map <- setNames(celltype_names, ordered_celltypes)
      
      cat("Rename mapping (original -> new):\n")
      for (i in 1:length(celltype_map)) {
        cat(sprintf("  %s -> %s\n", names(celltype_map)[i], celltype_map[i]))
      }
      
      SuSCs_meta_map <- SuSCs_meta
      SuSCs_meta_map[[celltype_col]] <- celltype_map[SuSCs_meta[[celltype_col]]]
      
      cat("Final cell type distribution (after rename):\n")
      final_counts <- table(SuSCs_meta_map[[celltype_col]])
      print(final_counts)
      
      ordered_indices <- order(match(SuSCs_meta_map[[celltype_col]], celltype_names))
      
      cat("=== Final Result ===\n")
      cat("Cells reordered by standard sequence, total cells:", length(ordered_indices), "\n")
      return(list(
        counts = SuSCs_counts[, ordered_indices], 
        meta = SuSCs_meta_map[ordered_indices, ], 
        cells = SuSCs_cells[ordered_indices]
      ))
    } else {
      cat("=== Using Original Names (no rename) ===\n")
      cat("Length mismatch - standard:", length(celltype_names), "vs ordered:", length(ordered_celltypes), "\n")
      
      ordered_indices <- order(match(SuSCs_meta[[celltype_col]], ordered_celltypes))
      cat("Cells reordered by count ascending, total cells:", length(ordered_indices), "\n")
      return(list(
        counts = SuSCs_counts[, ordered_indices], 
        meta = SuSCs_meta[ordered_indices, ], 
        cells = SuSCs_cells[ordered_indices]
      ))
    }
  } else {
    cat("Key cell types not found, returning all original data\n")
    cat("Total cells:", length(cells), "\n")
    return(list(counts = counts, meta = meta, cells = cells))
  }
  cat("=== End Validation ===\n")
}
