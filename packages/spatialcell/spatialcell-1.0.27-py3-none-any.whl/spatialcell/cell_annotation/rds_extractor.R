...
process_cell <- function(counts, meta, celltype_col, cells) {
  ...
  if (has_axin2 && has_gli1) {
    ...
    if (!is.null(celltype_names) && length(celltype_names) == length(ordered_celltypes)) {
      ...
      cat("=== DETAILED DEBUGGING START ===\n")
      cat("Total cells being processed:", length(SuSCs_cells), "\n")
      cat("Total cell types after filtering:", length(ordered_celltypes), "\n\n")
  
      # 显示完整的位置映射关系
      cat("Complete position mapping:\n")
      for (i in 1:length(ordered_celltypes)) {
        original_name <- ordered_celltypes[i]
        new_name <- celltype_names[i]
        original_count <- sum(SuSCs_meta[[celltype_col]] == original_name)
        cat(sprintf("Position %2d: %-35s (%4d cells) -> %s\n", 
                    i, original_name, original_count, new_name))
      }
  
      cat("\n=== BEFORE RENAME ===\n")
      cat("Sample cell counts before rename:\n")
      key_types <- c("Axin2⁺-SuSC", "Naive B cell", "Proliferating preosteoblast", "Proliferating meningeal fibroblast")
      for (type in key_types) {
        count <- sum(SuSCs_meta[[celltype_col]] == type)
      ...
      cat("Original celltype column sample (first 10 cells):\n")
      print(head(SuSCs_meta[[celltype_col]], 10))
  
      SuSCs_meta_map[[celltype_col]] <- celltype_map[SuSCs_meta[[celltype_col]]]
  
      cat("Renamed celltype column sample (first 10 cells):\n")
      print(head(SuSCs_meta_map[[celltype_col]], 10))
  
      cat("\n=== AFTER RENAME ===\n")
      cat("Sample cell counts after rename:\n")
      for (type in key_types) {
        count <- sum(SuSCs_meta_map[[celltype_col]] == type)
        cat(sprintf("  %-35s: %d cells\n", type, count))
      }
  
      cat("\n=== FINAL VERIFICATION ===\n")
      final_counts <- table(SuSCs_meta_map[[celltype_col]])
      cat("Complete final distribution:\n")
      sorted_names <- sort(names(final_counts))
      for (name in sorted_names) {
        cat(sprintf("  %-35s: %4d cells\n", name, final_counts[name]))
      }
  
      cat("\nTotal cells check:", sum(final_counts), "vs", length(SuSCs_cells), "\n")
      cat("=== DETAILED DEBUGGING END ===\n\n")
