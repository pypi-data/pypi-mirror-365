...
class ThreadSafeRabbitMQPool:
    ...
    def cleanup_thread_connection(self):
        """Clean up the current thread's connection"""
        if hasattr(self._thread_local, "rabbit_mq"):
            try:
                self._thread_local.rabbit_mq.close()
                self.logger.debug(
                    f"Cleaned up RabbitMQ connection for thread {threading.current_thread().name}"
                )
            except Exception as e:
                self.logger.error(f"Error cleaning up thread connection: {e}")
            finally:
                delattr(self._thread_local, "rabbit_mq")

    def cleanup(self):
        """This is called from main thread cleanup - individual threads handle their own cleanup"""
