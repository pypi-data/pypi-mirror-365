...
def example(resource_name, option_string):
    ...
    power_units = nirfmxspecan.SpectrumPowerUnits.DBM.value

    noise_compensation_enabled = nirfmxspecan.SpectrumNoiseCompensationEnabled.FALSE.value
    cleaner_spectrum = nirfmxinstr.CleanerSpectrum.DISABLED.value

    rbw_filter_type = nirfmxspecan.SpectrumRbwFilterType.GAUSSIAN.value
    rbw_auto = nirfmxspecan.SpectrumRbwAutoBandwidth.TRUE.value
    rbw = 10.0e3  # Hz

    vbw_auto = nirfmxspecan.SpectrumVbwFilterAutoBandwidth.TRUE.value
    vbw = 30.0e3  # Hz
    vbw_to_rbw_ratio = 3

    detector_type = nirfmxspecan.SpectrumDetectorType.NONE.value
    detector_points = 1001

    sweep_time_auto = nirfmxspecan.SpectrumSweepTimeAuto.TRUE.value
    sweep_time_interval = 1.00e-3  # seconds

    averaging_enabled = nirfmxspecan.SpectrumAveragingEnabled.FALSE.value
    averaging_count = 10
    averaging_type = nirfmxspecan.SpectrumAveragingType.RMS.value

    fft_window = nirfmxspecan.SpectrumFftWindow.FLAT_TOP.value
    fft_padding = -1.0
    fft_overlap_mode = nirfmxspecan.SpectrumFftOverlapMode.DISABLED.value
    fft_overlap_percent = 0
    fft_overlap_type = nirfmxspecan.SpectrumFftOverlapType.RMS.value
    ...
    try:
        ...
        specan.select_measurements("", nirfmxspecan.MeasurementTypes.SPECTRUM.value, True)
