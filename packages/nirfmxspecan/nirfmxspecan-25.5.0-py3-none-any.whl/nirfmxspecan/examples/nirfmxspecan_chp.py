...
def example(resource_name, option_string):
    ...
    rbw_filter_type = nirfmxspecan.ChpRbwFilterType.GAUSSIAN.value
    rbw_auto = nirfmxspecan.ChpRbwAutoBandwidth.TRUE.value
    rbw = 10.0e3

    sweep_time_auto = nirfmxspecan.ChpSweepTimeAuto.TRUE.value
    sweep_time_interval = 1.00e-3  # seconds

    averaging_enabled = nirfmxspecan.ChpAveragingEnabled.FALSE.value
    averaging_count = 10
    averaging_type = nirfmxspecan.ChpAveragingType.RMS.value

    rrc_filter_enabled = nirfmxspecan.ChpCarrierRrcFilterEnabled.FALSE.value
    rrc_alpha = 0.220

    fft_window = nirfmxspecan.ChpFftWindow.FLAT_TOP.value
    fft_padding = -1.0
    ...
    try:
        ...
        specan.select_measurements("", nirfmxspecan.MeasurementTypes.CHP.value, enable_all_traces)
