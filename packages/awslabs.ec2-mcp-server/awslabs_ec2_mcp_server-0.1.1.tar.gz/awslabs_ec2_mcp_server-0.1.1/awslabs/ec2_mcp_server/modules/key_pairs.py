...
async def create_key_pair(
    key_name: str,
    storage_method: Literal["secrets_manager", "s3_encrypted", "parameter_store"] = "secrets_manager",
    key_type: str = "rsa",
    ...
    """
    create_key_pair
            Creates an EC2 key pair and securely stores the private key using the specified method (Secrets Manager, S3, or Parameter Store).
            Private key storage is mandatory for security reasons.
            Required:
                key_name: Unique name for the key pair
            Optional:
                storage_method: Storage backend ("secrets_manager", "s3_encrypted", or "parameter_store", default "secrets_manager")
