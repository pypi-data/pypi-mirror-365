...
class IZISentinel:
    ...
    def get_composite_data(self, gdf, downloaded_dict, output_rgb=None, output_ndvi=None, output_evi=None, output_cir=None, output_cloud=None) -> Dict[str, CompositeRaster]:
        ...
        if output_cloud is not None:
            if not os.path.exists(output_cloud):
                raster_processing.create_cloud_mask(cropped_bands, output_cloud)
            else:
                logger.warning(f'Arquivo {output_cloud} já existe no diretório.')

    def get_satellite_image(self, geodataframe, end_date, cloud_cover_percentage, type='L2A', platform_name='SENTINEL-2', bands_dict={"L2A": {"10m": ["B02", "B03", "B04", "B08"],"20m": ["B05", "B06", "B07", "B8A", "B11", "B12"],"60m": ["B01", "B09", "B10"],"mask": ["datMask"]}}, output_rgb=None, output_ndvi=None, output_evi=None, output_cir=None, output_cloud=None):
        ...
        self.get_composite_data(geodataframe, downloaded_dict, output_rgb, output_ndvi, output_evi, output_cir, output_cloud)
...
if __name__ == '__main__':
    ...
    if zona_unica is True:
        today = datetime(2025, 4, 11, 1, 0, 0)

        for each_day in range(0, 10):
            today = today - timedelta(days=each_day)
            ...
            gdf = gdf.loc[(gdf['PROJETO'] == 'Andorinha')]
            gdf = gdf.dissolve(by='PROJETO').reset_index()
            #gdf = gpd.read_file('/home/ppz/Documentos/coding/izisat/src/new_layer.shp')
