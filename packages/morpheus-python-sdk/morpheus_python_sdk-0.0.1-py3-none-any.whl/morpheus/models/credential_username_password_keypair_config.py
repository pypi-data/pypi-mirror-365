...
class CredentialUsernamePasswordKeypairConfig(BaseModel):
    """
    ...
    password: StrictStr = Field(description="User password, API Key, or applicable secret")
    auth_key: AddCredentialsRequestCredentialOneOf2AuthKey = Field(alias="authKey")
    __properties: ClassVar[List[str]] = ["type", "name", "description", "enabled", "integration", "username", "password", "authKey"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['username-password-keypair']):
            raise ValueError("must be one of enum values ('username-password-keypair')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CredentialUsernamePasswordKeypairConfig from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CredentialUsernamePasswordKeypairConfig from a dict"""
        if obj is None:
            return None
