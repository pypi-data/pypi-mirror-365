...
class NetworkPoolServerCreateSolarWinds(BaseModel):
    """
    NetworkPoolServerCreateSolarWinds
    """ # noqa: E501
    type: StrictStr = Field(description="Type Code (SolarWinds)")
    ...
    __properties: ClassVar[List[str]] = ["type", "name", "enabled", "serviceUrl", "serviceUsername", "servicePassword", "serviceThrottleRate", "ignoreSsl", "config", "credential"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['solarwindsipam']):
            raise ValueError("must be one of enum values ('solarwindsipam')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of NetworkPoolServerCreateSolarWinds from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of NetworkPoolServerCreateSolarWinds from a dict"""
        if obj is None:
            return None
