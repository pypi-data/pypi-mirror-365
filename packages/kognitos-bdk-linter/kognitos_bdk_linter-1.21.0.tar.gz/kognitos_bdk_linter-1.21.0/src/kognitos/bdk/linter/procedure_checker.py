...
from kognitos.bdk.linter.procedure_extra_rules import (ProcedureExamplesRule,
                                                       ProcedureExtraRule)
...
class ProcedureChecker(BaseChecker):
    ...
    def __init__(self, linter: PyLinter, extra_rules: List[ProcedureExtraRule] | None = None) -> None:
        super().__init__(linter)
        if extra_rules is not None:
            self.extra_rules = extra_rules
        else:
            self.extra_rules = [ProcedureExamplesRule()]
    ...
    @classmethod
    def is_discover(cls, node: nodes.FunctionDef):
        return bool(util.get_decorator_by_name(node, "kognitos.bdk.decorators.discover_decorator.discover"))

    @classmethod
    def is_invoke(cls, node: nodes.FunctionDef):
        return bool(util.get_decorator_by_name(node, "kognitos.bdk.decorators.invoke_decorator.invoke"))
    ...
    def visit_functiondef(self, node: nodes.FunctionDef | nodes.AsyncFunctionDef) -> None:
        if ProcedureChecker.is_procedure(node):
            ...
            for extra_rule in self.extra_rules:
                extra_rule.check_rule(linter=self.linter, book_procedure=book_procedure, node=node)

        # Check for @discover and @invoke decorators
        check_discover_invoke_decorators(self, node)
...
def check_discover_invoke_decorators(checker: ProcedureChecker, node: nodes.FunctionDef | nodes.AsyncFunctionDef) -> None:
    """Check @discover and @invoke decorator requirements"""
    has_discover = ProcedureChecker.is_discover(node)
    has_invoke = ProcedureChecker.is_invoke(node)
    ...
    if has_discover:
        ...
        if node.returns:
            if not util.is_list_book_procedure_descriptor_type(node.returns):
