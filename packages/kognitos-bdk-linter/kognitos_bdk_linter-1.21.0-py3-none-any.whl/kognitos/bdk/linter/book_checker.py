...
def check_discover_and_invoke_procedures(checker, node):
    # Check for @discover and @invoke function coexistence
    discover_functions = BookChecker.discover_functions_from_classdef(node)
    invoke_functions = BookChecker.invoke_functions_from_classdef(node)
    if discover_functions and not invoke_functions:
        checker.add_message("book-discover-missing-invoke", args=node.repr_name(), node=node)
    elif invoke_functions and not discover_functions:
        checker.add_message("book-invoke-missing-discover", args=node.repr_name(), node=node)
    elif discover_functions and invoke_functions:
        # If both are present, ensure there's exactly one of each
        if len(discover_functions) > 1:
            checker.add_message("book-multiple-discover-functions", args=node.repr_name(), node=node)
        if len(invoke_functions) > 1:
            checker.add_message("book-multiple-invoke-functions", args=node.repr_name(), node=node)
...
class BookChecker(BaseChecker):
    ...
    @classmethod
    def discover_functions_from_classdef(cls, node: nodes.ClassDef):
        return [
            function
            for function in node.body
            if isinstance(function, nodes.FunctionDef) and util.get_decorator_by_name(function, "kognitos.bdk.decorators.discover_decorator.discover")
        ]

    @classmethod
    def invoke_functions_from_classdef(cls, node: nodes.ClassDef):
        return [
            function
            for function in node.body
            if isinstance(function, nodes.FunctionDef) and util.get_decorator_by_name(function, "kognitos.bdk.decorators.invoke_decorator.invoke")
        ]
    ...
    def visit_classdef(self, node: nodes.ClassDef) -> None:
        if BookChecker.is_book(node):
            ...
            check_discover_and_invoke_procedures(self, node)
