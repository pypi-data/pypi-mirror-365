...
def preprocess(
    ...
    if correct_spelling:
        CUSTOM_SPELL_FIXES = {"ths": "this"}
        words = [CUSTOM_SPELL_FIXES.get(w, w) for w in text.split()]
        text = ' '.join([get_spell()(w) for w in words])
    if remove_punct:
    ...
    CUSTOM_LEMMA_OVERRIDES = {"testimony": "test", "tests": "test"}
    ...
    for token in doc:
        if token.is_space:
            continue
        lemma = CUSTOM_LEMMA_OVERRIDES.get(token.lemma_, token.lemma_)
        if remove_stopwords:
            if token.is_alpha and not token.is_stop:
                if token.pos_ in {"NOUN", "VERB", "ADJ", "ADV", "INTJ"} or (preserve_pron_aux and token.pos_ in {"PRON", "AUX"}):
                    tokens.append(lemma if lemmatise else token.text)
        else:
            if token.is_alpha:
                tokens.append(lemma if lemmatise else token.text)
...
def get_tokens(
    ...
    # === Tokenize directly ===
    doc = get_nlp()(text)
    tokens = [token.lemma_ if lemmatise else token.text for token in doc if not token.is_space]

    return tokens  # ✅ preserves order, no stopword filtering or deduplication
