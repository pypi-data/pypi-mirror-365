...
def preprocess(
    ...
    if remove_stopwords:
        # Stopword removal + POS filtering (includes INTJ for "hello"-like words)
        tokens = [
            (token.lemma_ if lemmatise else token.text)
            for token in doc
            if token.is_alpha
            and not token.is_stop
            and token.pos_ in {"NOUN", "VERB", "ADJ", "ADV", "INTJ"}
        ]
    else:
        # Keep all tokens (no stopword removal, no POS restriction)
        tokens = [
            (token.lemma_ if lemmatise else token.text)
            for token in doc
            if token.is_alpha
