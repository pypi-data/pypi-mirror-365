...
class BollingerBands:
    ...
    def calculate_multiple_bollinger_bands(self, k: float, window: int) -> pd.DataFrame | None:
        ...
        - cov: covariance on basis of rolling std and mean for a particular column (std / mean) 
        ...
        try: 
            ...
            for column in REQUIRED_COLUMNS:
                ...
                ## COV -> SNA BASED
                df[f'{column}_sma_std'] = df[f'{column}_sma'].rolling(window=window).std()
                df[f'{column}_sma_mean'] = df[f'{column}_sma'].rolling(window=window).mean()
                
                df[f'{column}_sma_cov'] = df[f'{column}_sma_std'] / df[f'{column}_sma_mean']
                df[f"{column}_sma_agg_cov"] = df[f'{column}_sma'].std(ddof=1) / df[f'{column}_sma'].mean()
                df[f"{column}_sma_cov_ratio"] = df[f"{column}_sma_cov"] / df[f"{column}_sma_agg_cov"]
