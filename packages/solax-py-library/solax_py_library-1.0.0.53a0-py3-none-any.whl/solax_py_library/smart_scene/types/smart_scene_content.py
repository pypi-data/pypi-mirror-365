...
class SmartSceneContent(SmartSceneBaseModel):
    ...
    def rec_scene(cls, lang, cabinet_type):
        scene_rec_1 = {
            "name": cls._international_map["rec_name_01"][lang],
            "instruction": cls._international_map["instruction_01"][lang],
        ...
        scene_rec_2 = {
            "name": cls._international_map["rec_name_02"][lang],
            "instruction": cls._international_map["instruction_02"][lang],
            "repeatFunction": RepeatFunc.EVERYDAY,
            "weekList": [1, 2, 3, 4, 5, 6, 7],
            "if": {
                "operation": LogicFunc.AND,
    ...
    def repeat_text_info(self, lang):
        if self.repeatFunction != RepeatFunc.CUSTOM:
            return self.translate(str(self.repeatFunction.value), lang)
            return self._international_map[str(self.repeatFunction.value)][lang]
        else:
            return ",".join(
                [self._international_map[str(d)][lang] for d in self.weekList]
            )
