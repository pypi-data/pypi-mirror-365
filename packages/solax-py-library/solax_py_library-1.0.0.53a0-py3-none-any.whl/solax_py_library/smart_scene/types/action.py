...
class ActionChildData(SmartSceneBaseModel):
    data: List[Any]
...
class ActionItemData(SmartSceneBaseModel):
        ...
        if self.childType == SystemActionType.systemSwitch:
            ...
            if self.childData.data[0] == 0:
                ...
                if cabinet_type in TRENE_CABINET_ENUM:
                    ...
                    return self.translate(self.childType, lang).format(
                        self.translate("on", lang),
                        self.translate(msg, lang),
                        self.childData.data[2],
                    )
                else:
                    return self.translate("importControl_AELIO", lang).format(
                        self.translate("on", lang), self.childData.data[1]
                    )
    ...
    def work_mode_to_text(self, lang):
        ...
        if work_mode == WorkMode.manual_mode:
            ...
            if value_data[1] in [3, 4]:
                return self.translate(str(work_mode), lang).format(
                    self.translate(str(manual_mode), lang).format(
                        value_data[2], value_data[3]
                    )
                )
            else:
                return self.translate(str(work_mode), lang).format(
                    self.translate(str(manual_mode), lang)
                )
            ...
            if value_data[1] in [
                ...
                return self.translate(str(vpp_mode), lang).format(
                    value_data[2], value_data[3]
                )
            elif value_data[1] == VppMode.push_power_positive_negative_mode:
                return self.translate(str(vpp_mode), lang).format(value_data[2])
    ...
    def to_text(self, lang, cabinet_type):
        if self.childType == EmsActionType.DoControl:
            ...
            for do_info in self.childData.data:
                message += self.translate(self.childType, lang).format(
                    do_info.DoNumber,
                    do_info.DoValue,
                )
            return message
...
class SmartSceneAction(SmartSceneBaseModel):
    type: ActionType
