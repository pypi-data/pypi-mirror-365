...
from ptcc_library.ptcc_object import *
...
class PtccMessage:
    ...
    @classmethod
    def generate_msg_get_monitor(cls, module_type: ModuleType):
        """
        ...
        You can set those CallbackPtccObjectID to see the response:
         ...
         - PTCC_MONITOR_TH_ADC
        """
        return cls(generate_msg_get_monitor(module_type))
...
def generate_msg_get_monitor(module_type: ModuleType) -> list[int]:
    """
    Generates message for reading measured parameters of no memory module connected to PTCC device.
    ...
    if module_type == ModuleType.NONE:
        ...
        msg = create_get_ptcc_message(PtccObjectID.GET_PTCC_MONITOR)
    elif module_type == ModuleType.MEM:
        raise ValueError(f"Message not supported for this module type: {module_type}")
    elif module_type == ModuleType.LAB_M:
        msg = generate_msg_get_monitor(ModuleType.MEM)
    return msg
