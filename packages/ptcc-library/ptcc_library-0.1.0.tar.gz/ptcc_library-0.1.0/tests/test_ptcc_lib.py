...
class TestTecLibrary20(unittest.TestCase):
    ...
    def test(self):
        ...
        msg.reset()
        msg = PtccMessage.generate_msg_get_config(ModuleType.NOMEM)
        expected_msg = [ord(x) for x in "$050000040F01#"]
        assert msg.raw_message == expected_msg

        msg.reset()
        msg = PtccMessage(create_set_ptcc_message(PtccObjectID.SET_PTCC_CONFIG,
                                                  [PtccObject(obj_id=PtccObjectID.PTCC_CONFIG_VARIANT, data=[1]),
                                                 PtccObject(obj_id=PtccObjectID.PTCC_CONFIG_NO_MEM_COMPATIBLE,
                                                            data=[0])]))
        expected_msg = [ord(x) for x in "$051000121800000E1813000501182B000500DD84#"]
        assert msg.raw_message == expected_msg
